...
describe('runGovernanceChecks', () => {
  ...
  describe('CIS-2.1 CloudTrail multi-region', () => {
    it('flags when no trails exist', () => {
      const findings = runGovernanceChecks(
        govCtx({ cloudtrailTrails: [{ Name: 'test', IsMultiRegionTrail: false }] })
      );
  ...
  describe('CIS-2.2 CloudTrail log validation', () => {
    it('flags when log validation disabled', () => {
      const findings = runGovernanceChecks(
        govCtx({
          cloudtrailTrails: [
            { Name: 'test', IsMultiRegionTrail: true, LogFileValidationEnabled: false }
          ]
        })
      );
  ...
  describe('GOV-KMS1 KMS rotation', () => {
    it('flags rotation disabled', () => {
      const findings = runGovernanceChecks(
        govCtx({
          kmsKeys: [
            { KeyId: 'key-1', KeyManager: 'CUSTOMER', KeyState: 'Enabled', RotationEnabled: false }
          ]
        })
      );
    ...
    it('passes rotation enabled', () => {
      const findings = runGovernanceChecks(
        govCtx({
          kmsKeys: [
            { KeyId: 'key-1', KeyManager: 'CUSTOMER', KeyState: 'Enabled', RotationEnabled: true }
          ]
        })
      );
      const f = findControl(findings, 'GOV-KMS1');
    ...
    it('skips AWS-managed keys', () => {
      const findings = runGovernanceChecks(
        govCtx({
          kmsKeys: [
            { KeyId: 'key-1', KeyManager: 'AWS', KeyState: 'Enabled', RotationEnabled: false }
          ]
        })
      );
      const f = findControl(findings, 'GOV-KMS1');
