...
import {
  ipToNum,
  ipFromCidr,
  cidrContains as nrCidrContains,
  protoMatch,
  portInRange,
  protoName,
  evaluateRouteTable,
...
// Anonymous, opt-in telemetry. Dormant unless build.js sets __TELEMETRY_ENABLED__="true"
// (default false → init() is a no-op and the consent UI never mounts).
import * as Telemetry from './modules/telemetry.js';
import * as TelemetryUI from './modules/telemetry-ui.js';
...
function isHostedVercelPage() {
  ...
  return (
    host === 'awsmapper.schylerryan.com' ||
    host.endsWith('.awsmapper.schylerryan.com') ||
...
function applyRuntimeVersion() {
  ...
  document.querySelectorAll('.brand-ver').forEach((el) => {
  ...
  for (let node = footer.firstChild; node; node = node.nextSibling) {
    if (node.nodeType === 3 && node.nodeValue.trim().startsWith('v__VERSION__')) {
      node.nodeValue = node.nodeValue.replace('v__VERSION__', label);
      break;
    }
  }
}
...
window.AppModules = {
  ...
  // Telemetry (dormant unless enabled at build time; intentionally NOT spread onto window)
  Telemetry,
  TelemetryUI
...
// Initialize telemetry. No-op when __TELEMETRY_ENABLED__ is "false" (the default).
Telemetry.init();

// Auto-mount the consent prompt and settings row only when telemetry is enabled.
// Both helpers short-circuit when dormant, so this is a no-op in the default build.
(function mountTelemetryUI() {
  if (!Telemetry.isEnabled || !Telemetry.isEnabled()) {return;}
  if (document.body) {
    TelemetryUI.renderConsentPrompt(document.body, null);
    const pop = document.getElementById('settingsPopover');
    if (pop) {
      const row = TelemetryUI.buildSettingsRow();
