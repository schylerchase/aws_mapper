...
document.getElementById('searchInput').addEventListener('input', function () {
  ...
  _searchTimer = setTimeout(function () {
    ...
    matches.forEach((m) => {
      const acctBadge =
        isMA && m.acct && m.acct !== 'default'
          ? '<span style="font-size:8px;padding:1px 5px;border-radius:3px;background:' +
          ...
          : '';
      h +=
        '<div style="padding:8px 12px;cursor:pointer;border-bottom:1px solid var(--border);display:flex;align-items:center;gap:8px" onclick="closeSearch();_zoomToElement(\'' +
        esc(m.id) +
        "');_openDetailForSearch('" +
        esc(m.type) +
        "','" +
        esc(m.id) +
        '\')"><span style="font-size:9px;color:var(--accent-cyan);font-weight:600;width:50px">' +
        m.type +
        '</span><span style="flex:1;font-size:12px;color:var(--text-primary)">' +
        esc(m.name) +
        '</span>' +
        acctBadge +
    ...
    if (!matches.length)
      h =
        '<div style="padding:20px;text-align:center;color:var(--text-muted);font-size:12px">No results</div>';
    res.innerHTML = h;
  });
});
