...
function _buildSearchIndex(ctx) {
  ...
  const getName = function (obj, fallback) {
    const t = (obj.Tags || []).find(function (x) {
      return x.Key === 'Name';
    });
  ...
  (ctx.rdsInstances || []).forEach(function (d) {
    idx.push({
      ...
      extra: d.Engine || '',
      acct: d._accountLabel || d._accountId || '',
  ...
  (ctx.lambdaFns || []).forEach(function (f) {
    idx.push({
      ...
      id: f.FunctionName,
      extra: f.Runtime || '',
      acct: f._accountLabel || f._accountId || '',
...
document.getElementById('searchInput').addEventListener('input', function () {
  ...
  _searchTimer = setTimeout(function () {
    ...
    if (_searchIndexCtx !== _rlCtx) {
      _searchIndex = _buildSearchIndex(_rlCtx);
      _searchIndexCtx = _rlCtx;
    }
    // Filter cached index in a single pass (cap at 30)
    const matches = [];
    for (let si = 0; si < _searchIndex.length && matches.length < 30; si++) {
      if (_searchIndex[si].searchStr.includes(q)) {
        matches.push(_searchIndex[si]);
      }
    }
    // Notes are dynamic — search them live (typically small set)
    _getAllNotes().forEach(function (n) {
      if (matches.length >= 30) {
        return;
      }
      if (
        (n.text || '').toLowerCase().includes(q) ||
        (_getResourceName(n.resourceId) || '').toLowerCase().includes(q)
      ) {
        matches.push({
          type: 'Note',
          name: (n.text || '').slice(0, 50),
          id: n.resourceId,
          extra: n.category || '',
          acct: ''
        });
    ...
    matches.forEach((m) => {
      ...
      row.addEventListener('click', function () {
        closeSearch();
        _zoomToElement(m.id);
        _openDetailForSearch(m.type, m.id);
      });
      ...
      if (isMA && m.acct && m.acct !== 'default') {
        ...
        acctBadge.style.cssText =
          'font-size:8px;padding:1px 5px;border-radius:3px;background:' +
          (getAccountColor(m.acct) || 'var(--bg-tertiary)') +
...
function _zoomToElement(id) {
  if (!_mapSvg || !_mapZoom || !_mapG) {
    return;
  }
  let el = _queryMapResourceById(id, _mapG.node());
  ...
  if (!el && id && _rlCtx) {
    ...
    if (sg && sg.VpcId) {
      el = _queryMapResourceById(sg.VpcId, _mapG.node());
    }
  }
  if (!el) {
    return;
...
function _resolveResourceType(rid) {
  ...
  if (rid.startsWith('igw-')) {
    return 'IGW';
  }
  if (rid.startsWith('nat-')) {
    return 'NAT';
  }
  if (rid.startsWith('vgw-')) {
    return 'VGW';
  }
  if (rid.startsWith('vpce-')) {
    return 'VPCE';
  }
  if (rid.startsWith('tgw-')) {
    return 'TGW';
  }
  if (rid.startsWith('pcx-')) {
    return 'PCX';
  }
  // Check by lookup in context
  if (
    (_rlCtx.rdsInstances || []).find(function (d) {
  ...
  if (
    (_rlCtx.lambdaFns || []).find(function (f) {
