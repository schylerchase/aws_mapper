// IAC Generator — Terraform, CloudFormation, CDK code generation
...
/**
 * Detect circular references among security groups.
 * Returns an array of cycles (each cycle is an array of GroupIds).
 */
...
// --- HCL SG rule writer (inline block) ---
export function _writeSGRule(lines, rule) {
...
export function _writeSGRuleFlat(lines, rule) {
  const fromPort = rule.FromPort != null ? rule.FromPort : 0;
  const toPort = rule.ToPort != null ? rule.ToPort : 0;
  const proto = rule.IpProtocol || '-1';
  lines.push('  protocol         = "' + proto + '"');
...
/**
 * Main Terraform HCL generation engine.
...
/**
 * Generate CloudFormation template from AWS resource context.
...
/**
 * Generate a security-property-rich CloudFormation template for Checkov scanning.
...
export function _serializeCfnYaml(obj, indent) {
  indent = indent || 0;
...
/**
 * Basic HCL syntax highlighter.
 * Produces markup strings with span tags for color-coding.
 */
...
/**
 * Basic YAML/CloudFormation syntax highlighter.
 * Produces markup strings with span tags for color-coding.
 */
...
if (typeof window !== 'undefined') {
  ...
  // Backward-compatible aliases for inline DOM code
