...
function _runClassificationEngine(ctx) {
  if (typeof window.runClassificationEngine === 'function') {
    window.runClassificationEngine(ctx);
  }
}
...
const _BUDR_STRATEGY_LEGEND = [
  {
    ...
    color: '#ef4444',
    icon: '🔴',
    desc: 'Active-active — full replica running at all times. Near-zero RTO & RPO.'
  },
  {
    k: 'high',
    label: 'High (Warm)',
    color: '#f59e0b',
    icon: '🟡',
    desc: 'Scaled-down replica running. Scale up on failover. Minutes to recover.'
  },
  {
    k: 'medium',
    label: 'Medium (Pilot Light)',
    color: '#6366f1',
    icon: '🟣',
    desc: 'Data replicated continuously, compute stopped. Spin up on failover. ~10-30 min.'
  },
  {
    k: 'low',
    label: 'Low (Cold)',
    color: '#64748b',
    icon: '⚪',
    desc: 'Backups only, no standby. Rebuild from scratch. Hours to recover.'
  }
];
const _BUDR_RTO_RPO = {
...
const _BUDR_EST_MINUTES = {
  rds_multi_az: {
    rto: 5,
    rpo: 1,
    rtoWhy: 'Multi-AZ automatic failover completes in 1-2 min; DNS propagation adds ~3 min',
    rpoWhy:
      'Synchronous replication to standby — data loss limited to in-flight transactions (~seconds)'
  },
  rds_single_backup: {
    rto: 30,
    rpo: 1440,
    rtoWhy:
      'Restore from automated snapshot requires instance provisioning + data load (~20-30 min)',
    rpoWhy: 'Automated backups run daily — worst case RPO is 24 hours since last backup window'
  },
  rds_no_backup: {
    rto: 480,
    rpo: Infinity,
    rtoWhy: 'No backups — requires manual rebuild from application layer or external source',
    rpoWhy: 'No backup mechanism configured — all data since creation is unrecoverable'
  },
  rds_aurora: {
    rto: 0.5,
    rpo: 5,
    rtoWhy: 'Aurora automatic failover promotes read replica in <30 sec; DNS TTL is 5 sec',
    rpoWhy:
      'Aurora replicates 6 copies across 3 AZs — RPO limited to last committed transaction (~seconds)'
  },
  ec2_asg: {
    rto: 3,
    rpo: 0,
    rtoWhy:
      'ASG detects failure via health check (1-2 min) and launches replacement from AMI (~1-2 min)',
    rpoWhy: 'Stateless compute — no persistent data on instance; state lives in external stores'
  },
  ec2_ami_snap: {
    rto: 15,
    rpo: 10080,
    rtoWhy: 'Manual AMI launch + EBS restore from snapshot (~10-15 min depending on volume size)',
    rpoWhy: 'Snapshot frequency is typically weekly — worst case RPO is 7 days since last snapshot'
  },
  ec2_standalone: {
    rto: 480,
    rpo: Infinity,
    rtoWhy:
      'No AMI/snapshot — requires full OS install, config, and application deployment from scratch',
    rpoWhy: 'No backup mechanism — local EBS data is unrecoverable if instance or volume is lost'
  },
  ecs_multi: {
    rto: 1,
    rpo: 0,
    rtoWhy: 'ECS service scheduler replaces failed tasks in ~30-60 sec from container image',
    rpoWhy:
      'Stateless containers — no persistent data; state lives in external stores (RDS, S3, etc.)'
  },
  ecs_single: {
    rto: 5,
    rpo: 0,
    rtoWhy: 'Single task replacement takes ~2-5 min including image pull and health check',
    rpoWhy: 'Stateless containers — no persistent data; state lives in external stores'
  },
  lambda: {
    rto: 0,
    rpo: 0,
    rtoWhy: 'Fully managed — AWS handles all availability; cold start adds <1 sec latency',
    rpoWhy: 'Stateless execution — no persistent data; code stored in S3 with versioning'
  },
  ecache_multi: {
    rto: 2,
    rpo: 0.1,
    rtoWhy:
      'Multi-AZ auto-failover promotes replica in 1-2 min; DNS endpoint updates automatically',
    rpoWhy: 'Async replication lag is typically <100ms — data loss limited to replication lag'
  },
  ecache_single: {
    rto: 15,
    rpo: 10080,
    rtoWhy: 'Restore from snapshot requires new cluster provisioning + data load (~10-15 min)',
    rpoWhy: 'Snapshot frequency is typically daily/weekly — worst case RPO equals snapshot interval'
  },
  ecache_no_snap: {
    rto: 15,
    rpo: Infinity,
    rtoWhy: 'New cluster provisioning takes ~10-15 min but cache starts cold (empty)',
    rpoWhy: 'No snapshots — entire cache contents are lost; must be rebuilt from source of truth'
  },
  redshift_snap: {
    rto: 30,
    rpo: 1440,
    rtoWhy: 'Restore from snapshot creates new cluster (~20-30 min depending on data size)',
    rpoWhy: 'Automated snapshots run every 8 hours by default — worst case RPO is snapshot interval'
  },
  redshift_multi: {
    rto: 15,
    rpo: 5,
    rtoWhy: 'Multi-node cluster redistributes work to surviving nodes (~10-15 min recovery)',
    rpoWhy: 'Synchronous replication across nodes — RPO limited to in-flight queries (~minutes)'
  },
  redshift_none: {
    rto: 480,
    rpo: Infinity,
    rtoWhy: 'No snapshots — requires full data reload from S3/source systems (hours to days)',
    rpoWhy: 'No backup mechanism — all warehouse data is unrecoverable'
  },
  alb_multi_az: {
    rto: 0,
    rpo: 0,
    rtoWhy: 'Fully managed multi-AZ — AWS handles node replacement transparently',
    rpoWhy: 'Stateless load balancer — no data to lose; config stored in AWS control plane'
  },
  alb_single_az: {
    rto: 5,
    rpo: 0,
    rtoWhy: 'Single-AZ ALB may need DNS failover if AZ goes down (~3-5 min)',
    rpoWhy: 'Stateless load balancer — no data to lose'
  },
  s3: {
    rto: 0,
    rpo: 0,
    rtoWhy: '11 nines durability — service is always available across 3+ AZs',
    rpoWhy: 'Objects replicated across multiple AZs automatically'
  },
  s3_versioned: {
    rto: 0,
    rpo: 0,
    rtoWhy: 'Versioned objects can be restored to any previous version instantly',
    rpoWhy: 'Every object change creates a new version — zero data loss possible'
  },
  s3_unversioned: {
    rto: 0,
    rpo: Infinity,
    rtoWhy: 'Bucket is always available, but deleted/overwritten objects cannot be recovered',
    rpoWhy: 'No versioning — overwrites and deletes are permanent and unrecoverable'
  },
  s3_mfa_delete: {
    rto: 0,
    rpo: 0,
    rtoWhy: 'MFA Delete prevents accidental deletion — objects recoverable from versions',
    rpoWhy: 'Versioning + MFA Delete = immutable storage; data cannot be accidentally lost'
  },
  ebs_snap: {
    rto: 15,
    rpo: 10080,
    rtoWhy: 'Create new volume from snapshot + attach to instance (~10-15 min)',
    rpoWhy: 'Snapshot frequency is typically weekly — worst case RPO is 7 days since last snapshot'
  },
  ebs_no_snap: {
    rto: 480,
    rpo: Infinity,
    rtoWhy: 'No snapshots — volume data is unrecoverable if volume fails (rare but possible)',
    rpoWhy: 'No backup mechanism — all volume data is permanently lost on failure'
  }
};
...
function _budrTierCompliance(profileKey, classTier) {
  ...
  if (est.rpo === Infinity) {
    issues.push({
      field: 'RPO',
      severity: 'critical',
      msg: 'No backup — RPO unrecoverable (target: ' + target.rpoLabel + ')'
    });
  } else if (est.rpo > target.rpo) {
    issues.push({
      field: 'RPO',
      severity: 'warning',
      msg:
        'Est. RPO ~' + _fmtMin(est.rpo) + ' exceeds ' + classTier + ' target of ' + target.rpoLabel
    });
  }
  if (est.rto > target.rto) {
    issues.push({
      field: 'RTO',
      severity: 'warning',
      msg:
        'Est. RTO ~' + _fmtMin(est.rto) + ' exceeds ' + classTier + ' target of ' + target.rtoLabel
    });
  }
...
function runBUDRChecks(ctx) {
  ...
  (ctx.rdsInstances || []).forEach((db) => {
    ...
    if (hasMultiAZ && hasBackup) {
      ...
      f.push({
        ...
        resource: id,
        resourceName: name,
        message: 'RDS not Multi-AZ — single point of failure',
      ...
      f.push({
        ...
        resource: id,
        resourceName: name,
        message: 'RDS has no automated backups (retention=0)',
    ...
    if (!hasMultiAZ && !isMicro && hasBackup) {
      f.push({
        ...
        resource: id,
        resourceName: name,
        message: 'RDS single-AZ with backups only — extended RTO on AZ failure',
        remediation: 'Enable Multi-AZ or create cross-region read replica'
      });
    }
    if (!db.DeletionProtection && !isMicro) {
      f.push({
        ...
        remediation: 'Enable DeletionProtection to prevent accidental deletion'
      });
    }
    assessments.push({
      type: 'RDS',
      ...
      signals: {
        ...
        ReadReplicas: (db.ReadReplicaDBInstanceIdentifiers || []).length,
        PITR: hasPITR
      }
    });
  });
  ...
  (ctx.instances || []).forEach((inst) => {
    ...
    if (asgTag && asgTag.Value) {
      asgInstIds.add(inst.InstanceId);
    }
  });
  (ctx.instances || []).forEach((inst) => {
    const id = inst.InstanceId;
    const name = gn(inst, id);
    ...
    if (hasSnaps && snapAgeDays !== null && snapAgeDays > 7) {
      f.push({
        ...
        resource: id,
        resourceName: name,
        message: 'Newest EBS snapshot is ' + snapAgeDays + ' days old (>7 days)',
        remediation: 'Configure AWS Backup or DLM to take snapshots at least weekly'
      });
    }
    ...
    if (inASG) {
      ...
      f.push({
        ...
        resource: id,
        resourceName: name,
        message: 'EC2 not in Auto Scaling group — manual recovery required',
        remediation: 'Place behind ASG or create AMI + launch template for quick recovery'
      });
    } else {
      ...
      f.push({
        ...
        resource: id,
        resourceName: name,
        message: 'EC2 standalone with no EBS snapshots — unrecoverable on failure',
        remediation: 'Create regular EBS snapshots via AWS Backup or DLM; consider ASG'
      });
      if (!inASG) {
        f.push({
          severity: 'MEDIUM',
          ...
          resource: id,
          resourceName: name,
          message: 'EC2 has no disaster recovery strategy',
          remediation: 'Create AMI, configure ASG with multi-AZ, or use EBS snapshots'
        });
      }
    }
    assessments.push({
      ...
      name,
      profile,
      signals: { ASG: inASG, Snapshots: hasSnaps, SnapAgeDays: snapAgeDays, Encrypted: encrypted }
    });
  });
  // ECS
  (ctx.ecsServices || []).forEach((svc) => {
    ...
    if (multi) {
      ...
      f.push({
        ...
        resource: id,
        resourceName: name,
        message: 'ECS service has desiredCount=' + desired + ' — no redundancy',
        remediation: 'Set desiredCount ≥ 2 across multiple AZs'
      });
    }
    assessments.push({
      type: 'ECS',
      ...
      name,
      profile,
      signals: { DesiredCount: desired, MultiTask: multi }
    });
  });
  ...
  (ctx.lambdaFns || []).forEach((fn) => {
    assessments.push({
      ...
      profile: _BUDR_RTO_RPO.lambda,
      signals: { Managed: true }
    });
  });
  // ElastiCache
  (ctx.ecacheClusters || []).forEach((ec) => {
    ...
    if (multiNode) {
      ...
      f.push({
        ...
        resource: id,
        resourceName: name,
        message: 'ElastiCache single node — failover requires manual intervention',
        remediation: 'Add replicas or enable cluster mode for automatic failover'
      });
    } else {
      ...
      f.push({
        ...
        resource: id,
        resourceName: name,
        message: 'ElastiCache single node with no snapshots — data loss risk',
        remediation: 'Enable automatic snapshots and add read replicas'
      });
    }
    assessments.push({
      type: 'ElastiCache',
      ...
      name,
      profile,
      signals: { MultiNode: multiNode, Snapshots: hasSnap, AutoFailover: autoFailover }
    });
  });
  // Redshift
  (ctx.redshiftClusters || []).forEach((rs) => {
    ...
    if (multiNode && hasSnap) {
      ...
      f.push({
        ...
        resource: id,
        resourceName: name,
        message: 'Redshift single-node cluster — no compute redundancy',
        remediation: 'Resize to multi-node cluster for HA'
      });
    } else {
      ...
      f.push({
        ...
        resource: id,
        resourceName: name,
        message: 'Redshift with no automated snapshots — data loss risk',
        remediation: 'Enable automated snapshots with ≥7 day retention'
      });
    }
    assessments.push({
      type: 'Redshift',
      ...
      name,
      profile,
      signals: { MultiNode: multiNode, Snapshots: hasSnap }
    });
  });
  // ALBs
  (ctx.albs || []).forEach((alb) => {
    ...
    if (azs >= 2) {
      ...
      f.push({
        ...
        resource: id,
        resourceName: name,
        message: 'ALB in single AZ only — no failover',
        remediation: 'Register subnets in at least 2 AZs'
      });
    }
  ...
  (ctx.volumes || []).forEach((vol) => {
    ...
    if (snaps.length === 0) {
      f.push({
        ...
        remediation: 'Create snapshot schedule via AWS Backup or DLM lifecycle policy'
      });
    }
  });
  // S3
  (ctx.s3bk || []).forEach((bk) => {
    ...
    if (mfaDel) {
      ...
      f.push({
        ...
        resource: id,
        resourceName: name,
        message: 'S3 bucket has no versioning \u2014 data loss risk',
        remediation: 'Enable versioning to protect against accidental deletes and overwrites'
      });
    }
    assessments.push({
      type: 'S3',
      ...
      name,
      profile,
      signals: { Versioned: versioned, MFADelete: mfaDel }
    });
  });
  ...
  (ctx.subnets || []).forEach(function (s) {
    if (s.SubnetId) {
      _bSubVpc[s.SubnetId] = s.VpcId || '';
    }
  });
  (ctx.rdsInstances || []).forEach(function (r) {
    _budrLookup['RDS:' + r.DBInstanceIdentifier] = {
      a: r._accountId || '',
      r: r._region || '',
      v: (r.DBSubnetGroup && r.DBSubnetGroup.VpcId) || ''
    };
  });
  (ctx.instances || []).forEach(function (r) {
    _budrLookup['EC2:' + r.InstanceId] = {
      a: r._accountId || '',
      r: r._region || '',
      v: r.VpcId || _bSubVpc[r.SubnetId] || ''
    };
  });
  (ctx.ecsServices || []).forEach(function (r) {
    const nc = r.networkConfiguration && r.networkConfiguration.awsvpcConfiguration;
    const sid = nc && nc.subnets && nc.subnets[0] ? nc.subnets[0] : '';
    _budrLookup['ECS:' + (r.serviceName || r.serviceArn)] = {
      a: r._accountId || '',
      r: r._region || '',
      v: sid ? _bSubVpc[sid] || '' : ''
    };
  ...
  (ctx.lambdaFns || []).forEach(function (r) {
    _budrLookup['Lambda:' + r.FunctionName] = {
      a: r._accountId || '',
      r: r._region || '',
      v: (r.VpcConfig && r.VpcConfig.VpcId) || ''
    };
  });
  (ctx.ecacheClusters || []).forEach(function (r) {
    _budrLookup['ElastiCache:' + r.CacheClusterId] = {
      a: r._accountId || '',
      r: r._region || '',
      v: r.VpcId || ''
    };
  ...
  (ctx.redshiftClusters || []).forEach(function (r) {
    _budrLookup['Redshift:' + r.ClusterIdentifier] = {
      a: r._accountId || '',
      r: r._region || '',
      v: r.VpcId || ''
    };
  ...
  (ctx.albs || []).forEach(function (r) {
    _budrLookup['ALB:' + (r.LoadBalancerName || r.LoadBalancerArn)] = {
      a: r._accountId || '',
      r: r._region || '',
      v: r.VpcId || ''
    };
  ...
  if (_bAccts.size >= 1) {
    ...
    f.forEach(function (finding) {
      if (!finding._accountId) {
        finding._accountId = _bPri2;
      }
    });
  }
  budrFindings = f;
...
function _enrichBudrWithClassification(ctx, findings) {
  ...
  if (!classData.length && ctx) {
    _runClassificationEngine(ctx);
  }
  ...
  budrAssessments.forEach(function (a) {
    const cls =
      classMapTyped[a.type + '|' + a.id] ||
      classMapTyped[a.type + '|' + a.name] ||
      classMap[a.id] ||
      classMap[a.name];
    ...
    if (a.compliance.issues.length > 0) {
      a.compliance.issues.forEach(function (issue) {
        ...
        findings.push({
          ...
          remediation:
            issue.field === 'RPO'
              ? 'Configure automated backups to meet ' + a.classTier + ' RPO target'
              : 'Improve HA/DR strategy to meet ' + a.classTier + ' RTO target'
        });
      });
    }
    ...
    if (ov) {
      ...
      if (ov.rto) {
        a.profile.rto = ov.rto;
      }
      if (ov.rpo) {
        a.profile.rpo = ov.rpo;
      }
    }
  });
}
...
function _getBUDRTierCounts() {
  ...
  budrAssessments.forEach((a) => {
    if (a.profile) {
      counts[a.profile.tier] = (counts[a.profile.tier] || 0) + 1;
    }
  });
  return counts;
}
