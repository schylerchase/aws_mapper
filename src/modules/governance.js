...
// === State Accessors ===
export function getGovDashState() {
  return _govDashState;
}
export function setGovDashState(v) {
  _govDashState = v;
}
export function getIamDashState() {
  return _iamDashState;
}
export function setIamDashState(v) {
  _iamDashState = v;
}
export function getClassificationData() {
  return _classificationData;
}
export function setClassificationData(v) {
  _classificationData = v;
}
export function getClassificationOverrides() {
  return _classificationOverrides;
}
export function setClassificationOverrides(v) {
  _classificationOverrides = v;
}
export function getIamReviewData() {
  return _iamReviewData;
}
export function setIamReviewData(v) {
  _iamReviewData = v;
}
export function getInventoryData() {
  return _inventoryData;
}
export function setInventoryData(v) {
  _inventoryData = v;
}
export function getInvState() {
  return _invState;
}
export function setInvState(v) {
  _invState = v;
}
export function getAppRegistry() {
  return _appRegistry;
}
export function setAppRegistry(v) {
  _appRegistry = v;
}
export function getAppAutoDiscovered() {
  return _appAutoDiscovered;
}
export function setAppAutoDiscovered(v) {
  _appAutoDiscovered = v;
}
export function getAppSummaryState() {
  return _appSummaryState;
}
export function setAppSummaryState(v) {
  _appSummaryState = v;
}
export function getInvToolbarRendered() {
  return _invToolbarRendered;
}
export function setInvToolbarRendered(v) {
  _invToolbarRendered = v;
}
export function getInvFilterCache() {
  return _invFilterCache;
}
export function setInvFilterCache(v) {
  _invFilterCache = v;
}
export function getInvFilterKey() {
  return _invFilterKey;
}
export function setInvFilterKey(v) {
  _invFilterKey = v;
}
export function getClassificationRules() {
  return _classificationRules;
}
export function setClassificationRules(v) {
  _classificationRules = v;
}
export function getDiscoveredTags() {
  return _discoveredTags;
}
export function setDiscoveredTags(v) {
  _discoveredTags = v;
}
...
function _buildInventoryData() {
  ...
  (ctx.subnets || []).forEach(function (s) {
    if (s.SubnetId) {
      subVpcMap[s.SubnetId] = s.VpcId || '';
    }
  });
  const instVpcMap = {};
  (ctx.instances || []).forEach(function (i) {
    if (i.InstanceId) {
      instVpcMap[i.InstanceId] = i.VpcId || subVpcMap[i.SubnetId] || '';
    }
  });
  // 1. VPCs
  (ctx.vpcs || []).forEach(function (v) {
    rows.push(
      mkRow(v.VpcId, 'VPC', tag(v) || v.VpcId, v, {
        vpcId: v.VpcId,
        config: v.CidrBlock || '',
        state: v.State || ''
      })
  ...
  (ctx.subnets || []).forEach(function (s) {
    ...
    rows.push(
      mkRow(s.SubnetId, 'Subnet', tag(s) || s.SubnetId, s, {
        vpcId: s.VpcId,
        az: s.AvailabilityZone || '',
        config: (s.CidrBlock || '') + ' ' + (isPub ? 'public' : 'private'),
        state: s.State || ''
      })
    );
  });
  ...
  (ctx.instances || []).forEach(function (i) {
    ...
    rows.push(
      mkRow(i.InstanceId, 'EC2', tag(i) || i.InstanceId, i, {
        vpcId: i.VpcId || subVpcMap[i.SubnetId] || '',
        subnetId: i.SubnetId || '',
        az: i.Placement ? i.Placement.AvailabilityZone : '',
        config: i.InstanceType || '',
  ...
  (ctx.rdsInstances || []).forEach(function (db) {
    ...
    rows.push(
      mkRow(db.DBInstanceIdentifier, 'RDS', db.DBInstanceIdentifier, db, {
        vpcId: vpcId,
        az: db.AvailabilityZone || '',
        config: (db.Engine || '') + ' ' + (db.DBInstanceClass || ''),
  ...
  (ctx.lambdaFns || []).forEach(function (fn) {
    ...
    rows.push(
      mkRow(fn.FunctionName, 'Lambda', fn.FunctionName, fn, {
        vpcId: vpcId,
        subnetId: subId,
        config: (fn.Runtime || '') + (fn.MemorySize ? ' ' + fn.MemorySize + 'MB' : ''),
  ...
  (ctx.ecsServices || []).forEach(function (svc) {
    ...
    let mem = svc.memory || '';
    rows.push(
      mkRow(svc.serviceName, 'ECS', svc.serviceName, svc, {
        vpcId: vpcId,
        subnetId: subId,
        config: (svc.launchType || '') + ' ' + (cpu ? cpu + '/' : '') + (mem || ''),
        state: svc.status || ''
      })
    );
  });
  ...
  (ctx.albs || []).forEach(function (a) {
    rows.push(
      mkRow(a.LoadBalancerName, 'ALB', a.LoadBalancerName, a, {
        vpcId: a.VpcId || '',
        config: (a.Type || 'application') + ' ' + (a.Scheme || ''),
        state: a.State ? a.State.Code || '' : ''
      })
    );
  });
  // 8. ElastiCache
  (ctx.ecacheClusters || []).forEach(function (ec) {
    ...
    if (!vpcId && ec.CacheNodes && ec.CacheNodes[0]) {
      ...
      if (cn.SubnetId) {
        vpcId = subVpcMap[cn.SubnetId] || '';
      }
    }
    rows.push(
      mkRow(ec.CacheClusterId, 'ElastiCache', ec.CacheClusterId, ec, {
        vpcId: vpcId,
        config: (ec.Engine || '') + ' ' + (ec.CacheNodeType || ''),
        state: ec.CacheClusterStatus || ''
      })
    );
  });
  // 9. Redshift
  (ctx.redshiftClusters || []).forEach(function (rs) {
    rows.push(
      mkRow(rs.ClusterIdentifier, 'Redshift', rs.ClusterIdentifier, rs, {
        vpcId: rs.VpcId || '',
        config: (rs.NodeType || '') + ' x' + (rs.NumberOfNodes || 1),
        state: rs.ClusterStatus || '',
  ...
  (ctx.sgs || []).forEach(function (sg) {
    ...
    rows.push(
      mkRow(sg.GroupId, 'SG', sg.GroupName || sg.GroupId, sg, {
        vpcId: sg.VpcId || '',
        config: inCt + ' inbound / ' + outCt + ' outbound'
      })
    );
  });
  // 11. NACLs
  (ctx.nacls || []).forEach(function (n) {
    ...
    rows.push(
      mkRow(n.NetworkAclId, 'NACL', tag(n) || n.NetworkAclId, n, {
        vpcId: n.VpcId || '',
        config: ct + ' entries'
      })
    );
  ...
  (ctx.rts || []).forEach(function (rt) {
    ...
    rows.push(
      mkRow(rt.RouteTableId, 'Route Table', tag(rt) || rt.RouteTableId, rt, {
        vpcId: rt.VpcId || '',
        config: ct + ' routes'
      })
    );
  ...
  (ctx.igws || []).forEach(function (g) {
    ...
    rows.push(
      mkRow(g.InternetGatewayId, 'IGW', tag(g) || g.InternetGatewayId, g, {
        vpcId: attachedVpc,
        config: attachedVpc ? 'attached: ' + attachedVpc : 'detached'
      })
    );
  });
  // 14. NAT Gateways
  (ctx.nats || []).forEach(function (n) {
    rows.push(
      mkRow(n.NatGatewayId, 'NAT GW', tag(n) || n.NatGatewayId, n, {
        vpcId: n.VpcId || '',
        subnetId: n.SubnetId || '',
        config: (n.SubnetId || '') + ' ' + (n.State || ''),
        state: n.State || ''
      })
    );
  });
  ...
  (ctx.vpces || []).forEach(function (e) {
    rows.push(
      mkRow(e.VpcEndpointId, 'VPC Endpoint', tag(e) || e.VpcEndpointId, e, {
        vpcId: e.VpcId || '',
        config: e.ServiceName || '',
        state: e.State || ''
      })
    );
  });
  ...
  (ctx.enis || []).forEach(function (e) {
    rows.push(
      mkRow(e.NetworkInterfaceId, 'ENI', e.Description || e.NetworkInterfaceId, e, {
        vpcId: e.VpcId || '',
        subnetId: e.SubnetId || '',
        az: e.AvailabilityZone || '',
        config: e.PrivateIpAddress || '',
        state: e.Status || ''
      })
    );
  });
  ...
  (ctx.volumes || []).forEach(function (vol) {
    const attInsts = (vol.Attachments || [])
      .map(function (a) {
        return a.InstanceId;
      })
      .filter(Boolean);
    let vpcId = '';
    if (attInsts.length) {
      vpcId = instVpcMap[attInsts[0]] || '';
    }
    rows.push(
      mkRow(vol.VolumeId, 'EBS Volume', tag(vol) || vol.VolumeId, vol, {
        vpcId: vpcId,
        az: vol.AvailabilityZone || '',
        config: vol.Size + 'GB ' + (vol.VolumeType || ''),
        state: vol.State || '',
  ...
  (ctx.snapshots || []).forEach(function (snap) {
    rows.push(
      mkRow(snap.SnapshotId, 'Snapshot', snap.Description || snap.SnapshotId, snap, {
        config: (snap.VolumeSize || '') + 'GB',
        state: snap.State || '',
  ...
  (ctx.zones || []).forEach(function (z) {
    const recs =
      ctx.recsByZone && ctx.recsByZone[z.Id]
        ? ctx.recsByZone[z.Id].length
  ...
  (ctx.wafAcls || []).forEach(function (w) {
    ...
    rows.push(
      mkRow(w.Id || w.Name, 'WAF', w.Name || w.Id || '', w, { config: ruleCount + ' rules' })
    );
  });
  // 22. CloudFront
  (ctx.cfDistributions || []).forEach(function (cf) {
    rows.push(
      mkRow(cf.Id, 'CloudFront', cf.DomainName || cf.Id, cf, {
        config: cf.Status || '',
        state: cf.Status || ''
      })
    );
  ...
  (ctx.peerings || []).forEach(function (p) {
    ...
    rows.push(
      mkRow(p.VpcPeeringConnectionId, 'VPC Peering', tag(p) || p.VpcPeeringConnectionId, p, {
        config: req + '\u2194' + acc,
        state: p.Status ? p.Status.Code || '' : ''
      })
    );
  });
  // 24. VPN
  (ctx.vpns || []).forEach(function (v) {
    rows.push(
      mkRow(v.VpnConnectionId, 'VPN', tag(v) || v.VpnConnectionId, v, {
        config: (v.State || '') + ' ' + (v.Type || ''),
        state: v.State || ''
      })
    );
  });
  ...
  (ctx.tgwAttachments || []).forEach(function (t) {
    ...
    rows.push(
      mkRow(tid, 'TGW Attachment', tag(t) || tid, t, {
        vpcId: t.VpcId || '',
        config: (t.ResourceType || '') + ' ' + (t.TransitGatewayId || ''),
        state: t.State || ''
      })
    );
  });
  ...
  (ctx.tgs || []).forEach(function (tg) {
    rows.push(
      mkRow(tg.TargetGroupName, 'Target Group', tg.TargetGroupName, tg, {
        vpcId: tg.VpcId || '',
        config: (tg.Protocol || '') + ':' + (tg.Port || ''),
        state: tg.TargetType || ''
      })
    );
  });
  ...
  (budrAssessments || []).forEach(function (a) {
    budrMap[a.id] = {
      tier: a.profile ? a.profile.tier : null,
      strategy: a.profile ? a.profile.strategy : null,
      rto: a.profile ? a.profile.rto : null,
      rpo: a.profile ? a.profile.rpo : null
    };
  });
  ...
  (complianceFindings || []).forEach(function (f) {
    ...
    if (!compMap[f.resource]) {
      compMap[f.resource] = { pass: 0, fail: 0 };
    }
    if (f.status === 'PASS') {
      compMap[f.resource].pass++;
    } else {
      compMap[f.resource].fail++;
    }
  });
...
function _filterInventory() {
  ...
  const filterFn =
    (typeof window !== 'undefined' && window._udashFilterByAccount) ||
    function (x) {
      return x;
    };
  let items = filterFn(_inventoryData).slice();
  if (st.typeFilter !== 'all') {
    items = items.filter(function (r) {
      return r.type === st.typeFilter;
    });
  }
  if (st.regionFilter !== 'all') {
    items = items.filter(function (r) {
      return r.region === st.regionFilter;
    });
  }
  if (st.accountFilter !== 'all') {
    items = items.filter(function (r) {
  ...
  rules.forEach(function (rule) {
    ...
    if (rule.scope === 'any') {
      text =
        ...
        (vpcName || '') +
        ' ' +
        Object.values(tagMap).join(' ');
    } else if (rule.scope === 'vpc') {
      text = vpcName || '';
    } else if (rule.scope === 'type') {
      text = type || '';
    } else if (rule.scope === 'name') {
      text = name || '';
    } else if (rule.scope.indexOf('tag:') === 0) {
      text = tagMap[rule.scope.substring(4)] || '';
    } else {
      text = (name || '') + ' ' + (type || '') + ' ' + (vpcName || '');
    }
  ...
  function scan(arr, typeName) {
    (arr || []).forEach(function (obj) {
      ...
      tagArr.forEach(function (t) {
        if (!t.Key || t.Key.indexOf('aws:') === 0) {
          return;
        }
        if (!disc[t.Key]) {
          disc[t.Key] = { count: 0, samples: [], types: {} };
        }
        const d = disc[t.Key];
        d.count++;
        d.types[typeName] = true;
        if (d.samples.length < 5 && t.Value && d.samples.indexOf(t.Value) < 0) {
          d.samples.push(t.Value);
        }
      });
    });
  }
...
function runClassificationEngine(ctx) {
  ...
  (ctx.subnets || []).forEach(function (s) {
    if (s.VpcId) {
      subnetVpcMap[s.SubnetId] = s.VpcId;
    }
  });
  ...
  (ctx.instances || []).forEach(function (inst) {
    ...
    results.push({
      ...
      rpo: meta.rpo,
      rto: meta.rto,
      auto: !_classificationOverrides[inst.InstanceId],
      vpcId: vpcId,
      vpcName: vpcName,
      tags: tm
    });
  ...
  (ctx.rdsInstances || []).forEach(function (db) {
    ...
    results.push({
      ...
      rpo: meta.rpo,
      rto: meta.rto,
      auto: !_classificationOverrides[name],
      vpcId: vpcId,
      vpcName: vpcName,
      tags: tm
    });
  ...
  (ctx.lambdaFns || []).forEach(function (fn) {
    ...
    if (!vpcId && fn.VpcConfig && fn.VpcConfig.SubnetIds && fn.VpcConfig.SubnetIds[0]) {
      vpcId = subnetVpcMap[fn.VpcConfig.SubnetIds[0]] || '';
    }
    const vpcName = vpcNameMap[vpcId] || '';
  ...
  (ctx.sgs || []).forEach(function (sg) {
    ...
    results.push({
      ...
      rpo: meta.rpo,
      rto: meta.rto,
      auto: !_classificationOverrides[id],
      vpcId: vpcId,
      vpcName: vpcName,
      tags: tm
    });
...
function prepareIAMReviewData(iamData) {
  ...
  (iamData.roles || []).forEach(function (role) {
    ...
    const lastUsed =
      role.RoleLastUsed && role.RoleLastUsed.LastUsedDate
        ? new Date(role.RoleLastUsed.LastUsedDate)
        : null;
    ...
    const findings = (complianceFindings || []).filter(function (f) {
      return (
        f.framework === 'IAM' && (f.resource === role.RoleName || f.resource === (role.Arn || ''))
      );
    });
    const policyCount =
      (role.RolePolicyList || []).length + (role.AttachedManagedPolicies || []).length;
    ...
    items.push({
      ...
      findings: findings,
      _accountId: roleAcct ? roleAcct[1] : '',
      _raw: role
    });
  });
  // Process users
  (iamData.users || []).forEach(function (user) {
    ...
    const findings = (complianceFindings || []).filter(function (f) {
      return (
        f.framework === 'IAM' && (f.resource === user.UserName || f.resource === (user.Arn || ''))
      );
    });
    const policyCount =
      (user.UserPolicyList || []).length + (user.AttachedManagedPolicies || []).length;
    ...
    items.push({
      ...
      policies: policyCount,
      policyNames: policyNames,
      permBoundary: user.PermissionsBoundary ? user.PermissionsBoundary.PermissionsBoundaryArn : '',
      findings: findings,
      _accountId: userAcct ? userAcct[1] : '',
      hasMFA: hasMFA,
...
function matchResource(pattern, arn) {
  ...
  if (pParts.length !== aParts.length && pParts.length < 6) {
    return false;
  }
  const reStr = pParts
    .map((p, i) => {
      if (p === '*' && i === pParts.length - 1) {
        return '.*';
      }
      if (p === '*') {
        return '[^:]*';
...
function evaluateCondition(condBlock, context) {
  if (!condBlock || Object.keys(condBlock).length === 0) {
    return true;
  }
  for (const [op, keys] of Object.entries(condBlock)) {
    for (const [ck, cv] of Object.entries(keys)) {
      if (op === 'Bool' && ck === 'aws:MultiFactorAuthPresent') {
        if (context && context.mfa !== undefined) {
          return String(context.mfa) === String(cv);
        }
      }
      if (op === 'StringEquals' || op === 'StringLike') {
        if (context && context[ck]) {
          const vals = Array.isArray(cv) ? cv : [cv];
          if (!vals.some((v) => v === context[ck])) {
            return false;
          }
        }
      }
...
function _collectStatements(principal, iamData) {
  ...
  (principal.AttachedManagedPolicies || []).forEach((mp) => {
    const pol = (iamData.policies || []).find(
      (p) => p.Arn === mp.PolicyArn || p.PolicyName === mp.PolicyName
    );
    if (pol) {
...
function canDo(principal, action, resource, iamData) {
  ...
  for (const s of stmts) {
    ...
    if (acts.some((a) => matchAction(a, action)) && res.some((r) => matchResource(r, resource))) {
      if (evaluateCondition(s.Condition)) {
        return { effect: 'DENY', reason: 'Explicit deny in policy', statements: [s] };
      }
    }
  }
  if (principal.PermissionsBoundary) {
    const bArn =
      principal.PermissionsBoundary.PermissionsBoundaryArn || principal.PermissionsBoundary;
    ...
    if (bPol) {
      ...
      if (ver) {
        ...
        const allowed = bStmts.some((s) => {
          ...
          return (
            ba.some((a) => matchAction(a, action)) && br.some((r) => matchResource(r, resource))
          );
        });
        if (!allowed) {
          return {
  ...
  if (matchedStmts.length) {
    return { effect: 'ALLOW', reason: 'Allowed by policy', statements: matchedStmts };
  }
  ...
  stmts.forEach((s) => {
    ...
    if (s.NotAction) {
      ...
      na.forEach((a) => {
        ...
        if (!services[svc]) {
          services[svc] = { allowed: [], denied: [], resources: {} };
        }
    ...
    acts.forEach((a) => {
      ...
      if (s.Effect === 'Allow') {
        if (!services[svc].allowed.includes(actionName)) {
          services[svc].allowed.push(actionName);
        }
        res.forEach((r) => {
          services[svc].resources[actionName] = r;
        });
      } else if (s.Effect === 'Deny') {
        if (!services[svc].denied.includes(actionName)) {
          services[svc].denied.push(actionName);
        }
      }
    });
    if (res.some((r) => r === '*')) {
