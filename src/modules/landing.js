...
function renderLandingZoneMap(ctx) {
  ...
  lzAllRecSets.forEach((r) => {
    if (r.HostedZoneId) (lzRecsByZone[r.HostedZoneId] = lzRecsByZone[r.HostedZoneId] || []).push(r);
  });
  ...
  vL.forEach((vl) => {
    ...
    const _lzVpcLabels = buildVpcHeaderLabels({
      vpc: vl.vpc,
      width: vl.w,
      region,
      isMultiAccount: _multiAccount,
      nameOf: gn,
      nameLengthRatio: 0.55,
      accountSeparator: ' '
    });
    vG.append('text')
      ...
      .attr('x', vl.x + 12)
      .attr('y', vl.y + 16)
      .attr('textLength', _lzVpcLabels.nameTextLength)
      .attr('lengthAdjust', 'spacing')
      .text(_lzVpcLabels.name);
    ...
    if (_multiAccount && vl.vpc._accountId !== 'default') {
      ...
      if (lzAcCol) {
        vG.append('rect')
          ...
          .attr('rx', 2)
          .attr('opacity', 0.7);
        const lzAcLbl = buildAccountStripeLabel({
  ...
  function lzHlVpc(vid) {
    ...
    const hasInet = ndL
      .selectAll('.lz-gw-node[data-vpc="' + vid + '"]')
      .nodes()
      .some((n) => {
  ...
  const lzMainRT = {};
  rts.forEach((rt) => {
    if ((rt.Associations || []).some((a) => a.Main)) lzMainRT[rt.VpcId] = rt;
  });
  const lzSubRT = {};
  rts.forEach((rt) => {
    (rt.Associations || []).forEach((as) => {
      if (as.SubnetId) lzSubRT[as.SubnetId] = rt;
    });
  });
  subnets.forEach((s) => {
    if (!lzSubRT[s.SubnetId] && lzMainRT[s.VpcId]) lzSubRT[s.SubnetId] = lzMainRT[s.VpcId];
  });
