...
function _landingGatewaySelector(vpcId) {
  return '.lz-gw-node' + _dataAttrSelector('data-vpc', vpcId);
}
...
function _setLandingMapMoving(isMoving) {
  ...
  if (isMoving) {
    if (_landingMapMoveTimer) {
      clearTimeout(_landingMapMoveTimer);
      _landingMapMoveTimer = null;
    }
...
function _createLandingMapZoom(svg, g) {
  ...
  return zoom
    .on('start', () => {
      _setLandingMapMoving(true);
    })
    .on('zoom', (e) => {
      _applyLandingZoomTransform(g, e.transform);
      _updateLandingZoomLabel(e.transform.k);
    })
    .on('end', (e) => {
      _flushLandingZoomTransform(g, e.transform);
      _setLandingMapMoving(false);
    });
}
...
function renderLandingZoneMap(ctx) {
  ...
  const lzRecsByZone = {};
  lzAllRecSets.forEach((r) => {
    if (r.HostedZoneId) {
      (lzRecsByZone[r.HostedZoneId] = lzRecsByZone[r.HostedZoneId] || []).push(r);
    }
  });
  ...
  const HUB_X = 160,
    HUB_Y = 100;
  const SPOKE_START_X = 600,
    SPOKE_START_Y = 60;
  const SPOKE_COL_GAP = 200,
    SPOKE_ROW_GAP = 40; // Tighter gaps for flatter layout
  ...
  if (_detailLevel > 0) {
    subnets.forEach((s) => {
      lzSubTrees[s.SubnetId] = buildResTree(s.SubnetId, lzTreeCtx);
    });
  }
  ...
  function lzBuildSubLayouts(ss, baseX, baseY, sw) {
    ...
    sorted.forEach((s) => {
      ...
      if (az && lastAZ !== null && az !== lastAZ) {
        cy += LZ_AZ_HDR + SG;
        layouts.push({ azLabel: az, x: baseX + VP, y: cy - LZ_AZ_HDR - SG / 2, w: sw });
  ...
  function calcVpcSize(vpc, maxW) {
    ...
    ss.forEach((s) => {
      sw = Math.max(sw, gn(s, s.SubnetId).length * CW + 100);
    });
    sw = Math.min(sw, maxW || 350);
    let subnetHeight = 0;
    const sorted = lzSortByAZ(ss);
    sorted.forEach((s, i) => {
      subnetHeight += lzSubH(s.SubnetId) + (i < sorted.length - 1 ? SG : 0);
    });
  ...
  const minSpokeX = spokeLayouts.length
    ? Math.min(...spokeLayouts.map((s) => s.x))
    : actualSpokeStartX;

  // SVG setup
  const g = svg.append('g').attr('class', 'map-root');
  const zB = _createLandingMapZoom(svg, g);
  svg.call(zB);
  _mapSvg = svg;
  _mapZoom = zB;
  _mapG = g;
  ...
  if (tgwId) {
    ...
    if (!lzVpcRoutes.has(hubVpc.VpcId)) {
      lzVpcRoutes.set(hubVpc.VpcId, []);
    }
  ...
  if (tgwId) {
    ...
    tgwG
      .on('mouseenter', function () {
        ...
        // show all nodes
        ndL.selectAll('.lz-tgw-node,.lz-gw-node,.internet-node').classed('lz-hl', true);
        ...
        lzAllPaths.forEach((e) => {
          try {
            const d = e.p.attr('d'),
              s = e.p.attr('stroke');
            if (d) {
              const p = lzOlL.append('path').attr('d', d).style('stroke', s);
              Object.entries(olS).forEach(([k, v]) => p.style(k, v));
            }
          } catch (ex) {
            console.warn('Path overlay error:', ex);
          }
  ...
  vL.forEach((vl) => {
    ...
    vG.append('text')
      ...
      .attr('x', vl.x + 12)
      .attr('y', vl.y + 16)
      .attr('textLength', Math.min(_lzVpcName.length * 8, vl.w * 0.55))
    ...
    headerRect
      .on('mouseenter', function () {
        ...
        if (vpcVpces.length) {
          h +=
            '<div class="tt-sec"><div class="tt-sh">VPC Endpoints (' + vpcVpces.length + ')</div>';
          vpcVpces.slice(0, 5).forEach((v) => {
            const vi = vpces.find((x) => x.VpcEndpointId === v.id);
            h +=
              '<div class="tt-r"><span class="i">' +
              esc(vi?.ServiceName?.split('.').pop() || '?') +
              '</span></div>';
          });
          if (vpcVpces.length > 5) {
            h += '<div class="tt-r">... and ' + (vpcVpces.length - 5) + ' more</div>';
          }
          h += '</div>';
        }
        tt.innerHTML = h;
  ...
  function lzShowLock(v) {
    lzLockInd.style.display = v ? 'block' : 'none';
  }
  ...
  function lzHlVpc(vid) {
    ...
    ndL.selectAll(_landingGatewaySelector(vid)).classed('lz-hl', true);
    // Show NET if VPC has IGW or NAT
    const hasInet = ndL
      .selectAll(_landingGatewaySelector(vid))
      .nodes()
      .some((n) => {
        const t = n.getAttribute('data-gwtype');
        return t === 'IGW' || t === 'NAT';
      });
    if (hasInet) {
      ndL.selectAll('.internet-node').classed('lz-hl', true);
    }
  ...
  if (window._lzHlUnlockHandler) {
    document.removeEventListener('hl-unlock', window._lzHlUnlockHandler);
  }
  window._lzHlUnlockHandler = lzForceClr;
  document.addEventListener('hl-unlock', lzForceClr);
  svg.on('click', function (event) {
    if (
      !event.target.closest('.lz-gw-node') &&
      !event.target.closest('.lz-tgw-node') &&
  ...
  rts.forEach((rt) => {
    if ((rt.Associations || []).some((a) => a.Main)) {
      lzMainRT[rt.VpcId] = rt;
    }
  });
  const lzSubRT = {};
  rts.forEach((rt) => {
    (rt.Associations || []).forEach((as) => {
      if (as.SubnetId) {
        lzSubRT[as.SubnetId] = rt;
      }
    });
  });
  subnets.forEach((s) => {
    if (!lzSubRT[s.SubnetId] && lzMainRT[s.VpcId]) {
      lzSubRT[s.SubnetId] = lzMainRT[s.VpcId];
    }
  });
    ...
    vl.subs
      ...
      .forEach((sl) => {
        ...
        ndL
          .append('line')
          .attr('x1', sl.x + 38)
          .attr('y1', sl.y + LZ_AZ_HDR - 5)
          .attr('x2', sl.x + sl.w)
          .attr('y2', sl.y + LZ_AZ_HDR - 5)
      ...
      .forEach((sl) => {
        ...
        if (_detailLevel === 0 && lzTree.length > 0) {
          ...
          sG.append('text')
            ...
            .attr('y', sl.y + sl.h - 5)
            .attr('font-family', 'Segoe UI,system-ui,sans-serif')
            .style('font-size', 'calc(5px * var(--txt-scale,1))')
            .attr('fill', 'var(--text-muted)')
            .attr('opacity', 0.5)
            .text(summary);
          ...
          lzTree.forEach((res, ri) => {
            ...
            rG.append('text')
              ...
              .attr('font-family', 'Segoe UI,system-ui,sans-serif')
              .style('font-size', 'calc(5.5px * var(--txt-scale,1))')
              .attr('font-weight', '600')
            ...
            if (res.ip) {
              rG.append('text')
                ...
                .attr('clip-path', `url(#${nameClip})`)
                .attr('font-family', 'Segoe UI,system-ui,sans-serif')
                .style('font-size', 'calc(4.5px * var(--txt-scale,1))')
                .attr('fill', 'var(--text-muted)')
                .text(res.ip);
            }
            if (res.state) {
            ...
            if (nCh > 0) {
              (res.children || []).forEach((ch, ci) => {
                ...
                rG.append('text')
                  ...
                  .attr('y', cy2 + ch2 / 2 + 2)
                  .attr('font-family', 'Segoe UI,system-ui,sans-serif')
                  .style('font-size', 'calc(4px * var(--txt-scale,1))')
                  .attr('font-weight', '600')
                  .attr('fill', ch.col)
                  .text(ch.type);
                rG.append('text')
        ...
        sG.on('mouseenter', function () {
          ...
          h +=
            '<div class="tt-r">AZ: <span class="i">' +
            esc(sl.sub.AvailabilityZone || 'N/A') +
            '</span></div>';
          h += '<div class="tt-r">ID: <span class="i">' + esc(sl.sub.SubnetId) + '</span></div>';
          h += '</div>';
          if (subSi.length) {
            h +=
              '<div class="tt-sec"><div class="tt-sh">EC2 Instances (' + subSi.length + ')</div>';
            subSi.slice(0, 5).forEach((inst) => {
              ...
              h +=
                ...
                esc(nm) +
                '</span> ' +
                esc(inst.InstanceType) +
                ' [' +
                esc(inst.State?.Name) +
                ']</div>';
            });
            if (subSi.length > 5) {
              h += '<div class="tt-r">... and ' + (subSi.length - 5) + ' more</div>';
            }
            h += '</div>';
          }
          if (subSa.length) {
            h +=
              '<div class="tt-sec"><div class="tt-sh">Load Balancers (' + subSa.length + ')</div>';
            subSa.forEach((lb) => {
              h +=
                '<div class="tt-r"><span class="i">' +
                esc(lb.LoadBalancerName || 'ALB') +
                '</span> ' +
                esc(lb.Type) +
                '</div>';
            });
  ...
  hubGws.forEach((gw, i) => {
    ...
    const gG = ndL
      .append('g')
      .attr('class', 'lz-gw-node')
      .attr('data-vpc', hubVpc.VpcId)
      .attr('data-gwtype', gw.type)
      .style('cursor', 'pointer');
    ...
    gG.append('text')
      ...
      .attr('font-family', 'Segoe UI,system-ui,sans-serif')
      .style('font-size', 'calc(7px * var(--txt-scale,1))')
      .attr('fill', gwNm ? 'var(--text-secondary)' : 'var(--text-muted)')
      .text(lzLblTxt);
    ...
    gG.on('mouseenter', function () {
      ...
      if (gw.type === 'NAT') {
        ...
        if (natInfo) {
          ...
          h +=
            '<div class="tt-r">Subnet: <span class="i">' +
            esc(natInfo.SubnetId || 'N/A') +
            '</span></div>';
          h +=
            '<div class="tt-r">State: <span class="i">' +
            esc(natInfo.State || 'N/A') +
            '</span></div>';
          h += '</div>';
        }
      }
    ...
    lzVpcRoutes
      .get(hubVpc.VpcId)
      .push({ d: `M${gx + GR},${hubConnY} L${hubLayout.x},${hubConnY}`, stroke: col });
  ...
  spokeLayouts.forEach((spoke) => {
    ...
    lzRouteG
      ...
      .attr(
        'd',
        `M${spoke.x + spoke.w},${flowY} L${spoke.x + spoke.w + 20},${flowY} L${spoke.x + spoke.w + 20},${spoke.y + 30} L${sgX - 16},${spoke.y + 30}`
      )
    ...
    spokeGws.forEach((gw, i) => {
      ...
      gG.append('text')
        ...
        .attr('font-family', 'Segoe UI,system-ui,sans-serif')
        .style('font-size', 'calc(7px * var(--txt-scale,1))')
        .attr('fill', sgNm ? 'var(--text-secondary)' : 'var(--text-muted)')
        .text(skLblTxt);
      ...
      if (!lzVpcRoutes.has(spoke.vpc.VpcId)) {
        lzVpcRoutes.set(spoke.vpc.VpcId, []);
      }
      lzVpcRoutes
        .get(spoke.vpc.VpcId)
        .push({ d: `M${spoke.x + spoke.w},${sgY} L${sgX - 16},${sgY}`, stroke: col });

      // Gateway tooltip
      const gwName = gwNames[gw.id] || sid(gw.id);
      gG.on('mouseenter', function () {
        if (!_lzLocked) {
          lzHlVpc(spoke.vpc.VpcId);
        }
        let h = '<div class="tt-title">' + esc(gwName) + '</div>';
        h += '<div class="tt-sub">' + esc(gw.type) + ' Gateway</div>';
        h += '<div class="tt-sec"><div class="tt-sh">Details</div>';
        h += '<div class="tt-r">ID: <span class="i">' + esc(gw.id) + '</span></div>';
        h +=
          '<div class="tt-r">VPC: <span class="i">' +
          gn(spoke.vpc, spoke.vpc.VpcId) +
          '</span></div>';
        h += '</div>';
        tt.innerHTML = h;
        tt.style.display = 'block';
    ...
    spokeGws.forEach((gw, i) => {
      if (gw.type === 'IGW' || gw.type === 'NAT') {
        spokeInetGws.push({
          ...
          sgY,
          vid: spoke.vpc.VpcId,
          col: gw.type === 'IGW' ? 'var(--igw-color)' : 'var(--nat-color)'
        });
      }
      sgY += 50;
    });
  ...
  if (spokeInetGws.length && igwGws.length) {
    ...
    spokeInetGws.forEach((g, i) => {
      ...
      if (!lzVpcRoutes.has(g.vid)) {
        lzVpcRoutes.set(g.vid, []);
      }
  ...
  if (zones.length > 0) {
    ...
    const totalContentH = lzDnsRecExp
      ? lzZoneLayouts.length > 0
        ? lzZoneLayouts[lzZoneLayouts.length - 1].y + lzZoneLayouts[lzZoneLayouts.length - 1].h
        : 0
    ...
    lzTogG
      ...
      .attr('font-weight', '600')
      .attr('fill', '#0ea5e9')
      .text(lzDnsRecExp ? '\u25B2 Collapse' : '\u25BC Expand');
  ...
  legG
    ...
    .text('LANDING ZONE LAYOUT');
  const items = [
    { c: '#7C3AED', t: 'Hub VPC' },
    { c: 'var(--vpc-stroke)', t: 'Spoke VPC' },
    { c: 'var(--tgw-color)', t: 'Transit GW' },
    { c: 'var(--igw-color)', t: 'Internet GW' },
    { c: 'var(--nat-color)', t: 'NAT GW' },
    { c: 'var(--subnet-public)', t: 'Public' },
    { c: 'var(--subnet-private)', t: 'Private' }
  ];
  items.forEach((it, i) => {
    const ix = legX + i * 100;
    legG
...
function renderExecutiveOverview(ctx) {
  ...
  const vpcStats = vpcs.map((v) => {
    ...
    ss.forEach((s) => {
      ...
      (instBySub[s.SubnetId] || []).forEach((i) => {
        volC += (volByInst[i.InstanceId] || []).length;
      });
    });
    ...
    const region =
      ss.find((s) => s.AvailabilityZone)?.AvailabilityZone?.replace(/[a-z]$/, '') || '';
    return {
      vpc: v,
      subCount: ss.length,
      ec2,
  ...
  const CARD_W = 300,
    CARD_H = 220,
    CARD_GAP = 40,
    COLS = Math.max(2, Math.min(4, Math.ceil(Math.sqrt(vpcs.length))));
  const startX = 60;
  const _hdrStats = [
  ...
  // Peering connections
  peerings.forEach((p) => {
    const aVpc = p.AccepterVpcInfo?.VpcId,
      rVpc = p.RequesterVpcInfo?.VpcId;
    const pa = vpcPositions.get(aVpc),
      pr = vpcPositions.get(rVpc);
    if (!pa || !pr) {
      return;
    }
    const mx = (pa.cx + pr.cx) / 2,
      my = (pa.cy + pr.cy) / 2;
    connG
      .append('path')
      .attr('d', `M${pa.cx},${pa.cy} Q${mx},${my - 30} ${pr.cx},${pr.cy}`)
      .attr('fill', 'none')
  ...
  shGws
    .filter((g2) => g2.type === 'TGW')
    .forEach((tgw) => {
      const connected = [];
      vpcs.forEach((v) => {
        const ss = subByVpc[v.VpcId] || [];
        const rts2 = ctx.rts.filter(
          (rt) =>
            rt.VpcId === v.VpcId ||
            (rt.Associations || []).some((a) => ss.some((s) => s.SubnetId === a.SubnetId))
        );
        const hasTgw = rts2.some((rt) =>
          (rt.Routes || []).some((r) => r.TransitGatewayId === tgw.id)
        );
        if (hasTgw) {
          connected.push(v.VpcId);
        }
      });
      if (connected.length > 1) {
        tgwVpcs.set(tgw.id, connected);
      }
    });
  ...
  vpcStats.forEach((vs, i) => {
    ...
    resTypes.forEach((rt, ri) => {
      ...
      barG
        ...
        .attr('y', by + 9)
        .attr('font-family', 'Segoe UI,system-ui,sans-serif')
        .style('font-size', 'calc(6.5px * var(--txt-scale,1))')
        .attr('fill', rt.col)
        .text(rt.label);
      barG
        .append('rect')
      ...
      barG
        ...
        .attr('fill', rt.col)
        .text(rt.count);
      barG.on('click', function (event) {
        event.stopPropagation();
        openResourceList(rt.rlType);
      });
      barG
        .on('mouseenter', function () {
          d3.select(this)
    ...
    if (vs.gwCount) {
      ...
      gwG2.on('click', function (event) {
        event.stopPropagation();
        openResourceList('Gateways');
      });
    ...
    if (vs.vpceCount) {
      ...
      vpceG2.on('click', function (event) {
        event.stopPropagation();
        openResourceList('Endpoints');
      });
    }
    ...
    if (vs.total > 0) {
      ...
      cG.append('rect')
        .attr('x', cx + CARD_W - tw - 8)
        .attr('y', gwY - 10)
        .attr('width', tw)
        .attr('height', 14)
        .attr('rx', 3)
        .attr('fill', 'rgba(99,102,241,.15)')
        .attr('stroke', 'rgba(99,102,241,.4)')
        .attr('stroke-width', 0.5);
      cG.append('text')
        .attr('x', cx + CARD_W - tw / 2 - 8)
        .attr('y', gwY + 1)
        .attr('text-anchor', 'middle')
        .attr('font-family', 'Segoe UI,system-ui,sans-serif')
        .style('font-size', 'calc(7px * var(--txt-scale,1))')
    ...
    cG.on('mouseenter', function () {
      ...
      h +=
        '<div class="tt-r">Region: <span class="i">' +
        esc(vs.region || 'Unknown') +
        '</span></div>';
  ...
  const _hdrX = startX - 12,
    _hdrY = 8;
  hdrG
    .append('rect')
    .attr('x', _hdrX)
    .attr('y', _hdrY)
    .attr('width', _hdrTotalW)
    .attr('height', _hdrH)
    .attr('rx', 8)
    .attr('fill', 'rgba(17,24,39,.85)')
    .attr('stroke', 'var(--border)');
  hdrG
    .append('text')
    .attr('x', startX)
  ...
  const _regionLabel =
    _regions.size === 1 ? [..._regions][0] : _regions.size > 1 ? 'Multi-Region' : '';
  if (_regionLabel) {
    hdrG
      .append('text')
      .attr('x', _hdrX + _hdrTotalW - 12)
      .attr('y', _hdrY + 22)
      .attr('text-anchor', 'end')
      .attr('font-family', 'Segoe UI,system-ui,sans-serif')
      .style('font-size', 'calc(10px * var(--txt-scale,1))')
  ...
  _hdrStats.forEach((s, i) => {
    const col = i % _hdrCols,
      row = Math.floor(i / _hdrCols);
    ...
    mcG
      ...
      .attr('x', cx)
      .attr('y', cy)
      .attr('width', _hdrCardW)
      .attr('height', _hdrCardH)
      .attr('rx', 4)
      .attr('fill', 'rgba(255,255,255,.03)')
      .attr('class', 'hdr-stat-bg');
    mcG
      .append('rect')
      .attr('x', cx)
      .attr('y', cy + 4)
      .attr('width', 3)
      .attr('height', _hdrCardH - 8)
      .attr('fill', s.c)
      .attr('rx', 1);
    mcG
      .append('text')
      ...
      .attr('y', cy + 16)
      .attr('font-family', 'Segoe UI,system-ui,sans-serif')
      .style('font-size', 'calc(12px * var(--txt-scale,1))')
      .attr('font-weight', '700')
      .attr('fill', s.c)
      .text(s.v);
    mcG
      ...
      .attr('fill', 'var(--text-muted)')
      .text(s.l);
    mcG.on('click', function () {
      openResourceList(s.t);
    });
    mcG
      .on('mouseenter', function () {
        d3.select(this).select('.hdr-stat-bg').attr('fill', 'rgba(255,255,255,.08)');
      })
      .on('mouseleave', function () {
        d3.select(this).select('.hdr-stat-bg').attr('fill', 'rgba(255,255,255,.03)');
      });
  });
  const _compY = _gridY + _hdrGridH + 4;
  let _compFindings = [];
  try {
    _compFindings = runComplianceChecks(_rlCtx) || [];
  } catch (e) {
    console.warn('Landing compliance check failed:', e);
  }
  const _compN = _compFindings.length;
  const _compColor =
    _compN === 0 ? '#22c55e' : _compN <= 20 ? '#eab308' : _compN <= 50 ? '#f97316' : '#ef4444';
  const _compG = hdrG.append('g').style('cursor', 'pointer');
  _compG
    .append('rect')
    .attr('x', startX)
    .attr('y', _compY)
    .attr('width', _hdrTotalW - 24)
    .attr('height', 22)
    .attr('rx', 4)
    .attr('fill', 'rgba(255,255,255,.03)')
    .attr('class', 'comp-bar-bg');
  _compG
    .append('circle')
    .attr('cx', startX + 12)
    .attr('cy', _compY + 11)
    .attr('r', 4)
    .attr('fill', _compColor);
  _compG
    .append('text')
    .attr('x', startX + 22)
    .attr('y', _compY + 15)
    ...
    .style('font-size', 'calc(9px * var(--txt-scale,1))')
    .attr('fill', _compColor)
    .text(_compN === 0 ? 'No findings' : _compN + ' finding' + (_compN === 1 ? '' : 's'));
  _compG.on('click', function () {
    renderCompliancePanel(_compFindings);
  });
  _compG
    .on('mouseenter', function () {
      d3.select(this).select('.comp-bar-bg').attr('fill', 'rgba(255,255,255,.08)');
    })
    .on('mouseleave', function () {
      d3.select(this).select('.comp-bar-bg').attr('fill', 'rgba(255,255,255,.03)');
    });
...
function _cachedParse(id) {
  ...
  if (!val || !val.trim()) {
    delete _parseCache[id];
    return null;
  }
  if (_parseCache[id] && _parseCache[id].raw === val) {
    return _parseCache[id].parsed;
  }
