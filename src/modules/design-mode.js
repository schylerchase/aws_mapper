...
export function validateDesignChange(change, ctx) {
  ...
  if (change.action === 'split_subnet') {
    ...
    if (prefix >= _awsConstraints.subnet.cidrPrefixMax) {
      ...
      if (usable < 16) {
        warnings.push('Very small subnets — limited IP capacity');
      }
    }
    const insts = ctx ? (ctx.instBySub || {})[change.target.SubnetId] || [] : [];
    if (insts.length) {
      warnings.push(insts.length + ' instance(s) will require IP-based migration');
    }
  }
  ...
  if (change.action === 'add_route') {
    ...
    if (!parseCIDR(dest) && dest !== '0.0.0.0/0') {
      errors.push('Invalid destination CIDR: ' + dest);
    }
    if (
...
export function _generateCLI(ch) {
  ...
  if (ch.action === 'add_vpc') {
    cmds.push(
      `aws ec2 create-vpc --cidr-block ${ch.params.CidrBlock}${ch.params.Name ? " --tag-specifications 'ResourceType=vpc,Tags=[{Key=Name,Value=" + ch.params.Name + "}]'" : ''}`
    );
    cmds.push('# Default route table, NACL, and SG are created automatically by AWS');
  }
  if (ch.action === 'add_subnet') {
    cmds.push(
      `aws ec2 create-subnet --vpc-id ${ch.params.VpcId} --cidr-block ${ch.params.CidrBlock} --availability-zone ${ch.params.AZ}${ch.params.Name ? " --tag-specifications 'ResourceType=subnet,Tags=[{Key=Name,Value=" + ch.params.Name + "}]'" : ''}`
    );
  }
  if (ch.action === 'split_subnet') {
    ...
    if (halves) {
      ...
      cmds.push(
        `aws ec2 create-subnet --vpc-id $VPC_ID --cidr-block ${halves[0]} --availability-zone ${az}${ch.params.names?.[0] ? " --tag-specifications 'ResourceType=subnet,Tags=[{Key=Name,Value=" + ch.params.names[0] + "}]'" : ''}`
      );
      cmds.push(
        `aws ec2 create-subnet --vpc-id $VPC_ID --cidr-block ${halves[1]} --availability-zone ${az}${ch.params.names?.[1] ? " --tag-specifications 'ResourceType=subnet,Tags=[{Key=Name,Value=" + ch.params.names[1] + "}]'" : ''}`
      );
    }
  }
  if (ch.action === 'add_gateway') {
    if (ch.params.GatewayType === 'IGW') {
      cmds.push(
        `aws ec2 create-internet-gateway${ch.params.Name ? " --tag-specifications 'ResourceType=internet-gateway,Tags=[{Key=Name,Value=" + ch.params.Name + "}]'" : ''}`
      );
      cmds.push(
        `aws ec2 attach-internet-gateway --internet-gateway-id $IGW_ID --vpc-id ${ch.params.VpcId}`
      );
    }
    if (ch.params.GatewayType === 'NAT') {
      cmds.push('# Allocate EIP first: aws ec2 allocate-address --domain vpc');
      cmds.push(
        `aws ec2 create-nat-gateway --subnet-id ${ch.params.SubnetId || '$SUBNET_ID'} --allocation-id $EIP_ALLOC_ID${ch.params.Name ? " --tag-specifications 'ResourceType=natgateway,Tags=[{Key=Name,Value=" + ch.params.Name + "}]'" : ''}`
      );
    }
    if (ch.params.GatewayType === 'VPCE') {
      cmds.push(
        `aws ec2 create-vpc-endpoint --vpc-id ${ch.params.VpcId} --service-name ${ch.params.ServiceName || 'com.amazonaws.REGION.s3'} --vpc-endpoint-type ${ch.params.EndpointType || 'Gateway'}`
      );
    }
  }
  if (ch.action === 'add_route') {
    cmds.push(
      `aws ec2 create-route --route-table-id ${ch.target.RouteTableId} --destination-cidr-block ${ch.params.DestinationCidrBlock} --gateway-id ${ch.params.TargetId}`
    );
  }
  if (ch.action === 'add_resource') {
    if (ch.params.ResourceType === 'EC2') {
      cmds.push(
        `aws ec2 run-instances --subnet-id ${ch.params.SubnetId} --instance-type ${ch.params.InstanceType || 't3.micro'} --image-id $AMI_ID${ch.params.Name ? " --tag-specifications 'ResourceType=instance,Tags=[{Key=Name,Value=" + ch.params.Name + "}]'" : ''}`
      );
    }
    if (ch.params.ResourceType === 'RDS') {
      cmds.push(
        `aws rds create-db-instance --db-instance-identifier ${ch.params.Name || 'new-db'} --db-instance-class ${ch.params.InstanceClass || 'db.t3.micro'} --engine ${ch.params.Engine || 'mysql'} --master-username admin --master-user-password $DB_PASSWORD`
      );
    }
    if (ch.params.ResourceType === 'Lambda') {
      cmds.push(
        `aws lambda create-function --function-name ${ch.params.Name || 'new-function'} --runtime ${ch.params.Runtime || 'nodejs18.x'} --role $LAMBDA_ROLE_ARN --handler index.handler --zip-file fileb://function.zip --vpc-config SubnetIds=${ch.params.SubnetId}`
      );
    }
    if (ch.params.ResourceType === 'ElastiCache') {
      cmds.push(
        `aws elasticache create-cache-cluster --cache-cluster-id ${ch.params.Name || 'new-cache'} --cache-node-type ${ch.params.NodeType || 'cache.t3.micro'} --engine ${ch.params.Engine || 'redis'} --num-cache-nodes 1`
      );
    }
    if (ch.params.ResourceType === 'ECS') {
      cmds.push(
        `aws ecs create-service --cluster $CLUSTER --service-name ${ch.params.Name || 'new-service'} --task-definition $TASK_DEF --desired-count ${ch.params.DesiredCount || 1} --launch-type FARGATE --network-configuration "awsvpcConfiguration={subnets=[${ch.params.SubnetId}]}"`
      );
    }
    if (ch.params.ResourceType === 'Redshift') {
      cmds.push(
        `aws redshift create-cluster --cluster-identifier ${ch.params.Name || 'new-cluster'} --node-type ${ch.params.NodeType || 'dc2.large'} --master-username admin --master-user-password $RS_PASSWORD --number-of-nodes 1`
      );
    }
  }
  if (ch.action === 'add_security_group') {
    cmds.push(
      `aws ec2 create-security-group --group-name ${ch.params.Name || 'new-sg'} --description "${ch.params.Description || 'Design mode SG'}" --vpc-id ${ch.params.VpcId}`
    );
    if (ch.params.IngressRules) {
      ch.params.IngressRules.forEach((r) => {
        if (r.Protocol === '-1') {
          cmds.push(
            `aws ec2 authorize-security-group-ingress --group-id $SG_ID --protocol -1 --cidr ${r.CidrIp || '0.0.0.0/0'}`
          );
        } else {
          cmds.push(
            `aws ec2 authorize-security-group-ingress --group-id $SG_ID --protocol ${r.Protocol} --port ${r.FromPort} --cidr ${r.CidrIp || '0.0.0.0/0'}`
          );
        }
      });
    }
  ...
  if (ch.action === 'remove_resource') {
    ...
    if (t === 'EC2') {
      cmds.push(`aws ec2 terminate-instances --instance-ids ${id}`);
    }
    if (t === 'RDS') {
      cmds.push(`aws rds delete-db-instance --db-instance-identifier ${id} --skip-final-snapshot`);
    }
    if (t === 'Lambda') {
      cmds.push(`aws lambda delete-function --function-name ${id}`);
    }
    if (t === 'Subnet') {
      cmds.push(`aws ec2 delete-subnet --subnet-id ${id}`);
    }
    if (t === 'IGW') {
    ...
    if (t === 'NAT') {
      cmds.push(`aws ec2 delete-nat-gateway --nat-gateway-id ${id}`);
    }
    if (t === 'VPCE') {
      cmds.push(`aws ec2 delete-vpc-endpoints --vpc-endpoint-ids ${id}`);
    }
  }
  return cmds;
}
...
export function _generateWarnings() {
  ...
  if (splits.length) {
    w.push(splits.length + ' subnet split(s) require instance migration');
  }
  const removes = _designChanges.filter((c) => c.action === 'remove_resource');
  if (removes.length) {
    w.push(removes.length + ' resource removal(s) — verify dependencies first');
  }
  return w;
}
...
export function detectAZs(subnets) {
  ...
  (subnets || []).forEach((s) => {
    if (s.AvailabilityZone) {
      azSet.add(s.AvailabilityZone);
    }
  });
  if (azSet.size > 0) {
    return Array.from(azSet).sort();
  }
  if (_regionAZs[_designRegion]) {
    return _regionAZs[_designRegion];
  }
