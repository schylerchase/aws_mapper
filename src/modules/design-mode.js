...
export function setDesignMode(v) {
  _designMode = v;
}
...
export function setDesignChanges(v) {
  _designChanges = v;
}

export function getDesignBaseline() {
  return _designBaseline;
}
export function setDesignBaseline(v) {
  _designBaseline = v;
}

export function getDesignDebounce() {
  return _designDebounce;
}
export function setDesignDebounce(v) {
  _designDebounce = v;
}

export function getLastDesignValidation() {
  return _lastDesignValidation;
}
export function setLastDesignValidation(v) {
  _lastDesignValidation = v;
}

export function getSidebarWasCollapsed() {
  return _sidebarWasCollapsed;
}
export function setSidebarWasCollapsed(v) {
  _sidebarWasCollapsed = v;
}

export function getDesignRegion() {
  return _designRegion;
}
export function setDesignRegion(v) {
  _designRegion = v;
}
...
export function validateDesignChange(change, ctx) {
  ...
  if (change.action === 'add_vpc') {
    ...
    if (!isRfc1918)
      warnings.push('CIDR ' + p.CidrBlock + ' is not RFC 1918 — private subnets may have issues');
    ...
    vpcs.forEach((v) => {
      if (cidrOverlap(p.CidrBlock, v.CidrBlock))
        errors.push('Overlaps existing VPC ' + gn(v, v.VpcId) + ' (' + v.CidrBlock + ')');
    });
    if (vpcs.length >= _awsConstraints.vpc.maxPerRegion)
      warnings.push(
        'Exceeds default quota of ' +
          _awsConstraints.vpc.maxPerRegion +
          ' VPCs per region (adjustable)'
      );
  }
  ...
  if (change.action === 'add_subnet') {
    ...
    const prefix = parseInt(p.CidrBlock.split('/')[1], 10);
    if (
      prefix < _awsConstraints.subnet.cidrPrefixMin ||
      prefix > _awsConstraints.subnet.cidrPrefixMax
    )
    ...
    if (!cidrContains(vpc.CidrBlock, p.CidrBlock))
      errors.push('Subnet CIDR ' + p.CidrBlock + ' is not within VPC CIDR ' + vpc.CidrBlock);
    const vpcSubs = subnets.filter((s) => s.VpcId === p.VpcId);
    vpcSubs.forEach((s) => {
      if (cidrOverlap(p.CidrBlock, s.CidrBlock))
        errors.push('Overlaps subnet ' + gn(s, s.SubnetId) + ' (' + s.CidrBlock + ')');
    });
    if (vpcSubs.length >= _awsConstraints.subnet.maxPerVpc)
      warnings.push(
        'Exceeds default quota of ' + _awsConstraints.subnet.maxPerVpc + ' subnets per VPC'
      );
    const usable = Math.pow(2, 32 - prefix) - _awsConstraints.subnet.reservedIps;
    warnings.push(
      usable + ' usable IPs (' + _awsConstraints.subnet.reservedIps + ' reserved by AWS)'
    );
    ...
    if (vpcSubs.length > 0 && otherAZSubs.length === 0)
      warnings.push('All subnets in same AZ — consider multi-AZ for high availability');
  }

  if (change.action === 'split_subnet') {
    const prefix = parseInt((change.target.CidrBlock || '').split('/')[1], 10);
    if (prefix >= _awsConstraints.subnet.cidrPrefixMax)
      errors.push(
        'Cannot split /' +
          prefix +
          ' subnet (minimum is /' +
          _awsConstraints.subnet.cidrPrefixMax +
          ')'
      );
    else {
  ...
  if (change.action === 'add_gateway') {
    ...
    if (p.GatewayType === 'NAT') {
      if (p.SubnetId && !pubSubs.has(p.SubnetId))
        errors.push(
          'NAT Gateway must be placed in a public subnet (one with 0.0.0.0/0 → IGW route)'
        );
      const subnetAZ = (subnets.find((s) => s.SubnetId === p.SubnetId) || {}).AvailabilityZone;
      if (subnetAZ) {
        const azNats = nats.filter(
          (n) =>
            n.State !== 'deleted' &&
            (subnets.find((s) => s.SubnetId === n.SubnetId) || {}).AvailabilityZone === subnetAZ
        );
        if (azNats.length >= _awsConstraints.nat.maxPerAz)
          errors.push(
            'Exceeds limit of ' + _awsConstraints.nat.maxPerAz + ' NAT Gateways in ' + subnetAZ
          );
        else if (azNats.length > 0)
          warnings.push(
            subnetAZ + ' already has ' + azNats.length + ' NAT GW(s) — AWS recommends 1 per AZ'
          );
      }
    }
  }
  ...
  if (change.action === 'add_route') {
    ...
    if (
      _awsConstraints.routeTable.reservedDestinations.some(
        (r) => cidrContains(r, dest) || cidrContains(dest, r)
      )
    )
    ...
    if (rt) {
      ...
      if ((rt.Routes || []).length >= _awsConstraints.routeTable.maxRoutesPerTable)
        warnings.push(
          'Exceeds default quota of ' +
            _awsConstraints.routeTable.maxRoutesPerTable +
            ' routes per table'
        );
    }
  ...
  if (change.action === 'add_security_group') {
    ...
    if (vpcSgs.length >= _awsConstraints.securityGroup.maxPerVpc)
      warnings.push(
        'Exceeds default quota of ' + _awsConstraints.securityGroup.maxPerVpc + ' SGs per VPC'
      );
    if ((p.IngressRules || []).length > _awsConstraints.securityGroup.maxInboundRules)
      errors.push(
        'Exceeds limit of ' +
          _awsConstraints.securityGroup.maxInboundRules +
          ' inbound rules per SG'
      );
    (p.IngressRules || []).forEach((r) => {
      if (
        ...
        warnings.push(
          'Rule allows 0.0.0.0/0 on port ' + r.FromPort + ' — consider restricting source CIDR'
        );
    });
  }
    ...
    if (p.SubnetId) {
      ...
      if (sub) {
        ...
        if (remaining <= 0)
          ...
          warnings.push(
            'Subnet ' + gn(sub, sub.SubnetId) + ' has only ' + remaining + ' IPs remaining'
          );
      }
    }
  }
  ...
  if (change.action === 'remove_resource') {
    ...
    if (t.ResourceType === 'IGW') {
      const affectedRts = rts.filter((rt) =>
        (rt.Routes || []).some((r) => r.GatewayId === t.ResourceId)
      );
      if (affectedRts.length)
        warnings.push(
          affectedRts.length +
            ' route table(s) reference this IGW — subnets will lose internet access'
        );
    }
    if (t.ResourceType === 'NAT') {
      const affectedRts = rts.filter((rt) =>
        (rt.Routes || []).some((r) => r.NatGatewayId === t.ResourceId)
      );
      if (affectedRts.length)
        warnings.push(
          affectedRts.length +
            ' route table(s) route through this NAT — private subnets will lose outbound access'
        );
    }
    if (t.ResourceType === 'Subnet') {
      ...
      if (sub) {
        ...
        if (insts.length)
          warnings.push(insts.length + ' instance(s) in this subnet will be terminated');
        const subNats = nats.filter((n) => n.SubnetId === t.ResourceId);
        if (subNats.length)
          warnings.push(subNats.length + ' NAT Gateway(s) in this subnet will be deleted');
      }
    }
  }
  ...
  if (ctx) {
      ...
      for (let i = 0; i < subs.length; i++) {
        for (let j = i + 1; j < subs.length; j++) {
          if (cidrOverlap(subs[i].CidrBlock, subs[j].CidrBlock))
            errors.push(
              'Subnets ' +
                gn(subs[i], subs[i].SubnetId) +
                ' and ' +
                gn(subs[j], subs[j].SubnetId) +
    ...
    (ctx.nats || []).forEach((n) => {
      if (n.SubnetId && ctx.pubSubs && !ctx.pubSubs.has(n.SubnetId))
        warnings.push(
          'NAT Gateway ' + gn(n, n.NatGatewayId) + ' is in a private subnet — will not function'
        );
    });
    // Peering CIDR overlap check
    const peerPairs = [];
    (ctx.peerings || []).forEach((p) => {
      const rv = p.RequesterVpcInfo,
        av = p.AccepterVpcInfo;
      if (rv && av && rv.CidrBlock && av.CidrBlock) {
        if (cidrOverlap(rv.CidrBlock, av.CidrBlock))
          errors.push(
            'VPC Peering ' +
              gn(p, p.VpcPeeringConnectionId) +
              ' — CIDRs overlap: ' +
              rv.CidrBlock +
              ' / ' +
              av.CidrBlock
        ...
        if (peerPairs.includes(pair))
          warnings.push('Duplicate peering between ' + rv.VpcId + ' and ' + av.VpcId);
        peerPairs.push(pair);
      }
    });
  }
...
function _applyAddVpc(ch, getTa, setTa) {
  ...
  const vpc = {
    VpcId: id,
    CidrBlock: ch.params.CidrBlock,
    State: 'available',
    IsDefault: false,
    OwnerId: ch.params.AccountId || 'design',
    Tags: [{ Key: 'Name', Value: ch.params.Name || 'New VPC' }]
  };
  vpcs.push(vpc);
  ...
  rts.push({
    RouteTableId: rtId,
    VpcId: id,
    Associations: [{ Main: true, RouteTableId: rtId }],
    Routes: [
      {
        DestinationCidrBlock: ch.params.CidrBlock,
        GatewayId: 'local',
        State: 'active',
        Origin: 'CreateRouteTable'
      }
    ],
    Tags: [{ Key: 'Name', Value: (ch.params.Name || 'vpc') + '-main-rt' }]
  });
  ...
  nacls.push({
    ...
    Tags: [{ Key: 'Name', Value: (ch.params.Name || 'vpc') + '-default-nacl' }]
  });
  ...
  sgs.push({
    ...
    IpPermissions: [{ IpProtocol: '-1', IpRanges: [], UserIdGroupPairs: [{ GroupId: sgId }] }],
...
function _applyAddSubnet(ch, getTa, setTa) {
  ...
  subs.push({
    SubnetId: subId,
    VpcId: ch.params.VpcId,
    CidrBlock: ch.params.CidrBlock,
    AvailabilityZone: ch.params.AZ,
    MapPublicIpOnLaunch: ch.params.isPublic || false,
    Tags: [{ Key: 'Name', Value: ch.params.Name || 'New Subnet' }]
  });
...
function _applySplitSubnet(ch, getTa, setTa) {
  ...
  if (!halves) return;
  if (!ch.params.newIds)
    ch.params.newIds = ['subnet-split-a-' + Date.now(), 'subnet-split-b-' + Date.now()];
  const sub1 = {
    ...orig,
    SubnetId: ch.params.newIds[0],
    CidrBlock: halves[0],
    Tags: [{ Key: 'Name', Value: ch.params.names?.[0] || gn(orig, '') + '_a' }]
  };
  const sub2 = {
    ...orig,
    SubnetId: ch.params.newIds[1],
    CidrBlock: halves[1],
    Tags: [{ Key: 'Name', Value: ch.params.names?.[1] || gn(orig, '') + '_b' }]
  };
    ...
    reservations.forEach((res) => {
      (res.Instances || []).forEach((inst) => {
        if (inst.SubnetId === ch.target.SubnetId && inst.PrivateIpAddress) {
          inst.SubnetId = ipInCIDR(inst.PrivateIpAddress, halves[0])
            ? sub1.SubnetId
            : sub2.SubnetId;
        }
      });
...
function _applyAddGateway(ch, getTa, setTa) {
  ...
  if (type === 'IGW') {
    ...
    igws.push({
      ...
      Attachments: [{ VpcId: ch.params.VpcId, State: 'available' }],
      Tags: [{ Key: 'Name', Value: ch.params.Name || 'New IGW' }]
    });
    ...
    nats.push({
      ...
      State: 'available',
      Tags: [{ Key: 'Name', Value: ch.params.Name || 'New NAT' }]
    });
    ...
    vpces.push({
      VpcEndpointId: id,
      VpcId: ch.params.VpcId,
      ServiceName: ch.params.ServiceName || 'com.amazonaws.region.s3',
      VpcEndpointType: ch.params.EndpointType || 'Gateway',
      State: 'available',
      Tags: [{ Key: 'Name', Value: ch.params.Name || 'New VPCE' }]
    });
...
function _applyAddRoute(ch, getTa, setTa) {
  ...
  rt.Routes = rt.Routes || [];
  rt.Routes.push({
    DestinationCidrBlock: ch.params.DestinationCidrBlock,
    GatewayId: ch.params.TargetId,
    State: 'active'
  });
  setTa('in_rts', JSON.stringify({ RouteTables: rts }));
...
function _applyAddResource(ch, getTa, setTa) {
  ...
  if (type === 'EC2') {
      ...
      Instances: [
        {
          ...
          SubnetId: ch.params.SubnetId,
          VpcId: ch.params.VpcId,
          InstanceType: ch.params.InstanceType || 't3.micro',
          State: { Name: 'running' },
          PrivateIpAddress: ch.params.PrivateIp || '',
          Tags: [{ Key: 'Name', Value: ch.params.Name || 'New Instance' }]
        }
      ]
    });
    setTa('in_ec2', JSON.stringify({ Reservations: reservations }));
    ...
    rds.push({
      ...
      DBSubnetGroup: {
        VpcId: ch.params.VpcId,
        Subnets: [{ SubnetIdentifier: ch.params.SubnetId }]
      },
      DBInstanceClass: ch.params.InstanceClass || 'db.t3.micro',
      Engine: ch.params.Engine || 'mysql',
    ...
    clusters.push({
      CacheClusterId: id,
      CacheNodeType: ch.params.NodeType || 'cache.t3.micro',
      Engine: ch.params.Engine || 'redis',
      CacheClusterStatus: 'available',
      CacheNodes: [
        { CacheNodeId: '0001', Endpoint: { Address: id + '.cache.amazonaws.com', Port: 6379 } }
      ],
      ConfigurationEndpoint: { Address: id + '.cache.amazonaws.com', Port: 6379 }
    });
    ...
    ch.params.ResourceId = id;
    fns.push({
      FunctionName: ch.params.Name || 'new-function',
      FunctionArn: 'arn:aws:lambda:::function:' + (ch.params.Name || id),
      VpcConfig: {
        VpcId: ch.params.VpcId,
        SubnetIds: [ch.params.SubnetId],
        SecurityGroupIds: ch.params.SGIds || []
      },
      Runtime: ch.params.Runtime || 'nodejs18.x',
      MemorySize: 128
    });
    ...
    ch.params.ResourceId = id;
    svcs.push({
      serviceName: ch.params.Name || 'new-service',
      serviceArn: 'arn:aws:ecs:::service/' + id,
      networkConfiguration: {
        awsvpcConfiguration: {
          subnets: [ch.params.SubnetId],
          securityGroups: ch.params.SGIds || []
        }
      },
      runningCount: ch.params.DesiredCount || 1,
      desiredCount: ch.params.DesiredCount || 1,
      launchType: 'FARGATE'
    });
    ...
    clusters.push({
      ...
      NodeType: ch.params.NodeType || 'dc2.large',
      ClusterStatus: 'available',
      DBName: ch.params.DBName || 'dev',
      Endpoint: { Address: id + '.redshift.amazonaws.com', Port: 5439 },
      VpcId: ch.params.VpcId,
      ClusterSubnetGroupName: 'design-subnet-group'
    });
...
function _applyAddSecurityGroup(ch, getTa, setTa) {
  ...
  const sg = {
    GroupId: id,
    GroupName: ch.params.Name || 'new-sg',
    VpcId: ch.params.VpcId,
    Description: ch.params.Description || 'Design mode security group',
    IpPermissions: [],
    IpPermissionsEgress: [],
    Tags: [{ Key: 'Name', Value: ch.params.Name || 'new-sg' }]
  };
  if (ch.params.IngressRules) {
    ch.params.IngressRules.forEach((r) => {
      sg.IpPermissions.push({
        IpProtocol: r.Protocol || 'tcp',
        FromPort: r.FromPort,
        ToPort: r.ToPort,
        IpRanges: [{ CidrIp: r.CidrIp || '0.0.0.0/0' }]
      });
    });
  }
  sgs.push(sg);
...
function _applyRemoveResource(ch, getTa, setTa) {
  ...
  if (type === 'EC2') {
    ...
    setTa(
      'in_ec2',
      JSON.stringify({ Reservations: reservations.filter((r) => (r.Instances || []).length) })
    );
  } else if (type === 'RDS') {
    const raw = safeParse(getTa('in_rds'));
    const rds = raw ? ext(raw, ['DBInstances']) : [];
    setTa(
      'in_rds',
      JSON.stringify({
        DBInstances: rds.filter((d) => d.DBInstanceIdentifier !== ch.target.ResourceId)
      })
    );
  } else if (type === 'ElastiCache') {
    const raw = safeParse(getTa('in_elasticache'));
    ...
    setTa(
      'in_elasticache',
      JSON.stringify({ CacheClusters: c.filter((d) => d.CacheClusterId !== ch.target.ResourceId) })
    );
  } else if (type === 'Lambda') {
    const raw = safeParse(getTa('in_lambda'));
    const fns = raw ? ext(raw, ['Functions']) : [];
    setTa(
      'in_lambda',
      JSON.stringify({
        Functions: fns.filter(
          (f) => f.FunctionName !== ch.target.ResourceId && f.FunctionArn !== ch.target.ResourceId
        )
      })
    );
  } else if (type === 'Subnet') {
    ...
    setTa(
      'in_subnets',
      JSON.stringify({ Subnets: subs.filter((s) => s.SubnetId !== ch.target.ResourceId) })
    );
  } else if (type === 'Redshift') {
    const raw = safeParse(getTa('in_redshift'));
    ...
    setTa(
      'in_redshift',
      JSON.stringify({ Clusters: c.filter((d) => d.ClusterIdentifier !== ch.target.ResourceId) })
    );
  } else if (type === 'IGW') {
    const raw = safeParse(getTa('in_igws'));
    const igws = raw ? ext(raw, ['InternetGateways']) : [];
    setTa(
      'in_igws',
      JSON.stringify({
        InternetGateways: igws.filter((g) => g.InternetGatewayId !== ch.target.ResourceId)
      })
    );
  } else if (type === 'NAT') {
    const raw = safeParse(getTa('in_nats'));
    ...
    setTa(
      'in_nats',
      JSON.stringify({ NatGateways: nats.filter((g) => g.NatGatewayId !== ch.target.ResourceId) })
    );
  } else if (type === 'VPCE') {
    const raw = safeParse(getTa('in_vpces'));
    const vpces = raw ? ext(raw, ['VpcEndpoints']) : [];
    setTa(
      'in_vpces',
      JSON.stringify({
        VpcEndpoints: vpces.filter((g) => g.VpcEndpointId !== ch.target.ResourceId)
      })
    );
  }
}
...
function _shellArgOr(value, fallback) {
  return value ? _shellArg(value) : fallback;
}

function _tagSpec(resourceType, name) {
  return name
    ? ' --tag-specifications ' +
        _shellArg('ResourceType=' + resourceType + ',Tags=[{Key=Name,Value=' + name + '}]')
    : '';
}
...
export function _generateCLI(ch) {
  ...
  if (ch.action === 'add_vpc') {
    cmds.push(
      `aws ec2 create-vpc --cidr-block ${_shellArg(ch.params.CidrBlock)}${_tagSpec('vpc', ch.params.Name)}`
    );
    cmds.push('# Default route table, NACL, and SG are created automatically by AWS');
  }
  if (ch.action === 'add_subnet')
    cmds.push(
      `aws ec2 create-subnet --vpc-id ${_shellArg(ch.params.VpcId)} --cidr-block ${_shellArg(ch.params.CidrBlock)} --availability-zone ${_shellArg(ch.params.AZ)}${_tagSpec('subnet', ch.params.Name)}`
    );
  if (ch.action === 'split_subnet') {
    cmds.push('# Split subnet: delete original, create two new');
    cmds.push(`aws ec2 delete-subnet --subnet-id ${_shellArg(ch.target.SubnetId)}`);
    const halves = splitCIDR(ch.target.CidrBlock);
    if (halves) {
      const az = _shellArgOr(ch.params.AZ, '$AZ');
      cmds.push(
        `aws ec2 create-subnet --vpc-id $VPC_ID --cidr-block ${_shellArg(halves[0])} --availability-zone ${az}${_tagSpec('subnet', ch.params.names?.[0])}`
      );
      cmds.push(
        `aws ec2 create-subnet --vpc-id $VPC_ID --cidr-block ${_shellArg(halves[1])} --availability-zone ${az}${_tagSpec('subnet', ch.params.names?.[1])}`
      );
    }
  }
  if (ch.action === 'add_gateway') {
    if (ch.params.GatewayType === 'IGW') {
      cmds.push(`aws ec2 create-internet-gateway${_tagSpec('internet-gateway', ch.params.Name)}`);
      cmds.push(
        `aws ec2 attach-internet-gateway --internet-gateway-id $IGW_ID --vpc-id ${_shellArg(ch.params.VpcId)}`
      );
    }
    if (ch.params.GatewayType === 'NAT') {
      cmds.push('# Allocate EIP first: aws ec2 allocate-address --domain vpc');
      cmds.push(
        `aws ec2 create-nat-gateway --subnet-id ${_shellArgOr(ch.params.SubnetId, '$SUBNET_ID')} --allocation-id $EIP_ALLOC_ID${_tagSpec('natgateway', ch.params.Name)}`
      );
    }
    if (ch.params.GatewayType === 'VPCE')
      cmds.push(
        `aws ec2 create-vpc-endpoint --vpc-id ${_shellArg(ch.params.VpcId)} --service-name ${_shellArg(ch.params.ServiceName || 'com.amazonaws.REGION.s3')} --vpc-endpoint-type ${_shellArg(ch.params.EndpointType || 'Gateway')}`
      );
  }
  if (ch.action === 'add_route')
    cmds.push(
      `aws ec2 create-route --route-table-id ${_shellArg(ch.target.RouteTableId)} --destination-cidr-block ${_shellArg(ch.params.DestinationCidrBlock)} --gateway-id ${_shellArg(ch.params.TargetId)}`
    );
  if (ch.action === 'add_resource') {
    if (ch.params.ResourceType === 'EC2')
      cmds.push(
        `aws ec2 run-instances --subnet-id ${_shellArg(ch.params.SubnetId)} --instance-type ${_shellArg(ch.params.InstanceType || 't3.micro')} --image-id $AMI_ID${_tagSpec('instance', ch.params.Name)}`
      );
    if (ch.params.ResourceType === 'RDS')
      cmds.push(
        `aws rds create-db-instance --db-instance-identifier ${_shellArg(ch.params.Name || 'new-db')} --db-instance-class ${_shellArg(ch.params.InstanceClass || 'db.t3.micro')} --engine ${_shellArg(ch.params.Engine || 'mysql')} --master-username admin --master-user-password $DB_PASSWORD`
      );
    if (ch.params.ResourceType === 'Lambda')
      cmds.push(
        `aws lambda create-function --function-name ${_shellArg(ch.params.Name || 'new-function')} --runtime ${_shellArg(ch.params.Runtime || 'nodejs18.x')} --role $LAMBDA_ROLE_ARN --handler index.handler --zip-file fileb://function.zip --vpc-config ${_shellArg('SubnetIds=' + ch.params.SubnetId)}`
      );
    if (ch.params.ResourceType === 'ElastiCache')
      cmds.push(
        `aws elasticache create-cache-cluster --cache-cluster-id ${_shellArg(ch.params.Name || 'new-cache')} --cache-node-type ${_shellArg(ch.params.NodeType || 'cache.t3.micro')} --engine ${_shellArg(ch.params.Engine || 'redis')} --num-cache-nodes 1`
      );
    if (ch.params.ResourceType === 'ECS')
      cmds.push(
        `aws ecs create-service --cluster $CLUSTER --service-name ${_shellArg(ch.params.Name || 'new-service')} --task-definition $TASK_DEF --desired-count ${_shellArg(ch.params.DesiredCount || 1)} --launch-type FARGATE --network-configuration ${_shellArg('awsvpcConfiguration={subnets=[' + ch.params.SubnetId + ']}')}`
      );
    if (ch.params.ResourceType === 'Redshift')
      cmds.push(
        `aws redshift create-cluster --cluster-identifier ${_shellArg(ch.params.Name || 'new-cluster')} --node-type ${_shellArg(ch.params.NodeType || 'dc2.large')} --master-username admin --master-user-password $RS_PASSWORD --number-of-nodes 1`
      );
  }
  if (ch.action === 'add_security_group') {
    cmds.push(
      `aws ec2 create-security-group --group-name ${_shellArg(ch.params.Name || 'new-sg')} --description ${_shellArg(ch.params.Description || 'Design mode SG')} --vpc-id ${_shellArg(ch.params.VpcId)}`
    );
    if (ch.params.IngressRules) {
      ch.params.IngressRules.forEach((r) => {
        if (r.Protocol === '-1')
          cmds.push(
            `aws ec2 authorize-security-group-ingress --group-id $SG_ID --protocol -1 --cidr ${_shellArg(r.CidrIp || '0.0.0.0/0')}`
          );
        else
          cmds.push(
            `aws ec2 authorize-security-group-ingress --group-id $SG_ID --protocol ${_shellArg(r.Protocol)} --port ${_shellArg(r.FromPort)} --cidr ${_shellArg(r.CidrIp || '0.0.0.0/0')}`
          );
      });
    }
  }
  if (ch.action === 'remove_resource') {
    ...
    if (t === 'EC2') cmds.push(`aws ec2 terminate-instances --instance-ids ${_shellArg(id)}`);
    if (t === 'RDS')
      cmds.push(
        `aws rds delete-db-instance --db-instance-identifier ${_shellArg(id)} --skip-final-snapshot`
      );
    if (t === 'Lambda') cmds.push(`aws lambda delete-function --function-name ${_shellArg(id)}`);
    if (t === 'Subnet') cmds.push(`aws ec2 delete-subnet --subnet-id ${_shellArg(id)}`);
    if (t === 'IGW') {
      cmds.push(
        `aws ec2 detach-internet-gateway --internet-gateway-id ${_shellArg(id)} --vpc-id $VPC_ID`
      );
      cmds.push(`aws ec2 delete-internet-gateway --internet-gateway-id ${_shellArg(id)}`);
    }
    if (t === 'NAT') cmds.push(`aws ec2 delete-nat-gateway --nat-gateway-id ${_shellArg(id)}`);
    if (t === 'VPCE') cmds.push(`aws ec2 delete-vpc-endpoints --vpc-endpoint-ids ${_shellArg(id)}`);
  }
  return cmds;
}
...
export function importDesignPlan(json, enterFn, addChangeFn) {
  try {
    ...
    if (!plan.changes || !Array.isArray(plan.changes)) {
      showToast('Invalid plan format', 3000);
      return;
    }
    ...
    if (blocked > 0)
      showToast(
        'Imported ' +
          ...
          ' changes, ' +
          blocked +
          ' blocked by validation errors. Check the change log for details.',
        4000
      );
  } catch (e) {
