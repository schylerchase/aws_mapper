...
function _parseIAMData(raw) {
  return typeof window !== 'undefined' && window.parseIAMData ? window.parseIAMData(raw) : null;
}
...
// === State Accessors ===
export function getMultiViewMode() {
  return multiViewMode;
}
export function setMultiViewMode(v) {
  multiViewMode = v;
}
export function getLoadedContexts() {
  return loadedContexts;
}
export function setLoadedContexts(v) {
  loadedContexts = v;
}
export function getMergedCtx() {
  return mergedCtx;
}
export function setMergedCtx(v) {
  mergedCtx = v;
}
export function getSingleCtxBackup() {
  return singleCtxBackup;
}
export function setSingleCtxBackup(v) {
  singleCtxBackup = v;
}
...
export function detectRegionFromCtx(ctx) {
  ...
  if (sub && sub.AvailabilityZone) {
    return sub.AvailabilityZone.replace(/[a-z]$/, '');
  }
  return 'unknown';
}
...
export function buildRlCtxFromData(textareas, accountLabel) {
  try {
    ...
    let lambdaFns = ext(_val('in_lambda'), ['Functions']).filter(
    ...
    const vpcRegion = {};
    subnets.forEach((s) => {
      if (s.VpcId && s._region && s._region !== 'unknown') {
        vpcRegion[s.VpcId] = s._region;
      }
    });
    function fillRegion(r) {
      if (r && r._region === 'unknown' && r.VpcId && vpcRegion[r.VpcId]) {
        r._region = vpcRegion[r.VpcId];
      }
    }
    [
      vpcs,
      ...
      ecacheClusters,
      redshiftClusters
    ].forEach((arr) => arr.forEach(fillRegion));
    igwRaw.forEach((g) => {
      if (g._region === 'unknown') {
        const att = (g.Attachments || [])[0];
        if (att && att.VpcId && vpcRegion[att.VpcId]) {
          g._region = vpcRegion[att.VpcId];
        }
      }
    });
    const subVpcLookup = {};
    subnets.forEach((s) => {
      if (s.SubnetId) {
        subVpcLookup[s.SubnetId] = s.VpcId || '';
      }
    });
    ecsServices.forEach((svc) => {
      if (svc._region === 'unknown') {
        ...
        const sid = nc && nc.subnets && nc.subnets[0];
        if (sid) {
          const vid = subVpcLookup[sid];
          if (vid && vpcRegion[vid]) {
            svc._region = vpcRegion[vid];
          }
        }
      }
    });
          ...
          vpcs.forEach((vpc) => {
            if (vpc._region && vpc._region !== 'unknown') {
              (vpc.VpnGateways || []).forEach((g) => {
                if (g.VpnGatewayId === vgw) {
                  v._region = vpc._region;
                }
              });
            }
          });
    ...
    peerings.forEach((p) => {
      if (p._region === 'unknown') {
        ...
        if (rv && vpcRegion[rv]) {
          p._region = vpcRegion[rv];
        } else if (av && vpcRegion[av]) {
          p._region = vpcRegion[av];
        }
      }
    });
    const volRegion = {};
    volumes.forEach((v) => {
      if (v.VolumeId && v._region && v._region !== 'unknown') {
        volRegion[v.VolumeId] = v._region;
      }
    });
    snapshots.forEach((s) => {
      if (s._region === 'unknown' && s.VolumeId && volRegion[s.VolumeId]) {
        s._region = volRegion[s.VolumeId];
      }
    });
    const albArnRegion = {};
    albs.forEach((a) => {
      if (a.LoadBalancerArn && a._region && a._region !== 'unknown') {
        albArnRegion[a.LoadBalancerArn] = a._region;
      }
    });
    wafAcls.forEach((w) => {
    ...
    zones.forEach((z) => {
      if (z._region === 'unknown') {
        z._region = 'global';
      }
    });
    cfDistributions.forEach((d) => {
      if (d._region === 'unknown') {
        d._region = 'global';
      }
    });
    const acctRegion = {};
    [vpcs, subnets, instances].forEach((arr) => {
      arr.forEach((r) => {
        if (
          ...
          r._region !== 'unknown' &&
          r._region !== 'global' &&
          !acctRegion[r._accountId]
        ) {
          acctRegion[r._accountId] = r._region;
        }
      });
    });
    [
      ...
      arr.forEach((r) => {
        if (r._region === 'unknown' && r._accountId && acctRegion[r._accountId]) {
          r._region = acctRegion[r._accountId];
        }
      });
    });
    ...
    if (_accounts.size >= 1) {
      ...
      [
        ...
        arr.forEach((r) => {
          if (r && r._accountId === 'default') {
            r._accountId = _prAcct;
          }
        });
      });
    }
    ...
    igwRaw.forEach((g) => {
      (g.Attachments || []).forEach((a) => {
        if (vpcIds.has(a.VpcId)) {
          igws.push(Object.assign({}, g, { _vpcId: a.VpcId }));
        }
      });
    });
    natRaw.forEach((g) => {
      if (vpcIds.has(g.VpcId)) {
        nats.push(g);
      }
    });
    vpceRaw.forEach((g) => {
    ...
    const instBySub = new Map(),
      albBySub = new Map(),
      eniBySub = new Map();
    const rdsBySub = new Map(),
      ecsBySub = new Map(),
      lambdaBySub = new Map();
    instances.forEach((i) => {
      const s = i.SubnetId;
    ...
    const volByInst = new Map(),
      snapByVol = new Map();
    volumes.forEach((v) => {
      (v.Attachments || []).forEach((a) => {
        if (a.InstanceId) {
          if (!volByInst.has(a.InstanceId)) {
            volByInst.set(a.InstanceId, []);
          }
    ...
    const ecacheByVpc = new Map(),
      redshiftByVpc = new Map();
    ecacheClusters.forEach((c) => {
      const vid = c.VpcId || '';
      if (vid) {
        if (!ecacheByVpc.has(vid)) {
          ecacheByVpc.set(vid, []);
        }
      ...
      (rt.Routes || []).forEach((r) => {
        if (r.TransitGatewayId) {
          ...
          tgwAttachments.push({
            ...
            VpcId: vid,
            _accountId: rt._accountId || '',
            _region: rt._region || (vid && vpcRegion[vid]) || 'unknown'
          });
        }
      });
    });
    ...
    tgwAttachments.forEach((t) => {
      if ((!t._region || t._region === 'unknown') && t._accountId && acctRegion[t._accountId]) {
        t._region = acctRegion[t._accountId];
      }
      if (!t._region || t._region === 'unknown') {
        const vid = t.VpcId;
        if (vid && vpcRegion[vid]) {
          t._region = vpcRegion[vid];
        }
      }
    });
    ...
    cfDistributions.forEach((d) => {
      (d.Origins?.Items || []).forEach((o) => {
        ...
        if (matchAlb) {
          (cfByAlb[matchAlb.LoadBalancerArn] = cfByAlb[matchAlb.LoadBalancerArn] || []).push(d);
        }
      });
    });
    const recsByZone = {};
...
export function mergeContexts(contexts) {
  ...
  visible.forEach((c) => {
    if (!c.rlCtx && c.textareas) {
      c.rlCtx = buildRlCtxFromData(c.textareas, c.accountLabel);
    }
  });
  if (visible.length === 1) {
    return visible[0].rlCtx;
  }
  ...
  visible.forEach((ctx) => {
    ...
    if (c.pubSubs) {
      c.pubSubs.forEach((s) => merged.pubSubs.add(s));
    }
    if (c._accounts) {
      c._accounts.forEach((a) => merged._accounts.add(a));
    }
    merged._accounts.add(ctx.accountId);
    if (c._regions) {
      c._regions.forEach((r) => merged._regions.add(r));
    }
    if (ctx._isRegion && ctx.region) {
      merged._regions.add(ctx.region);
    }
