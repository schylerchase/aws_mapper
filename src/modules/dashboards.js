...
const _UDASH_TABS = [
  {
    ...
    prereq: function () {
      if (!_rlCtx) {
        _showToast('Render map data first', 'warn');
        return false;
      }
      if (!_classificationData.length) {
    ...
    prereq: function () {
      ...
      if (!_iamReviewData.length) {
        try {
          ...
          if (iamRaw) {
            const p = parseIAMData(iamRaw);
            if (p) {
              prepareIAMReviewData(p);
            }
          }
        } catch (e) {
          console.warn('IAM parse error in prereq:', e);
        }
      }
      return true;
    },
  ...
  },
  {
    ...
    prereq: function () {
      ...
      if (!_budrAssessments || !_budrAssessments.length) {
        runBUDRChecks(_rlCtx);
      }
      if (!_budrAssessments.length) {
        _showToast('No resources to assess');
        return false;
      }
      return true;
    },
...
function openUnifiedDash(tabId) {
  let tab = _UDASH_TABS.find(function (t) {
    return t.id === tabId;
  });
  if (!tab) {
    return;
  ...
  if (!wasOpen) {
    el.offsetHeight;
  } // force reflow only on first open
  _renderUdashTabs();
  tab.render();
}
...
function _switchUdashTab(tabId) {
  ...
  // Clear shared areas
  const udashBody = document.getElementById('udashBody');
  if (udashBody._fwDashActionHandler) {
    udashBody.removeEventListener('click', udashBody._fwDashActionHandler);
    udashBody._fwDashActionHandler = null;
  }
  document.getElementById('udashToolbar').innerHTML = '';
  udashBody.innerHTML = '';
  udashBody.className = 'udash-body';
  document.getElementById('udashFooter').innerHTML = '';
  document.getElementById('udashToolbar').style.display = '';
...
function _safeBUDRTier(tier) {
  if (!_BUDR_TIER_META || !_BUDR_TIER_META[tier]) {
    return 'partial';
  }
  return tier;
}
function _safeClassTier(tier) {
  if (!_TIER_RPO_RTO || !_TIER_RPO_RTO[tier]) {
    return 'low';
  }
  return tier;
}
function _safeDashSeverity(severity) {
...
function _renderBUDRDash() {
  ...
  let th =
    '<input id="budrSearch" type="text" placeholder="Filter by name, ID, type..." value="' +
    _escHtml(st.search) +
    '" style="background:var(--bg-tertiary);border:1px solid var(--border);color:var(--text-primary);padding:4px 10px;border-radius:4px;font-size:11px;font-family:Segoe UI,system-ui,sans-serif;width:200px">';
  th +=
    '<select id="budrSort" style="background:var(--bg-tertiary);border:1px solid var(--border);color:var(--text-secondary);padding:4px 8px;border-radius:4px;font-size:10px;font-family:Segoe UI,system-ui,sans-serif">';
  ...
  [
    { tier: 'all', label: 'All (' + total + ')' },
    { tier: 'protected', label: 'Protected (' + counts.protected + ')' },
    { tier: 'partial', label: 'Partial (' + counts.partial + ')' },
    { tier: 'at_risk', label: 'At Risk (' + counts.at_risk + ')' }
  ].forEach(function (p) {
    const btn = document.createElement('span');
    btn.className = 'budr-pill' + (st.tierFilter === p.tier ? ' active' : '');
    btn.dataset.tier = p.tier;
    btn.textContent = p.label;
    btn.addEventListener('click', function () {
      st.tierFilter = p.tier;
      _renderBUDRDash();
    });
    pillBox.appendChild(btn);
  });
  // Wire toolbar listeners
  document.getElementById('budrSearch').addEventListener('input', function () {
    st.search = this.value;
    _renderBUDRDash();
  });
  document.getElementById('budrSort').addEventListener('change', function () {
    st.sort = this.value;
    _renderBUDRDash();
  });
  // Filter assessments
  let items = _budrAssessments.slice();
  if (st.tierFilter !== 'all') {
    items = items.filter(function (a) {
      return a.profile && _safeBUDRTier(a.profile.tier) === st.tierFilter;
    });
  }
  if (st.search) {
  ...
  items.forEach(function (a) {
    if (a.profile) {
      groups[_safeBUDRTier(a.profile.tier)].push(a);
    }
  });
  // Render sections
  ['at_risk', 'partial', 'protected'].forEach(function (tier) {
    ...
    grp.forEach(function (a, i) {
      const findings = _budrFindings.filter(function (f) {
        return f.resource === a.id;
      });
      ...
      if (findings.length) {
        ...
        findings.forEach(function (f) {
          const sev = _safeDashSeverity(f.severity);
          h += '<div class="budr-finding">';
          h += '<span class="bf-sev ' + sev + '">' + _escHtml(sev) + '</span>';
          h += '<span class="bf-msg">' + esc(f.message) + '</span>';
          h += '</div>';
          if (f.remediation) {
            h +=
              '<div class="budr-finding"><span class="bf-sev" style="visibility:hidden">.</span><span class="bf-fix">\u21B3 ' +
              esc(f.remediation) +
              '</span></div>';
          }
        });
  ...
  if (!items.length) {
    h +=
      '<div style="padding:40px;text-align:center;color:var(--text-muted);font-family:Segoe UI,system-ui,sans-serif">No resources match current filter</div>';
  }
  body.innerHTML = h;
  body.scrollTop = 0;
  ...
  body.querySelectorAll('.budr-res-hdr').forEach(function (hdr) {
    hdr.addEventListener('click', function () {
      hdr.closest('.budr-res').classList.toggle('expanded');
    });
  });
  ...
  if (_isElectron) {
    fh += '<button id="budrExportXLSX">Export XLSX</button>';
  }
  fh +=
  ...
  document.getElementById('budrExportJSON').addEventListener('click', function () {
    ...
    const data = {
      ...
      summary: _getBUDRTierCounts(),
      assessments: _budrAssessments,
      findings: _budrFindings
    };
    downloadBlob(
      new Blob([JSON.stringify(data, null, 2)], { type: 'application/json' }),
      'budr-assessment.json'
    );
  });
...
function _closeAllDashboardsExcept(keep) {
  _closeAllDashboards(keep);
}
...
function openGovernanceDashboard(tab) {
  if (tab === 'iam') {
    ...
    openUnifiedDash('classification');
  }
}
...
function _renderClassificationTab() {
  ...
  if (_govToolbarTab !== 'classification') {
    ...
    th +=
      '<input id="govSearch" type="text" placeholder="Filter by name, type, VPC..." value="' +
      _escHtml(st.search) +
      '">';
    th += '<label>Tier</label>';
    th += '<select id="govFilter">';
    ['all', 'critical', 'high', 'medium', 'low'].forEach(function (v) {
      th +=
        '<option value="' +
        v +
        '"' +
        (st.filter === v ? ' selected' : '') +
        '>' +
    ...
    [25, 50, 100, 0].forEach(function (v) {
      th +=
        '<option value="' +
        v +
        '"' +
        (st.perPage === v ? ' selected' : '') +
        '>' +
        (v || 'All') +
        '</option>';
    });
    th += '</select>';
    th +=
      '<button id="govRulesBtn" style="margin-left:auto;background:rgba(139,92,246,.1);border:1px solid #8b5cf6;color:#8b5cf6;padding:4px 12px;border-radius:4px;font-size:10px;font-family:Segoe UI,system-ui,sans-serif;cursor:pointer">Configure Rules</button>';
    tb.innerHTML = th;
    ...
    document.getElementById('govFilter').addEventListener('change', function () {
      st.filter = this.value;
      st.page = 1;
      _renderClassificationTab();
    });
    document.getElementById('govPerPage').addEventListener('change', function () {
      st.perPage = parseInt(this.value) || 0;
      st.page = 1;
      _renderClassificationTab();
    });
  ...
  _classificationData.forEach(function (r) {
    const tier = _safeClassTier(r.tier);
    counts[tier] = (counts[tier] || 0) + 1;
  });
  let bh = '<div class="gov-tier-cards">';
  [
    { t: 'critical', l: 'Critical' },
    { t: 'high', l: 'High' },
    { t: 'medium', l: 'Medium' },
    { t: 'low', l: 'Low' }
  ].forEach(function (d) {
    ...
    bh +=
      '<div class="gov-tier-card" data-gov-tier="' +
      d.t +
      '" style="border-color:' +
      meta.color +
      '">';
    ...
    bh +=
      '<div class="gov-tier-count" style="color:' +
      meta.color +
      '">' +
      (counts[d.t] || 0) +
      '</div>';
  ...
  if (st.filter !== 'all') {
    items = items.filter(function (r) {
      return _safeClassTier(r.tier) === st.filter;
    });
  }
  if (st.search) {
    const q = st.search.toLowerCase();
  ...
  items.sort(function (a, b) {
    if (sortKey === 'tier') {
      return (
        ((_TIER_RPO_RTO[_safeClassTier(a.tier)] || { priority: 99 }).priority -
          (_TIER_RPO_RTO[_safeClassTier(b.tier)] || { priority: 99 }).priority) *
        dir
      );
    }
  ...
  const cols = [
    ...
    { key: 'type', label: 'Type' },
    { key: 'tier', label: 'Tier' },
    { key: 'rpo', label: 'RPO', nosort: true },
    { key: 'rto', label: 'RTO', nosort: true },
    { key: 'vpcName', label: 'VPC' },
    { key: 'auto', label: 'Source', nosort: true },
    { key: 'actions', label: '', nosort: true }
  ];
  bh += '<table class="gov-table"><thead><tr>';
  cols.forEach(function (c) {
    let cls = '';
    if (!c.nosort && st.sort === c.key) {
      cls = st.sortDir === 'asc' ? ' sort-asc' : ' sort-desc';
    }
    bh +=
      '<th' +
      (c.nosort ? '' : ' data-sort-col="' + c.key + '"') +
      ' class="' +
      cls +
      '">' +
      c.label +
  ...
  if (!pageItems.length) {
    bh +=
      '<tr><td colspan="' +
      cols.length +
      '" style="text-align:center;padding:30px;color:var(--text-muted)">No resources match filters</td></tr>';
  }
  pageItems.forEach(function (r) {
    const rowTier = _safeClassTier(r.tier);
    bh += '<tr>';
    bh +=
      '<td><span class="gov-res-link" data-rid="' +
      _escHtml(r.id) +
      '" style="color:var(--accent-cyan);cursor:pointer">' +
      _escHtml(r.name) +
      '</span></td>';
    bh += '<td>' + _escHtml(r.type) + '</td>';
    bh += '<td><span class="gov-tier-badge ' + rowTier + '">' + _escHtml(rowTier) + '</span></td>';
    bh += '<td>' + _escHtml(r.rpo) + '</td>';
    bh += '<td>' + _escHtml(r.rto) + '</td>';
    bh +=
      '<td style="font-size:10px;color:var(--text-muted)">' + _escHtml(r.vpcName || '—') + '</td>';
    bh +=
      '<td style="font-size:9px;color:var(--text-muted)">' +
      (r.auto ? 'Auto' : '<span style="color:#8b5cf6">Manual</span>') +
      '</td>';
    bh += '<td><select class="gov-override-select" data-res-id="' + _escHtml(r.id) + '">';
    ['critical', 'high', 'medium', 'low'].forEach(function (t) {
      bh +=
        '<option value="' + t + '"' + (rowTier === t ? ' selected' : '') + '>' + t + '</option>';
    });
    bh += '</select></td>';
    bh += '</tr>';
  });
  ...
  fh +=
    ...
    items.length +
    ' of ' +
    _classificationData.length +
    '</span>';
  if (totalPages > 1) {
    fh += '<button id="govPrev"' + (st.page <= 1 ? ' disabled' : '') + '>← Prev</button>';
    fh +=
      '<span style="font-size:10px;color:var(--text-muted)">Page ' +
      st.page +
      ' of ' +
      totalPages +
      '</span>';
  ...
  if (document.getElementById('govPrev')) {
    document.getElementById('govPrev').addEventListener('click', function () {
      st.page--;
      _renderClassificationTab();
    });
  }
  if (document.getElementById('govNext')) {
    document.getElementById('govNext').addEventListener('click', function () {
      st.page++;
      _renderClassificationTab();
    });
  }
  // Delegated click/change handlers for classification tab
  if (!body._govDelegated) {
    body._govDelegated = true;
    body.addEventListener('click', function (e) {
      let th = e.target.closest('th[data-sort-col]');
      if (th) {
        let col = th.dataset.sortCol;
        if (st.sort === col) {
      ...
      if (link) {
        e.stopPropagation();
        let rid = link.dataset.rid;
        if (!rid) {
          return;
        }
        closeUnifiedDash();
        setTimeout(function () {
          _zoomAndShowDetail(rid);
        }, 250);
      }
    });
    body.addEventListener('change', function (e) {
      ...
      const resId = sel.dataset.resId;
      let newTier = sel.value;
      _classificationOverrides[resId] = newTier;
      const item = _classificationData.find(function (r) {
        return r.id === resId;
      });
      if (item) {
        item.tier = newTier;
        item.rpo = _TIER_RPO_RTO[newTier].rpo;
        item.rto = _TIER_RPO_RTO[newTier].rto;
        item.auto = false;
      }
      _renderClassificationTab();
    });
...
function _renderIAMTab() {
  ...
  if (_govToolbarTab !== 'iam') {
    ...
    let th = '<label>Search</label>';
    th +=
      '<input id="govSearch" type="text" placeholder="Filter by name, ARN, type..." value="' +
      _escHtml(st.search) +
      '">';
    ...
    [
      { v: 'all', l: 'All' },
      { v: 'roles', l: 'Roles Only' },
      { v: 'users', l: 'Users Only' },
      { v: 'admin', l: 'Admin Access' },
      { v: 'findings', l: 'With Findings' }
    ].forEach(function (o) {
      th +=
        '<option value="' +
        o.v +
        '"' +
        (st.filter === o.v ? ' selected' : '') +
        '>' +
        o.l +
        '</option>';
    });
  ...
  if (!_iamReviewData.length) {
    body.innerHTML =
      '<div style="text-align:center;padding:60px;color:var(--text-muted);font-family:Segoe UI,system-ui,sans-serif"><p style="font-size:14px">No IAM data loaded</p><p style="font-size:11px">Paste IAM auth details (from <code>aws iam get-account-authorization-details</code>) in the IAM section of the left panel, then re-render.</p></div>';
    footer.innerHTML = '';
    return;
  }
  // Filter + sort
  ...
  } else if (st.filter === 'users') {
    items = items.filter(function (r) {
      return r.type === 'User';
    });
  } else if (st.filter === 'admin') {
    items = items.filter(function (r) {
      return r.isAdmin;
    });
  } else if (st.filter === 'findings') {
    items = items.filter(function (r) {
  ...
  let dir = st.sortDir === 'asc' ? 1 : -1;
  items.sort(function (a, b) {
    if (sortKey === 'lastUsed' || sortKey === 'created') {
  ...
  const pageItems = items.slice(start, start + perPage);
  // Summary
  const roleCt = _iamReviewData.filter(function (r) {
    return r.type === 'Role';
  }).length;
  const userCt = _iamReviewData.filter(function (r) {
    return r.type === 'User';
  }).length;
  const adminCt = _iamReviewData.filter(function (r) {
    return r.isAdmin;
  }).length;
  const findCt = _iamReviewData.filter(function (r) {
    return r.findings.length > 0;
  }).length;
  let bh = '<div class="gov-tier-cards" style="margin-bottom:16px">';
  [
    { l: 'Roles', c: roleCt, color: '#8b5cf6' },
    { l: 'Users', c: userCt, color: '#22d3ee' },
    { l: 'Admin', c: adminCt, color: '#ef4444' },
    { l: 'With Findings', c: findCt, color: '#f59e0b' }
  ].forEach(function (d) {
    bh +=
      '<div class="gov-tier-card" style="border-color:' +
      d.color +
      '"><h3 style="color:' +
      d.color +
      '">' +
      d.l +
      '</h3><div class="gov-tier-count" style="color:' +
      d.color +
      '">' +
      d.c +
      '</div></div>';
  ...
  const cols = [
    ...
    { key: 'policies', label: 'Policies', nosort: true },
    { key: 'admin', label: 'Admin', nosort: true },
    { key: 'findings', label: 'Findings', nosort: true },
    { key: 'cross', label: 'Cross-Acct', nosort: true }
  ];
  bh += '<table class="gov-table"><thead><tr>';
  cols.forEach(function (c) {
    let cls = '';
  ...
  if (!pageItems.length) {
    bh +=
      '<tr><td colspan="' +
      cols.length +
      '" style="text-align:center;padding:30px;color:var(--text-muted)">No IAM entities match filters</td></tr>';
  }
  pageItems.forEach(function (r, idx) {
    ...
    bh +=
      '<td>' +
      (r.findings.length > 0
        ? '<span class="gov-finding-badge">' + r.findings.length + '</span>'
        : '—') +
      '</td>';
    bh +=
      '<td>' +
      (r.crossAccounts.length > 0
        ? '<span style="color:#f59e0b">' + r.crossAccounts.length + '</span>'
        : '—') +
      '</td>';
    bh += '</tr>';
    ...
    bh +=
      '<div style="margin-bottom:6px"><b>ARN:</b> <code style="font-size:10px;background:var(--bg-input);padding:2px 6px;border-radius:3px">' +
      _escHtml(r.arn) +
      '</code></div>';
    if (r.policyNames && r.policyNames.length) {
      bh +=
        '<div style="margin-bottom:6px"><b>Policies:</b> ' +
        r.policyNames
          .map(function (p) {
            return (
              '<code style="font-size:9px;background:var(--bg-input);padding:1px 4px;border-radius:2px;margin-right:3px">' +
              _escHtml(p) +
              '</code>'
            );
          })
    ...
    if (r.permBoundary) {
      bh +=
        '<div style="margin-bottom:6px"><b>Permission Boundary:</b> <code style="font-size:9px">' +
        _escHtml(r.permBoundary) +
        '</code></div>';
    }
    if (r.crossAccounts.length) {
      bh +=
        '<div style="margin-bottom:6px"><b>Cross-Account Trusts:</b> ' +
        r.crossAccounts
          .map(function (a) {
            return (
              '<code style="font-size:10px;background:rgba(245,158,11,.1);padding:1px 4px;border-radius:2px;margin-right:3px">' +
              _escHtml(a) +
              '</code>'
            );
          })
    ...
    if (r.type === 'User') {
      bh +=
        '<div style="margin-bottom:6px"><b>MFA:</b> ' +
        (r.hasMFA
      ...
      if (r.hasConsole) {
        bh +=
          '<div style="margin-bottom:6px"><b>Console Access:</b> <span style="color:#f59e0b">Enabled</span></div>';
      }
      if (r.activeKeys) {
        bh +=
          '<div style="margin-bottom:6px"><b>Active Access Keys:</b> ' + r.activeKeys + '</div>';
      }
    }
    if (r.findings.length) {
      bh +=
        '<div><b>Findings (' +
        r.findings.length +
        '):</b><ul style="margin:4px 0 0;padding-left:20px;list-style:none">';
          ...
          sev === 'CRITICAL'
            ...
            : sev === 'HIGH'
              ...
              : sev === 'MEDIUM'
                ...
                : '#3b82f6';
        bh +=
          '<li style="font-size:10px;color:var(--text-secondary);margin:3px 0"><span style="font-size:8px;font-weight:700;padding:1px 4px;border-radius:2px;background:rgba(0,0,0,.2);color:' +
          sColor +
          ';margin-right:4px">' +
          _escHtml(sev) +
          '</span>' +
  ...
  body.querySelectorAll('tr[data-iam-row]').forEach(function (tr) {
    tr.addEventListener('click', function () {
      ...
      body.querySelectorAll('.gov-iam-expand').forEach(function (e) {
        e.classList.remove('open');
      });
      body.querySelectorAll('tr[data-iam-row]').forEach(function (r) {
  ...
  document.getElementById('govExportCSV').addEventListener('click', function () {
    ...
    items.forEach(function (r) {
      rows.push([
        ...
        r.lastUsed ? r.lastUsed.toISOString() : '',
        r.policies,
        r.isAdmin ? 'Yes' : 'No',
        r.findings.length,
        r.crossAccounts.join(';')
      ]);
    });
    let csv = rows
      .map(function (r) {
...
function _openRulesEditor() {
  ...
  function normalizeRule(rule) {
    ...
    const scope = ['vpc', 'type', 'name'].includes(rule.scope) ? rule.scope : 'name';
    const weight = parseInt(rule.weight, 10);
    return {
      pattern: String(rule.pattern || '').slice(0, 512),
      scope: scope,
      tier: _safeClassTier(rule.tier),
      weight: Number.isFinite(weight) ? weight : 0,
      enabled: rule.enabled !== false
    };
  }
  let workRules = structuredClone(
    Array.isArray(_classificationRules) ? _classificationRules : []
  ).map(normalizeRule);
  const groupCollapsed = { vpc: false, type: false, name: false };
  const scopeLabels = {
    vpc: 'VPC Name Rules',
    type: 'Resource Type Rules',
    name: 'Instance Name Rules'
  };
  ...
  function buildPreview(rules) {
    if (!_rlCtx) {
      return { critical: 0, high: 0, medium: 0, low: 0, samples: [] };
    }
    const counts = { critical: 0, high: 0, medium: 0, low: 0 };
    const samples = [];
    const vpcNameMap = {};
    ...
    function classify(name, type, vpcName, id) {
      let tier =
        _classificationOverrides[id] || _scoreClassification(name, type, vpcName, rules).tier;
      tier = _safeClassTier(tier);
      counts[tier] = (counts[tier] || 0) + 1;
      if (samples.length < 12) {
        samples.push({ name: name, type: type, tier: tier });
      }
    }
    (_rlCtx.instances || []).forEach(function (inst) {
      const name = inst.Tags
        ? (
            inst.Tags.find(function (t) {
              return t.Key === 'Name';
            }) || {}
    ...
    (_rlCtx.rdsInstances || []).forEach(function (db) {
      classify(
        db.DBInstanceIdentifier,
        'rds',
        vpcNameMap[(db.DBSubnetGroup || {}).VpcId] || '',
        db.DBInstanceIdentifier
      );
    });
    (_rlCtx.ecacheClusters || []).forEach(function (ec) {
      classify(ec.CacheClusterId, 'elasticache', '', ec.CacheClusterId);
    });
    (_rlCtx.albs || []).forEach(function (alb) {
      classify(
        alb.LoadBalancerName || '',
        'alb',
        vpcNameMap[alb.VpcId] || '',
        alb.LoadBalancerName || ''
      );
    });
    (_rlCtx.lambdaFns || []).forEach(function (fn) {
      classify(
        fn.FunctionName,
        'lambda',
        vpcNameMap[(fn.VpcConfig || {}).VpcId] || '',
        fn.FunctionName
      );
    });
    (_rlCtx.ecsServices || []).forEach(function (svc) {
      classify(svc.serviceName || '', 'ecs', '', svc.serviceName || '');
    });
    (_rlCtx.redshiftClusters || []).forEach(function (rs) {
      classify(rs.ClusterIdentifier, 'redshift', vpcNameMap[rs.VpcId] || '', rs.ClusterIdentifier);
    });
    return {
  ...
  function renderPreview() {
    const el = document.getElementById('govRulesPreview');
    if (!el) {
      return;
    }
    ...
    let ph =
      '<div class="gov-preview-card"><h5>Classification Distribution</h5><div class="gov-preview-bars">';
    [
      { k: 'critical', l: 'Critical', c: '#ef4444' },
      { k: 'high', l: 'High', c: '#f59e0b' },
      { k: 'medium', l: 'Medium', c: '#22d3ee' },
      { k: 'low', l: 'Low', c: '#64748b' }
    ].forEach(function (d) {
      const pct = Math.round((p[d.k] / total) * 100);
      ph +=
        '<div class="gov-preview-bar"><span class="gov-preview-bar-label" style="color:' +
        d.c +
        '">' +
        d.l +
        '</span>';
      ph +=
        '<div class="gov-preview-bar-track"><div class="gov-preview-bar-fill" style="width:' +
        pct +
        '%;background:' +
        d.c +
        '"></div></div>';
    ...
    if (deltas.length) {
      ph += '<div class="gov-preview-delta">' + deltas.join(' &nbsp; ') + '</div>';
    } else {
      ph +=
        '<div class="gov-preview-delta"><span class="same">No changes from current</span></div>';
    }
    // Sample
    ph +=
      '<div class="gov-preview-card" style="margin-top:10px"><h5>Sample Resources</h5><div class="gov-preview-sample">';
    p.samples.forEach(function (s) {
      const tier = _safeClassTier(s.tier);
      ph +=
        '<div class="gov-preview-sample-row"><span class="name">' +
        _escHtml(s.name) +
        '</span><span class="type">' +
        _escHtml(s.type) +
        '</span><span class="gov-tier-badge ' +
        tier +
  ...
  function renderRules() {
    const list = document.getElementById('govRulesList');
    if (!list) {
      return;
    }
    let h = '';
    scopeOrder.forEach(function (scope) {
      const rules = [];
      workRules.forEach(function (r, i) {
        if (r.scope === scope) {
          rules.push({ rule: r, idx: i });
        }
      });
      if (!rules.length && scope !== 'vpc') {
        return;
      }
      const collapsed = groupCollapsed[scope];
      h += '<div class="gov-rule-group" data-scope="' + scope + '">';
      h +=
        '<div class="gov-rule-group-hdr" data-toggle-scope="' +
        scope +
        '"><span class="gov-rule-group-arrow' +
        (collapsed ? ' collapsed' : '') +
        '">▼</span>';
      h += '<span class="gov-rule-group-label">' + (scopeLabels[scope] || scope) + '</span>';
      h +=
        '<span class="gov-rule-group-count">' +
        rules.length +
        ' rule' +
        (rules.length !== 1 ? 's' : '') +
        '</span></div>';
      h +=
        '<div class="gov-rule-group-body' +
        (collapsed ? ' collapsed' : '') +
        '" style="' +
        (collapsed ? 'max-height:0' : 'max-height:9999px') +
        '">';
      rules.forEach(function (d) {
        ...
        h +=
          '<div class="gov-rule-toggle' +
          (r.enabled !== false ? ' on' : '') +
          '" data-toggle-idx="' +
          i +
          '" title="' +
          (r.enabled !== false ? 'Enabled — click to disable' : 'Disabled — click to enable') +
          '"></div>';
        h +=
          '<input class="pattern' +
          (!isValid ? ' invalid-pattern' : '') +
          '" type="text" value="' +
          _escHtml(r.pattern) +
          '" data-field="pattern" placeholder="regex pattern…" title="' +
          (!isValid ? 'Invalid regex!' : 'Regex pattern') +
          '">';
        h +=
          '<select data-field="scope" style="display:none"><option value="vpc"' +
          (r.scope === 'vpc' ? ' selected' : '') +
          '>VPC Name</option><option value="type"' +
          (r.scope === 'type' ? ' selected' : '') +
          '>Type</option><option value="name"' +
          (r.scope === 'name' ? ' selected' : '') +
          '>Name</option></select>';
        h +=
          '<select data-field="tier"><option value="critical"' +
          (r.tier === 'critical' ? ' selected' : '') +
          '>Critical</option><option value="high"' +
          (r.tier === 'high' ? ' selected' : '') +
          '>High</option><option value="medium"' +
          (r.tier === 'medium' ? ' selected' : '') +
          '>Medium</option><option value="low"' +
          (r.tier === 'low' ? ' selected' : '') +
          '>Low</option></select>';
        h +=
          '<input class="weight" type="number" value="' +
          _escHtml(r.weight) +
          '" data-field="weight" title="Weight (higher wins)">';
        h +=
          '<span class="gov-rule-match-ct' +
          (mc > 0 ? ' has-matches' : '') +
          '" title="' +
          (mc < 0 ? 'Invalid regex' : mc + ' resources match') +
          '">' +
          (mc < 0 ? '!' : mc) +
          '</span>';
      ...
      h +=
        '<div style="padding:4px 0 8px 34px"><button class="gov-rule-add-scope" data-add-scope="' +
        scope +
        '" style="background:none;border:1px dashed var(--border);border-radius:3px;padding:3px 10px;font-size:9px;font-family:Segoe UI,system-ui,sans-serif;color:var(--text-muted);cursor:pointer;transition:all .15s">+ Add ' +
        (scopeLabels[scope] || scope).replace(' Rules', '') +
        ' Rule</button></div>';
      h += '</div></div>';
    });
    list.innerHTML = h;
    ...
    overlay.querySelectorAll('.gov-rule-row input,.gov-rule-row select').forEach(function (el) {
      el.addEventListener('input', function () {
        ...
        if (field === 'pattern') {
          ...
          if (mcEl) {
            mcEl.textContent = mc < 0 ? '!' : mc;
            mcEl.classList.toggle('has-matches', mc > 0);
          }
        } else if (field === 'tier') {
        ...
        workRules[idx] = normalizeRule(workRules[idx]);
  ...
  h +=
    '<div class="gov-rules-left"><p style="font-size:10px;color:var(--text-muted);margin:0 0 12px;line-height:1.5">Regex patterns matched against scope. Higher weight wins when multiple rules match. Toggle rules on/off to test without deleting.</p>';
  ...
  renderPreview();
  // Shell events
  document.getElementById('govRulesClose').addEventListener('click', function () {
    overlay.remove();
  });
  ...
  document.getElementById('govRuleReset').addEventListener('click', function () {
    workRules = structuredClone(_DEFAULT_CLASS_RULES).map(normalizeRule);
    renderRules();
    renderPreview();
  });
      ...
      reader.onload = function (e) {
        try {
          ...
          if (!Array.isArray(imported)) {
            _showToast('Invalid rules file', 'warn');
            return;
          }
          workRules = imported.map(normalizeRule);
          renderRules();
          renderPreview();
          _showToast(imported.length + ' rules imported');
        } catch (err) {
          _showToast('Failed to parse JSON', 'warn');
        }
      };
  ...
  document.getElementById('govRuleApply').addEventListener('click', function () {
    readRulesFromDom();
    _classificationRules = workRules.map(normalizeRule).filter(function (r) {
      return r.pattern;
    });
