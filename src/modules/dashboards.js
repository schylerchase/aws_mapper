...
function _renderBUDRDash() {
  ...
  if (st.tierFilter !== 'all')
    items = items.filter(function (a) {
      return a.profile && a.profile.tier === st.tierFilter;
    });
  if (st.search) {
  ...
  items.forEach(function (a) {
    if (a.profile) groups[a.profile.tier].push(a);
  });
  // Render sections
...
function _renderClassificationTab() {
  ...
  items.sort(function (a, b) {
    if (sortKey === 'tier')
      return (
        ((_TIER_RPO_RTO[a.tier] || { priority: 99 }).priority -
          (_TIER_RPO_RTO[b.tier] || { priority: 99 }).priority) *
        dir
      );
  ...
  cols.forEach(function (c) {
    const cls = '';
    if (!c.nosort && st.sort === c.key) cls = st.sortDir === 'asc' ? ' sort-asc' : ' sort-desc';
    bh +=
      '<th' +
  ...
  pageItems.forEach(function (r) {
    ...
    ['critical', 'high', 'medium', 'low'].forEach(function (t) {
      bh += '<option value="' + t + '"' + (r.tier === t ? ' selected' : '') + '>' + t + '</option>';
    });
    bh += '</select></td>';
    bh += '</tr>';
  });
...
function _openRulesEditor() {
  ...
  function renderRules() {
    ...
    scopeOrder.forEach(function (scope) {
      ...
      workRules.forEach(function (r, i) {
        if (r.scope === scope) rules.push({ rule: r, idx: i });
      });
      ...
      rules.forEach(function (d) {
        ...
        const isValid = true;
        try {
          if (r.pattern) new RegExp(r.pattern, 'i');
        } catch (e) {
          isValid = false;
        }
    ...
    overlay.querySelectorAll('.gov-rule-row input,.gov-rule-row select').forEach(function (el) {
      el.addEventListener('input', function () {
        ...
        if (field === 'pattern') {
          ...
          const valid = true;
          try {
            if (this.value) new RegExp(this.value, 'i');
          } catch (e) {
            valid = false;
          }
  ...
  document.getElementById('govRuleApply').addEventListener('click', function () {
    readRulesFromDom();
    _classificationRules = workRules.filter(function (r) {
      return r.pattern;
    });
