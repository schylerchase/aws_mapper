...
import {
  SEV_ORDER, FW_LABELS,
  EFFORT_LABELS, EFFORT_TIME,
  PRIORITY_META, TIER_META,
...
} catch (e) { console.warn('Failed to load muted findings:', e); }

export function _saveMuted() {
  try { localStorage.setItem(MUTE_KEY, JSON.stringify([..._mutedFindings])); }
  catch (e) { console.warn('Failed to save muted findings:', e); }
}
export function _muteKey(f) { return f.control + '::' + f.resource; }
export function _isMuted(f) { return _mutedFindings.has(_muteKey(f)); }
...
export function _rptAccountLabel(accountId) {
  if (!accountId) return '';
  const ctx = window._loadedContexts.find(function(c) { return c.accountId === accountId; });
...
export function _rptUniqueAccounts() {
  if (window._loadedContexts.length > 1) {
    return window._loadedContexts.map(function(c) { return {id: c.accountId, label: c.accountLabel || c.accountId}; });
  }
  return [];
}
...
export function _rptSecurityPosture(ctx, opts) {
  ...
  const trails = filt(c.cloudtrailTrails); const gd = filt(c.guarddutyDetectors);
  const fl = c.flowLogs || []; const rec = filt(c.configRecorders); const rules = filt(c.configRules);
  const sh = filt(c.securityHubStds); const aa = filt(c.accessAnalyzers);
  const keys = filt(c.kmsKeys); const secs = filt(c.secrets); const ecr = filt(c.ecrRepos);
  const logs = c.logGroups || []; const apis = filt(c.apiGateways); const vpcs = c.vpcs || [];
  ...
  const trailOk = trails.some(function(t) { return t.IsMultiRegionTrail && t.LogFileValidationEnabled && t.KmsKeyId && t.CloudWatchLogsLogGroupArn; });
  ...
  const gdOk = gd.some(function(d) { return d.Status === 'ENABLED'; });
  h += '<tr><td>GuardDuty</td><td>'+stp(gdOk, gd.length > 0 && !gdOk)+'</td><td>'+esc(gd.length ? gd.length + ' detector(s)' : 'No detectors')+'</td></tr>';
  const flVpcs = new Set(fl.filter(function(f) { return f.ResourceId && f.ResourceId.startsWith('vpc-'); }).map(function(f) { return f.ResourceId; }));
  const flCov = vpcs.length ? vpcs.filter(function(v) { return flVpcs.has(v.VpcId); }).length : 0;
  ...
  const logRet = logs.filter(function(l) { return l.retentionInDays; }).length;
...
export function _rptExecSummary(ctx, opts) {
  ...
  if (!c && window._importedReportData) {
    ...
    h += '<p class="rpt-import-notice">Data imported from previous report'+(window._importedReportData.date ? ' ('+esc(window._importedReportData.date)+')' : '')+'.</p>';
    ...
    h += '<div class="stat-box"><span class="val">'+_efF.length+'</span><span class="lbl">Findings</span></div>';
    h += '<div class="stat-box"><span class="val">'+_ebF.length+'</span><span class="lbl">BUDR Assessments</span></div>';
    h += '<div class="stat-box"><span class="val">'+_eiF.length+'</span><span class="lbl">Inventory Resources</span></div>';
    h += '</div>';
...
export function _rptExecBUDR() {
  ...
  filteredBudr.forEach(function(a) { if (a.profile && counts[a.profile.tier] !== undefined) counts[a.profile.tier]++; });
...
export function _rptCompliance(ctx, opts) {
  ...
  fwKeys.forEach(function(fw) {
    ...
    const hasAcctData = items.some(function(f) { return !!f._accountId; });
...
export function _rptGroupedFindingsTable(items) {
  ...
  const groups = Object.keys(ctrlMap).map(function(k) { return ctrlMap[k]; });
  ...
  groups.forEach(function(g) {
    ...
    const accts = {}; g.resources.forEach(function(f) { if (f._accountId) accts[f._accountId] = 1; });
...
export function _rptBUDR(ctx, opts) {
  ...
  const counts = {protected:0,partial:0,at_risk:0};
  _filteredBudr.forEach(function(a) { if (a.profile && counts[a.profile.tier] !== undefined) counts[a.profile.tier]++; });
  const filteredBudrFindings = _rptFilterByAccount(window._budrFindings || [], _rptGetAccountFilter());
  h += '<div class="rpt-section-body">';
  h += '<nav class="rpt-jump-nav">';
...
export function _rptIAMReview(ctx, opts) {
  if (!window._iamReviewData.length) {
    ...
    if (raw) { const p = _parseIAMData(raw); if (p) window.prepareIAMReviewData(p); }
  }
  let h = '<section class="rpt-section" id="s-iam-review"><h2 class="rpt-section-toggle">IAM Review</h2>';
  if (!window._iamReviewData.length) { return h + '<p>No IAM data available.</p></section>'; }
  h += '<div class="rpt-section-body">';
  const roles = window._iamReviewData.filter(function(r) { return r.type === 'Role'; });
  const users = window._iamReviewData.filter(function(r) { return r.type === 'User'; });
  const admins = window._iamReviewData.filter(function(r) { return r.isAdmin; });
  const withFindings = window._iamReviewData.filter(function(r) { return r.findings.length > 0; });
  const crossAcct = window._iamReviewData.filter(function(r) { return r.crossAccounts.length > 0; });
  ...
  const usersWithIssues = users.filter(function(u) { return u.findings.length > 0 || u.isAdmin || !u.hasMFA && u.hasConsole; });
...
export function _rptInventory(ctx, opts) {
  ...
  if (!window._rlCtx && !window._importedReportData) { return h + '<p>No data loaded.</p></section>'; }
  ...
  typeKeys.forEach(function(t) {
    ...
    const hasConfig = items.some(function(r) { return r.config && r.config !== '-'; });
    const hasState = items.some(function(r) { return r.state && r.state !== '-'; });
    const hasVpc = items.some(function(r) { return !!r.vpcId; });
...
export function _rptTagName(o) {
  const t = (o.Tags || []).find(function(t) { return t.Key === 'Name'; });
  return t ? t.Value : '';
}
...
export function _rptActionPlan(ctx, opts) {
  ...
  PRIORITY_KEYS.forEach(function(t) {
    const rgs = tiers[t]; if (!rgs || !rgs.length) return;
...
export function _rptIaCRecs(ctx, opts) {
  ...
  const f = _rptFilterByAccount(window._complianceFindings || [], _rptGetAccountFilter()).filter(function(x) {
