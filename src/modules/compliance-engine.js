...
let _mapSvg = null,
  _mapZoom = null,
  _mapG = null; // global map refs for navigation
...
function _naclCoversPort(e, port) {
  ...
  if (!pr || pr.From === undefined || pr.To === undefined) {
    return false;
  }
...
function runCISChecks(ctx) {
  ...
  (ctx.sgs || []).forEach((sg) => {
    (sg.IpPermissions || []).forEach((p) => {
      if (_hasPort(p, 22) && _hasOpenCidr(p)) {
        f.push({
          severity: 'HIGH',
          control: 'CIS 5.2',
          framework: 'CIS',
          ...
          message: 'SG allows SSH (22) from 0.0.0.0/0',
          remediation: 'Restrict SSH to specific CIDR ranges or bastion host SG'
        });
      }
    });
  });
  ...
  (ctx.sgs || []).forEach((sg) => {
    (sg.IpPermissions || []).forEach((p) => {
      if (_hasPort(p, 3389) && _hasOpenCidr(p)) {
        f.push({
          severity: 'HIGH',
          control: 'CIS 5.3',
          framework: 'CIS',
          resource: sg.GroupId,
          resourceName: sg.GroupName || '',
          message: 'SG allows RDP (3389) from 0.0.0.0/0',
          remediation: 'Restrict RDP to specific CIDR ranges or VPN'
        });
      }
    });
  });
    ...
    if (sg.GroupName === 'default') {
      ...
      if (hasIngress || hasEgress) {
        f.push({
          ...
          message: `Default SG in VPC ${sg.VpcId} has ${hasIngress ? 'ingress' : ''}${hasIngress && hasEgress ? ' and ' : ''}${hasEgress ? 'egress' : ''} rules`,
          remediation: 'Remove all rules from default SGs; use custom SGs instead'
        });
      }
    }
  });
  ...
  (ctx.nacls || []).forEach((nacl) => {
    (nacl.Entries || []).forEach((e) => {
      if (
        ...
        if (_naclCoversPort(e, 22)) {
          f.push({
            severity: 'HIGH',
            control: 'CIS 5.1',
            framework: 'CIS',
            resource: nacl.NetworkAclId,
            resourceName: gn(nacl, nacl.NetworkAclId),
            message: 'NACL allows SSH (22) from 0.0.0.0/0',
            remediation: 'Restrict NACL ingress to specific source CIDRs'
          });
        }
        if (_naclCoversPort(e, 3389)) {
          f.push({
            severity: 'HIGH',
            control: 'CIS 5.1',
            framework: 'CIS',
            resource: nacl.NetworkAclId,
            resourceName: gn(nacl, nacl.NetworkAclId),
            message: 'NACL allows RDP (3389) from 0.0.0.0/0',
            remediation: 'Restrict NACL ingress to specific source CIDRs'
          });
        }
      }
  ...
  (ctx.rts || []).forEach((rt) => {
    (rt.Routes || []).forEach((r) => {
      if (r.VpcPeeringConnectionId && r.DestinationCidrBlock === '0.0.0.0/0') {
        f.push({
          severity: 'MEDIUM',
          control: 'CIS 5.5',
          framework: 'CIS',
          resource: rt.RouteTableId,
          resourceName: gn(rt, rt.RouteTableId),
          message: 'Peering route has overly broad 0.0.0.0/0 destination',
          remediation: 'Use specific CIDR ranges for peering routes'
        });
      }
    });
  });
  ...
  (ctx.subnets || []).forEach((sub) => {
    if (pubSubIds.has && pubSubIds.has(sub.SubnetId)) {
      return;
    }
    ...
    const hasIgw = (rt.Routes || []).some(
      (r) => r.GatewayId && r.GatewayId.startsWith('igw-') && r.State === 'active'
    );
    if (hasIgw) {
      f.push({
        severity: 'MEDIUM',
        control: 'NET-1',
        framework: 'CIS',
        resource: sub.SubnetId,
        resourceName: gn(sub, sub.SubnetId),
        message: 'Private subnet has direct IGW route',
        remediation: 'Remove IGW route from private subnet route table'
      });
    }
  });
    ...
    (sg.IpPermissions || []).forEach((p) => {
      if (p.IpProtocol === '-1' && _hasOpenCidr(p)) {
        f.push({
          ...
          resource: sg.GroupId,
          resourceName: sg.GroupName || '',
          message: 'SG allows ALL traffic from 0.0.0.0/0',
  ...
  (ctx.instances || []).forEach((inst) => {
    ...
    if (mo.HttpTokens !== 'required') {
      f.push({
        severity: 'HIGH',
        control: 'CKV_AWS_79',
        framework: 'CIS',
        resource: inst.InstanceId,
        resourceName: gn(inst, inst.InstanceId),
        message: 'EC2 instance not enforcing IMDSv2 (HttpTokens != required)',
        remediation: 'Set MetadataOptions.HttpTokens to "required" to enforce IMDSv2'
      });
    }
  });
  ...
  if ((ctx.s3bk || []).some((b) => b.Versioning !== undefined)) {
    (ctx.s3bk || []).forEach((bk) => {
      if (!bk.Versioning || bk.Versioning.Status !== 'Enabled') {
        f.push({
          severity: 'MEDIUM',
          control: 'CKV_AWS_21',
          framework: 'CIS',
          resource: bk.Name,
          resourceName: bk.Name,
  ...
  if (
    (ctx.s3bk || []).some(
      (b) => b.LoggingConfiguration !== undefined || b.LoggingEnabled !== undefined
    )
  ) {
    (ctx.s3bk || []).forEach((bk) => {
      if (!bk.LoggingConfiguration && !bk.LoggingEnabled) {
        f.push({
          severity: 'LOW',
          control: 'CKV_AWS_18',
          framework: 'CIS',
          ...
          remediation: 'Enable server access logging to an audit bucket'
        });
      }
    });
  }
  ...
  (ctx.rdsInstances || []).forEach((db) => {
    if ((db.BackupRetentionPeriod || 0) < 7) {
      f.push({
        ...
        message:
          'RDS backup retention is ' + (db.BackupRetentionPeriod || 0) + ' days (should be >=7)',
  ...
  (ctx.lambdaFns || []).forEach((fn) => {
    if (
      ...
      f.push({
        ...
        message: 'Lambda has environment variables without KMS encryption',
        remediation: 'Set KMSKeyArn to encrypt environment variables at rest'
      });
    }
  });
  ...
  (ctx.lambdaFns || []).forEach((fn) => {
    if (!fn.TracingConfig || fn.TracingConfig.Mode !== 'Active') {
      f.push({
        ...
        resource: fn.FunctionName,
        resourceName: fn.FunctionName,
        message: 'Lambda X-Ray tracing not active',
        remediation: 'Enable active tracing for distributed tracing and debugging'
      });
    }
  });
  return f;
  ...
  acls.forEach((acl) => {
    if (!(acl.Rules || []).length) {
      f.push({
        ...
        control: 'WAF-1',
        framework: 'WAF',
        resource: acl.Id || acl.WebACLId || '',
        resourceName: acl.Name || '',
        message: 'WebACL has zero rules',
        remediation: 'Add rate-limiting and IP-filtering rules to WebACL'
      });
    }
  });
  // WAF-2: No rate-limiting rule
  acls.forEach((acl) => {
    const hasRate = (acl.Rules || []).some(
      (r) => (r.Statement && r.Statement.RateBasedStatement) || r.Type === 'RATE_BASED'
    );
    if (!hasRate && (acl.Rules || []).length > 0) {
      f.push({
        severity: 'MEDIUM',
        control: 'WAF-2',
        framework: 'WAF',
        resource: acl.Id || acl.WebACLId || '',
        resourceName: acl.Name || '',
        message: 'WebACL has no rate-limiting rule',
        remediation: 'Add a rate-based rule to prevent DDoS/brute-force'
      });
    }
  });
  // WAF-3: ALB not protected by WAF
  const protectedArns = new Set();
  acls.forEach((acl) => {
    (acl.ResourceArns || []).forEach((a) => protectedArns.add(a));
  });
  (ctx.albs || []).forEach((alb) => {
    if (alb.LoadBalancerArn && !protectedArns.has(alb.LoadBalancerArn)) {
      f.push({
        severity: 'MEDIUM',
        control: 'WAF-3',
        framework: 'WAF',
        ...
        message: 'ALB not associated with any WebACL',
        remediation: 'Associate this ALB with a WAF WebACL'
      });
    }
  });
  ...
  acls.forEach((acl) => {
    ...
    if (da.Allow || da.Type === 'ALLOW') {
      f.push({
        ...
        resource: acl.Id || acl.WebACLId || '',
        resourceName: acl.Name || '',
        message: 'WebACL default action is ALLOW (should be BLOCK)',
        remediation: 'Set default action to BLOCK and add explicit ALLOW rules'
      });
    }
  });
  return f;
...
function runArchChecks(ctx) {
  ...
  (ctx.subnets || []).forEach((sub) => {
    ...
    if (!hasIgw) {
      f.push({
        ...
        framework: 'ARCH',
        resource: sub.SubnetId,
        resourceName: gn2(sub, sub.SubnetId),
        message: 'Subnet has MapPublicIpOnLaunch=true but no IGW route',
        remediation: 'Add an IGW route to the route table or disable MapPublicIpOnLaunch'
      });
    }
  });
  // ARCH-N2: Private subnet resources with no NAT route
  (ctx.subnets || []).forEach((sub) => {
    if (pubSubs.has(sub.SubnetId)) {
      return;
    }
    const rt = subRT[sub.SubnetId];
    if (!rt) {
    ...
    const cnt =
      ((ctx.instBySub || {})[sub.SubnetId] || []).length +
      ((ctx.lambdaBySub || {})[sub.SubnetId] || []).length +
      ((ctx.ecsBySub || {})[sub.SubnetId] || []).length;
    if (cnt > 0 && !hasNat && !hasVpce) {
      f.push({
        severity: 'HIGH',
        control: 'ARCH-N2',
        framework: 'ARCH',
        ...
        resourceName: gn2(sub, sub.SubnetId),
        message:
          'Private subnet has ' + cnt + ' resources but no NAT gateway or VPC endpoint route',
        remediation: 'Add a NAT gateway in a public subnet and route 0.0.0.0/0 through it'
      });
    }
  });
  ...
  Object.entries(subByVpc).forEach(([vid, subs]) => {
    ...
    if (subs.length > 1 && azs.size === 1) {
      ...
      f.push({
        ...
        framework: 'ARCH',
        resource: vid,
        resourceName: gn2(vpc || {}, vid),
        message: 'All ' + subs.length + ' subnets in single AZ (' + [...azs][0] + ') — no HA',
        remediation: 'Create subnets across at least 2 AZs for fault tolerance'
      });
    }
  });
  ...
  (ctx.sgs || []).forEach((sg) => {
    ...
    (sg.IpPermissionsEgress || []).forEach((p) => {
      if (p.IpProtocol === '-1' && _hasOpenCidr(p)) {
        const inst = (ctx.instances || []).filter((i) =>
          (i.SecurityGroups || []).some((g) => g.GroupId === sg.GroupId)
        );
        if (inst.length > 0) {
          f.push({
            severity: 'LOW',
            ...
            resource: sg.GroupId,
            resourceName: sg.GroupName || '',
            message: 'SG has unrestricted egress attached to ' + inst.length + ' instance(s)',
            remediation: 'Restrict egress to required ports/CIDRs only'
          });
        }
      }
    });
  ...
  (ctx.instances || []).forEach((inst) => {
    ...
    if (hasBroad) {
      f.push({
        ...
        framework: 'ARCH',
        resource: inst.InstanceId,
        resourceName: gn2(inst, inst.InstanceId),
        message: 'EC2 in public subnet with SG allowing all traffic from 0.0.0.0/0',
        remediation: 'Restrict to specific ports; use ALB/NLB as entry point'
      });
    }
  });
  ...
  (ctx.instances || []).forEach((inst) => {
    ...
    if (unenc.length > 0) {
      f.push({
        ...
        resource: inst.InstanceId,
        resourceName: gn2(inst, inst.InstanceId),
        message: unenc.length + ' unencrypted EBS volume(s)',
  ...
  (ctx.lambdaFns || []).forEach((fn) => {
    ...
    if (!vc || !vc.SubnetIds || !vc.SubnetIds.length) {
      return;
    }
    const azs = new Set(
      vc.SubnetIds.map((sid) => {
    ...
    if (azs.size < 2) {
      f.push({
        ...
        resource: fn.FunctionName,
        resourceName: fn.FunctionName,
        message: 'Lambda in single AZ only',
        remediation: 'Configure Lambda VPC subnets across at least 2 AZs'
      });
    }
  });
  ...
  (ctx.rdsInstances || []).forEach((db) => {
    if (db.PubliclyAccessible) {
      f.push({
        ...
        remediation: 'Set PubliclyAccessible=false; access via VPN/bastion'
      });
    }
  });
  ...
  (ctx.rdsInstances || []).forEach((db) => {
    ...
    f.push({
      ...
      resource: db.DBInstanceIdentifier,
      resourceName: db.DBInstanceIdentifier,
      message: 'RDS not configured for Multi-AZ',
  ...
  (ctx.ecacheClusters || []).forEach((ec) => {
    if (ec.NumCacheNodes > 1 || (ec.CacheNodeType || '').includes('.micro')) {
      return;
    }
    f.push({
      severity: 'MEDIUM',
      control: 'ARCH-D4',
      framework: 'ARCH',
      resource: ec.CacheClusterId,
      resourceName: ec.CacheClusterId,
      message: 'ElastiCache cluster has only 1 node',
      remediation: 'Add read replicas or enable cluster mode'
    });
  });
  ...
  (ctx.redshiftClusters || []).forEach((rs) => {
    if (!rs.Encrypted) {
      f.push({
        ...
        control: 'ARCH-D5',
        framework: 'ARCH',
        resource: rs.ClusterIdentifier,
        resourceName: rs.ClusterIdentifier,
        message: 'Redshift cluster not encrypted at rest',
        remediation: 'Enable encryption (requires snapshot migration)'
      });
    }
  });
  ...
  (ctx.s3bk || []).forEach((bk) => {
    if (!bk.ServerSideEncryption && !bk.BucketEncryption) {
      f.push({
        ...
        resource: bk.Name,
        resourceName: bk.Name,
        message: 'S3 bucket may lack default encryption',
  ...
  (ctx.volumes || []).forEach((vol) => {
    ...
    if (snaps.length > 0 || vol.State !== 'in-use') {
      return;
    }
    f.push({
      severity: 'LOW',
      ...
      resourceName: gn2(vol, vol.VolumeId),
      message: 'In-use EBS volume has no snapshots',
      remediation: 'Create regular snapshots using AWS Backup or DLM'
    });
  });
  ...
  (ctx.albs || []).forEach((alb) => {
    if (alb.Scheme !== 'internet-facing' || cfOrigins.has(alb.DNSName)) {
      return;
    }
    f.push({
      severity: 'LOW',
      ...
      resourceName: alb.LoadBalancerName,
      message: 'Internet-facing ALB without CloudFront',
      remediation: 'Place CloudFront in front for caching and DDoS protection'
    });
  });
  ...
  Object.entries(natByVpc).forEach(([vid, nats]) => {
    ...
    if (nats.length >= 1 && azs.size === 1 && (subByVpc[vid] || []).length > 2) {
      ...
      f.push({
        ...
        resource: vid,
        resourceName: gn2(vpc || {}, vid),
        message: 'NAT Gateway(s) only in 1 AZ',
        remediation: 'Deploy NAT Gateways in each AZ for resilience'
      });
    }
  });
  ...
  const vpcHasS3Vpce = {};
  (ctx.vpces || []).forEach((e) => {
    if ((e.ServiceName || '').includes('.s3')) {
      vpcHasS3Vpce[e.VpcId] = true;
    }
  });
  (ctx.vpcs || []).forEach((vpc) => {
    const hasFns = (ctx.lambdaFns || []).some(
      (fn) => fn.VpcConfig && fn.VpcConfig.VpcId === vpc.VpcId
    );
    const hasInst = (ctx.instances || []).some((i) => i.VpcId === vpc.VpcId);
    if ((hasFns || hasInst) && !vpcHasS3Vpce[vpc.VpcId]) {
      f.push({
        severity: 'LOW',
        control: 'ARCH-G2',
        framework: 'ARCH',
        resource: vpc.VpcId,
        resourceName: gn2(vpc, vpc.VpcId),
        message: 'VPC has compute but no S3 Gateway Endpoint (traffic routes through NAT)',
        remediation: 'Create an S3 Gateway Endpoint (free) to reduce NAT costs'
      });
    }
  });
  ...
  (ctx.peerings || []).forEach((p) => {
    ...
    [p.RequesterVpcInfo?.VpcId, p.AccepterVpcInfo?.VpcId].forEach((vid) => {
      ...
      const hasRoute = (ctx.rts || []).some(
        (rt) =>
          rt.VpcId === vid &&
          (rt.Routes || []).some((r) => r.VpcPeeringConnectionId === p.VpcPeeringConnectionId)
      );
      if (!hasRoute) {
        f.push({
          severity: 'HIGH',
          control: 'ARCH-X1',
          framework: 'ARCH',
          resource: p.VpcPeeringConnectionId,
          resourceName: p.VpcPeeringConnectionId,
          message: 'VPC ' + vid + ' has no route for peering ' + p.VpcPeeringConnectionId,
          remediation: 'Add routes in both VPCs for the peering connection'
        });
      }
    });
  });
  ...
  (ctx.lambdaFns || []).forEach((fn) => {
    if (fn.Runtime && EOL_RUNTIMES.has(fn.Runtime)) {
      f.push({
        ...
        resource: fn.FunctionName,
        resourceName: fn.FunctionName,
        message: 'Lambda uses deprecated runtime: ' + fn.Runtime,
        remediation: 'Upgrade to a supported runtime version to receive security patches'
      });
    }
  });
  ...
  (ctx.lambdaFns || []).forEach((fn) => {
    if ((fn.Timeout || 3) > 300 && !fn.DeadLetterConfig?.TargetArn) {
      f.push({
        ...
        resource: fn.FunctionName,
        resourceName: fn.FunctionName,
        message: 'Lambda timeout >5min with no dead letter queue',
        remediation: 'Add an SQS or SNS DLQ for failed invocations'
      });
    }
  });
  ...
  (ctx.ecacheClusters || []).forEach((ec) => {
    if (
      ec.AtRestEncryptionEnabled === false ||
      (!ec.AtRestEncryptionEnabled && ec.Engine === 'redis')
    ) {
      f.push({
        severity: 'MEDIUM',
        control: 'ARCH-D6',
        ...
        resource: ec.CacheClusterId,
        resourceName: ec.CacheClusterId,
        message: 'ElastiCache cluster without encryption at rest',
        remediation: 'Enable at-rest encryption (requires creating a new cluster)'
      });
    }
  });
  ...
  (ctx.redshiftClusters || []).forEach((rs) => {
    if (rs.PubliclyAccessible) {
      f.push({
        ...
        remediation: 'Disable public access; connect via VPN or bastion in private subnet'
      });
    }
  });
  ...
  (ctx.cfDistributions || []).forEach((d) => {
    ...
    if (vpp === 'allow-all') {
      f.push({
        ...
        framework: 'ARCH',
        resource: d.Id || d.ARN || '',
        resourceName: d.DomainName || d.Id || '',
        message: 'CloudFront allows HTTP connections',
        remediation: 'Set ViewerProtocolPolicy to redirect-to-https or https-only'
      });
    }
  });
  ...
  (ctx.ecsServices || []).forEach((svc) => {
    if (svc.desiredCount > 0 && svc.runningCount === 0) {
      f.push({
        ...
        control: 'ARCH-C6',
        framework: 'ARCH',
        resource: svc.serviceName,
        resourceName: svc.serviceName,
        message: 'ECS service has 0 running tasks (desired: ' + svc.desiredCount + ')',
        remediation: 'Check task definition, IAM role, and container health checks'
      });
    }
  });
  return f;
...
function runSOC2Checks(ctx) {
      ...
      sensPorts.forEach((port) => {
        if (_hasPort(p, port) && _hasOpenCidr(p)) {
          f.push({
            ...
            resource: sg.GroupId,
            resourceName: sg.GroupName || '',
            message: 'SG allows port ' + port + ' from 0.0.0.0/0 — logical access control gap',
            remediation: 'Restrict to known CIDR ranges; use bastion hosts or SSM Session Manager'
          });
        }
      });
    });
  ...
  (ctx.sgs || []).forEach((sg) => {
    ...
    if ((sg.IpPermissions || []).length > 0) {
      f.push({
        ...
        resource: sg.GroupId,
        resourceName: 'default',
        message: 'Default SG has inbound rules — violates least-privilege',
        remediation: 'Remove all inbound rules from default SG; create explicit SGs per role'
      });
    }
  });
  ...
  (ctx.vpcs || []).forEach((vpc) => {
    ...
    if (nacls.length === 0) {
      f.push({
        ...
        resource: vpc.VpcId,
        resourceName: gn2(vpc, vpc.VpcId),
        message: 'VPC uses only default NACLs — no network boundary segmentation',
        remediation: 'Create custom NACLs for each subnet tier (public/private/data)'
      });
    }
  });
  ...
  (ctx.albs || []).forEach((alb) => {
    ...
    if (alb.Scheme === 'internet-facing' && !hasHttps) {
      f.push({
        severity: 'HIGH',
        control: 'SOC2-CC6.7',
        framework: 'SOC2',
        resource: alb.LoadBalancerName,
        resourceName: alb.LoadBalancerName,
        message: 'Internet-facing ALB has no HTTPS listener — data transmitted unencrypted',
        remediation: 'Add HTTPS listener with TLS 1.2+ certificate via ACM'
      });
    }
  });
  ...
  (ctx.instances || []).forEach((inst) => {
    ...
    const anyOpen = sgIds.some((gid) => {
      const sg = sgById.get(gid);
      return (
        sg && (sg.IpPermissionsEgress || []).some((p) => p.IpProtocol === '-1' && _hasOpenCidr(p))
      );
    });
    if (anyOpen) {
      f.push({
        ...
        resource: inst.InstanceId,
        resourceName: gn2(inst, inst.InstanceId),
        message: 'Public EC2 with unrestricted egress — C2 callback risk',
        remediation: 'Restrict outbound to required ports; use VPC endpoints for AWS services'
      });
    }
  });
    ...
    (ctx.vpcs || []).forEach((vpc) => {
      if (!vpc.FlowLogs || vpc.FlowLogs.length === 0) {
        f.push({
          ...
          resource: vpc.VpcId,
          resourceName: gn2(vpc, vpc.VpcId),
          message: 'VPC has no flow logs enabled — insufficient monitoring',
          remediation: 'Enable VPC Flow Logs to CloudWatch or S3 for audit trail'
        });
      }
    });
  }
  ...
  (ctx.rdsInstances || []).forEach((db) => {
    if (!(db.TagList || []).some((t) => t.Key === 'Name')) {
      untagged++;
    }
  });
  if (untagged > 0) {
    f.push({
      ...
      framework: 'SOC2',
      resource: 'Multiple',
      resourceName: untagged + ' resources',
      message: untagged + ' resource(s) missing Name tags — change tracking gap',
      remediation: 'Apply consistent tagging policy (Name, Environment, Owner, CostCenter)'
    });
  }
  ...
  (ctx.rdsInstances || []).forEach((db) => {
    if (!db.MultiAZ && !(db.DBInstanceClass || '').includes('.micro')) {
      f.push({
        ...
        resource: db.DBInstanceIdentifier,
        resourceName: db.DBInstanceIdentifier,
        message: 'RDS not Multi-AZ — does not meet availability commitment',
        remediation: 'Enable Multi-AZ for production databases'
      });
    }
  });
  ...
  (ctx.volumes || []).forEach((vol) => {
    ...
    if (snaps.length === 0 && vol.State === 'in-use') {
      f.push({
        ...
        resource: vol.VolumeId,
        resourceName: gn2(vol, vol.VolumeId),
        message: 'In-use EBS volume with no backup snapshots — recovery gap',
        remediation: 'Configure AWS Backup or DLM lifecycle policy'
      });
    }
  });
  ...
  (ctx.s3bk || []).forEach((bk) => {
    if (!bk.ServerSideEncryption && !bk.BucketEncryption) {
      f.push({
        ...
        resource: bk.Name,
        resourceName: bk.Name,
        message: 'S3 bucket without default encryption — confidentiality risk',
  ...
  (ctx.volumes || []).forEach((vol) => {
    if (!vol.Encrypted && vol.State === 'in-use') {
      f.push({
        ...
        resource: vol.VolumeId,
        resourceName: gn2(vol, vol.VolumeId),
        message: 'EBS volume not encrypted at rest — data protection gap',
        remediation: 'Enable EBS encryption by default in account settings'
      });
    }
  });
  ...
  (ctx.ecacheClusters || []).forEach((ec) => {
    if (
      ec.TransitEncryptionEnabled === false ||
      (ec.Engine === 'redis' && !ec.TransitEncryptionEnabled)
    ) {
      f.push({
        severity: 'HIGH',
        control: 'SOC2-C1.3',
        ...
        resource: ec.CacheClusterId,
        resourceName: ec.CacheClusterId,
        message: 'ElastiCache without in-transit encryption — data exposure risk',
        remediation: 'Enable transit encryption (requires new cluster for existing Redis)'
      });
    }
  });
  ...
  (ctx.cfDistributions || []).forEach((d) => {
    ...
    if (vpp === 'allow-all') {
      f.push({
        ...
        resource: d.Id || d.ARN || '',
        resourceName: d.DomainName || d.Id || '',
        message: 'CloudFront allows unencrypted HTTP — monitoring blind spot',
    ...
    if (svc.desiredCount > 0 && svc.runningCount < svc.desiredCount) {
      f.push({
        ...
        message:
          ...
          '/' +
          svc.desiredCount +
          ' tasks — availability gap',
        remediation: 'Check task health, resource limits, and container image availability'
      });
    }
  });
  ...
  (ctx.lambdaFns || []).forEach((fn) => {
    if (fn.Runtime && EOL_RUNTIMES.has(fn.Runtime)) {
      f.push({
        ...
        resource: fn.FunctionName,
        resourceName: fn.FunctionName,
        message: 'Lambda using EOL runtime ' + fn.Runtime + ' — no security patches',
  ...
  (ctx.vpcs || []).forEach((vpc) => {
    const customRts = (ctx.rts || []).filter(
      (rt) => rt.VpcId === vpc.VpcId && !(rt.Associations || []).some((a) => a.Main)
    );
    if (
      customRts.length === 0 &&
      (ctx.subnets || []).filter((s) => s.VpcId === vpc.VpcId).length > 1
    ) {
      f.push({
        severity: 'LOW',
        control: 'SOC2-PI1.1',
        ...
        resource: vpc.VpcId,
        resourceName: gn2(vpc, vpc.VpcId),
        message: 'VPC uses only main route table for all subnets — processing integrity risk',
        remediation: 'Create custom route tables per subnet tier for explicit routing control'
      });
    }
  });
  return f;
...
function runPCIDSSChecks(ctx) {
      ...
      dbPorts.forEach((port) => {
        if (_hasPort(p, port) && _hasOpenCidr(p)) {
          f.push({
            ...
            resource: sg.GroupId,
            resourceName: sg.GroupName || '',
            message: 'SG allows DB port ' + port + ' from 0.0.0.0/0 — CDE exposure',
            remediation:
              'Restrict database ports to application-tier SGs only; never expose to 0.0.0.0/0'
          });
        }
      });
    });
  ...
  (ctx.rdsInstances || []).forEach((db) => {
    ...
    if (allOpen) {
      f.push({
        ...
        resource: db.DBInstanceIdentifier,
        resourceName: db.DBInstanceIdentifier,
        message: 'RDS SG has unrestricted outbound — must restrict CDE egress',
        remediation: 'Restrict egress to specific app-tier SGs and required AWS service endpoints'
      });
    }
  });
  // 1.3.4 – Network segmentation: public subnet resources sharing SG with private
  const pubSgIds = new Set();
  const privSgIds = new Set();
  (ctx.instances || []).forEach((inst) => {
    const sgs = (inst.SecurityGroups || []).map((g) => g.GroupId);
    if (pubSubs.has(inst.SubnetId)) {
      sgs.forEach((g) => pubSgIds.add(g));
    } else {
      sgs.forEach((g) => privSgIds.add(g));
    }
  });
  pubSgIds.forEach((gid) => {
    if (privSgIds.has(gid)) {
      ...
      f.push({
        ...
        framework: 'PCI',
        resource: gid,
        resourceName: (sg ? sg.GroupName : '') || gid,
        message: 'SG shared between public and private subnets — network segmentation failure',
        remediation: 'Create separate SGs for each network tier; never share across CDE boundary'
      });
    }
  });
  ...
  (ctx.instances || []).forEach((inst) => {
    ...
    if (hasDefault) {
      f.push({
        ...
        resource: inst.InstanceId,
        resourceName: gn2(inst, inst.InstanceId),
        message: 'EC2 using default SG — non-compliant with configuration standards',
        remediation: 'Replace default SG with purpose-built SG following least privilege'
      });
    }
  });
  ...
  (ctx.rdsInstances || []).forEach((db) => {
    if (!db.StorageEncrypted) {
      f.push({
        ...
        resource: db.DBInstanceIdentifier,
        resourceName: db.DBInstanceIdentifier,
        message: 'RDS storage not encrypted — cardholder data at risk',
        remediation:
          'Enable encryption at rest (requires snapshot + restore for existing instances)'
      });
    }
  });
  (ctx.volumes || []).forEach((vol) => {
    if (!vol.Encrypted && vol.State === 'in-use') {
      f.push({
        ...
        resource: vol.VolumeId,
        resourceName: gn2(vol, vol.VolumeId),
        message: 'EBS volume not encrypted — stored data exposure risk',
        remediation: 'Enable EBS encryption by default; migrate existing volumes via snapshot'
      });
    }
  });
  (ctx.s3bk || []).forEach((bk) => {
    if (!bk.ServerSideEncryption && !bk.BucketEncryption) {
      f.push({
        ...
        resource: bk.Name,
        resourceName: bk.Name,
        message: 'S3 bucket without default encryption — data at rest violation',
        remediation:
          'Enable SSE-S3 or SSE-KMS default encryption; add bucket policy to deny unencrypted uploads'
      });
    }
  });
  ...
  (ctx.rdsInstances || []).forEach((db) => {
    if (db.StorageEncrypted && db.KmsKeyId && db.KmsKeyId.includes('aws/rds')) {
      f.push({
        ...
        resource: db.DBInstanceIdentifier,
        resourceName: db.DBInstanceIdentifier,
        message: 'RDS using AWS-managed key instead of CMK — limited key control',
        remediation: 'Use customer-managed KMS key for full key rotation and access control'
      });
    }
  });
  ...
  (ctx.albs || []).forEach((alb) => {
    ...
    if (alb.Scheme === 'internet-facing' && !hasHttps) {
      f.push({
        ...
        resource: alb.LoadBalancerName,
        resourceName: alb.LoadBalancerName,
        message: 'Internet-facing ALB without HTTPS — data in transit unencrypted',
        remediation: 'Add HTTPS listener with TLS 1.2+ via ACM certificate; redirect HTTP to HTTPS'
      });
    }
  });
  // 6.4.1 – Web application firewall: internet-facing ALB without WAF
  const albsWithWaf = new Set();
  ...
  (ctx.albs || []).forEach((alb) => {
    ...
    if (!albsWithWaf.has(alb.LoadBalancerName)) {
      f.push({
        severity: 'HIGH',
        control: 'PCI-6.4.1',
        framework: 'PCI',
        resource: alb.LoadBalancerName,
        resourceName: alb.LoadBalancerName,
        message: 'Internet-facing ALB without WAF — web app firewall required',
        remediation: 'Associate AWS WAF WebACL with ALB; add OWASP Top 10 managed rule group'
      });
    }
  });
    ...
    (sg.IpPermissions || []).forEach((p) => {
      if (p.IpProtocol === '-1' && _hasOpenCidr(p)) {
        f.push({
          ...
          resource: sg.GroupId,
          resourceName: sg.GroupName || '',
          message: 'SG allows all inbound traffic — violates least privilege',
          remediation: 'Replace with specific port/protocol rules matching business need'
        });
      }
    });
  });
    ...
    (ctx.vpcs || []).forEach((vpc) => {
      if (!vpc.FlowLogs || vpc.FlowLogs.length === 0) {
        f.push({
          ...
          resource: vpc.VpcId,
          resourceName: gn2(vpc, vpc.VpcId),
          message: 'VPC flow logs not enabled — insufficient audit logging',
          remediation: 'Enable VPC Flow Logs with at least 1 year retention for PCI compliance'
        });
      }
    });
  }
  ...
  (ctx.rdsInstances || []).forEach((db) => {
    if (db.PubliclyAccessible) {
      f.push({
        ...
        resource: db.DBInstanceIdentifier,
        resourceName: db.DBInstanceIdentifier,
        message: 'RDS publicly accessible — immediate vulnerability exposure',
        remediation: 'Set PubliclyAccessible=false; access only via private subnet or VPN'
      });
    }
  });
  ...
  (ctx.vpcs || []).forEach((vpc) => {
    ...
    if (instCount > 5 && vpc.FlowLogs !== undefined && !vpc.FlowLogs) {
      f.push({
        severity: 'MEDIUM',
        control: 'PCI-12.10.1',
        framework: 'PCI',
        ...
        resourceName: gn2(vpc, vpc.VpcId),
        message:
          'Large VPC (' + instCount + ' instances) without monitoring — incident response gap',
        remediation: 'Enable GuardDuty, CloudTrail, and VPC Flow Logs; create SNS alerts'
      });
    }
  });
  ...
  (ctx.ecacheClusters || []).forEach((ec) => {
    if (
      ...
      f.push({
        ...
        resource: ec.CacheClusterId,
        resourceName: ec.CacheClusterId,
        message: 'ElastiCache without in-transit encryption — data exposed in network',
        remediation: 'Enable transit encryption; use TLS for Redis connections'
      });
    }
  });
  ...
  (ctx.ecacheClusters || []).forEach((ec) => {
    if (
      ec.AtRestEncryptionEnabled === false ||
      (ec.Engine === 'redis' && !ec.AtRestEncryptionEnabled)
    ) {
      f.push({
        severity: 'HIGH',
        control: 'PCI-3.4.1',
        ...
        resource: ec.CacheClusterId,
        resourceName: ec.CacheClusterId,
        message: 'ElastiCache without at-rest encryption — cached data at risk',
        remediation: 'Enable at-rest encryption (requires new cluster)'
      });
    }
  });
  ...
  (ctx.lambdaFns || []).forEach((fn) => {
    if (fn.Runtime && EOL_RUNTIMES.has(fn.Runtime)) {
      f.push({
        ...
        resource: fn.FunctionName,
        resourceName: fn.FunctionName,
        message: 'Lambda on EOL runtime ' + fn.Runtime + ' — unpatched vulnerabilities',
        remediation: 'Upgrade to supported runtime for security patch coverage'
      });
    }
  });
  ...
  (ctx.cfDistributions || []).forEach((d) => {
    ...
    if (vpp === 'allow-all') {
      f.push({
        ...
        resource: d.Id || d.ARN || '',
        resourceName: d.DomainName || d.Id || '',
        message: 'CloudFront allows unencrypted HTTP — data in transit violation',
        remediation: 'Set ViewerProtocolPolicy to redirect-to-https or https-only'
      });
    }
  });
  ...
  (ctx.redshiftClusters || []).forEach((rs) => {
    if (rs.PubliclyAccessible) {
      f.push({
        ...
        resource: rs.ClusterIdentifier,
        resourceName: rs.ClusterIdentifier,
        message: 'Redshift publicly accessible — data warehouse exposed',
        remediation: 'Disable public access; use private subnet with VPN access only'
      });
    }
  });
  return f;
...
export function runGovernanceChecks(ctx) {
  ...
  trails.forEach((t) => {
    if (!t.IsMultiRegionTrail) {
      f.push({
        ...
        control: 'CIS-2.1',
        framework: 'CIS',
        resource: t.TrailARN || t.Name || '',
        resourceName: t.Name || '',
        message: 'CloudTrail "' + t.Name + '" is not multi-region',
        remediation: 'Enable multi-region trail to capture all API activity across regions'
      });
    }
  });
  ...
  trails.forEach((t) => {
    if (!t.LogFileValidationEnabled) {
      f.push({
        ...
        resource: t.TrailARN || t.Name || '',
        resourceName: t.Name || '',
        message: 'CloudTrail "' + t.Name + '" log file validation disabled',
  ...
  trails.forEach((t) => {
    if (!t.KmsKeyId) {
      f.push({
        ...
        resource: t.TrailARN || t.Name || '',
        resourceName: t.Name || '',
        message: 'CloudTrail "' + t.Name + '" not encrypted with KMS',
        remediation: 'Configure KMS encryption for CloudTrail logs'
      });
    }
  });
  ...
  trails.forEach((t) => {
    if (!t.CloudWatchLogsLogGroupArn) {
      f.push({
        ...
        resource: t.TrailARN || t.Name || '',
        resourceName: t.Name || '',
        message: 'CloudTrail "' + t.Name + '" not integrated with CloudWatch Logs',
  ...
  const flVpcs = new Set(
    flowLogs
      .filter((fl) => fl.ResourceId && fl.ResourceId.startsWith('vpc-'))
  ...
  (ctx.vpcs || []).forEach((v) => {
    if (!flVpcs.has(v.VpcId)) {
      f.push({
        ...
        framework: 'CIS',
        resource: v.VpcId,
        resourceName: gn(v) || v.VpcId,
        message: 'VPC ' + v.VpcId + ' has no flow logs',
        remediation: 'Enable VPC Flow Logs for traffic visibility and audit compliance'
      });
    }
  });
  ...
  detectors.forEach((d) => {
    if (d.Status && d.Status !== 'ENABLED') {
      f.push({
        ...
        resourceName: 'GuardDuty',
        message: 'GuardDuty detector is not enabled (status: ' + d.Status + ')',
    ...
    (d.Features || []).forEach((feat) => {
      if (feat.Status === 'DISABLED') {
        f.push({
          ...
          resource: d.DetectorId || '',
          resourceName: 'GuardDuty',
          message: 'GuardDuty feature ' + feat.Name + ' is disabled',
          remediation: 'Enable ' + feat.Name + ' for comprehensive threat detection'
        });
      }
    });
  });
  ...
  recorders.forEach((r) => {
    ...
    if (!rg.allSupported) {
      f.push({
        ...
        message: 'Config recorder "' + r.name + '" not recording all supported resources',
        remediation: 'Enable all-supported resource recording in AWS Config'
      });
    }
  });
  ...
  if (recorders.length > 0 && rules.length === 0) {
    f.push({
      ...
      resource: '',
      resourceName: 'AWS Config',
      message: 'AWS Config recorder active but no Config rules configured',
      remediation: 'Add AWS Config rules to evaluate resource compliance'
    });
  }
  ...
  if (shStds.length === 0 && (trails.length > 0 || recorders.length > 0 || detectors.length > 0)) {
    f.push({
      ...
      framework: 'GOV',
      resource: '',
      resourceName: 'Security Hub',
      message: 'Security Hub has no enabled standards',
      remediation:
        'Enable Security Hub with AWS Foundational Security Best Practices or CIS standards'
    });
  }
  ...
  if (analyzers.length > 0 && !analyzers.some((a) => a.status === 'ACTIVE')) {
    f.push({
      ...
      remediation: 'Create an IAM Access Analyzer to identify unintended resource access'
    });
  }
  ...
  keys.forEach((k) => {
    if (k.KeyManager === 'CUSTOMER' && k.KeyState === 'Enabled' && !k.RotationEnabled) {
      f.push({
        ...
        control: 'GOV-KMS1',
        framework: 'GOV',
        resource: k.KeyId || k.KeyArn || '',
        resourceName: k.KeyId || '',
        message: 'KMS key ' + k.KeyId + ' does not have automatic rotation enabled',
  ...
  logs.forEach((lg) => {
    if (!lg.retentionInDays) {
      f.push({
        ...
        control: 'GOV-LOG1',
        framework: 'GOV',
        resource: lg.arn || lg.logGroupName || '',
        resourceName: lg.logGroupName || '',
        message:
          'Log group "' +
          lg.logGroupName +
          '" has no retention policy (logs retained indefinitely)',
        remediation: 'Set a retention policy to control storage costs and compliance'
      });
    }
  });
  ...
  repos.forEach((r) => {
    if (r.imageTagMutability === 'MUTABLE') {
      f.push({
        ...
        control: 'GOV-ECR1',
        framework: 'GOV',
        resource: r.repositoryArn || r.repositoryName || '',
        resourceName: r.repositoryName || '',
        message: 'ECR repo "' + r.repositoryName + '" has mutable image tags',
        remediation: 'Enable tag immutability to prevent image overwrites'
      });
    }
  });
  ...
  repos.forEach((r) => {
    ...
    if (!sc.scanOnPush) {
      f.push({
        ...
        resource: r.repositoryArn || r.repositoryName || '',
        resourceName: r.repositoryName || '',
        message: 'ECR repo "' + r.repositoryName + '" does not have scan-on-push enabled',
        remediation: 'Enable scan-on-push to detect vulnerabilities in container images'
      });
    }
  });
  ...
  secs.forEach((s) => {
    if (!s.RotationEnabled) {
      f.push({
        ...
        control: 'GOV-SEC1',
        framework: 'GOV',
        resource: s.ARN || s.Name || '',
        resourceName: s.Name || '',
        message: 'Secret "' + s.Name + '" does not have rotation enabled',
  ...
  apis.forEach((a) => {
    ...
    if (policy !== 'TLS_1_2') {
      f.push({
        ...
        control: 'GOV-APIGW1',
        framework: 'GOV',
        resource: a.id || a.name || '',
        resourceName: a.name || '',
        message:
          'API Gateway "' +
          a.name +
          '" is not enforcing TLS 1.2' +
          (policy ? ' (using ' + policy + ')' : ''),
        remediation: 'Set minimum TLS version to TLS 1.2 on API Gateway'
      });
    }
  });
  return f;
...
export function runComplianceChecks(ctx) {
  ...
  _complianceFindings.forEach((f) => {
    if (_CKV_MAP[f.control]) {
      f.ckv = _CKV_MAP[f.control];
    }
  });
