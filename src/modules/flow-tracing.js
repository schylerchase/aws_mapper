...
export function setFlowMode(v) {
  _flowMode = v;
}

export function getFlowSource() {
  return _flowSource;
}
export function setFlowSource(v) {
  _flowSource = v;
}
...
export function setFlowTarget(v) {
  _flowTarget = v;
}
...
export function setFlowConfig(v) {
  _flowConfig = v;
}
...
export function setFlowPath(v) {
  _flowPath = v;
}

export function getFlowBlocked() {
  return _flowBlocked;
}
export function setFlowBlocked(v) {
  _flowBlocked = v;
}
...
export function setFlowStepIndex(v) {
  _flowStepIndex = v;
}

export function getFlowSelecting() {
  return _flowSelecting;
}
export function setFlowSelecting(v) {
  _flowSelecting = v;
}

export function getFlowWaypoints() {
  return _flowWaypoints;
}
export function setFlowWaypoints(v) {
  _flowWaypoints = v;
}

export function getFlowLegs() {
  return _flowLegs;
}
export function setFlowLegs(v) {
  _flowLegs = v;
}

export function getFlowActiveLeg() {
  return _flowActiveLeg;
}
export function setFlowActiveLeg(v) {
  _flowActiveLeg = v;
}

export function getFlowSelectingWaypoint() {
  return _flowSelectingWaypoint;
}
export function setFlowSelectingWaypoint(v) {
  _flowSelectingWaypoint = v;
}

export function getFlowSuggestions() {
  return _flowSuggestions;
}
export function setFlowSuggestions(v) {
  _flowSuggestions = v;
}
...
export function suggestPort(targetType, targetResource) {
  if (targetType === 'rds') {
    return (targetResource && targetResource.Endpoint && targetResource.Endpoint.Port) || 3306;
  }
  if (targetType === 'ecache') {
    return 6379;
  }
  if (targetType === 'alb') {
    return 443;
...
export function resolveNetworkPosition(type, id, ctx) {
  ...
  if (type === 'rds') {
    ...
    const rSgs2 = (rds2.VpcSecurityGroups || [])
      .map(function (s) {
    ...
    return {
      ...
      cidr: rSubCidr,
      sgs: rSgs2,
      name: rds2.DBInstanceIdentifier
    };
  }
  ...
  if (type === 'alb') {
    ...
    const aSgs = (alb2.SecurityGroups || [])
      .map(function (gid) {
        return (ctx.sgs || []).find(function (sg) {
  ...
  if (type === 'lambda') {
    ...
    const fSgs2 = ((fn2.VpcConfig || {}).SecurityGroupIds || [])
      .map(function (gid) {
        return (ctx.sgs || []).find(function (sg) {
          return sg.GroupId === gid;
        });
  ...
  if (type === 'ecs') {
    ...
    const eSgs2 = (eNc.securityGroups || [])
      .map(function (gid) {
        return (ctx.sgs || []).find(function (sg) {
          return sg.GroupId === gid;
        });
  ...
  if (type === 'ecache') {
    ...
    const ecSgs = (ec2.SecurityGroups || [])
      .map(function (s) {
        return (ctx.sgs || []).find(function (sg) {
    ...
    if (ecVpc) {
      (ctx.subnets || []).forEach(function (s) {
        if (!ecSid && s.VpcId === ecVpc && !(ctx.pubSubs && ctx.pubSubs.has(s.SubnetId))) {
          ecSid = s.SubnetId;
        }
      });
    }
    if (!ecSid && ecVpc) {
      (ctx.subnets || []).forEach(function (s) {
        if (!ecSid && s.VpcId === ecVpc) {
          ecSid = s.SubnetId;
        }
      });
    }
    const ecSubCidr = ecSid
      ? (
          (ctx.subnets || []).find(function (s) {
  ...
  if (type === 'redshift') {
    ...
    const rsSgs = (rs2.VpcSecurityGroups || [])
      .map(function (s) {
        return (ctx.sgs || []).find(function (sg) {
    ...
    if (rsVpc) {
      (ctx.subnets || []).forEach(function (s) {
        if (!rsSid && s.VpcId === rsVpc && !(ctx.pubSubs && ctx.pubSubs.has(s.SubnetId))) {
          rsSid = s.SubnetId;
        }
      });
    }
    if (!rsSid && rsVpc) {
      (ctx.subnets || []).forEach(function (s) {
        if (!rsSid && s.VpcId === rsVpc) {
          rsSid = s.SubnetId;
        }
      });
    }
    const rsSubCidr = rsSid
      ? (
          (ctx.subnets || []).find(function (s) {
...
export function resolveClickTarget(el, ctx, buildResTreeFn) {
  ...
  if (inetNode) {
    return { type: 'internet', id: 'internet' };
  }
  ...
  if (resNode && subNode) {
    ...
    if (tree && tree[resIdx]) {
      ...
      if (res.type === 'EC2') {
        return { type: 'instance', id: res.rid || '' };
      }
      if (res.type === 'ALB') {
        return { type: 'alb', id: res.rid || res.name };
      }
      if (res.type === 'RDS') {
        return { type: 'rds', id: res.rid || res.name };
      }
      if (res.type === 'FN') {
        return { type: 'lambda', id: res.rid || res.name };
      }
      if (res.type === 'ECS') {
        return { type: 'ecs', id: res.rid || res.name };
      }
      if (res.type === 'CACHE') {
        return { type: 'ecache', id: res.rid || res.name };
      }
      if (res.type === 'RS') {
        return { type: 'redshift', id: res.rid || res.name };
      }
      if (res.type === 'ENI') {
...
export function traceInternetToResource(target, config, ctx, opts) {
  ...
  if (!tgtPos) {
    return {
      path: [{ hop: 1, type: 'error', id: '-', action: 'block', detail: 'Cannot resolve target' }],
      blocked: { hop: 1, reason: 'Target not found' }
    };
  }
  ...
  path.push({
    ...
    id: 'Internet',
    action: 'allow',
    detail: 'Source: Internet (0.0.0.0/0)'
  });
  ...
  if (!igw) {
    path.push({
      ...
      id: 'No IGW',
      action: 'block',
      detail: 'No Internet Gateway attached to VPC ' + vpcId
    });
    path.push({
      hop: hopN++,
      type: 'target',
      id: tgtPos.name || target.id,
      action: 'block',
      detail: 'Target unreachable',
      subnetId: tgtPos.subnetId
    });
    return {
      path: path,
      blocked: {
        hop: 2,
        reason: 'No Internet Gateway in target VPC',
        suggestion: 'Attach an Internet Gateway to VPC ' + vpcId
      }
    };
  }
  path.push({
    hop: hopN++,
    type: 'igw-check',
    id: igw.InternetGatewayId || 'IGW',
    action: 'allow',
    detail: 'Internet Gateway ' + igw.InternetGatewayId + ' attached to VPC'
  });
  ...
  if (!isPublic) {
    path.push({
      ...
      id: 'No IGW route',
      action: 'block',
      detail: 'Target subnet ' + tgtPos.subnetId + ' has no route to IGW (private subnet)'
    });
    path.push({
      hop: hopN++,
      type: 'target',
      id: tgtPos.name || target.id,
      action: 'block',
      detail: 'Target in private subnet',
      subnetId: tgtPos.subnetId
    });
    return {
      path: path,
      blocked: {
        hop: hopN - 2,
        reason: 'Target is in a private subnet with no IGW route',
        suggestion: 'Move resource to a public subnet or use an ALB/NAT'
      }
    };
  }
  path.push({
  ...
  path.push({
    hop: hopN++,
    type: 'nacl-inbound',
    id: tgtNacl ? tgtNacl.NetworkAclId || 'NACL' : 'Default NACL',
    action: naclIn.action,
    detail: 'Target subnet NACL inbound from Internet',
    rule: naclIn.rule
  });
  if (naclIn.action === 'deny') {
    ...
    return {
      ...
      blocked: {
        hop: hopN - 2,
        reason: 'NACL denies inbound from Internet',
        suggestion:
          'Add NACL inbound rule allowing ' +
          config.protocol +
          '/' +
          config.port +
          ' from 0.0.0.0/0'
  ...
  path.push({
    ...
    id: 'Target SG',
    action: sgIn.action,
    detail: 'Security Group inbound from Internet',
    rule: sgIn.rule
  });
  if (sgIn.action === 'deny') {
    path.push({
    ...
    return {
      ...
      blocked: {
        hop: hopN - 2,
        reason:
          'Security group denies inbound ' + config.protocol + '/' + config.port + ' from Internet',
        suggestion:
          'Add SG inbound rule allowing ' + config.protocol + '/' + config.port + ' from 0.0.0.0/0'
      }
    };
  }
  ...
  path.push({
    ...
    id: tgtPos.name || target.id,
    action: 'allow',
    detail: 'Target: ' + (tgtPos.name || target.id) + ' (' + target.type + ')',
    subnetId: tgtPos.subnetId
  });
...
export function traceResourceToInternet(source, config, ctx, opts) {
  ...
  if (!srcPos) {
    return {
      path: [{ hop: 1, type: 'error', id: '-', action: 'block', detail: 'Cannot resolve source' }],
      blocked: { hop: 1, reason: 'Source not found' }
    };
  }
  ...
  path.push({
    hop: hopN++,
    type: 'source',
    id: srcPos.name || source.id,
    action: 'allow',
    detail: 'Source: ' + (srcPos.name || source.id) + ' (' + source.type + ')',
    subnetId: srcPos.subnetId
  });

  // SG outbound check
  const sgOpts = opts && opts.discovery ? { assumeAllow: true } : null;
  const sgOut = evaluateSG(
    srcPos.sgs,
    'outbound',
    config.protocol,
    config.port,
  ...
  path.push({
    ...
    id: 'Source SG',
    action: sgOut.action,
    detail: 'SG outbound to Internet',
    rule: sgOut.rule
  });
  if (sgOut.action === 'deny') {
    ...
    return {
      ...
      blocked: {
        hop: 2,
        reason: 'Security group denies outbound',
        suggestion:
          'Add SG outbound rule allowing ' + config.protocol + '/' + config.port + ' to 0.0.0.0/0'
      }
    };
  }
  ...
  path.push({
    hop: hopN++,
    type: 'nacl-outbound',
    id: srcNacl ? srcNacl.NetworkAclId || 'NACL' : 'Default NACL',
    action: naclOut.action,
    detail: 'Source subnet NACL outbound to Internet',
    rule: naclOut.rule
  });
  if (naclOut.action === 'deny') {
    ...
    return {
      ...
      blocked: {
        hop: hopN - 2,
        reason: 'NACL denies outbound to Internet',
        suggestion: 'Add NACL outbound rule allowing ' + config.protocol + '/' + config.port
      }
    };
  }
  ...
  if (hasIgwRoute || hasNatRoute) {
    path.push({
      hop: hopN++,
      type: 'route-table',
      id: srcRT ? srcRT.RouteTableId || 'RT' : 'RT',
      action: 'allow',
      detail: 'Route to Internet via ' + (hasIgwRoute ? 'IGW' : 'NAT') + ' (' + routeTarget + ')',
      rule: '0.0.0.0/0 \u2192 ' + routeTarget
    });
  } else {
    path.push({
      hop: hopN++,
      ...
      id: 'No route',
      action: 'block',
      detail: 'No route to Internet (no IGW or NAT Gateway route in route table)'
    });
    path.push({
      hop: hopN++,
      type: 'target',
      id: 'Internet',
      action: 'block',
      detail: 'No Internet route'
    });
    return {
      path: path,
      blocked: {
        hop: hopN - 2,
        reason: 'No route to Internet in route table',
        suggestion: 'Add a route 0.0.0.0/0 \u2192 IGW or NAT Gateway'
      }
    };
  }
  ...
  path.push({
    ...
    id: 'Internet',
    action: 'allow',
    detail: 'Target: Internet (0.0.0.0/0)'
  });
  return { path: path, blocked: null };
}
...
export function traceFlowLeg(source, target, config, ctx, opts) {
  if (source.type === 'internet') {
    return traceInternetToResource(target, config, ctx, opts);
  }
  if (target.type === 'internet') {
    return traceResourceToInternet(source, config, ctx, opts);
  }
...
export function traceFlow(source, target, config, ctx) {
      ...
      path: [
        {
          ...
          id: '-',
          action: 'block',
          detail: 'Cannot resolve source position'
        }
      ],
      blocked: { hop: 1, reason: 'Source not found' }
    };
      ...
      path: [
        {
          ...
          id: '-',
          action: 'block',
          detail: 'Cannot resolve target position'
        }
      ],
      blocked: { hop: 1, reason: 'Target not found' }
    };
  ...
  const srcSgIds = srcPos.sgs
    .map(function (s) {
      return s.GroupId;
    })
    .filter(Boolean);
  const tgtSgIds = tgtPos.sgs
    .map(function (s) {
      return s.GroupId;
    })
    .filter(Boolean);
  ...
  if (srcPos.subnetId === tgtPos.subnetId) {
    const sgOut = evaluateSG(srcPos.sgs, 'outbound', config.protocol, config.port, tgtCidr, {
      sourceSgIds: tgtSgIds
    });
    path.push({
      hop: hopN++,
      type: 'sg-outbound',
      id: 'Source SG',
      action: sgOut.action,
      detail: 'Security Group outbound check',
      rule: sgOut.rule
    });
    if (sgOut.action === 'deny') {
      const sgInSkip = evaluateSG(tgtPos.sgs, 'inbound', config.protocol, config.port, srcCidr, {
        sourceSgIds: srcSgIds
      });
      path.push({
        hop: hopN++,
        type: 'sg-inbound',
        id: 'Target SG',
        action: 'skip',
        detail: 'Skipped (blocked upstream)',
        rule: sgInSkip.rule
      });
      path.push({
        hop: hopN++,
        ...
        id: tgtPos.name || target.id,
        action: 'block',
        detail: 'Target: ' + (tgtPos.name || target.id),
        subnetId: tgtPos.subnetId
      });
      return {
        path: path,
        blocked: {
          hop: 2,
          reason: 'Source security group denies outbound ' + config.protocol + '/' + config.port,
          suggestion:
            'Add outbound rule to source SG allowing ' +
            config.protocol +
            '/' +
            config.port +
            ' to ' +
    ...
    const sgIn = evaluateSG(tgtPos.sgs, 'inbound', config.protocol, config.port, srcCidr, {
      sourceSgIds: srcSgIds
    });
    path.push({
      hop: hopN++,
      ...
      id: 'Target SG',
      action: sgIn.action,
      detail: 'Security Group inbound check',
      rule: sgIn.rule
    });
    if (sgIn.action === 'deny') {
      path.push({
      ...
      return {
        ...
        blocked: {
          hop: hopN - 2,
          reason: 'Target security group denies inbound ' + config.protocol + '/' + config.port,
          suggestion:
            'Add inbound rule to target SG allowing ' +
            config.protocol +
            '/' +
            config.port +
            ' from ' +
  ...
  if (srcPos.vpcId === tgtPos.vpcId) {
    ...
    path.push({
      hop: hopN++,
      type: 'route-table',
      id: srcRT ? srcRT.RouteTableId || 'RT' : 'Main RT',
      action: rtResult.type === 'blackhole' ? 'block' : 'allow',
      detail:
        'Route table lookup for ' +
        tgtCidr +
        ' => ' +
        rtResult.type +
        (rtResult.target !== 'local' ? ' (' + rtResult.target + ')' : ''),
      rule:
        'Route: ' + rtResult.type + (rtResult.target !== 'local' ? ' via ' + rtResult.target : '')
    });
    if (rtResult.type === 'blackhole') {
      ...
      return {
        ...
        blocked: {
          hop: hopN - 2,
          reason: 'Route table has no route to destination',
          suggestion: 'Add a route to ' + tgtCidr + ' in the source subnet route table'
        }
      };
    }
    ...
    path.push({
      ...
      id: srcNacl ? srcNacl.NetworkAclId || 'NACL' : 'Default NACL',
      action: naclOut.action,
      detail: 'Source subnet NACL outbound',
      rule: naclOut.rule
    });
    if (naclOut.action === 'deny') {
      path.push({
      ...
      return {
        ...
        blocked: {
          hop: hopN - 2,
          reason: 'Source NACL denies outbound traffic',
          suggestion: 'Add NACL outbound rule allowing ' + config.protocol + '/' + config.port
        }
      };
    }

    const sgOut2 = evaluateSG(srcPos.sgs, 'outbound', config.protocol, config.port, tgtCidr, {
      sourceSgIds: tgtSgIds
    });
    path.push({
      hop: hopN++,
      type: 'sg-outbound',
      id: 'Source SG',
      action: sgOut2.action,
      detail: 'Source SG outbound',
      rule: sgOut2.rule
    });
    if (sgOut2.action === 'deny') {
      ...
      return {
        ...
        blocked: {
          hop: hopN - 2,
          reason: 'Source security group denies outbound',
          suggestion: 'Add SG outbound rule for ' + config.protocol + '/' + config.port
        }
      };
    }
    ...
    path.push({
      ...
      id: tgtNacl ? tgtNacl.NetworkAclId || 'NACL' : 'Default NACL',
      action: naclIn.action,
      detail: 'Target subnet NACL inbound',
      rule: naclIn.rule
    });
    if (naclIn.action === 'deny') {
      path.push({
      ...
      return {
        ...
        blocked: {
          hop: hopN - 2,
          reason: 'Target NACL denies inbound traffic',
          suggestion:
            'Add NACL inbound rule allowing ' +
            config.protocol +
            '/' +
    ...
    const sgIn2 = evaluateSG(tgtPos.sgs, 'inbound', config.protocol, config.port, srcCidr, {
      sourceSgIds: srcSgIds
    });
    path.push({
      hop: hopN++,
    ...
    if (sgIn2.action === 'deny') {
      ...
      return {
        ...
        blocked: {
          hop: hopN - 2,
          reason: 'Target security group denies inbound',
          suggestion:
            'Add SG inbound rule for ' + config.protocol + '/' + config.port + ' from source'
        }
      };
    }
  ...
  path.push({
    hop: hopN++,
    type: 'route-table',
    id: srcRTx ? srcRTx.RouteTableId || 'RT' : 'Main RT',
    action: rtResultX.type === 'blackhole' ? 'block' : 'allow',
    detail:
      'Route table lookup for ' +
      tgtCidr +
      ' => ' +
      rtResultX.type +
      (rtResultX.target !== 'local' ? ' (' + rtResultX.target + ')' : ''),
    rule:
      'Route: ' + rtResultX.type + (rtResultX.target !== 'local' ? ' via ' + rtResultX.target : '')
  });
  if (rtResultX.type === 'blackhole') {
    ...
    return {
      ...
      blocked: {
        hop: hopN - 2,
        reason: 'Route table has no route to destination',
        suggestion: 'Add a route to ' + tgtCidr + ' via peering or TGW'
      }
    };
  }
  ...
  path.push({
    hop: hopN++,
    type: 'nacl-outbound',
    id: srcNaclX ? srcNaclX.NetworkAclId || 'NACL' : 'Default NACL',
    action: naclOutX.action,
    detail: 'Source subnet NACL outbound',
    rule: naclOutX.rule
  });
  ...
  const sgOutX = evaluateSG(srcPos.sgs, 'outbound', config.protocol, config.port, tgtCidr, {
    sourceSgIds: tgtSgIds
  });
  path.push({
    hop: hopN++,
  ...
  (ctx.peerings || []).forEach(function (p) {
    ...
    if (
      (req.VpcId === srcPos.vpcId && acc.VpcId === tgtPos.vpcId) ||
      (acc.VpcId === srcPos.vpcId && req.VpcId === tgtPos.vpcId)
    ) {
      peeringRoute = p;
    }
  });
  ...
  if (peeringRoute) {
    path.push({
      hop: hopN++,
      type: 'peering',
      id: peeringRoute.VpcPeeringConnectionId || 'PCX',
      action: 'allow',
      detail: 'VPC Peering connection between ' + srcPos.vpcId + ' and ' + tgtPos.vpcId,
      rule: 'Peering: ' + peeringRoute.VpcPeeringConnectionId
    });
  } else {
    let tgwRoute = false;
    (ctx.tgwAttachments || []).forEach(function (att) {
      if (att.ResourceId === srcPos.vpcId || att.ResourceId === tgtPos.vpcId) {
        tgwRoute = true;
      }
    });
    if (tgwRoute) {
      path.push({
        ...
        id: 'Transit Gateway',
        action: 'allow',
        detail: 'Transit Gateway route between VPCs'
      });
    } else {
      path.push({
        hop: hopN++,
        ...
        id: 'No route',
        action: 'block',
        detail: 'No peering or TGW connection between VPCs'
      });
      path.push({
        hop: hopN++,
        type: 'target',
      ...
      return {
        ...
        blocked: {
          hop: hopN - 2,
          reason: 'No connectivity between VPCs',
          suggestion: 'Create a VPC peering connection or Transit Gateway attachment'
        }
      };
    }
  }
  ...
  path.push({
    hop: hopN++,
    type: 'nacl-inbound',
    id: tgtNaclX ? tgtNaclX.NetworkAclId || 'NACL' : 'Default NACL',
    action: naclInX.action,
    detail: 'Target subnet NACL inbound',
    rule: naclInX.rule
  });
  ...
  const sgIn3 = evaluateSG(tgtPos.sgs, 'inbound', config.protocol, config.port, srcCidr, {
    sourceSgIds: srcSgIds
  });
  path.push({
    hop: hopN++,
    ...
    id: 'Target SG',
    action: sgIn3.action,
    detail: 'Target SG inbound (cross-VPC)',
    rule: sgIn3.rule
  });
  if (sgIn3.action === 'deny') {
    ...
    return {
      ...
      blocked: {
        hop: hopN - 2,
        reason: 'Target SG denies inbound from cross-VPC source',
        suggestion: 'Add SG inbound rule for ' + config.protocol + '/' + config.port
      }
    };
  }
...
export function findAlternatePaths(source, target, config, ctx) {
  ...
  (ctx.instances || []).forEach(function (inst) {
    ...
    if (!isInternet && instVpc !== vpcId) {
      return;
    }
    if (inst.InstanceId === (target.type === 'instance' ? target.id : '')) {
      return;
    }
    if (inst.InstanceId === (source.type === 'instance' ? source.id : '')) {
      return;
    }
    ...
    const gn2 = inst.Tags
      ...
      : inst.InstanceId;
    candidates.push({
      ref: { type: 'instance', id: inst.InstanceId },
      name: gn2,
      isPub: isPub,
      defaultPort: 22
    });
  ...
  Object.keys(ctx.albBySub || {}).forEach(function (sid) {
    const sub = (ctx.subnets || []).find(function (s) {
      return s.SubnetId === sid;
    });
    if (!sub || (!isInternet && sub.VpcId !== vpcId)) {
      return;
    }
    (ctx.albBySub[sid] || []).forEach(function (alb) {
      const albId = alb.LoadBalancerArn ? alb.LoadBalancerArn.split('/').pop() : '';
      if (albId === (target.type === 'alb' ? target.id : '')) {
        return;
      }
      candidates.push({
        ref: { type: 'alb', id: albId || alb.LoadBalancerName },
        name: alb.LoadBalancerName || albId,
        isPub: true,
        defaultPort: 443
      });
    });
  ...
  for (let i = 0; i < candidates.length && tested < 20 && results.length < 5; i++) {
    ...
    results.push({
      via: { type: cand.ref.type, id: cand.ref.id, name: cand.name },
      leg1Result: leg1,
      leg2Result: leg2,
      leg1Config: leg1Config
    });
  }
  return results;
}
