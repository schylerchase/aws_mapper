...
function _fwDashRender() {
  ...
  sgs.forEach(function (sg) {
    ...
    (sg.IpPermissions || []).forEach(function (p) {
      (p.IpRanges || []).forEach(function (r) {
        if (r.CidrIp === '0.0.0.0/0') hasOpen = true;
      });
      (p.Ipv6Ranges || []).forEach(function (r) {
        if (r.CidrIpv6 === '::/0') hasOpen = true;
      });
    });
    rows.push({
      type: 'sg',
  ...
  rows.forEach(function (r) {
    if (r.comp) {
      ...
      if ((sevOrder[r.comp.worst] || 9) < (sevOrder[worstSev] || 9)) worstSev = r.comp.worst;
    }
  ...
  filtered.forEach(function (r, idx) {
    let trCls = '';
    if (r.edits) trCls += ' has-edits';
    if (r.comp) trCls += ' has-findings';
    ...
    if (cmds.length && navigator.clipboard && navigator.clipboard.writeText) {
      navigator.clipboard
        ...
        .then(function () {
          ...
          setTimeout(function () {
            if (btn.parentNode) btn.textContent = 'Export All CLI';
          }, 1500);
        })
        .catch(function () {
