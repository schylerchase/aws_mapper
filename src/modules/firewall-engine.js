...
function _fwApplyRule(type, resourceId, direction, ruleData) {
  if (type === 'nacl') {
    ...
    if (idx >= 0) {
      arr[idx] = Object.assign({}, ruleData);
    } else {
      arr.push(Object.assign({}, ruleData));
    }
  } else if (type === 'route') {
    ...
    if (idx >= 0) {
      rt.Routes[idx] = Object.assign({}, ruleData);
    } else {
      rt.Routes.push(Object.assign({}, ruleData));
    }
  }
}
...
function _fwRuleMatch(a, b) {
  ...
  if (String(a.IpProtocol) !== String(b.IpProtocol)) {
    return false;
  }
  if ((a.FromPort || 0) !== (b.FromPort || 0)) {
    return false;
  }
  if ((a.ToPort || 0) !== (b.ToPort || 0)) {
    return false;
  }
...
function _fwValidateNaclRule(rule, existingEntries, direction) {
  ...
  if (existingEntries && !isNaN(num)) {
    ...
    if (dup) {
      errs.push('Duplicate rule number ' + num + ' in ' + direction + ' direction');
    }
  }
  if (!_fwValidateCidr(rule.CidrBlock)) {
    errs.push('Invalid CIDR format');
  }
    ...
    if (!rule.PortRange) {
      ...
      if (isNaN(from) || isNaN(to)) {
        ...
        if (from < 0 || from > 65535) {
          errs.push('From port must be 0-65535');
        }
        if (to < 0 || to > 65535) {
          errs.push('To port must be 0-65535');
        }
        if (from > to) {
          errs.push('From port must be <= To port');
        }
      }
    }
  }
...
function _fwValidateSgRule(rule) {
  ...
  if (!validProtos.includes(proto)) {
    errs.push('Invalid protocol: ' + proto);
  }
  if (proto === 'tcp' || proto === 'udp') {
    ...
    if (isNaN(from) || isNaN(to)) {
      ...
      if (from < 0 || from > 65535) {
        errs.push('FromPort must be 0-65535');
      }
      if (to < 0 || to > 65535) {
        errs.push('ToPort must be 0-65535');
      }
      if (from > to) {
        errs.push('FromPort must be <= ToPort');
      }
    }
  }
  ...
  if (hasCidr) {
    (rule.IpRanges || []).forEach((r) => {
      if (r.CidrIp && !_fwValidateCidr(r.CidrIp)) {
        errs.push('Invalid CIDR: ' + r.CidrIp);
      }
    });
  }
  return errs;
...
function _fwValidateRoute(route, existingRoutes) {
  const errs = [];
  if (!_fwValidateCidr(route.DestinationCidrBlock)) {
    errs.push('Invalid destination CIDR');
  }
  if (existingRoutes) {
    const dup = existingRoutes.some((r) => r.DestinationCidrBlock === route.DestinationCidrBlock);
    if (dup) {
      errs.push('Duplicate destination CIDR: ' + route.DestinationCidrBlock);
    }
  }
  const hasTarget =
    route.GatewayId ||
    ...
    route.VpcEndpointId;
  if (!hasTarget) {
    errs.push('Route target required');
  }
  return errs;
}
...
function _fwGenerateCli(edits) {
  ...
  list.forEach((edit) => {
    if (edit.type === 'nacl') {
      _fwGenNaclCli(edit, cmds);
    } else if (edit.type === 'sg') {
      _fwGenSgCli(edit, cmds);
    } else if (edit.type === 'route') {
      _fwGenRouteCli(edit, cmds);
    }
  });
  return cmds;
}
...
function _fwSgRuleCmd(verb, sgId, rule) {
  ...
  if (cidrs.length) {
    cmd += ' --cidr ' + cidrs[0];
  } else if (sgRefs.length) {
    cmd += ' --source-group ' + sgRefs[0];
  }
  return cmd;
}
...
function _fwRouteCmd(verb, rtId, route) {
  ...
  if (route.GatewayId) {
    cmd += ' --gateway-id ' + route.GatewayId;
  } else if (route.NatGatewayId) {
    cmd += ' --nat-gateway-id ' + route.NatGatewayId;
  } else if (route.TransitGatewayId) {
    cmd += ' --transit-gateway-id ' + route.TransitGatewayId;
    ...
    cmd += ' --vpc-endpoint-id ' + route.VpcEndpointId;
  }
  return cmd;
}
...
function _fwRenderNaclInline(nacl, sub) {
  ...
  let h =
    '<div class="dp-kv"><span class="k">ID</span><span class="v">' +
    naclId +
    (ec ? '<span class="fw-badge edits">' + ec + ' edit' + (ec > 1 ? 's' : '') + '</span>' : '') +
    '</span></div>';
  h +=
    '<div class="dp-kv"><span class="k">Name</span><span class="v">' +
    gn(nacl, naclId) +
    '</span></div>';
  h += _fwRenderNaclDirection(
    nacl,
    (nacl.Entries || [])
      .filter(function (e) {
        return !e.Egress;
  ...
  if (allWarn.length) {
    h +=
      '<div style="margin-top:6px;padding:4px 6px;background:rgba(245,158,11,.08);border:1px solid rgba(245,158,11,.2);border-radius:4px;font-size:calc(8px * var(--txt-scale) * var(--dp-txt-scale));font-family:Segoe UI,system-ui,sans-serif">';
    h +=
      '<div style="color:var(--accent-orange);font-weight:600;margin-bottom:2px">Shadow Warnings</div>';
    allWarn.forEach(function (w) {
      h += '<div style="color:var(--text-muted);margin:1px 0">' + _escHtml(w) + '</div>';
    });
    h += '</div>';
  }
  ...
  h +=
    '<button data-fw-action="export-cli" data-fw-type="nacl" data-nacl-id="' +
    naclId +
    '">Export CLI</button>';
...
function _fwRenderNaclDirection(nacl, entries, direction, sub) {
  ...
  const isEgress = direction === 'egress';
  let h =
    '<div style="margin-top:6px"><div class="dp-row"><span class="lbl">' + label + '</span></div>';
  const userRules = entries.filter(function (e) {
    return e.RuleNumber !== 32767;
  });
  userRules.forEach(function (e) {
    ...
    h +=
      '<div class="fw-arrow ' +
      cls +
      '"><div class="fw-arrow-line"></div><div class="fw-arrow-head"></div></div>';
    ...
    h +=
      '<button class="fw-edit-btn edit" data-fw-action="edit-nacl" data-nacl-id="' +
      naclId +
      '" data-rule-num="' +
      e.RuleNumber +
      '" data-egress="' +
      ...
      '" title="Edit">&#9998;</button>';
    h +=
      '<button class="fw-edit-btn del" data-fw-action="delete-nacl" data-nacl-id="' +
      naclId +
      '" data-rule-num="' +
      e.RuleNumber +
      '" data-egress="' +
  ...
  h +=
    '<div class="fw-arrow deny"><div class="fw-arrow-line"></div><div class="fw-arrow-head"></div></div>';
  h += '<span class="fw-proto">ALL</span>';
  h += '<span class="fw-port deny">#* ALL</span>';
  h += '<span class="fw-src">0.0.0.0/0</span>';
  h += '</div>';
  h +=
    '<button class="fw-edit-btn add" data-fw-action="add-nacl" data-nacl-id="' +
    naclId +
    '" data-egress="' +
    isEgress +
    '" data-direction="' +
...
function _fwShowNaclEditForm(naclId, ruleNum, egress, container) {
  const nacl = (_rlCtx.nacls || []).find(function (n) {
    return n.NetworkAclId === naclId;
  });
  if (!nacl) {
    return;
  }
  ...
  if (ruleNum !== null && ruleNum !== undefined) {
    ...
    existing = (nacl.Entries || []).find(function (e) {
      return e.RuleNumber === rn && e.Egress === isEgress;
    });
  }
  ...
  const formHtml =
    '<input class="fw-input" data-field="ruleNum" type="number" min="1" max="32766" placeholder="#" value="' +
    rNum +
    '" style="width:48px" title="Rule Number">' +
    '<select class="fw-select" data-field="action" title="Action">' +
    '<option value="allow"' +
    (rAction === 'allow' ? ' selected' : '') +
    '>Allow</option>' +
    '<option value="deny"' +
    (rAction === 'deny' ? ' selected' : '') +
    '>Deny</option>' +
    '</select>' +
    ...
    '>ALL</option>' +
    '</select>' +
    '<input class="fw-input" data-field="portFrom" type="number" min="0" max="65535" placeholder="From" value="' +
    portFrom +
    '" style="width:52px" ' +
    disablePorts +
    '>' +
    '<input class="fw-input" data-field="portTo" type="number" min="0" max="65535" placeholder="To" value="' +
    portTo +
    '" style="width:52px" ' +
    disablePorts +
    '>' +
    '<input class="fw-input" data-field="cidr" placeholder="CIDR" value="' +
    _escHtml(cidr) +
    '" style="width:100px">' +
    '<button class="fw-edit-btn save" data-fw-action="save-nacl" data-nacl-id="' +
    _escHtml(naclId) +
    '" data-egress="' +
    _escHtml(String(isEgress)) +
    '" data-direction="' +
    _escHtml(direction) +
    '"' +
    (existing ? ' data-editing="' + _escHtml(String(rNum)) + '"' : '') +
    '>Save</button>' +
...
function _fwRenderSgInline(sg) {
  ...
  let h =
    '<div style="margin-bottom:8px;padding:6px;background:rgba(255,255,255,.02);border:1px solid var(--border);border-radius:4px">';
  h +=
    '<div class="dp-kv"><span class="k">' +
    _escHtml(sg.GroupName) +
    '</span><span class="v">' +
    sgId +
    (ec ? '<span class="fw-badge edits">' + ec + ' edit' + (ec > 1 ? 's' : '') + '</span>' : '') +
    '</span></div>';
  if (sg.Description) {
    h +=
      '<div style="font-size:calc(8px * var(--txt-scale) * var(--dp-txt-scale));color:var(--text-muted);margin-bottom:4px">' +
      _escHtml(sg.Description) +
      '</div>';
  }
  ...
  h +=
    '<button data-fw-action="export-cli" data-fw-type="sg" data-sg-id="' +
    sgId +
    '">Export CLI</button>';
  h += '<button data-fw-action="undo">Undo</button>';
  h += '<button data-fw-action="reset">Reset</button>';
...
function _fwRenderSgDirection(sg, direction) {
  ...
  let h =
    '<div style="margin-top:4px"><div class="dp-row"><span class="lbl">' + label + '</span></div>';
  rules.forEach(function (p, idx) {
    ...
    const port =
      p.IpProtocol === '-1'
        ...
        : p.FromPort === p.ToPort
          ? p.FromPort || 'ALL'
          : (p.FromPort || '*') + '-' + (p.ToPort || '*');
    const sources = (p.IpRanges || [])
      .map(function (r) {
        return r.CidrIp;
      })
      .concat(
        (p.UserIdGroupPairs || []).map(function (g) {
          return g.GroupId;
        })
      );
    ...
    h += '<div class="fw-edit-row">';
    h +=
      '<div class="fw-arrow allow"><div class="fw-arrow-line"></div><div class="fw-arrow-head"></div></div>';
    h += '<span class="fw-proto">' + proto + '</span>';
    h += '<span class="fw-port allow">' + port + '</span>';
    h += '<span class="fw-src">' + _escHtml(srcStr) + '</span>';
    h += '<span style="margin-left:auto;display:flex;gap:2px">';
    h +=
      '<button class="fw-edit-btn edit" data-fw-action="edit-sg" data-sg-id="' +
      sgId +
      '" data-rule-idx="' +
      idx +
      '" data-direction="' +
      ...
      '" title="Edit">&#9998;</button>';
    h +=
      '<button class="fw-edit-btn del" data-fw-action="delete-sg" data-sg-id="' +
      sgId +
      '" data-rule-idx="' +
      idx +
      '" data-direction="' +
  ...
  h +=
    '<button class="fw-edit-btn add" data-fw-action="add-sg" data-sg-id="' +
    sgId +
    '" data-direction="' +
    direction +
    '">+ Add Rule</button>';
...
function _fwShowSgEditForm(sgId, ruleIdx, direction, container) {
  ...
  const portFrom =
    existing && existing.FromPort !== undefined && existing.FromPort !== -1
      ? existing.FromPort
      : '';
  const portTo =
    existing && existing.ToPort !== undefined && existing.ToPort !== -1 ? existing.ToPort : '';
  let source = '0.0.0.0/0';
  if (existing) {
    const cidrs = (existing.IpRanges || [])
      .map(function (r) {
        return r.CidrIp;
      })
      .filter(Boolean);
    const sgRefs = (existing.UserIdGroupPairs || [])
      .map(function (g) {
        return g.GroupId;
      })
      .filter(Boolean);
  ...
  const formHtml =
    ...
    disablePorts +
    '>' +
    '<input class="fw-input" data-field="source" placeholder="CIDR or sg-xxx" value="' +
    _escHtml(source) +
    '" style="width:110px">' +
    '<button class="fw-edit-btn save" data-fw-action="save-sg" data-sg-id="' +
    _escHtml(sgId) +
    '" data-direction="' +
    _escHtml(direction) +
    '"' +
    (existing ? ' data-editing="' + _escHtml(String(ruleIdx)) + '"' : '') +
    '>Save</button>' +
    '<button class="fw-edit-btn cancel" data-fw-action="cancel-edit">Cancel</button>';
  row.innerHTML = formHtml;
  container.appendChild(row);
...
function _fwRenderRtInline(rt, vpcId, lk) {
  ...
  (rt.Routes || []).forEach(function (r, idx) {
    ...
    if (!isLocal) {
      h += '<span style="margin-left:auto;display:flex;gap:2px">';
      h +=
        '<button class="fw-edit-btn edit" data-fw-action="edit-rt" data-rt-id="' +
        rtId +
        '" data-rule-idx="' +
        idx +
        '" title="Edit">&#9998;</button>';
      h +=
        '<button class="fw-edit-btn del" data-fw-action="delete-rt" data-rt-id="' +
        rtId +
        '" data-rule-idx="' +
        idx +
        '" title="Delete">&#10005;</button>';
  ...
  h +=
    '<button class="fw-edit-btn add" data-fw-action="add-rt" data-rt-id="' +
    rtId +
    '">+ Add Route</button>';
  h += '<div class="fw-toolbar">';
  h += '<button data-fw-action="full-editor" data-rt-id="' + rtId + '">Full Editor</button>';
  h +=
    '<button data-fw-action="export-cli" data-fw-type="route" data-rt-id="' +
    rtId +
    '">Export CLI</button>';
  h += '<button data-fw-action="undo">Undo</button>';
  h += '<button data-fw-action="reset">Reset</button>';
...
function _fwShowRtEditForm(rtId, routeIdx, container, vpcId, lk) {
  if (!lk) {
    lk = { igws: [], nats: [], vpces: [], peerings: [], tgwAttachments: [] };
  }
  const rt = (_rlCtx.rts || []).find(function (r) {
    return r.RouteTableId === rtId;
  });
  if (!rt) {
    return;
  }
  const existing =
    routeIdx !== null && routeIdx !== undefined ? (rt.Routes || [])[parseInt(routeIdx, 10)] : null;
  const dest = existing ? existing.DestinationCidrBlock || '' : '';
  let selTgt = '';
  if (existing) {
    selTgt =
      existing.GatewayId ||
      existing.NatGatewayId ||
      existing.TransitGatewayId ||
      existing.VpcPeeringConnectionId ||
      existing.VpcEndpointId ||
      '';
  }
  ...
  (lk.nats || []).forEach(function (n) {
    if (n.VpcId === vpcId) {
      ...
      opts +=
        '<option value="' + n.NatGatewayId + '"' + sel + '>' + n.NatGatewayId + ' (NAT)</option>';
    }
  });
  ...
  const formHtml =
    '<input class="fw-input" data-field="dest" placeholder="Dest CIDR" value="' +
    _escHtml(dest) +
    '" style="width:110px">' +
    '<span class="p" style="margin:0 4px">-&gt;</span>' +
    '<select class="fw-select" data-field="target" style="width:160px" title="Target">' +
    opts +
    '</select>' +
    '<button class="fw-edit-btn save" data-fw-action="save-rt" data-rt-id="' +
    _escHtml(rtId) +
    '"' +
    (existing ? ' data-editing="' + _escHtml(String(routeIdx)) + '"' : '') +
    '>Save</button>' +
    '<button class="fw-edit-btn cancel" data-fw-action="cancel-edit">Cancel</button>';
  row.innerHTML = formHtml;
  container.appendChild(row);
...
function _fwOpenFullEditor(type, resourceId, sub, vpcId, lk) {
  ...
  if (type === 'nacl') {
    const nacl = (_rlCtx.nacls || []).find(function (n) {
      return n.NetworkAclId === resourceId;
    });
    if (nacl) {
      name = gn(nacl, resourceId);
    }
  } else if (type === 'sg') {
    const sg = (_rlCtx.sgById || {})[resourceId];
    if (sg) {
      name = sg.GroupName || gn(sg, resourceId);
    }
  } else if (type === 'route') {
    const rt = (_rlCtx.rts || []).find(function (r) {
      return r.RouteTableId === resourceId;
    });
    if (rt) {
      name = gn(rt, resourceId);
    }
  }
  const vpcObj = vpcId
    ? (_rlCtx.vpcs || []).find(function (v) {
        return v.VpcId === vpcId;
      })
    : null;
  const vpcLabel = vpcObj ? gn(vpcObj, vpcId) : vpcId || '';
  titleEl.innerHTML =
    esc(label + ': ' + (name || resourceId)) +
    (vpcId
      ? ' <span class="fw-link" id="fwFpVpcLink" style="font-size:10px;font-weight:400">' +
        esc(vpcLabel) +
        '</span>'
      : '');
  const vpcLink = document.getElementById('fwFpVpcLink');
  if (vpcLink) {
    vpcLink.addEventListener('click', function (e) {
      e.stopPropagation();
      document.getElementById('fwFullPanel').classList.remove('open');
      closeUnifiedDash();
      setTimeout(function () {
        _zoomToElement(vpcId);
      }, 250);
    });
  }
...
function _fwRefreshFullPanel() {
  ...
  if (_fwFpType === 'nacl') {
    const nacl = (_rlCtx.nacls || []).find(function (n) {
      return n.NetworkAclId === _fwFpResId;
    });
    if (!nacl) {
      bodyEl.innerHTML = '<div style="color:var(--text-muted);padding:12px">NACL not found</div>';
      return;
    }
    const isEgress = _fwFpDir === 'egress';
    const entries = (nacl.Entries || [])
      .filter(function (e) {
        return isEgress ? e.Egress : !e.Egress;
      })
      .sort(function (a, b) {
        return a.RuleNumber - b.RuleNumber;
      });
    ...
    if (warns.length) {
      h +=
        '<div style="margin-top:6px;padding:4px 6px;background:rgba(245,158,11,.08);border:1px solid rgba(245,158,11,.2);border-radius:4px;font-size:10px;font-family:Segoe UI,system-ui,sans-serif">';
      h +=
        '<div style="color:var(--accent-orange);font-weight:600;margin-bottom:2px">Shadow Warnings</div>';
    ...
    if (!sg) {
      bodyEl.innerHTML =
        '<div style="color:var(--text-muted);padding:12px">Security Group not found</div>';
      return;
    }
    h += _fwRenderSgDirection(sg, _fwFpDir);
  } else if (_fwFpType === 'route') {
    const rt = (_rlCtx.rts || []).find(function (r) {
      return r.RouteTableId === _fwFpResId;
    });
    if (!rt) {
      bodyEl.innerHTML =
        '<div style="color:var(--text-muted);padding:12px">Route Table not found</div>';
      return;
    }
  ...
  if (_fpResComp && _fpResComp.findings.length) {
    ...
    h +=
      '<div style="font-size:11px;font-weight:600;color:var(--accent-orange);margin-bottom:8px">Compliance Findings (' +
      _fpResComp.count +
      ')</div>';
    _fpResComp.findings.forEach(function (f) {
      ...
      h +=
        ...
        f.severity +
        '" style="font-size:8px;padding:1px 5px;margin-right:6px">' +
        f.severity +
        '</span>';
      h +=
        '<span class="fw-finding-ctrl" data-fw-ctrl="' +
        _escHtml(f.control) +
        '">' +
        _escHtml(f.control) +
        '</span>';
      if (f.ckv) {
        h += ' <span style="opacity:.5;font-size:8px">(' + _escHtml(f.ckv) + ')</span>';
      }
      h +=
        '<div style="margin:4px 0 2px;color:var(--text-secondary);font-size:10px">' +
        _escHtml(f.message) +
        '</div>';
      h +=
        '<div style="color:var(--text-muted);font-size:9px">Remediation: ' +
        _escHtml(f.remediation) +
        '</div>';
      h += '</div>';
    });
    h += '</div>';
  ...
  bodyEl.querySelectorAll('.fw-finding-ctrl').forEach(function (el) {
    el.addEventListener('click', function (e) {
      ...
      _compDashState = {
        ...
        showMuted: false,
        execSummary: false,
        view: _compDashState.view || 'action'
      };
  ...
  if (bodyEl._fwFullPanelActionHandler) {
    bodyEl.removeEventListener('click', bodyEl._fwFullPanelActionHandler);
  }
  bodyEl._fwFullPanelActionHandler = function (ev) {
    if (ev.target.closest('.fw-finding-ctrl')) {
      return;
    } // already handled above
    _fwHandleAction(ev, _fwFpSub, _fwFpVpcId, _fwFpLk);
  };
  bodyEl.addEventListener('click', bodyEl._fwFullPanelActionHandler);

  // Render visual pane: show arrow visualization for current direction
  let vH = '';
  if (_fwFpType === 'nacl') {
    const vNacl = (_rlCtx.nacls || []).find(function (n) {
      return n.NetworkAclId === _fwFpResId;
    });
    if (vNacl) {
      const vIsEgress = _fwFpDir === 'egress';
      const vEntries = (vNacl.Entries || [])
        .filter(function (e) {
          return vIsEgress ? e.Egress : !e.Egress;
        })
        .sort(function (a, b) {
          return a.RuleNumber - b.RuleNumber;
        });
      ...
      vH +=
        '<div style="font-size:10px;font-weight:600;color:var(--text-secondary);margin-bottom:6px;font-family:Segoe UI,system-ui,sans-serif">' +
        vLabel +
        ' FLOW</div>';
      vEntries
        .filter(function (e) {
        ...
        .forEach(function (e) {
          ...
          vH +=
            ...
            port +
            ' ' +
            (e.RuleAction === 'allow' ? 'ALLOW' : 'DENY') +
            '</span>';
          vH += '</div>';
        });
      vH +=
        '<div class="fw-edit-row" style="padding:2px 0;opacity:.4"><div class="fw-arrow deny"><div class="fw-arrow-line"></div><div class="fw-arrow-head"></div></div><span style="font-size:9px;color:var(--text-muted)">#* DENY ALL</span></div>';
    }
  } else if (_fwFpType === 'sg') {
    const vSg = (_rlCtx.sgById || {})[_fwFpResId];
    if (vSg) {
      const vRules =
        _fwFpDir === 'ingress' ? vSg.IpPermissions || [] : vSg.IpPermissionsEgress || [];
    ...
    const vRt = (_rlCtx.rts || []).find(function (r) {
      return r.RouteTableId === _fwFpResId;
    });
    if (vRt) {
      vH +=
        '<div style="font-size:10px;font-weight:600;color:var(--text-secondary);margin-bottom:6px;font-family:Segoe UI,system-ui,sans-serif">ROUTE FLOW</div>';
  ...
  const filtered = _fwEdits.filter(function (ed) {
    return ed.resourceId === _fwFpResId;
  });
  ...
  if (cmds.length) {
    ...
    cliEl.innerHTML =
      '<span style="color:var(--text-muted);font-style:italic">No pending edits</span>';
  }
}
...
function _fwHandleAction(e, sub, vpcId, lk) {
  ...
  const dpBody = _inFullPanel
    ? document.getElementById('fwFpBody')
  ...
  if (action === 'undo') {
    _fwUndo();
    if (sub && lk) {
      openSubnetPanel(sub, vpcId, lk);
    }
    if (
      typeof _fwDashRender === 'function' &&
      _udashTab === 'firewall' &&
      document.getElementById('udash') &&
      document.getElementById('udash').classList.contains('open')
    ) {
      _fwDashRender();
    }
    if (document.getElementById('fwFullPanel').classList.contains('open')) {
      _fwRefreshFullPanel();
    }
    return;
  }
  ...
  if (action === 'export-cli') {
    ...
    const resId =
      el.getAttribute('data-nacl-id') ||
      el.getAttribute('data-sg-id') ||
      el.getAttribute('data-rt-id');
    const filtered = _fwEdits.filter(function (ed) {
      return ed.type === fwType && ed.resourceId === resId;
    });
    const cmds = _fwGenerateCli(filtered);
    if (cmds.length) {
      const txt = cmds.join('\n');
  ...
  // NACL actions
  if (action === 'edit-nacl') {
    dpBody.querySelectorAll('[data-fw-form]').forEach(function (f) {
      f.remove();
    });
  ...
  if (action === 'delete-nacl') {
    ...
    const dNacl = (_rlCtx.nacls || []).find(function (n) {
      return n.NetworkAclId === dNaclId;
    });
    if (!dNacl) {
      return;
    }
    const dEntry = (dNacl.Entries || []).find(function (en) {
      return en.RuleNumber === dRuleNum && en.Egress === dEgress;
    });
    if (!dEntry) {
      return;
    }
    _fwTakeSnapshot();
    const dIdx = (dNacl.Entries || []).findIndex(function (en) {
      return en.RuleNumber === dRuleNum && en.Egress === dEgress;
    });
    if (dIdx >= 0) {
      dNacl.Entries.splice(dIdx, 1);
    }
    _fwEdits.push({
      type: 'nacl',
      resourceId: dNaclId,
      ...
      action: 'delete',
      rule: Object.assign({}, dEntry),
      originalRule: Object.assign({}, dEntry)
    });
    _fwRebuildLookups();
    if (sub && lk) {
      openSubnetPanel(sub, vpcId, lk);
  ...
  if (action === 'save-nacl') {
    ...
    const sNacl = (_rlCtx.nacls || []).find(function (n) {
      return n.NetworkAclId === sNaclId;
    });
    const otherEntries = sNacl
      ? (sNacl.Entries || []).filter(function (en) {
          if (
            sEditing !== null &&
            sEditing !== undefined &&
            en.RuleNumber === parseInt(sEditing, 10) &&
            en.Egress === sEgress
          ) {
            return false;
          }
    ...
    formRow.querySelectorAll('.fw-input,.fw-select').forEach(function (inp) {
      inp.classList.remove('invalid');
    });
    if (errs.length) {
      errs.forEach(function (er) {
        if (er.indexOf('Rule number') >= 0) {
          formRow.querySelector('[data-field="ruleNum"]').classList.add('invalid');
        }
        if (er.indexOf('CIDR') >= 0) {
          formRow.querySelector('[data-field="cidr"]').classList.add('invalid');
        }
        if (er.indexOf('Port') >= 0 || er.indexOf('port') >= 0) {
          ...
          if (pfEl) {
            pfEl.classList.add('invalid');
          }
          if (ptEl) {
            ptEl.classList.add('invalid');
          }
        }
      });
      return;
    ...
    if (sEditing !== null && sEditing !== undefined) {
      ...
      origRule = (sNacl.Entries || []).find(function (en) {
        return en.RuleNumber === editRn && en.Egress === sEgress;
      });
      if (origRule) {
        origRule = Object.assign({}, origRule);
      }
      const oldIdx = (sNacl.Entries || []).findIndex(function (en) {
        return en.RuleNumber === editRn && en.Egress === sEgress;
      });
      if (oldIdx >= 0) {
        sNacl.Entries.splice(oldIdx, 1);
      }
      editAction = 'modify';
    }
    ...
    const editObj = {
      ...
      action: editAction,
      rule: Object.assign({}, ruleObj)
    };
    if (origRule) {
      editObj.originalRule = origRule;
    }
  ...
  if (action === 'save-sg') {
    ...
    sgForm.querySelectorAll('.fw-input,.fw-select').forEach(function (inp) {
      inp.classList.remove('invalid');
    });
    if (sgErrs.length) {
      sgErrs.forEach(function (er) {
        if (er.indexOf('protocol') >= 0) {
          sgForm.querySelector('[data-field="protocol"]').classList.add('invalid');
        }
        if (er.indexOf('Port') >= 0 || er.indexOf('port') >= 0) {
          const spf = sgForm.querySelector('[data-field="portFrom"]');
          const spt = sgForm.querySelector('[data-field="portTo"]');
          if (spf) {
            spf.classList.add('invalid');
          }
          if (spt) {
            spt.classList.add('invalid');
          }
        }
        if (er.indexOf('source') >= 0 || er.indexOf('CIDR') >= 0) {
          sgForm.querySelector('[data-field="source"]').classList.add('invalid');
        }
      });
      return;
    }
    ...
    const sgEdit = {
      ...
      direction: sDir,
      action: sgEditAct,
      rule: structuredClone(sgRule)
    };
    if (sgOrig) {
      sgEdit.originalRule = sgOrig;
    }
  ...
  if (action === 'delete-rt') {
    ...
    const dRt = (_rlCtx.rts || []).find(function (r) {
      return r.RouteTableId === dRtId;
    });
    if (!dRt || !dRt.Routes || dRtIdx >= dRt.Routes.length) {
      return;
    }
  ...
  if (action === 'save-rt') {
    ...
    if (rtTgt === 'local') {
      routeObj.GatewayId = 'local';
    } else if (rtTgt.startsWith('igw-')) {
      routeObj.GatewayId = rtTgt;
    } else if (rtTgt.startsWith('nat-')) {
      routeObj.NatGatewayId = rtTgt;
    } else if (rtTgt.startsWith('vpce-')) {
      routeObj.VpcEndpointId = rtTgt;
    } else if (rtTgt.startsWith('pcx-')) {
      routeObj.VpcPeeringConnectionId = rtTgt;
    } else if (rtTgt.startsWith('tgw-')) {
      routeObj.TransitGatewayId = rtTgt;
    } else {
      routeObj.GatewayId = rtTgt;
    }
    const sRt = (_rlCtx.rts || []).find(function (r) {
      return r.RouteTableId === sRtId;
    });
    const otherRoutes = sRt
      ? (sRt.Routes || []).filter(function (r, ri) {
          if (sRtEdit !== null && sRtEdit !== undefined && ri === parseInt(sRtEdit, 10)) {
            return false;
          }
          return true;
        })
    ...
    rtForm.querySelectorAll('.fw-input,.fw-select').forEach(function (inp) {
      inp.classList.remove('invalid');
    });
    if (rtErrs.length) {
      rtErrs.forEach(function (er) {
        if (er.indexOf('destination') >= 0 || er.indexOf('CIDR') >= 0) {
          rtForm.querySelector('[data-field="dest"]').classList.add('invalid');
        }
        if (er.indexOf('target') >= 0) {
          rtForm.querySelector('[data-field="target"]').classList.add('invalid');
        }
      });
      return;
    }
    ...
    const rtEditObj = {
      ...
      direction: 'egress',
      action: rtEditAct,
      rule: Object.assign({}, routeObj)
    };
    if (rtOrig) {
      rtEditObj.originalRule = rtOrig;
    }
...
function _renderFirewallTab() {
  ...
  if (!_rlCtx) {
    tb.innerHTML = '<span style="color:var(--text-muted)">No data loaded</span>';
    return;
  }
  const sgs = _rlCtx.sgs || [],
    nacls = _rlCtx.nacls || [],
    rts = _rlCtx.rts || [];
  ...
  const sortOpts = [
    { k: 'type', l: 'Sort: Type' },
    { k: 'name', l: 'Sort: Name' },
    { k: 'severity', l: 'Sort: Severity' },
    { k: 'rules', l: 'Sort: Rules' }
  ];
  let sortHtml = '';
  sortOpts.forEach(function (o) {
    sortHtml +=
      ...
      o.k +
      '"' +
      (_fwDashState.sort === o.k ? ' selected' : '') +
      '>' +
      o.l +
      '</option>';
  });
  tb.innerHTML =
    ...
    '<input id="fwDashSearch" type="text" placeholder="Search resources..." value="' +
    _escHtml(_fwDashState.search) +
    '" style="background:var(--bg-tertiary);border:1px solid var(--border);color:var(--text-primary);padding:4px 10px;border-radius:4px;font-size:11px;font-family:Segoe UI,system-ui,sans-serif;width:180px">' +
    '<select id="fwDashVpcFilter" style="background:var(--bg-tertiary);border:1px solid var(--border);color:var(--text-secondary);padding:4px 8px;border-radius:4px;font-size:10px;font-family:Segoe UI,system-ui,sans-serif">' +
    vpcOpts +
    '</select>' +
    '<select id="fwDashSort" style="background:var(--bg-tertiary);border:1px solid var(--border);color:var(--text-secondary);padding:4px 8px;border-radius:4px;font-size:10px;font-family:Segoe UI,system-ui,sans-serif">' +
    sortHtml +
    '</select>' +
  ...
  [
    ...
    el.className = 'fw-dash-pill' + (p.t === _fwDashFilter ? ' active' : '');
    el.dataset.type = p.t;
    el.textContent = p.l;
    el.addEventListener('click', function () {
      _fwDashFilter = p.t;
      _fwDashState.cardFilter = null;
      pills.querySelectorAll('.fw-dash-pill').forEach(function (x) {
...
function _fwDashRender() {
  ...
  if (!_rlCtx) {
    if (body) {
      body.innerHTML =
        '<div style="padding:40px;text-align:center;color:var(--text-muted)">No data loaded</div>';
    }
    return;
  }
  const sgs = _rlCtx.sgs || [],
  ...
  sgs.forEach(function (sg) {
    ...
    const inCount = (sg.IpPermissions || []).length;
    const outCount = (sg.IpPermissionsEgress || []).length;
    ...
    rows.push({
      ...
      rulesLabel: inCount + ' in / ' + outCount + ' out',
      edits: ec,
      desc: sg.Description || '',
      obj: sg,
      comp: cf,
  ...
  nacls.forEach(function (n) {
    ...
    const inCount = entries.filter(function (e) {
      return !e.Egress && e.RuleNumber !== 32767;
    }).length;
    const outCount = entries.filter(function (e) {
      return e.Egress && e.RuleNumber !== 32767;
    }).length;
  ...
  let totalFindings = 0,
    worstSev = 'LOW',
    openIngress = 0,
    totalEdits = _fwEdits ? _fwEdits.length : 0;
  rows.forEach(function (r) {
    if (r.comp) {
      totalFindings += r.comp.count;
      if ((sevOrder[r.comp.worst] || 9) < (sevOrder[worstSev] || 9)) {
        worstSev = r.comp.worst;
      }
    }
    if (r.openIngress) {
      openIngress++;
  ...
  h +=
    '<div class="fw-card-sub">' +
    (totalFindings ? worstSev + ' worst' : 'All clear') +
    '</div></div>';
  ...
  h +=
    '<div class="fw-card-sub">' +
    (openIngress ? 'Unrestricted ingress' : 'None detected') +
    '</div></div>';
  ...
  h +=
    '<div class="fw-card-sub">' +
    (totalEdits ? totalEdits + ' rule' + (totalEdits > 1 ? 's' : '') + ' modified' : 'No changes') +
    '</div></div>';
  h += '</div>';
  ...
  if (_fwDashFilter !== 'all') {
    filtered = filtered.filter(function (r) {
      return r.type === _fwDashFilter;
    });
  }
  if (_fwDashState.vpcFilter !== 'all') {
    filtered = filtered.filter(function (r) {
      return r.vpc === _fwDashState.vpcFilter;
    });
  }
  if (_fwDashState.search) {
    filtered = filtered.filter(function (r) {
      return (
        (r.name + ' ' + r.id + ' ' + r.vpc + ' ' + r.desc)
          .toLowerCase()
          .indexOf(_fwDashState.search) !== -1
      );
    });
  }

  // Sort
  const sortBy = _fwDashState.sort;
  const dir = _fwDashState.sortDir === 'asc' ? 1 : -1;
  filtered.sort(function (a, b) {
    if (sortBy === 'type') {
      return (a.type.localeCompare(b.type) || a.name.localeCompare(b.name)) * dir;
    }
    if (sortBy === 'name') {
      return a.name.localeCompare(b.name) * dir;
    }
    if (sortBy === 'rules') {
      return (b.rules - a.rules) * dir;
    }
    if (sortBy === 'severity') {
      const sa = a.comp ? sevOrder[a.comp.worst] || 9 : 99;
      const sb = b.comp ? sevOrder[b.comp.worst] || 9 : 99;
      return (sa - sb) * dir;
    }
    return 0;
  });
  ...
  if (!filtered.length) {
    const emptyMsg =
      cf === 'edits'
        ? 'No resources with pending edits'
        : cf === 'findings'
          ? 'No resources with compliance findings'
          : cf === 'open'
            ? 'No SGs with open 0.0.0.0/0 ingress'
            : _fwDashState.search
              ? 'No resources match "' + _escHtml(_fwDashState.search) + '"'
              : 'No firewall resources found';
    body.innerHTML =
      h +
      '<div style="padding:60px 20px;text-align:center;color:var(--text-muted);font-family:Segoe UI,system-ui,sans-serif;font-size:12px">' +
      emptyMsg +
      '</div>';
    _fwWireCards(body);
  ...
  const cols = [
    ...
    { k: 'vpc', l: 'VPC' },
    { k: 'rules', l: 'Rules' },
    { k: 'severity', l: 'Compliance' },
    { k: 'edits', l: 'Edits', nosort: true }
  ];
  h += '<table class="fw-dash-table"><thead><tr>';
  cols.forEach(function (c) {
    let cls = '';
    if (!c.nosort && _fwDashState.sort === c.k) {
      cls = _fwDashState.sortDir === 'asc' ? ' sort-asc' : ' sort-desc';
    }
    h +=
      '<th' +
      (c.nosort ? '' : ' data-fw-sort="' + c.k + '"') +
      ' class="' +
      cls +
      '">' +
      c.l +
  ...
  filtered.forEach(function (r, idx) {
    ...
    const sevColor = r.comp
      ? { CRITICAL: '#dc2626', HIGH: '#f59e0b', MEDIUM: '#3b82f6', LOW: '#10b981' }[r.comp.worst] ||
        'transparent'
      : 'transparent';
    h +=
      '<tr class="' +
    ...
    // Type badge
    h +=
      '<td><span class="fw-type-badge ' +
      r.type +
      '">' +
      (r.type === 'sg' ? 'SG' : r.type === 'nacl' ? 'NACL' : 'RT') +
      '</span></td>';
    // Name link
    h +=
      '<td><span class="fw-link" data-fw-action="open" data-fw-type="' +
      r.type +
      '" data-fw-id="' +
      esc(r.id) +
      '" data-fw-vpc="' +
    ...
    if (r.desc) {
      h +=
        '<br><span style="font-size:9px;color:var(--text-muted)">' +
        esc(r.desc.substring(0, 60)) +
        '</span>';
    }
    h += '</td>';
    // VPC link
    const vpcObj = (_rlCtx.vpcs || []).find(function (v) {
      return v.VpcId === r.vpc;
    });
    const vpcLabel = vpcObj ? gn(vpcObj, r.vpc) : r.vpc || '--';
    h +=
      '<td><span class="fw-link" data-fw-action="zoom-vpc" data-fw-vpc="' +
      esc(r.vpc || '') +
      '">' +
      esc(vpcLabel) +
      '</span></td>';
    ...
    if (r.comp) {
      h +=
        ...
        r.comp.worst +
        '</span> <span style="font-size:9px;color:var(--text-muted)">' +
        r.comp.count +
        '</span></td>';
    } else {
  ...
  body.querySelectorAll('.fw-dash-table th[data-fw-sort]').forEach(function (th) {
    th.addEventListener('click', function () {
      ...
      if (_fwDashState.sort === col) {
        _fwDashState.sortDir = _fwDashState.sortDir === 'asc' ? 'desc' : 'asc';
      } else {
        _fwDashState.sort = col;
        _fwDashState.sortDir = 'asc';
      }
      _fwDashRender();
    });
  });

  // Wire link clicks via event delegation without accumulating listeners on re-render.
  if (body._fwDashActionHandler) {
    body.removeEventListener('click', body._fwDashActionHandler);
  }
  body._fwDashActionHandler = function (e) {
  ...
  body.addEventListener('click', body._fwDashActionHandler);

  // Wire row clicks
  body.querySelectorAll('.fw-dash-table tbody tr[data-fw-idx]').forEach(function (tr) {
    tr.addEventListener('click', function (e) {
      if (e.target.closest('[data-fw-action]')) {
        return;
      }
...
function _fwRenderFooter(shown, total) {
  ...
  footer.innerHTML =
    ...
    (totalEdits
      ? totalEdits + ' edit' + (totalEdits > 1 ? 's' : '') + ' pending'
      : 'No pending edits') +
    ' | ' +
    shown +
    ' of ' +
  ...
  document.getElementById('fwDashExportAll').addEventListener('click', function () {
    if (!_fwEdits || !_fwEdits.length) {
      showToast('No edits to export', 3000);
      return;
    }
    ...
    if (cmds.length && navigator.clipboard && navigator.clipboard.writeText) {
      navigator.clipboard
        .writeText(cmds.join('\n'))
        .then(function () {
          let btn = document.getElementById('fwDashExportAll');
          if (!btn) {
            return;
          }
          ...
          setTimeout(function () {
            if (btn.parentNode) {
              btn.textContent = 'Export All CLI';
            }
          }, 1500);
        })
        .catch(function () {
          const btn = document.getElementById('fwDashExportAll');
          if (!btn) {
            return;
          }
  ...
  document.getElementById('fwDashResetAll').addEventListener('click', function () {
    if (!_fwEdits || !_fwEdits.length) {
      showToast('No edits to reset', 3000);
      return;
    }
    if (!confirm('Reset all ' + _fwEdits.length + ' firewall edits?')) {
      return;
    }
    _fwResetAll();
    _fwDashRender();
