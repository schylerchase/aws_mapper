...
function _mapResourceSelector(id) {
  const v = _cssAttrValue(id);
  return (
    '[data-vpc-id="' +
    v +
    '"],[data-subnet-id="' +
...
function _queryMapResourceById(id, root) {
  if (id == null || !root) {
    return null;
  }
  try {
    return root.querySelector(_mapResourceSelector(id));
  } catch (e) {
    return null;
  }
}

function _wireDpSectionToggles(dpBody) {
  dpBody.querySelectorAll('.dp-sec-hdr').forEach(function (header) {
...
function _openResourceSpotlight(rid) {
  ...
  const el = _queryMapResourceById(rid, _mapG.node());
  if (!el) {
    return;
  }
  ...
  const cx = bb.x + bb.width / 2,
    cy = bb.y + bb.height / 2;
  const svgW = _mapSvg.node().clientWidth,
    svgH = _mapSvg.node().clientHeight;
  ...
  h +=
    '<span class="sl-type-badge" style="background:' +
    tc +
    '22;color:' +
    tc +
    ';border:1px solid ' +
  ...
  h +=
    '<span class="sl-id" title="Click to copy" data-spotlight-copy="' +
    _escHtml(rid) +
    '">' +
    _escHtml(rid) +
    '</span>';
  ...
  if (info.details && info.details.length) {
    ...
    info.details.forEach(function (d) {
      h += '<dt>' + _escHtml(d[0]) + '</dt><dd>' + _escHtml(d[1]) + '</dd>';
    });
    h += '</dl></div>';
  }
  // Compliance findings
  if (info.findings && info.findings.length) {
    h +=
      '<div class="spotlight-section"><div class="spotlight-section-title">Compliance (' +
      info.findings.length +
      ')</div>';
    info.findings.slice(0, 5).forEach(function (f) {
      const sc =
        { CRITICAL: '#ef4444', HIGH: '#f97316', MEDIUM: '#eab308', LOW: '#3b82f6' }[f.severity] ||
        '#64748b';
      ...
      h +=
        '<span style="color:' +
        sc +
        ';font-weight:600;width:55px;flex-shrink:0">' +
        _escHtml(f.severity) +
        '</span>';
      h +=
    ...
    if (info.findings.length > 5) {
      h +=
        '<div style="font-size:9px;color:var(--text-muted);padding-top:4px">+' +
  ...
  if (info.related && info.related.length) {
    h +=
      '<div class="spotlight-section"><div class="spotlight-section-title">Related Resources</div>';
  ...
  if (info.nearby && info.nearby.length) {
    h +=
      '<div class="spotlight-section"><div class="spotlight-section-title">Nearby Resources</div>';
    h += '<div class="spotlight-nearby">';
  ...
  h +=
    '<button class="primary" data-spotlight-detail="' + _escHtml(rid) + '">Full Details</button>';
  ...
  if (copyBtn) {
    copyBtn.addEventListener('click', function (e) {
      ...
      if (navigator.clipboard && navigator.clipboard.writeText) {
        navigator.clipboard.writeText(this.dataset.spotlightCopy || this.textContent.trim());
      }
    });
  }
  ...
  card.querySelector('[data-spotlight-detail]').addEventListener('click', function () {
    ...
    if (type) {
      _openDetailForSearch(type, drid);
    }
  });
  ...
  if (depsBtn) {
    depsBtn.addEventListener('click', function () {
      ...
      _closeSpotlight();
      if (typeof showDependencies === 'function') {
        showDependencies(drid);
      }
    });
  }
}
...
function _gatherResourceInfo(rid) {
  ...
  const gn2 = function (obj, fallback) {
    return obj && obj.Tags
      ? (
          obj.Tags.find(function (t) {
            return t.Key === 'Name';
          }) || {}
        ).Value || fallback
      : fallback;
  ...
  if (rid.startsWith('i-')) {
    const inst = (_rlCtx.instances || []).find(function (i) {
      return i.InstanceId === rid;
    });
    if (!inst) {
      return null;
    }
    info.type = 'EC2';
    info.name = gn2(inst, rid);
    info.details = [
    ...
    (inst.SecurityGroups || []).forEach(function (sg) {
      info.related.push({ id: sg.GroupId, name: sg.GroupName || sg.GroupId, type: 'SG' });
    });
    if (inst.SubnetId) {
      const sub = (_rlCtx.subnets || []).find(function (s) {
        return s.SubnetId === inst.SubnetId;
      });
      if (sub) {
        info.related.push({ id: sub.SubnetId, name: gn2(sub, sub.SubnetId), type: 'Subnet' });
      }
    }
    // Nearby: other instances in same subnet
    (_rlCtx.instances || [])
      .filter(function (i) {
        return i.SubnetId === inst.SubnetId && i.InstanceId !== rid;
      })
      .slice(0, 6)
      .forEach(function (i) {
        info.nearby.push({ id: i.InstanceId, name: gn2(i, i.InstanceId), type: 'EC2' });
      });
    // Also add RDS, ALBs in same subnet
    (_rlCtx.rdsInstances || []).forEach(function (db) {
      const subs = ((db.DBSubnetGroup && db.DBSubnetGroup.Subnets) || []).map(function (s) {
        return s.SubnetIdentifier;
      });
      if (subs.indexOf(inst.SubnetId) !== -1) {
        info.nearby.push({
    ...
    const sub = (_rlCtx.subnets || []).find(function (s) {
      return s.SubnetId === rid;
    });
    if (!sub) {
      return null;
    }
    info.type = 'Subnet';
    info.name = gn2(sub, rid);
    ...
    info.details = [
      ...
      ['VPC', sub.VpcId || '—'],
      ['Available IPs', '' + (sub.AvailableIpAddressCount || 0)]
    ];
    // Nearby: resources in this subnet
    (_rlCtx.instances || [])
      .filter(function (i) {
        return i.SubnetId === rid;
      })
      .slice(0, 8)
      .forEach(function (i) {
        info.nearby.push({ id: i.InstanceId, name: gn2(i, i.InstanceId), type: 'EC2' });
    ...
    const vpc = (_rlCtx.vpcs || []).find(function (v) {
      return v.VpcId === rid;
    });
    if (!vpc) {
      return null;
    ...
    info.type = 'VPC';
    info.name = gn2(vpc, rid);
    const vpcSubs = (_rlCtx.subnets || []).filter(function (s) {
      return s.VpcId === rid;
    });
    info.details = [
      ['CIDR', vpc.CidrBlock || '—'],
      ['State', vpc.State || '—'],
      ['Subnets', '' + vpcSubs.length],
      [
        'Instances',
        '' +
          (_rlCtx.instances || []).filter(function (i) {
            return vpcSubs.some(function (s) {
              return s.SubnetId === i.SubnetId;
            });
          }).length
      ]
    ];
    vpcSubs.slice(0, 8).forEach(function (s) {
      info.nearby.push({ id: s.SubnetId, name: gn2(s, s.SubnetId), type: 'Subnet' });
    });
  } else if (rid.startsWith('sg-')) {
    const sg = (_rlCtx.sgs || []).find(function (s) {
      return s.GroupId === rid;
    });
    if (!sg) {
      return null;
    }
    info.type = 'SG';
    info.name = sg.GroupName || rid;
    info.details = [
      ['Group ID', rid],
    ...
    if (sg.VpcId) {
      info.related.push({ id: sg.VpcId, name: sg.VpcId, type: 'VPC' });
    }
    // Instances using this SG
    (_rlCtx.instances || [])
      .filter(function (i) {
        return (i.SecurityGroups || []).some(function (s) {
          return s.GroupId === rid;
        });
      })
      .slice(0, 6)
    ...
    const igw = (_rlCtx.igws || []).find(function (g) {
      return g.InternetGatewayId === rid;
    });
    if (!igw) {
      return null;
    }
    info.type = 'IGW';
    info.name = gn2(igw, rid);
    const attachedVpcs = (igw.Attachments || []).map(function (a) {
      return a.VpcId;
    });
    info.details = [
      ['Gateway ID', rid],
      ['Attached VPCs', attachedVpcs.join(', ') || 'None']
    ];
    attachedVpcs.forEach(function (v) {
      info.related.push({ id: v, name: v, type: 'VPC' });
    });
  } else if (rid.startsWith('nat-')) {
    const nat = (_rlCtx.nats || []).find(function (g) {
      return g.NatGatewayId === rid;
    });
    if (!nat) {
      return null;
    }
    info.type = 'NAT';
    info.name = gn2(nat, rid);
    info.details = [
      ['Gateway ID', rid],
      ['State', nat.State || '—'],
      ['Subnet', nat.SubnetId || '—'],
    ...
    if (nat.VpcId) {
      info.related.push({ id: nat.VpcId, name: nat.VpcId, type: 'VPC' });
    }
    if (nat.SubnetId) {
      info.related.push({ id: nat.SubnetId, name: nat.SubnetId, type: 'Subnet' });
    }
  } else {
    // Try RDS, Lambda, etc. by name lookup
    const rds = (_rlCtx.rdsInstances || []).find(function (d) {
      return d.DBInstanceIdentifier === rid;
    });
    if (rds) {
      info.type = 'RDS';
      info.name = rds.DBInstanceIdentifier;
      info.details = [
        ['Engine', (rds.Engine || '') + ' ' + (rds.EngineVersion || '')],
        ['Class', rds.DBInstanceClass || '—'],
        ['Status', rds.DBInstanceStatus || '—'],
        ['Multi-AZ', rds.MultiAZ ? 'Yes' : 'No'],
        ['Storage', (rds.AllocatedStorage || '?') + ' GB'],
        ['Encrypted', rds.StorageEncrypted ? 'Yes' : 'No']
      ];
      const rdsVpc = rds.DBSubnetGroup ? rds.DBSubnetGroup.VpcId : '';
      if (rdsVpc) {
        info.related.push({ id: rdsVpc, name: rdsVpc, type: 'VPC' });
      }
    } else {
      const lam = (_rlCtx.lambdaFns || []).find(function (f) {
        return f.FunctionName === rid;
      });
      if (lam) {
        info.type = 'Lambda';
        info.name = lam.FunctionName;
        info.details = [
          ['Runtime', lam.Runtime || '—'],
          ['Memory', (lam.MemorySize || '?') + ' MB'],
          ['Timeout', (lam.Timeout || '?') + ' sec'],
          ['Handler', lam.Handler || '—'],
          ['Code Size', ((lam.CodeSize || 0) / 1024).toFixed(1) + ' KB']
        ];
        const lamVpc = lam.VpcConfig ? lam.VpcConfig.VpcId : '';
        if (lamVpc) {
          info.related.push({ id: lamVpc, name: lamVpc, type: 'VPC' });
        }
      } else {
        return null;
      }
  ...
  (_complianceFindings || []).forEach(function (f) {
    if (f.resource === rid && !_isMuted(f)) {
      info.findings.push(f);
    }
  });
  return info;
}
...
function _dpBackBtnHtml() {
  return _navStack.length > 0
    ? '<span id="dpBack" style="cursor:pointer;color:var(--accent-blue);font-size:calc(10px * var(--txt-scale,1) * var(--dp-txt-scale,1));font-family:Segoe UI,system-ui,sans-serif;margin-right:8px" title="Back">&lt; Back</span>'
    : '';
}
...
function _dpTypeBadge(type) {
  ...
  return (
    '<span style="display:inline-block;font-size:9px;font-weight:600;padding:2px 8px;border-radius:4px;margin-right:8px;background:' +
    tc +
    '22;color:' +
    tc +
    ';border:1px solid ' +
...
function _openDetailForSearch(type, id) {
  ...
  if (!_dpSkipPush && dp.classList.contains('open') && dpBody.textContent) {
    const prevType = dp.dataset.dpType;
    const prevId = dp.dataset.dpId;
    if (prevType && prevId) {
      _navStack.push({
        fn: () => {
          _dpSkipPush = true;
          _openDetailForSearch(prevType, prevId);
          _dpSkipPush = false;
        }
      });
    }
  }
  dp.dataset.dpType = type;
  dp.dataset.dpId = id;
  _lastRlType = null;
  ...
  if (type === 'Subnet') {
    ...
    if (sub) {
      openSubnetPanel(sub, sub.VpcId, {
        pubSubs: _rlCtx.pubSubs,
        subRT: _rlCtx.subRT,
        subNacl: _rlCtx.subNacl,
        instBySub: _rlCtx.instBySub,
        eniBySub: _rlCtx.eniBySub,
        albBySub: _rlCtx.albBySub,
        sgByVpc: _rlCtx.sgByVpc,
        volByInst: _rlCtx.volByInst,
        enis: _rlCtx.enis,
        snapByVol: _rlCtx.snapByVol,
        tgByAlb: _rlCtx.tgByAlb,
        wafByAlb: _rlCtx.wafByAlb,
        rdsBySub: _rlCtx.rdsBySub,
        ecsBySub: _rlCtx.ecsBySub,
        lambdaBySub: _rlCtx.lambdaBySub,
        ecacheByVpc: _rlCtx.ecacheByVpc,
        redshiftByVpc: _rlCtx.redshiftByVpc,
        cfByAlb: _rlCtx.cfByAlb
      });
      return;
    }
  }
  if (
    ...
    const gwType = type;
    openGatewayPanel(id, gwType, {
      gwNames: gwNames,
      igws: _rlCtx.igws,
      nats: _rlCtx.nats,
      vpns: _rlCtx.vpns,
      vpces: _rlCtx.vpces,
      peerings: _rlCtx.peerings,
      rts: _rlCtx.rts,
      subnets: _rlCtx.subnets,
      subRT: _rlCtx.subRT,
      pubSubs: _rlCtx.pubSubs,
      vpcs: _rlCtx.vpcs,
      tgwAttachments: _rlCtx.tgwAttachments || []
    });
    return;
  }
  ...
  if (type === 'VPC') {
    ...
    const gws = [];
    (_rlCtx.igws || []).forEach((g) => {
      if ((g.Attachments || []).some((a) => a.VpcId === id)) {
        gws.push({ type: 'IGW', id: g.InternetGatewayId, name: gn(g, g.InternetGatewayId) });
      }
    });
    (_rlCtx.nats || []).forEach((g) => {
      if (g.VpcId === id) {
        gws.push({ type: 'NAT', id: g.NatGatewayId, name: gn(g, g.NatGatewayId) });
      }
    });
    const sgs = (_rlCtx.sgs || []).filter((s) => s.VpcId === id);
    const insts = (_rlCtx.instances || []).filter((i) =>
      subs.some((s) => s.SubnetId === i.SubnetId)
    );
    ...
    dpSub.innerHTML =
      ...
      esc(vpc.CidrBlock || '');
    h +=
      '<div class="dp-section"><div class="dp-sec-hdr"><span class="dp-sec-title">Overview</span><span class="dp-sec-arr">&#9660;</span></div><div class="dp-sec-body">';
    ...
    if (subs.length) {
      h +=
        '<div class="dp-section"><div class="dp-sec-hdr"><span class="dp-sec-title">Subnets</span><span><span class="dp-sec-count">' +
        subs.length +
        '</span><span class="dp-sec-arr">&#9660;</span></span></div><div class="dp-sec-body">';
      subs.forEach((s) => {
        const sn = gn(s, s.SubnetId);
        const isPub = _rlCtx.pubSubs && _rlCtx.pubSubs.has(s.SubnetId);
        h +=
          '<div class="dp-link" style="padding:4px 0;cursor:pointer;color:var(--accent-cyan);font-size:calc(11px * var(--dp-txt-scale,1))" data-detail-type="Subnet" data-detail-id="' +
          esc(s.SubnetId) +
          '" data-zoom-id="' +
          esc(s.SubnetId) +
          '">' +
    ...
    if (gws.length) {
      h +=
        '<div class="dp-section"><div class="dp-sec-hdr"><span class="dp-sec-title">Gateways</span><span><span class="dp-sec-count">' +
        gws.length +
        '</span><span class="dp-sec-arr">&#9660;</span></span></div><div class="dp-sec-body">';
      gws.forEach((g) => {
        h +=
          '<div class="dp-link" style="padding:4px 0;cursor:pointer;color:' +
          gcv(g.type) +
          ';font-size:calc(11px * var(--dp-txt-scale,1))" data-detail-type="' +
          esc(g.type) +
          '" data-detail-id="' +
          esc(g.id) +
          '">' +
    ...
    h +=
      '<div class="dp-section"><div class="dp-sec-hdr"><span class="dp-sec-title">Details</span><span class="dp-sec-arr">&#9660;</span></div><div class="dp-sec-body">';
    h += '<table class="dp-kv">';
    h += '<tr><td>Type</td><td>' + esc(inst.InstanceType || '—') + '</td></tr>';
    h +=
      '<tr><td>State</td><td><span style="color:' +
      stateColor +
      '">' +
      esc(stateName) +
      '</span></td></tr>';
    h +=
      '<tr><td>AZ</td><td>' +
      esc((inst.Placement && inst.Placement.AvailabilityZone) || '—') +
      '</td></tr>';
    h += '<tr><td>Private IP</td><td>' + esc(inst.PrivateIpAddress || '—') + '</td></tr>';
    h += '<tr><td>Public IP</td><td>' + esc(inst.PublicIpAddress || '—') + '</td></tr>';
    h +=
      '<tr><td>Subnet</td><td><span class="dp-link" style="cursor:pointer;color:var(--accent-cyan)" data-detail-type="Subnet" data-detail-id="' +
      esc(inst.SubnetId || '') +
      '" data-zoom-id="' +
      esc(inst.SubnetId || '') +
      '">' +
      ...
      '</span></td></tr>';
    h +=
      '<tr><td>VPC</td><td><span class="dp-link" style="cursor:pointer;color:var(--accent-cyan)" data-detail-type="VPC" data-detail-id="' +
      esc(inst.VpcId || '') +
      '">' +
      esc(inst.VpcId || '—') +
      '</span></td></tr>';
    ...
    if (sgsArr.length) {
      h +=
        '<div class="dp-section"><div class="dp-sec-hdr"><span class="dp-sec-title">Security Groups</span><span><span class="dp-sec-count">' +
        sgsArr.length +
        '</span><span class="dp-sec-arr">&#9660;</span></span></div><div class="dp-sec-body">';
      sgsArr.forEach((s) => {
        h +=
          '<div class="dp-link" style="padding:4px 0;cursor:pointer;color:var(--accent-cyan);font-size:calc(11px * var(--dp-txt-scale,1))" data-detail-type="SG" data-detail-id="' +
          esc(s.GroupId) +
          '">' +
          esc(s.GroupName || s.GroupId) +
          ' <span style="color:var(--text-muted);font-size:9px">' +
          esc(s.GroupId) +
          '</span></div>';
      });
    ...
    h +=
      '<tr><td>Storage</td><td>' +
      esc(db.AllocatedStorage || '—') +
      ' GB ' +
      esc(db.StorageType || '') +
      '</td></tr>';
    h +=
      '<tr><td>Endpoint</td><td>' + esc((db.Endpoint && db.Endpoint.Address) || '—') + '</td></tr>';
    ...
    if (vpcCfg && vpcCfg.VpcId) {
      ...
      h +=
        ...
        (vpcCfg.SubnetIds || [])
          .map(
            (s) =>
              '<span class="dp-link" style="cursor:pointer;color:var(--accent-cyan)" data-detail-type="Subnet" data-detail-id="' +
              esc(s) +
              '" data-zoom-id="' +
              esc(s) +
              '">' +
    ...
    h +=
      '<div class="dp-section"><div class="dp-sec-hdr"><span class="dp-sec-title">Overview</span><span class="dp-sec-arr">&#9660;</span></div><div class="dp-sec-body">';
    h +=
      '<table class="dp-kv"><tr><td>Description</td><td>' +
      esc(sg.Description || '—') +
      '</td></tr>';
    h +=
      '<tr><td>VPC</td><td><span class="dp-link" style="cursor:pointer;color:var(--accent-cyan)" data-detail-type="VPC" data-detail-id="' +
    ...
    const inRules = sg.IpPermissions || [];
    const outRules = sg.IpPermissionsEgress || [];
    if (inRules.length) {
      h +=
        '<div class="dp-section"><div class="dp-sec-hdr"><span class="dp-sec-title">Inbound Rules</span><span><span class="dp-sec-count">' +
        inRules.length +
        '</span><span class="dp-sec-arr">&#9660;</span></span></div><div class="dp-sec-body"><table class="dp-tbl"><tr><th>Proto</th><th>Port</th><th>Source</th></tr>';
      inRules.forEach((r) => {
        ...
        const src =
          (r.IpRanges || [])
            .map((x) => x.CidrIp)
            .concat((r.UserIdGroupPairs || []).map((x) => x.GroupId))
            .join(', ') || '—';
        h +=
          '<tr><td>' + esc(proto) + '</td><td>' + esc(port) + '</td><td>' + esc(src) + '</td></tr>';
      });
      h += '</table></div></div>';
    }
    if (outRules.length) {
      h +=
        '<div class="dp-section"><div class="dp-sec-hdr collapsed"><span class="dp-sec-title">Outbound Rules</span><span><span class="dp-sec-count">' +
        outRules.length +
        '</span><span class="dp-sec-arr">&#9660;</span></span></div><div class="dp-sec-body hidden"><table class="dp-tbl"><tr><th>Proto</th><th>Port</th><th>Dest</th></tr>';
      outRules.forEach((r) => {
        const proto = r.IpProtocol === '-1' ? 'All' : r.IpProtocol;
        const port =
          r.FromPort === r.ToPort
        ...
        const dst =
          (r.IpRanges || [])
            ...
            .join(', ') || '—';
        h +=
          '<tr><td>' + esc(proto) + '</td><td>' + esc(port) + '</td><td>' + esc(dst) + '</td></tr>';
      });
      h += '</table></div></div>';
    }
  } else if (type === 'Note') {
  ...
  dpBody.querySelectorAll('[data-detail-type][data-detail-id]').forEach(function (el) {
    el.addEventListener('click', function (e) {
      ...
      if (this.dataset.zoomId) {
        _zoomToElement(this.dataset.zoomId);
      }
...
try {
  const s = localStorage.getItem(_SNAP_KEY);
  if (s) {
    _snapshots = JSON.parse(s);
  }
} catch (e) {
  _snapshots = [];
}
function _saveSnapshots() {
  try {
    localStorage.setItem(_SNAP_KEY, JSON.stringify(_snapshots));
  } catch (e) {
    // If storage full, trim oldest half
    if (_snapshots.length > 4) {
      _snapshots = _snapshots.slice(Math.floor(_snapshots.length / 2));
      try {
        localStorage.setItem(_SNAP_KEY, JSON.stringify(_snapshots));
      } catch (e2) {
...
function takeSnapshot(label, auto) {
  ...
  if (_snapshots.length > 0 && _snapshots[_snapshots.length - 1].checksum === checksum) {
    return null;
  }
  const snap = {
    id: 'snap-' + Date.now(),
  ...
  _renderTimeline();
  if (!auto) {
    _showToast('Snapshot saved: ' + (label || 'Manual'));
  }
  return snap;
}
function _renderTimeline() {
  ...
  if (!_snapshots.length) {
    document.getElementById('timelineLabel').textContent = 'No snapshots';
    return;
  }
  document.getElementById('timelineLabel').textContent =
    _snapshots.length + ' snap' + (_snapshots.length !== 1 ? 's' : '');
...
function _addTimelineDot(container, snap, x, idx) {
  ...
  dot.addEventListener('contextmenu', function (ev) {
    ev.preventDefault();
    if (typeof _compareWithSnapshot === 'function') {
...
function _viewSnapshot(idx) {
  ...
  if (!_viewingHistory) {
    ...
    _currentSnapshot = {};
    document.querySelectorAll('.ji').forEach((el) => {
      _currentSnapshot[el.id] = el.value;
    });
  ...
  Object.entries(snap.textareas || {}).forEach(([id, val]) => {
    ...
    if (el) {
      ...
      try {
        JSON.parse(val);
        el.className = 'ji valid';
      } catch (e) {
        el.className = 'ji invalid';
      }
    }
  });
  if (snap.accountLabel) {
    ...
    if (al) {
      al.value = snap.accountLabel;
    }
  }
  if (snap.annotations) {
    _annotations = structuredClone(snap.annotations);
  }
  renderMap();
  // Show history banner
  const d = new Date(snap.timestamp);
...
function _returnToCurrent() {
  if (!_viewingHistory || !_currentSnapshot) {
    return;
  }
  ...
  if (_currentSnapshot._accountLabel) {
    const al = document.getElementById('accountLabel');
    if (al) {
      al.value = _currentSnapshot._accountLabel;
    }
  }
  if (_currentSnapshot._annotations) {
    _annotations = structuredClone(_currentSnapshot._annotations);
  }
...
function openTimeline() {
  document.getElementById('timelineBar').classList.add('open');
  _renderTimeline();
}
function closeTimeline() {
  document.getElementById('timelineBar').classList.remove('open');
}
