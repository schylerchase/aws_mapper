...
function _dataAttrSelector(attr, value) {
  return '[' + attr + '="' + _cssAttrValue(value) + '"]';
}
function _gatewayNodeSelector(id) {
  return '.gw-node' + _dataAttrSelector('data-gwid', id);
}
function _vpcGroupSelector(id) {
  return '.vpc-group' + _dataAttrSelector('data-vpc-id', id);
}
...
function _setMapRendering(isRendering) {
  ...
  if (isRendering) {
    if (_mapRenderTimer) {
      ...
      _mapRenderTimer = null;
    }
    main.classList.add('map-rendering');
    if (svg) {
      svg.classList.add('map-rendering');
    }
    return;
  }
  if (_mapRenderTimer) {
  ...
  _mapRenderTimer = setTimeout(() => {
    main.classList.remove('map-rendering');
    if (svg) {
      svg.classList.remove('map-rendering');
    }
    _mapRenderTimer = null;
...
function _createMapZoom(svg, g) {
  ...
  if (typeof window !== 'undefined' && typeof window._mapWheelDelta === 'function') {
    zoom.wheelDelta(window._mapWheelDelta);
  }
  return zoom
    .on('start', () => {
      _setMapMoving(true);
    ...
    .on('zoom', (e) => {
      _applyZoomTransform(g, e.transform);
      _updateZoomLabel(e.transform.k);
    })
    .on('end', (e) => {
      _flushZoomTransform(g, e.transform);
      _setMapMoving(false);
    });
}
...
function _parseInputs() {
  ...
  allRecSets.forEach((r) => {
    if (r.HostedZoneId) {
      (recsByZoneMap[r.HostedZoneId] = recsByZoneMap[r.HostedZoneId] || []).push(r);
    }
  });
  ...
  let lambdaFns = ext(_cachedParse('in_lambda'), ['Functions']).filter(
    (f) => f.VpcConfig && f.VpcConfig.VpcId
  );
...
function _buildLookupMaps(d) {
  ...
  gwNames = {};
  igws.forEach((g) => {
    gwNames[g.InternetGatewayId] = gn(g, g.InternetGatewayId);
  });
  nats.forEach((g) => {
    gwNames[g.NatGatewayId] = gn(g, g.NatGatewayId);
  });
  vpces.forEach((g) => {
    gwNames[g.VpcEndpointId] = gn(g, g.VpcEndpointId);
  });
  vpns.forEach((g) => {
    if (g.VpnGatewayId) {
      gwNames[g.VpnGatewayId] = gn(g, g.VpnGatewayId);
    }
  });
  peerings.forEach((g) => {
    gwNames[g.VpcPeeringConnectionId] = gn(g, g.VpcPeeringConnectionId);
  });
  tgwAttachments.forEach((g) => {
    if (g.TransitGatewayId && !gwNames[g.TransitGatewayId]) {
      gwNames[g.TransitGatewayId] = gn(g, g.TransitGatewayId);
    }
  });
  const mainRT = {};
  rts.forEach((rt) => {
  ...
  rts.forEach((rt) => {
    ...
    (rt.Routes || []).forEach((r) => {
      if (r.GatewayId && r.GatewayId !== 'local') {
        gwSet.set(r.GatewayId, { type: clsGw(r.GatewayId), id: r.GatewayId, vpcId: rt.VpcId });
      }
      if (r.NatGatewayId) {
        gwSet.set(r.NatGatewayId, { type: 'NAT', id: r.NatGatewayId, vpcId: rt.VpcId });
      }
      if (r.TransitGatewayId) {
        gwSet.set(r.TransitGatewayId, { type: 'TGW', id: r.TransitGatewayId, vpcId: 'shared' });
      }
      if (r.VpcPeeringConnectionId) {
        gwSet.set(r.VpcPeeringConnectionId, {
          type: 'PCX',
  ...
  igws.forEach((g) => {
    if (!gwSet.has(g.InternetGatewayId)) {
      const v = (g.Attachments || [])[0];
      gwSet.set(g.InternetGatewayId, {
        type: 'IGW',
  ...
  nats.forEach((g) => {
    if (!gwSet.has(g.NatGatewayId)) {
      gwSet.set(g.NatGatewayId, { type: 'NAT', id: g.NatGatewayId, vpcId: g.VpcId || 'unk' });
    }
  });
  vpces.forEach((g) => {
    if (!gwSet.has(g.VpcEndpointId)) {
      gwSet.set(g.VpcEndpointId, { type: 'VPCE', id: g.VpcEndpointId, vpcId: g.VpcId || 'unk' });
    }
  });
  subnets.forEach((s) => {
    if (!subRT[s.SubnetId] && mainRT[s.VpcId]) {
      subRT[s.SubnetId] = mainRT[s.VpcId];
      const hasIgw = (mainRT[s.VpcId].Routes || []).some(
        (r) => r.GatewayId && r.GatewayId.startsWith('igw-') && r.State !== 'blackhole'
      );
      if (hasIgw) {
        pubSubs.add(s.SubnetId);
      }
    }
  });
  const subNacl = {};
  ...
  if (_iamData) {
    (instances || []).forEach((i) => {
      ...
      if (pa) {
        ...
        if (!iamRoleResources[rn]) {
          iamRoleResources[rn] = { ec2: [], lambda: [], ecs: [] };
        }
  ...
  enis.forEach((e) => {
    if (e.SubnetId) {
      (eniBySub[e.SubnetId] = eniBySub[e.SubnetId] || []).push(e);
    }
    if (e.Attachment && e.Attachment.InstanceId) {
      (eniByInst[e.Attachment.InstanceId] = eniByInst[e.Attachment.InstanceId] || []).push(e);
    }
  });
  const albBySub = {};
  albs.forEach((lb) => {
  ...
  volumes.forEach((v) => {
    (v.Attachments || []).forEach((a) => {
      if (a.InstanceId) {
        (volByInst[a.InstanceId] = volByInst[a.InstanceId] || []).push(v);
      }
    });
  });
  const knownInstIds = new Set(instances.map((i) => i.InstanceId));
  const instSubFromEni = {};
  enis.forEach((e) => {
    if (e.SubnetId && e.Attachment && e.Attachment.InstanceId) {
      instSubFromEni[e.Attachment.InstanceId] = e.SubnetId;
    }
  });
  const volBySub = {};
  volumes.forEach((v) => {
    ...
    if (att && att.InstanceId) {
      if (knownInstIds.has(att.InstanceId)) {
        return;
      }
      const sid2 = instSubFromEni[att.InstanceId];
      if (sid2) {
        (volBySub[sid2] = volBySub[sid2] || []).push(v);
      }
    }
  });
  const snapByVol = {};
  snapshots.forEach((s) => {
    if (s.VolumeId) {
      (snapByVol[s.VolumeId] = snapByVol[s.VolumeId] || []).push(s);
    }
  });
  const tgByAlb = {};
  tgs.forEach((tg) => {
  ...
  ecacheClusters.forEach((c) => {
    if (c.VpcId) {
      (ecacheByVpc[c.VpcId] = ecacheByVpc[c.VpcId] || []).push(c);
    }
  });
  const redshiftByVpc = {};
  redshiftClusters.forEach((c) => {
    if (c.VpcId) {
      (redshiftByVpc[c.VpcId] = redshiftByVpc[c.VpcId] || []).push(c);
    }
  });
  const cfByAlb = {};
  cfDistributions.forEach((dd) => {
    (dd.Origins?.Items || []).forEach((o) => {
      const matchAlb = albs.find(
        (a) => a.DNSName && o.DomainName && o.DomainName.includes(a.DNSName)
      );
      if (matchAlb) {
        (cfByAlb[matchAlb.LoadBalancerArn] = cfByAlb[matchAlb.LoadBalancerArn] || []).push(dd);
      }
    });
  });
  const pvGws = {},
  ...
  [...gwSet.values()].forEach((gw) => {
    if (gw.type === 'VPCE') {
      (vpceByVpc[gw.vpcId] = vpceByVpc[gw.vpcId] || []).push(gw);
      vpceIds.add(gw.id);
      return;
    }
    if (isShared(gw.type)) {
      if (!shGws.find((g) => g.id === gw.id)) {
        shGws.push(gw);
      }
    } else {
      (pvGws[gw.vpcId] = pvGws[gw.vpcId] || []).push(gw);
    }
  });
  return {
    subByVpc,
...
function _renderMapInner() {
  try {
    ...
    vpcs.forEach((v) => {
      if (v._region && v._region !== 'unknown') {
        _regions.add(v._region);
      }
    });
    const _multiRegion = _regions.size > 1;
    // Deduplicate: if same VpcId from same account, keep last pasted
    const seenVpcs = new Map();
    const vpcDupes = [];
    vpcs.forEach((v) => {
      const key = v._accountId + ':' + v.VpcId;
      if (seenVpcs.has(key)) {
        vpcDupes.push(v.VpcId);
      }
      seenVpcs.set(key, v);
    });
    if (vpcDupes.length) {
      console.warn('Duplicate VPCs detected (kept latest):', vpcDupes);
    }
    vpcs = [...seenVpcs.values()];
    // Collect unique accounts for rendering
    const _accounts = new Set();
    vpcs.forEach((v) => {
      if (v._accountId && v._accountId !== 'default') {
        _accounts.add(v._accountId);
      }
    });
    ...
    if (!vpcs.length && !subnets.length) {
      if (_designMode) {
        ...
        document.getElementById('emptyDesc').textContent =
          'No infrastructure loaded — create your first VPC to start designing';
        const eBtn = document.getElementById('emptyDesignBtn');
        eBtn.style.display = 'inline-block';
        if (eBtn._designEmptyHandler) {
          eBtn.removeEventListener('click', eBtn._designEmptyHandler);
        }
        eBtn._designEmptyHandler = function () {
          showDesignForm('add_vpc', {});
        };
        eBtn.addEventListener('click', eBtn._designEmptyHandler);
        svg.style('display', 'none');
        return;
      }
      document.getElementById('emptyTitle').textContent = 'No data loaded';
      document.getElementById('emptyDesc').textContent =
        'Paste AWS CLI JSON exports and click Render Map';
    ...
    function countAZHeaders(ss) {
      ...
      return Math.max(0, azs.size - 1);
    }
    function calcVpcHeight(ss) {
      let ih = 0;
      const sorted = sortByAZ(ss);
      sorted.forEach((s, i) => {
        ih += subHeight(s.SubnetId) + (i < sorted.length - 1 ? SG : 0);
      });
    ...
    function buildSubLayouts(ss, baseX, baseY, sw) {
      ...
      sorted.forEach((s) => {
        ...
        if (az && lastAZ !== null && az !== lastAZ) {
          cy += AZ_HDR + SG;
          layouts.push({ azLabel: az, x: baseX + VP, y: cy - AZ_HDR - SG / 2, w: sw });
        } else if (az && lastAZ === null && sorted.filter((x) => x.AvailabilityZone).length > 1) {
          layouts.push({ azLabel: az, x: baseX + VP, y: cy - 2, w: sw });
          cy += AZ_HDR;
        }
        lastAZ = az;
    ...
    knownVpcs.forEach((vpc, idx) => {
      ...
      if (_prevLayoutRegion && _vpcRegion !== _prevLayoutRegion && _multiRegion) {
        cx += REGION_GAP;
      }
    ...
    const W = _mainEl.clientWidth,
      H = _mainEl.clientHeight;
    ...
    preAllS.forEach((sl) => {
      const rt = subRT[sl.sub.SubnetId];
      if (!rt) {
        return;
      }
      (rt.Routes || []).forEach((r) => {
        ...
        if (!tid || vpceIds.has(tid)) {
          return;
        }
        if (!gwSubnetYs.has(tid)) {
          gwSubnetYs.set(tid, []);
        }
    ...
    vL.forEach((vl) => {
      const gs = [...(pvGws[vl.vpc.VpcId] || [])].sort((a, b) => {
        const oa = gwOrder[a.type] ?? 9,
          ob = gwOrder[b.type] ?? 9;
        return oa - ob;
      });
    ...
    const lE = knownVL[0]?.x || 0,
      rE = knownVL.length ? knownVL[knownVL.length - 1].x + knownVL[knownVL.length - 1].w : W;
    const sCX = (lE + rE) / 2,
      sY = knownVpcBot + 80;
    const ssX = sCX - ((shGws.length - 1) * 80) / 2;
    shGws.forEach((gw, i) => {
      gwP.set(gw.id, { x: ssX + i * 80, y: sY, gw });
    });
    ...
    const allVpcRight = Math.max(
      ...vL.map((v) => (v.gwSide === 'left' ? v.x + v.w : v.x + v.w + v.chanW))
    );
    ...
    allS.forEach((sl) => {
      ...
      const ov = vL.find((v) => v.subs.includes(sl));
      if (!ov) {
        return;
      }
      (rt.Routes || []).forEach((r) => {
        ...
        const tid = r.GatewayId || r.NatGatewayId || r.TransitGatewayId || r.VpcPeeringConnectionId;
        if (!tid || !gwP.has(tid) || vpceIds.has(tid)) {
          return;
        }
        ...
        (tG[k] = tG[k] || []).push({
          sl,
          sid: sl.sub.SubnetId,
          dst: r.DestinationCidrBlock || r.DestinationPrefixListId || '?',
          gid: tid,
          vid: ov.vpc.VpcId
        });
      });
    ...
    const zB = _createMapZoom(svg, g);
    svg.call(zB);
    _mapSvg = svg;
    _mapZoom = zB;
    _mapG = g;
    if (typeof window !== 'undefined' && typeof window._bindSafariGestureZoom === 'function') {
      window._bindSafariGestureZoom();
    }
    bindZoomButtons();
    ...
    function showLockInd(v) {
      lockInd.style.display = v ? 'block' : 'none';
    }
    function setGwHl(gid) {
      ...
      if (gid) {
        ndL.selectAll('.gw-node').each(function () {
          ...
          if (el.datum && el.datum() === gid) {
            el.classed('gw-hl', true);
          }
        });
      }
    }
    ...
    function hlGw(gid) {
      ...
      routeG.style('opacity', '0.03');
      structG.style('opacity', '0.03');
      ...
      ndL.selectAll(_gatewayNodeSelector(gid)).classed('gw-hl', true);

      // Find all VPCs connected to this gateway and highlight them
      const gwVids = new Set();
      structG
        .node()
        .querySelectorAll('[data-gid="' + gid + '"][data-vid]')
        .forEach((el) => {
          gwVids.add(el.getAttribute('data-vid'));
        });
      gwVids.forEach((vid) =>
        ndL.selectAll(_vpcGroupSelector(vid)).each(function () {
          d3.select(this)
            .select('rect')
            .style('stroke-width', '3px')
            .style('filter', 'drop-shadow(0 0 8px rgba(99,180,255,.7))');
        })
      );
      ...
      sNode.querySelectorAll('[data-gid="' + gid + '"]').forEach((el) => {
        if (el.hasAttribute('data-net-vert')) {
          hasNet = true;
        }
        clonePathToOl(el);
      });
      ...
      if (hasNet) {
        ...
        if (netLine && gp) {
          ...
          if (nlM) {
            const nlX1 = parseFloat(nlM[1]),
              nlY = parseFloat(nlM[2]),
              nlX2 = parseFloat(nlM[3]);
        ...
        if (gp) {
          ...
          ndL.selectAll('.gw-node').each(function () {
            ...
            if (t.text() === 'IGW' && Math.abs(cx - gwX) < 2) {
              d3.select(this).classed('gw-hl', true);
            }
          });
        }
      }
    ...
    function hlSub(sid) {
      ...
      if (subVid) {
        ndL.selectAll(_vpcGroupSelector(subVid)).each(function () {
          d3.select(this)
            .select('rect')
            .style('stroke-width', '3px')
            .style('filter', 'drop-shadow(0 0 8px rgba(99,180,255,.7))');
      ...
      mg.forEach((gid) => {
        ...
        const subRouteEl = sNode.querySelector(
          '[data-gid="' + gid + '"][data-sid="' + sid + '"].route-line'
        );
        let subY = subMidY;
        if (subRouteEl) {
          const rm = subRouteEl.getAttribute('d').match(/M[\d.]+,([\d.]+)/);
          if (rm) {
            subY = parseFloat(rm[1]);
          }
        }
        ...
        sNode
          .querySelectorAll('[data-gid="' + gid + '"][data-sid="' + sid + '"]')
          .forEach((el) => clonePathToOl(el));
        ...
        sNode
          .querySelectorAll(
            '[data-gid="' +
              ...
              '"][data-vid="' +
              subVid +
              '"]:not([data-sid]):not([data-net-vert])'
          )
          .forEach((el) => {
        ...
        sNode
          ...
          .forEach((el) => {
            ...
            const isDashed =
              el.style.strokeDasharray !== 'none' && !el.classList.contains('route-junction');
        ...
        sNode
          .querySelectorAll(
            '[data-gid="' + gid + '"]:not([data-vid]):not([data-net-vert]):not([data-net-line])'
          )
          .forEach((el) => clonePathToOl(el));
          ...
          sNode.querySelectorAll('[data-net-vert]').forEach((el) => {
            ...
            if (dm) {
              ...
              if (Math.abs(segX - gwX) < 2) {
                ...
                if (segTop <= gwBotY + 2 && segBot <= gwBotY + 2) {
                  clonePathToOl(el);
                }
              }
            }
    ...
    function clr() {
      ...
      olL.selectAll('*').remove();
      routeG.style('opacity', null);
      structG.style('opacity', null);
      allLb.forEach((l) => l.g.classed('visible', false));
    ...
    if (window._hlUnlockHandler) {
      document.removeEventListener('hl-unlock', window._hlUnlockHandler);
    }
    window._hlUnlockHandler = forceClr;
    document.addEventListener('hl-unlock', forceClr);
    svg.on('click', function (event) {
      if (
        !event.target.closest('.gw-node') &&
        !event.target.closest('.subnet-node') &&
        !event.target.closest('.res-node') &&
        !event.target.closest('.route-hitarea') &&
        !event.target.closest('.peering-hitarea') &&
        !event.target.closest('.internet-node')
      ) {
        forceClr();
        if (_spotlightActive) {
          _closeSpotlight();
    ...
    ndL.selectAll('.subnet-node').each(function () {
      ...
      el.on('mouseenter.hl', function () {
        if (!_hlLocked) {
          hlSub(sid);
        }
      })
        .on('mouseleave.hl', function () {
          if (!_hlLocked) {
            clr();
          }
        })
        .on('click.hl', function (event) {
          if (event.target.closest('.res-node')) {
            return;
          }
          event.stopPropagation();
          if (_hlLocked && _hlKey === sid && _hlType === 'sub') {
            forceClr();
            return;
          }
          forceClr();
    ...
    const activePeerings = peerings
      ...
      .map((pcx) => {
        const rv = pcx.RequesterVpcInfo?.VpcId,
          av = pcx.AccepterVpcInfo?.VpcId;
        const v1 = vL.find((v) => v.vpc.VpcId === rv),
          v2 = vL.find((v) => v.vpc.VpcId === av);
        if (!v1 || !v2) {
          return null;
        }
    ...
    function _peeringTooltipHtml(pcx) {
      ...
      let h =
        '<div class="tt-title">' +
        gn(pcx, id) +
        '</div><div class="tt-sub">VPC Peering | ' +
        esc(id) +
        '</div>';
      h += '<div class="tt-sec"><div class="tt-sh">Connection</div>';
      h +=
        '<div class="tt-r">Requester: <span class="i">' +
        esc(_peeringVpcText(req)) +
        '</span></div>';
      h +=
        '<div class="tt-r">Accepter: <span class="i">' +
        esc(_peeringVpcText(acc)) +
        '</span></div>';
      h +=
        '<div class="tt-r">Status: <span class="a">' +
        esc((pcx.Status && pcx.Status.Code) || 'active') +
        '</span></div></div>';
      return h;
    }
    function _setPeeringHover(refs, pcx, on) {
      refs.forEach((r) => {
        r.line.classed('peering-hl', on);
        if (r.label) {
          r.label.classed('peering-hl', on);
        }
      });
      [
        pcx.RequesterVpcInfo && pcx.RequesterVpcInfo.VpcId,
        pcx.AccepterVpcInfo && pcx.AccepterVpcInfo.VpcId
      ].forEach((vid) => {
        if (!vid) {
          return;
        }
        ndL
          .selectAll(_vpcGroupSelector(vid))
          .select('rect')
          .style('stroke-width', on ? 3 : null)
          .style('filter', on ? 'drop-shadow(0 0 8px rgba(251,146,60,.75))' : null);
      });
    }
    function _addPeeringHitarea(d, refs, pcx) {
      ...
      lnL
        .append('path')
        .attr('class', 'peering-hitarea')
        .attr('d', d)
        .on('mouseenter', function (event) {
          if (_hlLocked) {
            return;
          }
          _setPeeringHover(refs, pcx, true);
          tt.innerHTML = _peeringTooltipHtml(pcx);
        ...
        .on('mouseleave', function () {
          if (_hlLocked && _hlKey === id && _hlType === 'pcx') {
            return;
          }
          _setPeeringHover(refs, pcx, false);
          tt.style.display = 'none';
        })
    ...
    activePeerings.forEach((p, idx) => {
      ...
      pg.append('text')
        ...
        .style('font-size', 'calc(9px * var(--txt-scale,1))')
        .attr('fill', '#fb923c')
        .text(pn);
    ...
    vpns.forEach((vpn) => {
      ...
      const vgwId = vpn.VpnGatewayId;
      if (!vgwId || !gwP.has(vgwId)) {
        return;
      }
      const pos = gwP.get(vgwId);
      const vpnLbl = 'VPN: ' + gn(vpn, 'VPN');
      const vpnLblY = pos.y - GR - 12;
      const vpnTw = vpnLbl.length * 5.2 + 14;
      ndL
        .append('rect')
        .attr('x', pos.x - vpnTw / 2)
        .attr('y', vpnLblY - 9)
        .attr('width', vpnTw)
        .attr('height', 14)
        .attr('rx', 4)
        .attr('class', 'gw-label-bg')
        .attr('fill', 'var(--overlay-bg)')
        .attr('stroke', 'rgba(249,115,22,.3)')
        .attr('stroke-width', 0.5);
      ndL
        .append('text')
        .attr('x', pos.x)
        ...
        .attr('text-anchor', 'middle')
        .attr('font-family', 'Segoe UI,system-ui,sans-serif')
        .style('font-size', 'calc(7px * var(--txt-scale,1))')
        .attr('fill', '#f97316')
        .text(vpnLbl);
    });
    ...
    sortedTgEntries.forEach(([key, conns]) => {
      ...
      const gi = gp.gw,
        col = gcv(gi.type),
        colH = gch(gi.type);
      const ov = vL.find((v) => v.vpc.VpcId === vid);
      if (!ov) {
        return;
      }
      ...
      if (sh) {
        ...
        if (goingUp) {
          ...
          tx = gwLeft
            ? cl - baseOffset - upCount * sp - (tn - 1) * sp
            : cl + baseOffset + upCount * sp + (tn - 1) * sp;
        }
      }
      ...
      gwP.forEach((pos, id) => {
        if (!isShared(pos.gw.type)) {
          const ovGw = vL.find(
            (v) => v.subs.some((s) => true) && pvGws[v.vpc.VpcId]?.some((g) => g.id === id)
          );
          if (ovGw && ovGw.vpc.VpcId === vid) {
            vpcGwPositions.push(pos);
          }
        }
      });
      ...
      uniqueConns.forEach((c) => {
        ...
        if (gwLeft) {
          ...
          const crossingGw = vpcGwPositions.find(
            (g) => g !== gp && Math.abs(g.y - sy) < GR + 8 && g.x < subLeft && g.x > tx
          );
          if (stepN > 0) {
          ...
          const crossingGw = vpcGwPositions.find(
            (g) => g !== gp && Math.abs(g.y - sy) < GR + 8 && g.x > subRight && g.x < tx
          );
          if (stepN > 0) {
        ...
        const rl = structG
          .append('path')
          .attr('class', 'route-line route-structural')
          .attr('d', d)
          .attr('stroke', col)
          .attr('data-gid', gid)
          .attr('data-vid', vid)
        ...
        structG
          .append('path')
          .attr('class', 'route-junction route-structural')
          .attr('d', jd)
          .attr('stroke', col)
          .attr('fill', col)
          .attr('stroke-width', 1)
        ...
        lnL
          ...
          .on('click', function (event) {
            event.stopPropagation();
            if (_hlLocked && _hlKey === c.sid && _hlType === 'sub') {
              forceClr();
              return;
            }
            forceClr();
      ...
      const topY = goingUp
        ? uniqueConns[0].sl.y + 6
        : uniqueConns[0].sl.y + uniqueConns[0].sl.h - 6;
      const botY = goingUp
        ? uniqueConns[uniqueConns.length - 1].sl.y + 6
        : uniqueConns[uniqueConns.length - 1].sl.y + uniqueConns[uniqueConns.length - 1].sl.h - 6;
      let lx, ly;
      ...
      if (!sh) {
        ...
        lnL
          ...
          .on('click', function (event) {
            event.stopPropagation();
            if (_hlLocked && _hlKey === gid && _hlType === 'gw') {
              forceClr();
              return;
            }
            forceClr();
        ...
        lx = gwLeft ? tx - 4 : tx + 4;
        ly =
          uniqueConns[uniqueConns.length - 1].sl.y + uniqueConns[uniqueConns.length - 1].sl.h + 10;
      }
      ...
      if (!skip) {
        const lt = uniqueConns[0].dst + ' > ' + gi.type,
          lw = lt.length * 5.4 + 16;
        ...
        lg.append('text')
          ...
          .attr('text-anchor', 'middle')
          .attr('font-family', 'Segoe UI,system-ui,sans-serif')
          .style('font-size', 'calc(8px * var(--txt-scale,1))')
    ...
    knownVL.forEach((vl, i) => {
      ...
      if (regionVpcs.length === 0) {
        regionStartX = vl.x;
      }
      regionVpcs.push(vl);
      prevRegion = r;
      if (i === knownVL.length - 1 && regionVpcs.length) {
        regionGroups.push({ region: r, vpcs: [...regionVpcs] });
      }
    });
    ...
    regionGroups.forEach((rg) => {
      const first = rg.vpcs[0],
        last = rg.vpcs[rg.vpcs.length - 1];
      const ry = first.y - 30;
      ...
      if (mr) {
        ...
        const labelText =
          rg.region + ' (' + rg.vpcs.length + ' VPC' + (rg.vpcs.length > 1 ? 's' : '') + ')';
    ...
    vL.forEach((vl) => {
      ...
      vG.append('rect')
        ...
        .attr('width', vl.w)
        .attr('height', vl.h)
        .attr('fill', 'rgba(59,130,246,.03)')
        .attr('stroke', 'var(--vpc-stroke)')
      ...
      vG.append('text')
        ...
        .attr('x', vl.x + 14)
        .attr('y', vl.y + 16)
        .attr('textLength', Math.min(_vpcName.length * 8, vl.w * 0.7))
        .attr('lengthAdjust', 'spacing')
        .text(_vpcName);
      const regionTag = vpcRegionMap[vl.vpc.VpcId] || '';
      const acctTag =
        _multiAccount && vl.vpc._accountId && vl.vpc._accountId !== 'default'
          ? ' [' + vl.vpc._accountId + ']'
          : '';
      vG.append('text')
        .attr('class', 'vpc-cidr')
        .attr('x', vl.x + 14)
        .attr('y', vl.y + 34)
        .text(vl.vpc.CidrBlock + (regionTag ? ' | ' + regionTag : '') + (acctTag ? acctTag : ''));
      ...
      if (_multiAccount && vl.vpc._accountId !== 'default') {
        ...
        if (acCol) {
          ...
          vG.append('text')
            .attr('x', vl.x + 5)
            .attr('y', vl.y + vl.h - 6)
            .attr('transform', 'rotate(-90,' + (vl.x + 5) + ',' + (vl.y + vl.h - 6) + ')')
            .attr('font-family', 'Segoe UI,system-ui,sans-serif')
            .style('font-size', 'calc(7px * var(--txt-scale,1))')
            .attr('fill', '#fff')
            .attr('font-weight', '600')
      ...
      if (vl.subs.length === 0) {
        vG.append('text')
          .attr('x', vl.x + vl.w / 2)
          .attr('y', vl.y + vl.h / 2 + 10)
          .attr('text-anchor', 'middle')
          .attr('font-family', 'Segoe UI,system-ui,sans-serif')
          .style('font-size', 'calc(10px * var(--txt-scale,1))')
    ...
    vL.forEach((vl) => {
      ...
      vl.subs
        .filter((sl) => sl.azLabel)
        .forEach((sl) => {
          ndL
            .append('text')
            .attr('x', sl.x)
            .attr('y', sl.y + AZ_HDR - 2)
            .attr('font-family', 'Segoe UI,system-ui,sans-serif')
            .style('font-size', 'calc(7px * var(--txt-scale,1))')
            ...
            .text('AZ: ' + sl.azLabel.slice(-2).toUpperCase());
          ndL
            .append('line')
            .attr('x1', sl.x + 38)
            .attr('y1', sl.y + AZ_HDR - 5)
            .attr('x2', sl.x + sl.w)
            .attr('y2', sl.y + AZ_HDR - 5)
      ...
      vl.subs
        ...
        .forEach((sl) => {
          const sG = ndL
            .append('g')
            .attr('class', 'subnet-node')
            .attr('data-subnet-id', sl.sub.SubnetId);
          ...
          sG.append('rect')
            ...
            .attr('width', sl.w)
            .attr('height', sl.h)
            .attr('fill', sl.pub ? 'rgba(6,182,212,.15)' : 'rgba(139,92,246,.15)')
            .attr('stroke', col)
          ...
          tG2
            ...
            .attr('x', sl.x + 8)
            .attr('y', sl.y + 16)
            .text(gn(sl.sub, sl.sub.SubnetId));
          tG2
            .append('text')
            ...
            .text(
              sl.sub.CidrBlock +
                (sl.sub.AvailabilityZone ? '  ' + sl.sub.AvailabilityZone.slice(-2) : '')
            );
          sG.append('text')
            .attr('x', sl.x + sl.w - 8)
            .attr('y', sl.y + 14)
          ...
          if (_detailLevel === 0 && tree.length > 0) {
            ...
            sG.append('text')
              ...
              .attr('y', sl.y + sl.h - 6)
              .attr('font-family', 'Segoe UI,system-ui,sans-serif')
              .style('font-size', 'calc(6px * var(--txt-scale,1))')
              .attr('fill', 'var(--text-muted)')
              .attr('opacity', 0.5)
              .text(summary);
            ...
            tree.forEach((res, ri) => {
              ...
              rG.append('text')
                ...
                .attr('text-anchor', 'middle')
                .attr('font-family', 'Segoe UI,system-ui,sans-serif')
                .style('font-size', 'calc(7.5px * var(--txt-scale,1))')
                .attr('font-weight', '700')
                .attr('fill', res.col)
                .text(res.type);
              ...
              if (nCh > 0) {
                (res.children || []).forEach((ch, ci) => {
                  ...
                  rG.append('text')
                    ...
                    .text(ch.name + (ch.detail ? ' ' + ch.detail : ''));
                });
              }
              rx += iconW + RES_GAP;
              rci++;
          ...
          sG.on('mouseenter', function () {
            ...
            .on('click', function (event) {
              ...
              _lastRlType = null;
              _navStack = [];
              openSubnetPanel(sl.sub, vl.vpc.VpcId, {
                pubSubs,
                subRT,
                subNacl,
                instBySub,
    ...
    gwP.forEach((pos, id) => {
      ...
      const gw = pos.gw,
        gG = ndL.append('g').attr('class', 'gw-node').attr('data-gwid', id),
        col = gcv(gw.type);
      gG.append('circle')
        .attr('cx', pos.x)
        .attr('cy', pos.y)
      ...
      gG.on('mouseenter', function () {
        ...
        if (natInfo) {
          h += `<div class="tt-sec"><div class="tt-sh">NAT Gateway</div><div class="tt-r">Subnet: <span class="i">${esc(natInfo.SubnetId || 'N/A')}</span></div><div class="tt-r">State: ${esc(natInfo.State || 'N/A')}</div></div>`;
        }
        tt.innerHTML = h;
        tt.style.display = 'block';
      })
        ...
        .on('click', function (event) {
          event.stopPropagation();
          if (_hlLocked && _hlKey === id && _hlType === 'gw') {
            forceClr();
            return;
          }
          forceClr();
          ...
          _lastRlType = null;
          _navStack = [];
          openGatewayPanel(gw.id, gw.type, {
            gwNames,
            igws,
            nats,
            vpns,
    ...
    if (iGwList.length) {
      ...
      iG.append('text')
        ...
        .attr('text-anchor', 'middle')
        .attr('font-family', 'Segoe UI,system-ui,sans-serif')
        .style('font-size', 'calc(13px * var(--txt-scale,1))')
        .attr('font-weight', '700')
        .attr('fill', 'var(--igw-color)')
        .text('NET');
      // Text below circle
      iG.append('text')
        .attr('x', iX)
        ...
        .attr('text-anchor', 'middle')
        .attr('font-family', 'Segoe UI,system-ui,sans-serif')
        .style('font-size', 'calc(11px * var(--txt-scale,1))')
        .attr('font-weight', '500')
        .attr('fill', 'var(--text-secondary)')
        .text('Internet');
      iG.append('text')
        ...
        .style('font-size', 'calc(9px * var(--txt-scale,1))')
        .attr('fill', 'var(--igw-color)')
        .text(iGwList.length + ' Gateway' + (iGwList.length > 1 ? 's' : ''));
        ...
        for (let i = 0; i < group.length; i++) {
          ...
          if (Math.abs(botY - topY) > 2) {
            structG
              ...
              .attr('stroke', col)
              .attr('stroke-width', 3)
              .attr('data-gid', group[i].gw.id)
              .attr('data-net-vert', '1');
            if (topY === iY) {
              reachesBus = true;
            }
          }
    ...
    vL.forEach((vl) => {
      ...
      eG.append('rect')
        ...
        .attr('height', nh)
        .attr('rx', 3)
        .attr('fill', 'rgba(167,139,250,.2)')
        .attr('stroke', 'var(--vpce-color)')
        .attr('stroke-width', 1);
      eG.append('text')
        .attr('x', gx)
        .attr('y', ny + 12)
        ...
        .style('font-size', 'calc(8px * var(--txt-scale,1))')
        .attr('font-weight', '600')
        .attr('fill', 'var(--vpce-color)')
        .text(vpcVpces.length + ' VPCE');
      // tooltip on hover
      eG.on('mouseenter', function () {
        ...
        vpcVpces.forEach((v) => {
          ...
          h +=
            '<div class="tt-r"><span class="i">' +
            esc(nm || sid(v.id)) +
            '</span> ' +
            esc(svc.split('.').pop()) +
            ' [' +
            esc(vi?.VpcEndpointType) +
            ']</div>';
        });
        h += '</div>';
        tt.innerHTML = h;
    ...
    if (zones.length > 0) {
      ...
      zones.forEach((z) => {
        if (z.Config?.PrivateZone && z.VPCs) {
          z.VPCs.forEach((v) => {
            ...
            if (vid) {
              (privZonesByVpc[vid] = privZonesByVpc[vid] || []).push(z);
            }
          });
        }
      });
      ...
      const totalContentH = dnsRecExp
        ? zoneLayouts.length > 0
          ? zoneLayouts[zoneLayouts.length - 1].y + zoneLayouts[zoneLayouts.length - 1].h
          : 0
      ...
      dnsG
        ...
        .attr('y', dnsY + 22)
        .attr('font-family', 'Segoe UI,system-ui,sans-serif')
        .style('font-size', 'calc(14px * var(--txt-scale,1))')
        .attr('font-weight', '700')
        .attr('fill', '#0ea5e9')
      ...
      dnsG
        ...
        .style('font-size', 'calc(10px * var(--txt-scale,1))')
        .attr('fill', 'var(--text-muted)')
        .text(pubZones.length + ' public, ' + privZones.length + ' private');
      ...
      togG
        ...
        .attr('font-weight', '600')
        .attr('fill', '#0ea5e9')
        .text(dnsRecExp ? '\u25B2 Collapse' : '\u25BC Expand');
      ...
      zones.forEach((z, zi) => {
        ...
        zG.append('rect')
          ...
          .attr('stroke', isPub ? '#10b981' : '#0ea5e9')
          .attr('stroke-width', 1.5);
        ...
        const dispName = dnsRecExp
          ? z.Name
          : z.Name.length > maxNameLen
            ? z.Name.substring(0, maxNameLen - 2) + '..'
            : z.Name;
        zG.append('text')
          .attr('x', zx + 24)
          .attr('y', zy + 15)
        ...
        if (dnsRecExp) {
          ...
          if (lay.recs.length > 0) {
            ...
            lay.recs.forEach((rec) => {
              ...
              const rVal = rec.AliasTarget
                ? 'ALIAS \u2192 ' + (rec.AliasTarget.DNSName || '').replace(/\.$/, '')
        ...
        zG.on('mouseenter', function () {
          ...
          h +=
            '<div class="tt-r"><span class="i">Records</span> ' +
            esc(z.ResourceRecordSetCount) +
            '</div>';
          h +=
            '<div class="tt-r"><span class="i">Type</span> ' +
            (isPub ? 'Public' : 'Private') +
            '</div>';
    ...
    if (s3bk.length > 0) {
      ...
      s3bk.forEach((bk, bi) => {
        ...
        bG.append('rect')
          .attr('x', bx)
          .attr('y', by)
          .attr('width', s3ColW - 10)
          .attr('height', s3RowH)
          .attr('rx', 3)
          .attr('fill', 'rgba(234,88,12,.1)')
          .attr('stroke', '#ea580c')
          .attr('stroke-width', 0.8);

        const maxChars = Math.floor((s3ColW - 20) / 6);
        const dispName =
          bk.Name.length > maxChars ? bk.Name.substring(0, maxChars - 2) + '..' : bk.Name;
        bG.append('text')
          .attr('x', bx + 6)
          .attr('y', by + 16)
          .attr('font-family', 'Segoe UI,system-ui,sans-serif')
        ...
        bG.on('mouseenter', function () {
          ...
          h += '<div class="tt-sec">';
          h +=
            '<div class="tt-r"><span class="i">Created</span> ' +
            esc((bk.CreationDate || 'N/A').split('T')[0]) +
            '</div>';
          h += '</div>';
          tt.innerHTML = h;
          tt.style.display = 'block';
    ...
    if (cfDistributions.length > 0) {
      ...
      cfG
        ...
        .style('font-size', 'calc(10px * var(--txt-scale,1))')
        .attr('fill', 'var(--text-muted)')
        .text(cfDistributions.length + ' distributions');
      ...
      cfDistributions.forEach((d, di) => {
        ...
        cG.append('rect')
          .attr('x', cx)
          .attr('y', cy)
          .attr('width', cfColW - 10)
          .attr('height', cfRowH)
          .attr('rx', 3)
          .attr('fill', 'rgba(139,92,246,.12)')
          .attr('stroke', '#8b5cf6')
          .attr('stroke-width', 0.8);
        cG.append('text')
          .attr('x', cx + 6)
        ...
        cG.on('mouseenter', function () {
          ...
          if (aliases.length) {
            h +=
              '<div class="tt-r"><span class="i">Aliases</span> ' +
              esc(aliases.join(', ')) +
              '</div>';
          }
          ...
          if (d.WebACLId) {
            h +=
              '<div class="tt-r"><span class="i">WAF</span> ' +
              esc(d.WebACLId.split('/').pop()) +
              '</div>';
          }
          h += '</div>';
          tt.innerHTML = h;
    ...
    [
      { l: 'VPCs', v: vpcs.length },
      { l: 'Subnets', v: subnets.length },
      { l: 'Public', v: pubSubs.size },
      { l: 'Private', v: subnets.length - pubSubs.size },
      { l: 'Gateways', v: gwSet.size },
      { l: 'RTs', v: rts.length },
      { l: 'NACLs', v: nacls.length },
      { l: 'SGs', v: sgs.length },
      { l: 'EC2', v: instances.length },
      { l: 'ENIs', v: enis.length },
      { l: 'ALBs', v: albs.length },
      { l: 'TGs', v: tgs.length },
      { l: 'RDS', v: rdsInstances.length },
      { l: 'ECS', v: ecsServices.length },
      { l: 'Lambda', v: lambdaFns.length },
      { l: 'Cache', v: ecacheClusters.length },
      { l: 'Redshift', v: redshiftClusters.length },
      { l: 'Peering', v: peerings.length },
      { l: 'VPNs', v: vpns.length },
      { l: 'Endpoints', v: vpces.length },
      { l: 'Volumes', v: volumes.length },
      { l: 'Snapshots', v: snapshots.length },
      { l: 'S3', v: s3bk.length },
      { l: 'R53', v: zones.length },
      { l: 'WAF', v: wafAcls.length },
      { l: 'CF', v: cfDistributions.length }
    ].forEach((s) => {
      if (s.v > 0) {
        const c = document.createElement('div');
        c.className = 'stat-chip';
        c.dataset.type = s.l;
        c.innerHTML = `<b>${s.v}</b>${s.l}`;
        c.addEventListener('click', () => {
          _setMapMoving(true);
          requestAnimationFrame(() =>
            setTimeout(() => {
              try {
                openResourceList(s.l);
              } finally {
                _setMapMoving(false);
              }
            }, 0)
    ...
    try {
      const findings = runComplianceChecks(_rlCtx);
      if (findings.length) {
        addComplianceChip(sb2, findings);
      }
      _addBUDRChip(sb2);
    } catch (ce) {
      console.warn('Compliance check error:', ce);
    }
    if (_iamData) {
      const _ic = (_iamData.roles?.length || 0) + (_iamData.users?.length || 0);
      if (_ic > 0) {
        const ic = document.createElement('div');
        ic.className = 'stat-chip';
        ic.classList.add('accent-amber');
        ic.innerHTML = '<b>' + _ic + '</b> IAM';
        ic.addEventListener('click', () => {
          _setMapMoving(true);
          requestAnimationFrame(() =>
            setTimeout(() => {
              try {
                openResourceList('IAM');
              } finally {
                _setMapMoving(false);
              }
            }, 0)
    ...
    try {
      ...
      console.warn('Note badges error:', ne);
    }
    try {
      _renderComplianceBadges();
    } catch (cbe) {
      console.warn('Compliance badge error:', cbe);
    }
    try {
      if (typeof _scheduleAutoRenderSnapshot === 'function') {
        _scheduleAutoRenderSnapshot();
      } else if (Date.now() - _lastAutoSnap > 120000) {
        takeSnapshot('Render', true);
        _lastAutoSnap = Date.now();
      }
    } catch (se) {
      console.warn('Auto-snapshot error:', se);
    }
    ...
    if (_designMode && _lastDesignValidation) {
      ...
      dvC.className =
        'compliance-chip ' +
        (sv.errors.length ? 'critical' : sv.warnings.length ? 'warn' : 'clean');
    ...
    try {
      if (_diffMode) {
        setTimeout(_applyDiffOverlay, 150);
      }
    } catch (de) {
      console.warn('Diff overlay error:', de);
    }
    ...
    if (typeof _fitMapWhenReady === 'function') {
      _fitMapWhenReady();
    } else {
    ...
    showToast('Render error — see console', 4000);
