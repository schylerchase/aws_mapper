...
function _buildLookupMaps(d) {
  ...
  vpns.forEach((g) => {
    if (g.VpnGatewayId) gwNames[g.VpnGatewayId] = gn(g, g.VpnGatewayId);
  });
  peerings.forEach((g) => {
  ...
  rts.forEach((rt) => {
    ...
    (rt.Associations || []).forEach((a) => {
      if (a.SubnetId) {
        subRT[a.SubnetId] = rt;
        if (hasIgw) pubSubs.add(a.SubnetId);
      }
    });
    (rt.Routes || []).forEach((r) => {
  ...
  volumes.forEach((v) => {
    ...
    if (att && att.InstanceId) {
      if (knownInstIds.has(att.InstanceId)) return;
  ...
  ecacheClusters.forEach((c) => {
    if (c.VpcId) (ecacheByVpc[c.VpcId] = ecacheByVpc[c.VpcId] || []).push(c);
  });
  const redshiftByVpc = {};
  redshiftClusters.forEach((c) => {
    if (c.VpcId) (redshiftByVpc[c.VpcId] = redshiftByVpc[c.VpcId] || []).push(c);
  });
  const cfByAlb = {};
  ...
  [...gwSet.values()].forEach((gw) => {
    ...
    if (isShared(gw.type)) {
      if (!shGws.find((g) => g.id === gw.id)) shGws.push(gw);
...
function _renderMapInner() {
  try {
    ...
    vpcs.forEach((v) => {
      ...
      if (seenVpcs.has(key)) vpcDupes.push(v.VpcId);
      seenVpcs.set(key, v);
    });
    ...
    function countAZHeaders(ss) {
      const azs = new Set();
      ss.forEach((s) => {
        if (s.AvailabilityZone) azs.add(s.AvailabilityZone);
      });
    ...
    function hlSub(sid) {
      ...
      if (subVid)
        ndL.selectAll('.vpc-group[data-vpc-id="' + subVid + '"]').each(function () {
          d3.select(this)
            .select('rect')
            .style('stroke-width', '3px')
            .style('filter', 'drop-shadow(0 0 8px rgba(99,180,255,.7))');
    ...
    function _setPeeringHover(refs, pcx, on) {
      refs.forEach((r) => {
        ...
        if (r.label) r.label.classed('peering-hl', on);
      });
      [
        ...
        if (!vid) return;
        ndL
          .selectAll('.vpc-group[data-vpc-id="' + vid + '"]')
          .select('rect')
    ...
    function _addPeeringHitarea(d, refs, pcx) {
      ...
      lnL
        ...
        .on('mouseleave', function () {
          if (_hlLocked && _hlKey === id && _hlType === 'pcx') return;
    ...
    vL.forEach((vl) => {
      ...
      const vpcHeader = buildVpcHeaderLabels({
        vpc: vl.vpc,
        width: vl.w,
        region: regionTag,
        isMultiAccount: _multiAccount,
        nameOf: gn,
        nameLengthRatio: 0.7,
        accountSeparator: ' | '
      });
      vG.append('text')
        ...
        .attr('x', vl.x + 14)
        .attr('y', vl.y + 16)
        .attr('textLength', vpcHeader.nameTextLength)
      ...
      if (_multiAccount && vl.vpc._accountId !== 'default') {
        ...
        if (acCol) {
          vG.append('rect')
            ...
            .attr('rx', 2)
            .attr('opacity', 0.7);
          const acLbl = buildAccountStripeLabel({
            accountId: vl.vpc._accountId,
            accountLabel: vl.vpc._accountLabel,
            height: vl.h
          });
          vG.append('text')
            .attr('x', vl.x + 5)
    ...
    vL.forEach((vl) => {
      ...
      vl.subs
        ...
        .forEach((sl) => {
          ndL
            ...
            .text(formatAzSeparatorLabel(sl.azLabel));
          ndL
            .append('line')
            .attr('x1', sl.x + 38)
      ...
      vl.subs
        ...
        .forEach((sl) => {
          ...
          const subnetHeader = buildSubnetHeaderLabels({
            subnet: sl.sub,
            isPublic: sl.pub,
            nameOf: gn,
            exposureStyle: 'long'
          ...
          tG2
            ...
            .attr('x', sl.x + 8)
            .attr('y', sl.y + 16)
            .text(subnetHeader.name);
          tG2
            .append('text')
            .attr('class', 'subnet-cidr')
            .attr('x', sl.x + 8)
            .attr('y', sl.y + 32)
            .text(subnetHeader.cidrLine);
          sG.append('text')
            .attr('x', sl.x + sl.w - 8)
            .attr('y', sl.y + 14)
            ...
            .attr('font-weight', '600')
            .attr('fill', col)
            .text(subnetHeader.exposureLabel);
    ...
    [
      ...
      if (s.v > 0) {
        ...
        c.addEventListener('click', () => openResourceList(s.l));
        sb2.appendChild(c);
      }
    });
    ...
    if (_iamData) {
      ...
      if (_ic > 0) {
        ...
        ic.addEventListener('click', () => openResourceList('IAM'));
        sb2.appendChild(ic);
      }
    }
    _depGraph = null;
    ...
    try {
      if (Date.now() - _lastAutoSnap > 120000) {
    ...
    try {
      if (_diffMode) setTimeout(_applyDiffOverlay, 150);
    } catch (de) {
