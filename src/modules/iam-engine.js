...
let _iamData = null;
let _showIAM = false;
export function setIamData(v) {
  _iamData = v;
}
export function setShowIAM(v) {
  _showIAM = v;
}
export function getIamData() {
  return _iamData;
}
export function getShowIAM() {
  return _showIAM;
}
function parseIAMData(raw) {
  ...
  if (raw.RoleDetailList) {
    data.roles = raw.RoleDetailList;
  } else if (raw.Roles) {
    data.roles = raw.Roles;
  }
  if (raw.UserDetailList) {
    data.users = raw.UserDetailList;
  }
  if (raw.Policies) {
    data.policies = raw.Policies;
  }
  if (raw.PasswordPolicy) {
    data.passwordPolicy = raw.PasswordPolicy;
  }
  if (raw.AccountPasswordPolicy) {
    data.passwordPolicy = raw.AccountPasswordPolicy;
  }
  // OPTIMIZED: Pre-index policies for O(1) lookup
  const policyMap = new Map();
  data.policies.forEach((dp) => {
    if (dp.Arn) {
      policyMap.set(dp.Arn, dp);
    }
    if (dp.PolicyName) {
      policyMap.set(dp.PolicyName, dp);
    }
  });
  ...
  data.roles.forEach((role) => {
    ...
    policies.forEach((p) => {
      ...
      stmts.forEach((stmt) => {
        ...
        if (hasWildAction && hasWildResource) {
          role._isAdmin = true;
        }
        if (hasWildResource) {
          role._hasWildcard = true;
        }
        // Check for VPC conditions
        const cond = stmt.Condition || {};
        Object.values(cond).forEach((cv) => {
          if (cv['aws:SourceVpc']) {
            role._vpcAccess.push(
              ...(Array.isArray(cv['aws:SourceVpc']) ? cv['aws:SourceVpc'] : [cv['aws:SourceVpc']])
            );
          }
          if (cv['ec2:Vpc']) {
            role._vpcAccess.push(
              ...(Array.isArray(cv['ec2:Vpc']) ? cv['ec2:Vpc'] : [cv['ec2:Vpc']])
            );
          }
        });
        ...
        if (actions.some((a) => a.startsWith('ec2:')) || actions.some((a) => a === 'ec2:*')) {
          resources.forEach((r) => {
            if (r.includes(':vpc/') || r.includes(':subnet/')) {
              role._vpcAccess.push(r);
            }
          });
        }
      });
...
function getIAMAccessForVpc(iamData, vpcId) {
  ...
  return iamData.roles.filter(
    (r) =>
      r._isAdmin ||
      r._hasWildcard ||
      r._vpcAccess.some((v) => v === vpcId || v.includes(vpcId) || v === '*')
  );
}
function runIAMChecks(iamData) {
  ...
  (iamData.policies || []).forEach((p) => {
    if (p.Arn) {
      policyByArn.set(p.Arn, p);
    }
    if (p.PolicyName) {
      policyByArn.set(p.PolicyName, p);
    }
  });
  ...
  (iamData.roles || []).forEach((role) => {
    ...
    if (role._isAdmin) {
      f.push({
        ...
        control: 'IAM-1',
        framework: 'IAM',
        resource: role.RoleName || role.Arn || '',
        resourceName: role.RoleName || '',
        message: 'Role has admin (*:*) permissions',
        remediation: 'Apply least-privilege: scope actions and resources'
      });
    }
    if (role._hasWildcard && !role._isAdmin) {
      f.push({
        severity: 'HIGH',
        control: 'IAM-2',
        framework: 'IAM',
        ...
        remediation: 'Scope Resource ARNs to specific resources'
      });
    }
    ...
    if (role.AssumeRolePolicyDocument) {
      ...
      const hasMFA = stmts.some((s) =>
        JSON.stringify(s.Condition || {}).includes('aws:MultiFactorAuth')
      );
      if (crossAccount && !hasMFA) {
        f.push({
          severity: 'MEDIUM',
          control: 'IAM-3',
          framework: 'IAM',
          ...
          resourceName: role.RoleName || '',
          message: 'Cross-account role without MFA condition',
          remediation: 'Add Condition with aws:MultiFactorAuthPresent'
        });
      }
    }
    ...
    allStmts4.forEach((stmt) => {
      ...
      if (acts.some((a) => /^[a-z0-9]+:\*$/i.test(a))) {
        ...
        f.push({
          ...
          resource: role.RoleName || '',
          resourceName: role.RoleName || '',
          message: 'Role uses service-level wildcard actions (e.g. s3:*)',
          remediation: 'Scope actions to specific API calls needed'
        });
      }
    });
    // IAM-5: Unused role (>90 days or never used)
    const lastUsed = role.RoleLastUsed?.LastUsedDate;
    if (!lastUsed || Date.now() - new Date(lastUsed).getTime() > 90 * 86400000) {
      f.push({
        severity: 'LOW',
        control: 'IAM-5',
        framework: 'IAM',
        resource: role.RoleName || '',
        resourceName: role.RoleName || '',
        message: 'Role unused for >90 days or never used',
        remediation: 'Review and remove unused roles'
      });
    }
      ...
      _stmtArr(tr6.Statement).forEach((s) => {
        ...
        const hasExtId =
          ...
          (s.Condition.StringEquals?.['sts:ExternalId'] ||
            s.Condition.StringLike?.['sts:ExternalId']);
        if (isCross && !hasExtId) {
          f.push({
            severity: 'HIGH',
            control: 'IAM-6',
            ...
            resource: role.RoleName || '',
            resourceName: role.RoleName || '',
            message: 'Cross-account trust without ExternalId condition',
            remediation: 'Add sts:ExternalId condition to assume role policy'
          });
        }
      });
    }
    // IAM-7: Inline policies
    if (role.RolePolicyList?.length > 0) {
      f.push({
        severity: 'LOW',
        control: 'IAM-7',
        framework: 'IAM',
        resource: role.RoleName || '',
        resourceName: role.RoleName || '',
        message: 'Role uses inline policies instead of managed',
        remediation: 'Convert inline policies to managed policies for reusability'
      });
    }
    // IAM-8: Admin/wildcard without permission boundary
    if ((role._isAdmin || role._hasWildcard) && !role.PermissionsBoundary) {
      f.push({
        severity: 'MEDIUM',
        control: 'IAM-8',
        framework: 'IAM',
        resource: role.RoleName || '',
        resourceName: role.RoleName || '',
        message: 'Privileged role without permission boundary',
        remediation: 'Attach a permission boundary to limit effective permissions'
      });
    }
  });
  // User checks
  (iamData.users || []).forEach((user) => {
    ...
    if (uHasWildcard && !uIsAdmin) {
      f.push({
        ...
        resource: user.UserName || '',
        resourceName: user.UserName || '',
        message: 'User has wildcard Resource: "*"',
        remediation: 'Scope Resource ARNs to specific resources'
      });
    }
    if (!(user.MFADevices || []).length) {
      f.push({
        ...
        resource: user.UserName || '',
        resourceName: user.UserName || '',
        message: 'User has no MFA device configured',
        remediation: 'Enable MFA for all IAM users'
      });
    }
    if (user.UserPolicyList?.length > 0) {
      f.push({
        severity: 'LOW',
        control: 'IAM-7',
        framework: 'IAM',
        resource: user.UserName || '',
        resourceName: user.UserName || '',
        message: 'User uses inline policies instead of managed',
      ...
      if (stmt.Effect === 'Allow') {
        ...
        if (acts.some((a) => /^[a-z0-9]+:\*$/i.test(a))) {
          ...
          f.push({
            ...
            resource: user.UserName || '',
            resourceName: user.UserName || '',
            message: 'User uses service-level wildcard actions',
            remediation: 'Scope actions to specific API calls needed'
          });
        }
      }
    ...
    (user.AccessKeys || []).forEach((ak) => {
      if (ak.Status === 'Active' && ak.CreateDate) {
        ...
        const age = (Date.now() - cd.getTime()) / 86400000;
        if (age > 90) {
          f.push({
            severity: 'MEDIUM',
            control: 'IAM-9',
            ...
            resource: user.UserName || '',
            resourceName: user.UserName || '',
            message: 'Access key ' + ak.AccessKeyId + ' is ' + Math.floor(age) + ' days old',
            remediation: 'Rotate access keys every 90 days; use IAM roles for EC2/Lambda instead'
          });
        }
      }
    });
    ...
    if (activeKeys.length > 1) {
      f.push({
        ...
        remediation: 'Maintain only one active key; rotate and deactivate old keys'
      });
    }
    // IAM-11: Console password without MFA
    if (user.LoginProfile && !(user.MFADevices || []).length) {
      f.push({
        severity: 'CRITICAL',
        control: 'IAM-11',
        framework: 'IAM',
        resource: user.UserName || '',
        resourceName: user.UserName || '',
        message: 'Console login enabled without MFA',
        remediation: 'Enable MFA immediately; enforce MFA via IAM policy condition'
      });
    }
    // IAM-12: Excessive managed policy attachments (>10)
    if ((user.AttachedManagedPolicies || []).length > 10) {
      f.push({
        severity: 'LOW',
        control: 'IAM-12',
        framework: 'IAM',
        ...
        resourceName: user.UserName || '',
        message:
          'User has ' + (user.AttachedManagedPolicies || []).length + ' managed policies attached',
        remediation: 'Consolidate policies; use groups with managed policies instead'
      });
    }
  });
  ...
  if (iamData.passwordPolicy) {
    ...
    if (
      ...
      f.push({
        ...
        message: 'Password policy does not require all character types',
        remediation: 'Require uppercase, lowercase, numbers, and symbols'
      });
    }
    if ((pp.MinimumPasswordLength || 0) < 14) {
      f.push({
        ...
        message:
          ...
          (pp.MinimumPasswordLength || 'not set') +
          ' (should be 14+)',
        remediation: 'Set MinimumPasswordLength to at least 14 characters'
      });
    }
    if ((pp.MaxPasswordAge || 0) > 90 || !pp.MaxPasswordAge) {
      f.push({
        ...
        resource: 'AccountPasswordPolicy',
        resourceName: 'Password Policy',
        message: 'Password max age is ' + (pp.MaxPasswordAge || 'unlimited') + ' days',
