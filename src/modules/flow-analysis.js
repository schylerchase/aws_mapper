...
// === State Accessors ===
export function getFlowAnalysisMode() {
  return flowAnalysisMode;
}
export function setFlowAnalysisMode(v) {
  flowAnalysisMode = v;
}
export function getFlowAnalysisCache() {
  return flowAnalysisCache;
}
export function setFlowAnalysisCache(v) {
  flowAnalysisCache = v;
}
export function getFaDashState() {
  return faDashState;
}
export function setFaDashState(v) {
  faDashState = v;
}
export function getFaDashRows() {
  return faDashRows;
}
export function setFaDashRows(v) {
  faDashRows = v;
}
...
export function findIngressPaths(ctx) {
  ...
  (ctx.igws || []).forEach((igw) => {
    ...
    (ctx.subnets || []).forEach((sub) => {
      ...
      if (!ctx.pubSubs || !ctx.pubSubs.has(sub.SubnetId)) {
        return;
      }
      (ctx.instBySub[sub.SubnetId] || []).forEach((inst) => {
        [443, 80, 22].forEach((port) => {
          const r = _traceInbound(
            { type: 'instance', id: inst.InstanceId },
            { protocol: 'tcp', port },
            ctx,
            { discovery: true }
          );
          if (!r.blocked) {
            paths.push({
              from: 'internet',
              to: { type: 'instance', id: inst.InstanceId },
              toName: _gn3(inst),
              path: r.path,
              port,
              type: 'direct',
      ...
      (ctx.albBySub[sub.SubnetId] || []).forEach((alb) => {
        ...
        const r = _traceInbound(
          { type: 'alb', id: albId || alb.LoadBalancerName },
        ...
        if (!r.blocked) {
          paths.push({
            from: 'internet',
            to: { type: 'alb', id: albId || alb.LoadBalancerName },
            toName: alb.LoadBalancerName || albId,
            path: r.path,
            port: 443,
            type: 'loadbalancer',
            vpcId
...
export function findEgressPaths(ctx) {
  ...
  (ctx.instances || []).forEach((inst) => {
    if (checked.has(inst.SubnetId)) {
      return;
    }
    const r = _traceOutbound(
      { type: 'instance', id: inst.InstanceId },
      { protocol: 'tcp', port: 443 },
      ctx,
      { discovery: true }
...
export function detectBastions(ctx) {
  ...
  const hasSgData = (ctx.instances || []).some((i) => (i.SecurityGroups || []).length > 0);
  (ctx.instances || []).forEach((inst) => {
    if (!ctx.pubSubs || !ctx.pubSubs.has(inst.SubnetId)) {
      return;
    }
    ...
    if (hasSgData) {
      const sgs = (inst.SecurityGroups || [])
        .map((s) => (ctx.sgs || []).find((sg) => sg.GroupId === s.GroupId))
        .filter(Boolean);
      const hasSSH = sgs.some((sg) =>
        (sg.IpPermissions || []).some((r) => r.FromPort <= 22 && r.ToPort >= 22)
      );
      if (!hasSSH && !nameMatch) {
        return;
      }
    ...
    bastions.push({
      ...
      subnetId: inst.SubnetId,
      vpcId:
        inst.VpcId || ((ctx.subnets || []).find((s) => s.SubnetId === inst.SubnetId) || {}).VpcId
    });
  });
  return bastions;
}
...
export function findBastionChains(bastions, ctx) {
  ...
  bastions.forEach((bastion) => {
    ...
    (ctx.instances || []).forEach((inst) => {
      if (inst.InstanceId === bastion.id) {
        return;
      }
      const instVpc =
        inst.VpcId || ((ctx.subnets || []).find((s) => s.SubnetId === inst.SubnetId) || {}).VpcId;
      if (instVpc !== bastion.vpcId) {
        return;
      }
      if (ctx.pubSubs && ctx.pubSubs.has(inst.SubnetId)) {
        return;
    ...
    (ctx.rdsInstances || []).forEach((db) => {
      let rSid = null;
      Object.keys(ctx.rdsBySub || {}).forEach((sid) => {
        (ctx.rdsBySub[sid] || []).forEach((d) => {
          if (d.DBInstanceIdentifier === db.DBInstanceIdentifier) {
            rSid = sid;
          }
        });
      });
