...
export const ipToInt = (ip) => {
  ...
  for (let i = 0; i < 4; i++) {
    ...
    if (isNaN(o) || o < 0 || o > 255 || parts[i] !== String(o)) {
      return null;
    }
    n = n * 256 + o;
  }
...
function generateDemo() {
  ...
  const vpcDefs = [
    {
      ...
      azsUsed: 3,
      instancesPer: [4, 6, 10, 8, 6, 4, 3, 2],
      albCount: 5
    },
    {
      name: 'Staging',
      cidr: '10.1.0.0/16',
      tiers: ['public', 'private-web', 'private-app', 'private-data', 'private-cache'],
      azsUsed: 3,
      instancesPer: [2, 4, 6, 4, 2],
      albCount: 3
    },
    {
      name: 'Development',
      cidr: '10.2.0.0/16',
      tiers: ['public', 'private-app', 'private-data', 'private-test'],
      azsUsed: 2,
      instancesPer: [2, 6, 3, 4],
      albCount: 2
    },
    {
      name: 'QA-Automation',
      cidr: '10.3.0.0/16',
      tiers: ['public', 'private-runners', 'private-selenium', 'private-data'],
      azsUsed: 2,
      instancesPer: [1, 8, 6, 2],
      albCount: 1
    },
    {
    ...
    },
    {
      ...
      azsUsed: 3,
      instancesPer: [5, 4, 8, 4, 3, 6],
      albCount: 2
    },
    {
      name: 'Security',
      cidr: '10.30.0.0/16',
      tiers: ['public', 'private-siem', 'private-scanner', 'private-forensics', 'private-logging'],
      azsUsed: 2,
      instancesPer: [1, 4, 3, 2, 4],
      albCount: 1
    },
    {
      name: 'DR-Recovery',
      cidr: '10.40.0.0/16',
      tiers: ['public', 'private-web', 'private-app', 'private-data'],
      azsUsed: 2,
      instancesPer: [2, 3, 5, 3],
      albCount: 2
    },
    {
      name: 'Edge-Services',
      cidr: '10.50.0.0/16',
      tiers: ['public', 'private-proxy', 'private-waf', 'private-cdn-origin'],
      azsUsed: 3,
      instancesPer: [3, 4, 3, 2],
      albCount: 3
    },
    {
      name: 'Management',
      cidr: '10.100.0.0/16',
      tiers: ['public', 'private-bastion', 'private-logging', 'private-backup', 'private-config'],
      azsUsed: 2,
      instancesPer: [1, 3, 4, 3, 2],
      albCount: 1
    },
    {
      name: 'Sandbox',
      cidr: '10.200.0.0/16',
      tiers: ['public', 'private-dev1', 'private-dev2', 'private-experiment'],
      azsUsed: 2,
      instancesPer: [1, 5, 5, 3],
      albCount: 1
    },
    {
      name: 'PCI-Compliant',
      cidr: '10.60.0.0/16',
      tiers: ['private-dmz', 'private-app', 'private-tokenize', 'private-vault', 'private-audit'],
      azsUsed: 3,
      instancesPer: [3, 6, 4, 2, 2],
      albCount: 2
    }
  ];
  ...
  vpcDefs.forEach((vd, vi) => {
    ...
    if (hasPublic) {
      igwsList.push({
        ...
        Tags: [{ Key: 'Name', Value: vd.name + '-igw' }]
      });
    }
    ...
    if (hasPublic) {
      for (let a = 0; a < Math.min(vd.azsUsed, 2); a++) {
        ...
        natsList.push({
          ...
          SubnetId: null,
          State: 'available',
          Tags: [{ Key: 'Name', Value: vd.name + '-nat-' + AZS[a].slice(-2) }]
        });
      }
    }
        ...
        IpPermissions: [
          {
            ...
            IpRanges: [
              {
                CidrIp:
                  sgName.includes('ssh') || sgName.includes('rdp') ? '10.0.0.0/8' : '0.0.0.0/0'
              }
            ]
          },
    ...
    vd.tiers.forEach((tier, ti) => {
      ...
      if (isPub) {
        routes.push({ DestinationCidrBlock: '0.0.0.0/0', GatewayId: igwId });
      }
      // cross-VPC via TGW
      ...
      for (let a = 0; a < vd.azsUsed; a++) {
        ...
        subnets.push({
          ...
          Tags: [
            { Key: 'Name', Value: vd.name.toLowerCase() + '-' + tier + '-' + AZS[a].slice(-2) }
          ]
        });
          ...
          rts.push({
            ...
            Routes: [
              { DestinationCidrBlock: vd.cidr, GatewayId: 'local' },
              {
                ...
                NatGatewayId: natIds[Math.min(a, natIds.length - 1)]
              },
            ...
            Tags: [
              {
                Key: 'Name',
                Value: vd.name.toLowerCase() + '-' + tier + '-' + AZS[a].slice(-2) + '-rt'
              }
            ]
          });
        } else {
        ...
        if (
          isPub &&
          a < natIds.length &&
          natsList[natsList.length - vd.azsUsed + a + Math.min(a, natIds.length - 1)]
        ) {
          natsList.forEach((n) => {
            if (n.NatGatewayId === natIds[a] && !n.SubnetId) {
              n.SubnetId = subId;
            }
          });
        }
        ...
        for (let ii = 0; ii < instCount; ii++) {
          ...
          const instName = isBastionSlot
            ? vd.name.toLowerCase() + '-bastion'
            : vd.name.toLowerCase() +
              '-' +
              tier.replace('private-', '') +
              '-' +
          ...
          const _ec2Entry = {
            ...
            PrivateIpAddress:
              '10.' +
              Math.floor(_random() * 255) +
              '.' +
              Math.floor(_random() * 255) +
              '.' +
              Math.floor(_random() * 254 + 1),
            Placement: { AvailabilityZone: AZS[a] },
            State: {
              Name: isBastionSlot ? 'running' : st,
              Code: isBastionSlot ? 16 : st === 'running' ? 16 : 80
            },
            Tags: [{ Key: 'Name', Value: instName }]
          };
          if (vi === 0) {
            _ec2Entry.IamInstanceProfile = {
              Arn: 'arn:aws:iam::111222333444:instance-profile/EC2InstanceRole',
              Id: 'AIPA000000001'
            };
          }
    ...
    for (let lb = 0; lb < vd.albCount; lb++) {
      const pubSubs = subnets.filter(
        (s) => s.VpcId === vpcId && (s.Tags[0]?.Value || '').includes('public')
      );
      const prvSubs = subnets.filter(
        (s) => s.VpcId === vpcId && !(s.Tags[0]?.Value || '').includes('public')
      );
      ...
      albsList.push({
        LoadBalancerArn:
          'arn:aws:elasticloadbalancing:us-east-1:111222333444:loadbalancer/' +
          lbType +
          '/' +
          vd.name.toLowerCase() +
          '-' +
        ...
        Scheme: scheme,
        VpcId: vpcId,
        AvailabilityZones: lbSubs
          .slice(0, vd.azsUsed)
          .map((s) => ({ SubnetId: s.SubnetId, ZoneName: s.AvailabilityZone })),
        State: { Code: 'active' },
    ...
    albsList
      .filter((a) => a.VpcId === vpcId)
      .forEach((alb, li) => {
        const tgInsts = vpcInstances.slice(0, 3 + li);
        const tgType = alb.Type === 'application' ? 'instance' : 'ip';
        tgsList.push({
          TargetGroupArn:
            'arn:aws:elasticloadbalancing:us-east-1:111222333444:targetgroup/' +
            vd.name.toLowerCase() +
            '-tg-' +
            li +
            '/abc' +
  ...
  const vpnConns = [
    {
      VpnConnectionId: nid('vpn'),
      State: 'available',
      VpnGatewayId: 'vgw-onprem01',
      CustomerGatewayId: 'cgw-dc01',
      Tags: [{ Key: 'Name', Value: 'datacenter-east-primary' }]
    },
    {
      VpnConnectionId: nid('vpn'),
      State: 'available',
      VpnGatewayId: 'vgw-onprem01',
      CustomerGatewayId: 'cgw-dc02',
      Tags: [{ Key: 'Name', Value: 'datacenter-east-backup' }]
    },
    {
      VpnConnectionId: nid('vpn'),
      State: 'available',
      VpnGatewayId: 'vgw-onprem02',
      CustomerGatewayId: 'cgw-dc03',
      Tags: [{ Key: 'Name', Value: 'datacenter-west-primary' }]
    },
    {
      VpnConnectionId: nid('vpn'),
      State: 'available',
      VpnGatewayId: 'vgw-onprem02',
      CustomerGatewayId: 'cgw-dc04',
      Tags: [{ Key: 'Name', Value: 'datacenter-west-backup' }]
    }
  ];
  ...
  [
    ...
    s3Buckets.push({
      Name: n + '-' + Math.floor(_random() * 99999),
      CreationDate: '2024-' + String(Math.floor(_random() * 12) + 1).padStart(2, '0') + '-15'
    });
  });
  ...
  const zones = [
    {
      ...
      ResourceRecordSetCount: 187
    },
    {
      Id: '/hostedzone/Z002',
      Name: 'internal.example.com.',
      Config: { PrivateZone: true },
      ResourceRecordSetCount: 342,
      VPCs: [
        { VPCId: 'vpc-production' },
        { VPCId: 'vpc-dataplatform' },
        { VPCId: 'vpc-management' }
      ]
    },
    {
      Id: '/hostedzone/Z003',
      ...
      Config: { PrivateZone: false },
      ResourceRecordSetCount: 64
    },
    {
      Id: '/hostedzone/Z004',
      Name: 'api.example.com.',
      Config: { PrivateZone: false },
      ResourceRecordSetCount: 96
    },
    {
      Id: '/hostedzone/Z005',
      Name: 'dev.example.com.',
      Config: { PrivateZone: false },
      ResourceRecordSetCount: 78
    },
    {
      Id: '/hostedzone/Z006',
      Name: 'data.internal.example.com.',
      Config: { PrivateZone: true },
      ResourceRecordSetCount: 124,
      VPCs: [{ VPCId: 'vpc-dataplatform' }]
    },
    {
      Id: '/hostedzone/Z007',
      Name: 'pci.example.com.',
      Config: { PrivateZone: true },
      ResourceRecordSetCount: 45,
      VPCs: [{ VPCId: 'vpc-pcicompliant' }, { VPCId: 'vpc-security' }]
    },
    {
      Id: '/hostedzone/Z008',
      Name: 'dr.example.com.',
      Config: { PrivateZone: false },
      ResourceRecordSetCount: 52
    }
  ];
  ...
  zones.forEach((z) => {
    ...
    r53records.push({
      ...
      Type: 'NS',
      TTL: 172800,
      ResourceRecords: [{ Value: 'ns-1.awsdns-01.org.' }, { Value: 'ns-2.awsdns-02.co.uk.' }]
    });
    r53records.push({
      HostedZoneId: zid,
      Name: base,
    ...
    r53records.push({
      ...
      Type: 'A',
      AliasTarget: {
        DNSName: 'dualstack.elb-prod-123456.us-east-1.elb.amazonaws.com.',
    ...
    r53records.push({
      ...
      Type: 'TXT',
      TTL: 300,
      ResourceRecords: [{ Value: '"v=DMARC1; p=quarantine; rua=mailto:dmarc@' + base + '"' }]
    });
  });
  ...
  if (internetFacingAlbs.length > 0) {
    wafAcls.push({
      ...
      Rules: [
        ...
        { Name: 'RateLimit-1000', Priority: 3 }
      ],
  ...
  rdsConfigs.forEach((rc, ri) => {
    ...
    rdsInstances.push({
      ...
      Endpoint: {
        Address: rc.name + '.cluster-abc.us-east-1.rds.amazonaws.com',
        Port: rc.engine.includes('postgres') ? 5432 : 3306
      },
      DBSubnetGroup: {
        VpcId: vpcId,
        DBSubnetGroupName: rc.name + '-subnet-group',
        Subnets: prvSubs.slice(0, 3).map((s) => ({
          SubnetIdentifier: s.SubnetId,
          SubnetAvailabilityZone: { Name: s.AvailabilityZone }
        }))
      },
      VpcSecurityGroups: sgs
        .filter((sg) => sg.VpcId === vpcId)
        .slice(0, 1)
        .map((sg) => ({ VpcSecurityGroupId: sg.GroupId, Status: 'active' })),
  ...
  ecConfigs.forEach((ec) => {
    ...
    ecacheClusters.push({
      ...
      SecurityGroups: sgs
        .filter((sg) => sg.VpcId === vpcId)
        .slice(0, 1)
        .map((sg) => ({ SecurityGroupId: sg.GroupId, Status: 'active' }))
    });
  });
  ...
  if (vpcs[3]) {
    ...
    redshiftClusters.push({
      ...
      DBName: 'analytics',
      Endpoint: {
        Address: 'data-analytics-cluster.abc.us-east-1.redshift.amazonaws.com',
        Port: 5439
      },
      VpcId: vpcId,
      ...
      VpcSecurityGroups: sgs
        ...
        .map((sg) => ({ VpcSecurityGroupId: sg.GroupId, Status: 'active' })),
      Encrypted: true,
      ClusterNodes: Array.from({ length: 4 }, (_, i) => ({
        NodeRole: i === 0 ? 'LEADER' : 'COMPUTE'
      }))
    });
  }
  ...
  vpcs.forEach((vpc) => {
    const hasRoute = rts.some(
      (rt) => rt.VpcId === vpc.VpcId && (rt.Routes || []).some((r) => r.TransitGatewayId)
    );
    if (hasRoute) {
      tgwAttachments.push({
  ...
  cfAlbs.forEach((alb, ci) => {
    cfDistributions.push({
      ...
      ViewerCertificate: {
        ACMCertificateArn: 'arn:aws:acm:us-east-1:111222333444:certificate/abc-' + ci
      },
      Aliases: {
        Items: ci === 0 ? ['www.example.com', 'api.example.com'] : ['staging.example.com']
      },
  ...
  const iamRoles = [
          ...
          PolicyDocument: {
            ...
            Statement: [
              ...
              },
              {
                ...
                Resource: 'arn:aws:logs:*:111222333444:*'
              }
            ]
          }
        }
          ...
          PolicyDocument: {
            ...
            Statement: [
              ...
              },
              {
                ...
                Resource: 'arn:aws:dynamodb:us-east-1:111222333444:table/prod-*'
              }
            ]
          }
        }
          ...
          PolicyDocument: {
            ...
            Statement: [
              ...
              },
              {
                ...
                Resource: ['arn:aws:s3:::prod-assets', 'arn:aws:s3:::prod-assets/*']
              },
              {
                Effect: 'Allow',
                Action: ['sqs:SendMessage', 'sqs:ReceiveMessage', 'sqs:DeleteMessage'],
                Resource: 'arn:aws:sqs:us-east-1:111222333444:prod-*'
              }
            ]
          }
        }
    ...
    },
    {
      ...
      PermissionsBoundary: {
        ...
        PermissionsBoundaryArn: 'arn:aws:iam::111222333444:policy/DeployBoundary'
      },
      RolePolicyList: [
        {
          PolicyName: 'DeployInline',
          PolicyDocument: {
            Version: '2012-10-17',
            Statement: [
              {
                Effect: 'Allow',
                Action: ['codedeploy:*', 's3:GetObject', 's3:PutObject', 's3:ListBucket'],
                Resource: '*'
              },
              {
                Effect: 'Allow',
    ...
    },
    {
      ...
      AssumeRolePolicyDocument: {
        ...
        Statement: [
          {
            Effect: 'Allow',
            Principal: { AWS: 'arn:aws:iam::555666777888:root' },
            Action: 'sts:AssumeRole'
          }
        ]
      },
