...
export function buildDependencyGraph(ctx) {
  ...
  (ctx.vpcs || []).forEach((v) => {
    (ctx.subnets || [])
      .filter((s) => s.VpcId === v.VpcId)
      .forEach((s) => addEdge(v.VpcId, s.SubnetId, 'contains', 'hard'));
    (ctx.igws || []).forEach((ig) => {
      if ((ig.Attachments || []).some((a) => a.VpcId === v.VpcId)) {
        addEdge(v.VpcId, ig.InternetGatewayId, 'attached', 'hard');
      }
    });
    (ctx.nats || [])
      .filter((n) => n.VpcId === v.VpcId)
      .forEach((n) => addEdge(v.VpcId, n.NatGatewayId, 'contains', 'hard'));
    (ctx.vpces || [])
      .filter((e) => e.VpcId === v.VpcId)
      .forEach((e) => addEdge(v.VpcId, e.VpcEndpointId, 'contains', 'soft'));
    (ctx.rts || [])
      .filter((rt) => rt.VpcId === v.VpcId)
      .forEach((rt) => addEdge(v.VpcId, rt.RouteTableId, 'contains', 'config'));
    (ctx.nacls || [])
      .filter((n) => n.VpcId === v.VpcId)
      .forEach((n) => addEdge(v.VpcId, n.NetworkAclId, 'contains', 'config'));
    (ctx.sgs || [])
      .filter((sg) => sg.VpcId === v.VpcId)
      .forEach((sg) => addEdge(v.VpcId, sg.GroupId, 'contains', 'config'));
  });

  // Subnet -> resources
  (ctx.subnets || []).forEach((sub) => {
    ((ctx.instBySub || {})[sub.SubnetId] || []).forEach((i) =>
      addEdge(sub.SubnetId, i.InstanceId, 'contains', 'hard')
    );
    ((ctx.rdsBySub || {})[sub.SubnetId] || []).forEach((r) =>
      addEdge(sub.SubnetId, r.DBInstanceIdentifier, 'contains', 'hard')
    );
    ((ctx.ecsBySub || {})[sub.SubnetId] || []).forEach((e) =>
      addEdge(sub.SubnetId, e.serviceName, 'contains', 'hard')
    );
    ((ctx.lambdaBySub || {})[sub.SubnetId] || []).forEach((l) =>
      addEdge(sub.SubnetId, l.FunctionName, 'contains', 'hard')
    );
    ((ctx.albBySub || {})[sub.SubnetId] || []).forEach((a) =>
      addEdge(sub.SubnetId, a.LoadBalancerName, 'contains', 'hard')
    );
    const rt = (ctx.subRT || {})[sub.SubnetId];
    if (rt) {
      addEdge(sub.SubnetId, rt.RouteTableId, 'associated', 'config');
    }
    const nacl = (ctx.subNacl || {})[sub.SubnetId];
    if (nacl) {
      addEdge(sub.SubnetId, nacl.NetworkAclId, 'associated', 'config');
    }
  });
  ...
  (ctx.instances || []).forEach((inst) => {
    (inst.SecurityGroups || []).forEach((sg) =>
      addEdge(inst.InstanceId, sg.GroupId, 'secured_by', 'soft')
    );
    (inst.BlockDeviceMappings || []).forEach((b) => {
      if (b.Ebs && b.Ebs.VolumeId) {
        addEdge(inst.InstanceId, b.Ebs.VolumeId, 'attached', 'hard');
      }
    });
  });
  ...
  (ctx.rdsInstances || []).forEach((db) => {
    (db.VpcSecurityGroups || []).forEach((sg) =>
      addEdge(db.DBInstanceIdentifier, sg.VpcSecurityGroupId, 'secured_by', 'soft')
    );
  });

  // ALB -> SGs + target groups
  (ctx.albs || []).forEach((alb) => {
    (alb.SecurityGroups || []).forEach((gid) =>
      addEdge(alb.LoadBalancerName, gid, 'secured_by', 'soft')
    );
    ...
    [...(sg.IpPermissions || []), ...(sg.IpPermissionsEgress || [])].forEach((p) => {
      (p.UserIdGroupPairs || []).forEach((pair) => {
        if (pair.GroupId && pair.GroupId !== sg.GroupId) {
          addEdge(sg.GroupId, pair.GroupId, 'references', 'config');
        }
      });
    });
  });
  ...
  (ctx.rts || []).forEach((rt) => {
    (rt.Routes || []).forEach((r) => {
      if (r.GatewayId && r.GatewayId !== 'local') {
        addEdge(rt.RouteTableId, r.GatewayId, 'routes_through', 'config');
      }
      if (r.NatGatewayId) {
        addEdge(rt.RouteTableId, r.NatGatewayId, 'routes_through', 'config');
      }
      if (r.TransitGatewayId) {
        addEdge(rt.RouteTableId, r.TransitGatewayId, 'routes_through', 'config');
      }
      if (r.VpcPeeringConnectionId) {
        addEdge(rt.RouteTableId, r.VpcPeeringConnectionId, 'routes_through', 'config');
      }
    });
  });
  ...
  (ctx.peerings || []).forEach((p) => {
    if (p.RequesterVpcInfo) {
      addEdge(p.VpcPeeringConnectionId, p.RequesterVpcInfo.VpcId, 'connects', 'soft');
    }
    if (p.AccepterVpcInfo) {
      addEdge(p.VpcPeeringConnectionId, p.AccepterVpcInfo.VpcId, 'connects', 'soft');
    }
  });
...
export function getResType(id) {
  ...
  const ctx = rlCtx;
  if (ctx) {
    if ((ctx.rdsInstances || []).some((r) => r.DBInstanceIdentifier === id)) {
      return 'RDS';
    }
    if ((ctx.lambdaFns || []).some((f) => f.FunctionName === id)) {
      return 'Lambda';
    }
    if ((ctx.ecsServices || []).some((e) => e.serviceName === id)) {
      return 'ECS';
    }
    if ((ctx.albs || []).some((a) => a.LoadBalancerName === id)) {
      return 'ALB';
    }
    if ((ctx.ecacheClusters || []).some((c) => c.CacheClusterId === id)) {
      return 'ElastiCache';
    }
    if ((ctx.redshiftClusters || []).some((c) => c.ClusterIdentifier === id)) {
      return 'Redshift';
    }
  }
  return 'Resource';
...
export function clearBlastRadius() {
  ...
  mg.selectAll('.blast-dimmed,.blast-glow-hard,.blast-glow-soft,.blast-glow-config')
    ...
    .classed('blast-glow-hard', false)
    .classed('blast-glow-soft', false)
    .classed('blast-glow-config', false);
}
