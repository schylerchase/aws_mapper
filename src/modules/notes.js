...
try {
  ...
  if (s) _annotations = JSON.parse(s);
} catch (e) {
...
function _populateResourceSelect() {
  ...
  if (!sel || !_rlCtx) return;
  let opts = '<option value="">-- Select resource --</option>';
  (_rlCtx.vpcs || []).forEach(
    (v) => (opts += '<option value="' + v.VpcId + '">VPC: ' + gn(v, v.VpcId) + '</option>')
  );
  (_rlCtx.subnets || []).forEach(
    (s) => (opts += '<option value="' + s.SubnetId + '">Subnet: ' + gn(s, s.SubnetId) + '</option>')
  );
  (_rlCtx.instances || []).forEach(
    (i) =>
      (opts += '<option value="' + i.InstanceId + '">EC2: ' + gn(i, i.InstanceId) + '</option>')
  );
  (_rlCtx.rdsInstances || []).forEach(
    (d) =>
      (opts +=
        '<option value="' +
        esc(d.DBInstanceIdentifier) +
        '">RDS: ' +
        esc(d.DBInstanceIdentifier) +
        '</option>')
  );
  (_rlCtx.lambdaFns || []).forEach(
    (f) =>
      (opts +=
        '<option value="' + esc(f.FunctionName) + '">Lambda: ' + esc(f.FunctionName) + '</option>')
  );
  (_rlCtx.sgs || []).forEach(
    (s) =>
      (opts +=
        '<option value="' + esc(s.GroupId) + '">SG: ' + esc(s.GroupName || s.GroupId) + '</option>')
  );
  sel.innerHTML = opts;
}
function _showEditNote(rid, ni) {
  const notes = _annotations[rid];
  if (!notes || !notes[ni]) return;
  const n = notes[ni];
  const card = document.querySelector('.note-card[data-rid="' + rid + '"][data-ni="' + ni + '"]');
  if (!card) return;
  card.innerHTML =
...
function _renderComplianceBadges() {
  ...
  Object.entries(lookup).forEach(([rid, data]) => {
    ...
    const badge = nodesLayer
      ...
      .attr('transform', 'translate(' + (bb.x + 8) + ',' + (bb.y + 4) + ')')
      .style('cursor', 'pointer');
...
function openNoteFormForResource(resourceId) {
  ...
  setTimeout(() => {
    ...
    if (sel) {
      ...
      if (!sel.value) _populateResourceSelect();
      sel.value = resourceId;
    }
...
document.getElementById('timelineBtn').addEventListener('click', () => {
  ...
  if (tb.classList.contains('open')) closeTimeline();
  else openTimeline();
});
