...
window.generateDemoSnapshots = function () {
  ...
  const fieldMap = {
    ...
    in_ecs: 'ecs',
    in_lambda: 'lambda',
    in_ecache: 'elasticache',
    in_redshift: 'redshift'
  };
  const fullTA = {};
  Object.entries(fieldMap).forEach(([taId, dKey]) => {
    if (d[dKey]) {
      fullTA[taId] = JSON.stringify(d[dKey], null, 2);
    }
  });
  ...
  scales.forEach((scale, i) => {
    ...
    if (scale >= 0.5) {
      ta.in_subnets = fullTA.in_subnets;
    }
    if (scale >= 0.7) {
      ta.in_sgs = fullTA.in_sgs;
      ta.in_rts = fullTA.in_rts;
    }
    if (scale >= 0.85) {
      ta.in_inst = fullTA.in_inst;
      ta.in_igws = fullTA.in_igws;
    }
    if (scale >= 1.0) {
      Object.assign(ta, fullTA);
    }
...
window._demoAnnotations = (function () {
  ...
  return {
    [vpc0]: [
      {
        text: 'Production VPC - primary workloads. Contact: platform-team@example.com',
        category: 'owner',
        author: 'admin',
      ...
      {
        text: 'Incident INC-4521: Elevated latency observed 2026-01-20. Root cause: misconfigured NAT gateway.',
        category: 'incident',
        author: 'oncall',
    ...
    [sub0]: [
      {
        text: 'TODO: Migrate to larger CIDR range before Q2 scaling',
        category: 'todo',
        author: 'architect',
        created: '2026-01-18T11:00:00Z',
        updated: '2026-01-18T11:00:00Z',
        pinned: false
      }
    ],
    [inst0]: [
      {
        text: 'Bastion host - SSH key rotation due 2026-03-01',
        category: 'warning',
        author: 'security',
      ...
      {
        text: 'Running custom AMI with hardened OS config',
        category: 'info',
        author: 'ops',
    ...
    [sg0]: [
      {
        text: 'Reviewed 2026-01-25 - rules compliant with CIS benchmarks',
        category: 'status',
        author: 'auditor',
...
window._edgeCaseTests.multiAccount = function () {
  ...
  T('Same VPC ID different accounts', () => {
    const v1 = {
      ...
      Tags: [{ Key: 'Name', Value: 'Acct1' }]
    };
    const v2 = {
      VpcId: 'vpc-shared01',
      CidrBlock: '10.1.0.0/16',
      OwnerId: '222222222222',
      Tags: [{ Key: 'Name', Value: 'Acct2' }]
    };
  ...
  T('Cross-account peering unknown VPC', () => {
    const peering = {
      VpcPeeringConnectionId: 'pcx-test01',
      Status: { Code: 'active' },
    ...
    return {
      pass: missingRef && reqAcct === '111111111111' && accAcct === '999999999999',
  ...
  T('TGW shared across accounts', () => {
    ...
    return {
      pass: acctIds.size === 2 && attachments.every((a) => a.TransitGatewayId === tgwId),
  ...
  T('RAM-shared subnets cross-account', () => {
    ...
    const crossAccount =
      subnet._accountId !== instance._accountId && subnet.SubnetId === instance.SubnetId;
    return {
      pass: crossAccount,
      detail:
        'Instance in acct ' + instance._accountId + ' uses subnet from acct ' + subnet._accountId
    };
  });
  ...
  T('Different regions AZ handling', () => {
    ...
    return {
      pass: r1 === 'us-east-1' && r2 === 'eu-west-1' && r1 !== r2,
  ...
  T('Cross-account SG references', () => {
    ...
    return {
      pass: crossAcct,
      detail: 'SG ' + sg1.GroupId + ' refs SG ' + sgRef.GroupId + ' in account ' + sgRef.UserId
    };
  });
  ...
  T('Compliance findings per account', () => {
    ...
    const byAcct = {};
    findings.forEach((f) => {
      if (!byAcct[f._accountId]) {
        byAcct[f._accountId] = [];
      }
      byAcct[f._accountId].push(f);
    });
    return {
      pass: Object.keys(byAcct).length === 2 && byAcct['111111111111'].length === 2,
      detail: Object.entries(byAcct)
        .map(([a, f]) => a + ':' + f.length)
        .join(', ')
    };
  });
  ...
  T('Save/load multi-account project', () => {
    ...
    return {
      pass:
        parsed._version === '2.0' && parsed.accounts.length === 2 && parsed.multiViewMode === true,
      detail:
        'v' +
        parsed._version +
        ', ' +
  ...
  T('Return to single account', () => {
    ...
    const simContexts = [
      { accountId: 'a1', visible: true },
      { accountId: 'a2', visible: true }
    ];
    simContexts.splice(0, simContexts.length);
    const singleMode = simContexts.length <= 1;
    return {
      pass: singleMode && simContexts.length === 0,
...
window._edgeCaseTests.snapshots = function () {
  ...
  T('localStorage quota handling', () => {
    ...
    _snapshots = Array.from({ length: 10 }, (_, i) => ({
      id: 'snap-q-' + i,
  ...
  T('Checksum dedup consecutive snapshots', () => {
    ...
    return {
      pass: cs1 === cs2 && dupeCheck,
      detail: 'Checksums match: ' + cs1 + '===' + cs2 + ', dedup active'
    };
  });
  ...
  T('Restore during design mode warning', () => {
    ...
    return {
      pass: typeof _restoreSnapshot === 'function',
      detail: '_restoreSnapshot exists, design mode check is caller responsibility'
    };
  });
  ...
  T('Clear all snapshots', () => {
    ...
    let stored = null;
    try {
      stored = localStorage.getItem(_SNAP_KEY);
    } catch (e) {}
    return {
      pass: _snapshots.length === 0 && (stored === null || stored === undefined),
  ...
  T('Multi-account snapshot data', () => {
    const ta = {
      in_vpcs:
        '{"Vpcs":[{"VpcId":"vpc-1","OwnerId":"111111111111"},{"VpcId":"vpc-2","OwnerId":"222222222222"}]}'
    };
  ...
  T('50+ snapshots handling', () => {
    ...
    while (_snapshots.length > _MAX_SNAPSHOTS) {
      _snapshots.shift();
    }
    return {
      pass: _snapshots.length === _MAX_SNAPSHOTS,
  ...
  T('Corrupted snapshot graceful handling', () => {
    ...
    return {
      pass: valid.length === 2,
      detail: 'Filtered ' + (snaps.length - valid.length) + ' corrupted, kept ' + valid.length
    };
  });
  ...
  T('Timezone ISO 8601 handling', () => {
    ...
    return {
      pass: roundtrip === ts && !isNaN(d.getTime()) && localStr.length > 0,
  ...
  T('Snapshot captures textarea values', () => {
    ...
    return {
      pass: Object.keys(snap.textareas).length === 2 && snap.textareas.in_vpcs.includes('vpc-edit'),
...
window._edgeCaseTests.notes = function () {
  ...
  T('Orphaned notes detection', () => {
    ...
    return {
      pass: isOrph === true || !_rlCtx,
      detail: 'Orphaned: ' + isOrph + ' (rlCtx exists: ' + !!_rlCtx + ')'
    };
  });
  ...
  T('Very long note text (500+ chars)', () => {
    ...
    return {
      pass: textLen === 600 && note && note.text.length === 600,
  ...
  T('Notes survive design mode context', () => {
    ...
    return {
      pass: JSON.stringify(before) === JSON.stringify(after) && Object.keys(after).length === 1,
  ...
  T('Multiple notes on same resource', () => {
    ...
    for (let i = 0; i < 5; i++) {
      addAnnotation(rid, 'Note ' + i, _NOTE_CATEGORIES[i % _NOTE_CATEGORIES.length], i === 0);
    }
    const notes = _annotations[rid];
    const all = _getAllNotes().filter((n) => n.resourceId === rid);
    return {
      pass: notes.length === 5 && all.length === 5 && notes[0].pinned === true,
      detail:
        notes.length + ' notes, first pinned: ' + notes[0].pinned + ', getAllNotes: ' + all.length
    };
  });

  // 5. XSS in note text
  T('XSS sanitization in notes', () => {
    const xss = '<scr' + 'ipt>alert("xss")<\/scr' + 'ipt><img onerror="alert(1)" src=x>'; // readability-audit-ignore intentional XSS fixture
    const escaped = _escHtml(xss);
    const hasScript = escaped.includes('<script>');
    const hasTag = escaped.includes('<img');
    return {
      pass: !hasScript && !hasTag && escaped.includes('&lt;script&gt;'),
  ...
  T('Multi-account note key collision', () => {
    ...
    return {
      pass: key1 !== key2 && key3 === 'vpc-001' && key1 === '111111111111:vpc-001',
  ...
  T('Annotations save/load round-trip', () => {
    ...
    return {
      pass: match && Object.keys(loaded.annotations).length === 2,
      detail:
        'Round-trip match: ' + match + ', resources: ' + Object.keys(loaded.annotations).length
    };
  });
  ...
  T('Notes in search results', () => {
    ...
    return {
      pass: found.length === 1 && found[0].resourceId === 'vpc-searchable',
  ...
  T('CRUD annotation lifecycle', () => {
    ...
    const u1 =
      updated.text === 'Updated text' && updated.category === 'warning' && updated.pinned === true;
    // Delete
  ...
  T('Bulk annotation multiple resources', () => {
    ...
    rids.forEach((rid) =>
      addAnnotation(rid, 'Bulk maintenance window 2026-03-01', 'status', false)
    );
    const allNotes = _getAllNotes();
    const bulkNotes = allNotes.filter((n) => n.text.includes('Bulk maintenance'));
    return {
      pass: bulkNotes.length === 5 && Object.keys(_annotations).length === 5,
      detail:
        bulkNotes.length + ' bulk notes across ' + Object.keys(_annotations).length + ' resources'
    };
  });
...
window._edgeCaseTests.iacExport = function () {
  ...
  T('AWS IDs to resource references', () => {
    ...
    return {
      pass: hasResourceRef && !hasLiteralVpcId,
  ...
  T('Circular SG reference splitting', () => {
    const sg1 = {
      GroupId: 'sg-circ1',
      GroupName: 'circ1',
      VpcId: ctx.vpcs[0].VpcId,
      IpPermissions: [
        {
          IpProtocol: 'tcp',
          FromPort: 443,
          ToPort: 443,
          UserIdGroupPairs: [{ GroupId: 'sg-circ2' }]
        }
      ],
      IpPermissionsEgress: [],
      Tags: [{ Key: 'Name', Value: 'circ1' }]
    };
    const sg2 = {
      GroupId: 'sg-circ2',
      GroupName: 'circ2',
      VpcId: ctx.vpcs[0].VpcId,
      IpPermissions: [
        { IpProtocol: 'tcp', FromPort: 80, ToPort: 80, UserIdGroupPairs: [{ GroupId: 'sg-circ1' }] }
      ],
      IpPermissionsEgress: [],
      Tags: [{ Key: 'Name', Value: 'circ2' }]
    };
    const cycles = window._core.detectCircularSGs([sg1, sg2]);
    const ctxCopy = Object.assign({}, ctx, { sgs: [sg1, sg2] });
    const tf = window._core.generateTerraform(ctxCopy, {
      mode: 'create',
      scopeVpcId: ctx.vpcs[0].VpcId
    });
    const code = typeof tf === 'string' ? tf : tf.code || tf;
    const hasSgRule = code.includes('aws_security_group_rule');
    return {
      pass: cycles.length > 0 && hasSgRule,
  ...
  T('Dependency ordering subnet->VPC', () => {
    ...
    return {
      pass: vpcPos < subPos && subRefVpc,
      detail:
        'VPC at pos ' + vpcPos + ', Subnet at pos ' + subPos + ', subnet refs VPC: ' + subRefVpc
    };
  });
  ...
  T('CloudFormation 500-resource limit warning', () => {
    ...
    return {
      pass: typeof cfn === 'object' && Array.isArray(warnings),
      detail:
        'Resources: ' +
        (stats.resources || '?') +
        ', warnings: ' +
        warnings.length +
        (warnAt450 ? ' (limit warning present)' : '')
    };
  });
  ...
  T('Encrypted resources KMS handling', () => {
    ...
    return {
      pass: encryptedRds.length > 0 && hasEncrypted,
  ...
  T('Design mode create-only export', () => {
    ...
    const code = typeof tf === 'string' ? tf : tf.code || tf;
    const hasDesignVpc =
      code.includes('DesignVPC') || code.includes('design_vpc') || code.includes('designvpc');
    return {
      pass: hasDesignVpc && code.includes('aws_vpc'),
      detail: 'Design VPC in output: ' + hasDesignVpc
    };
  });
  ...
  T('Default VPC/SG export handling', () => {
    ...
    return {
      pass: code.includes('aws_vpc') && hasDefaultSg,
...
function _buildTestCtx(demoData) {
  ...
  rts.forEach((rt) => {
    const hasIgw = (rt.Routes || []).some(
      (r) => r.GatewayId && r.GatewayId.startsWith('igw-') && r.State !== 'blackhole'
    );
    (rt.Associations || []).forEach((a) => {
      if (a.SubnetId && hasIgw) {
        pubSubs.add(a.SubnetId);
      }
    });
  });
  const subNacl = {};
  ...
  instances.forEach((i) => {
    if (i.SubnetId) {
      (instBySub[i.SubnetId] = instBySub[i.SubnetId] || []).push(i);
    }
  });
  const albBySub = {};
  albs.forEach((lb) => {
    (lb.AvailabilityZones || []).forEach((az) => {
      if (az.SubnetId) {
        (albBySub[az.SubnetId] = albBySub[az.SubnetId] || []).push(lb);
      }
    });
  });
  const rdsBySub = {};
  rdsInstances.forEach((db) => {
    ...
    (sg.Subnets || []).forEach((s) => {
      if (s.SubnetIdentifier) {
        (rdsBySub[s.SubnetIdentifier] = rdsBySub[s.SubnetIdentifier] || []).push(db);
      }
    });
  });
  const ecsBySub = {};
  ecsServices.forEach((svc) => {
    ...
    (nc.subnets || []).forEach((sid) => {
      (ecsBySub[sid] = ecsBySub[sid] || []).push(svc);
    });
  });
  const lambdaBySub = {};
  lambdaFns.forEach((fn) => {
    (fn.VpcConfig?.SubnetIds || []).forEach((sid) => {
      (lambdaBySub[sid] = lambdaBySub[sid] || []).push(fn);
    });
  });
  const sgByVpc = {};
...
function _demoToDiffObj(demoData) {
  ...
  return {
    ...
    vpces: ctx.vpces,
    albs: ctx.albs,
    rdsInstances: ctx.rdsInstances,
    ecsServices: ctx.ecsServices,
    lambdaFns: ctx.lambdaFns,
    ecacheClusters: ctx.ecacheClusters,
    redshiftClusters: ctx.redshiftClusters,
...
window.generateDemoBaseline = function () {
  ...
  const bVpcs = b.vpcs.Vpcs;
  const bSubs = b.subnets.Subnets;
  const bInsts = b.ec2.Reservations[0].Instances;
  const bSgs = b.sgs.SecurityGroups;
  const bNats = b.nats.NatGateways;
  const bPeerings = b.peer.VpcPeeringConnections;
  // 1. Remove DR-Recovery and Sandbox VPCs + their resources
  const removeIds = new Set();
  ['DR-Recovery', 'Sandbox'].forEach((name) => {
    const idx = bVpcs.findIndex((v) =>
      (v.Tags || []).some((t) => t.Key === 'Name' && t.Value === name)
    );
    if (idx >= 0) {
      removeIds.add(bVpcs[idx].VpcId);
      bVpcs.splice(idx, 1);
    }
  });
  b.subnets.Subnets = bSubs.filter((s) => !removeIds.has(s.VpcId));
  b.ec2.Reservations[0].Instances = bInsts.filter((i) => {
    const sub = bSubs.find((s) => s.SubnetId === i.SubnetId);
    return !sub || !removeIds.has(sub.VpcId);
  });
  ...
  b.igws.InternetGateways = b.igws.InternetGateways.filter(
    (ig) => !(ig.Attachments || []).some((a) => removeIds.has(a.VpcId))
  );
  // 2. Add 3 extra instances in Production VPC
  const prodVpc = bVpcs.find((v) =>
    (v.Tags || []).some((t) => t.Key === 'Name' && t.Value === 'Production')
  );
  if (prodVpc) {
    const prodSubs = b.subnets.Subnets.filter((s) => s.VpcId === prodVpc.VpcId).slice(0, 3);
    for (let x = 0; x < 3; x++) {
      b.ec2.Reservations[0].Instances.push({
        InstanceId: 'i-baseline-extra-' + x,
        SubnetId: prodSubs[x % prodSubs.length].SubnetId,
        InstanceType: 't3.micro',
        PrivateIpAddress: '10.0.99.' + x,
        State: { Name: 'running', Code: 16 },
        Tags: [{ Key: 'Name', Value: 'baseline-extra-' + x }]
      });
    }
  }
  ...
  const ssVpc = bVpcs.find((v) =>
    (v.Tags || []).some((t) => t.Key === 'Name' && t.Value === 'Shared-Services')
  );
  if (ssVpc) {
    const idx = b.nats.NatGateways.findIndex((n) => n.VpcId === ssVpc.VpcId);
    if (idx >= 0) {
      b.nats.NatGateways.splice(idx, 1);
    }
  }
  // 5. Change instance types on 5 instances
  b.ec2.Reservations[0].Instances.filter((i) => i.InstanceType === 't3.micro')
    .slice(0, 5)
    .forEach((i) => {
      i.InstanceType = 't3.small';
    });
  // 6. Add an extra peering
  bPeerings.push({
    VpcPeeringConnectionId: 'pcx-baseline-extra',
    Status: { Code: 'active' },
    RequesterVpcInfo: { VpcId: 'vpc-production', CidrBlock: '10.0.0.0/16' },
    AccepterVpcInfo: { VpcId: 'vpc-development', CidrBlock: '10.2.0.0/16' },
    Tags: [{ Key: 'Name', Value: 'baseline-extra-peering' }]
  });
...
const _demoFlowScenarios = [
  {
    name: 'Same-subnet instance to instance',
...
window._edgeCaseTests.diff = function () {
  ...
  T('Removed VPCs detected', () => {
    ...
    bl.vpcs.push({
      VpcId: 'vpc-removed-test',
      CidrBlock: '10.99.0.0/16',
      Tags: [{ Key: 'Name', Value: 'Removed-VPC' }]
    });
    bl.subnets.push({
      SubnetId: 'subnet-rem-1',
      VpcId: 'vpc-removed-test',
      CidrBlock: '10.99.0.0/24',
      Tags: [{ Key: 'Name', Value: 'rem-sub' }]
    });
    const d = computeDiff(bl, current);
    return {
      pass:
        d.removed.some((r) => r.key === 'vpc-removed-test') &&
        d.removed.some((r) => r.key === 'subnet-rem-1'),
      detail: 'removed=' + d.total.removed
    };
  });
  ...
  T('New VPCs detected as added', () => {
    ...
    if (idx >= 0) {
      bl.vpcs.splice(idx, 1);
    }
    const d = computeDiff(bl, current);
    return {
      pass: d.added.some((r) => r.key === 'vpc-sandbox'),
      detail: 'added vpc-sandbox=' + d.added.some((r) => r.key === 'vpc-sandbox')
    };
  });
  ...
  T('Reordered SG rules: no false positive', () => {
    ...
    if (sg) {
      sg.IpPermissions = [...sg.IpPermissions].reverse();
      if (sg.IpPermissions[0]?.IpRanges?.length > 1) {
        sg.IpPermissions[0].IpRanges = [...sg.IpPermissions[0].IpRanges].reverse();
      }
    }
    const d = computeDiff(bl, current);
    return {
      pass: d.modified.filter((r) => r.type === 'sgs').length === 0,
      detail: 'sg mods from reorder=' + d.modified.filter((r) => r.type === 'sgs').length
    };
  });
  ...
  T('InstanceType change is structural', () => {
    ...
    if (inst) {
      inst.InstanceType = 't3.xlarge';
    }
    const d = computeDiff(bl, current);
    const mod = d.modified.find((r) => r.type === 'instances' && r.key === inst?.InstanceId);
    return {
      pass: !!mod && mod.hasStructural,
      detail: 'modified=' + !!mod + ', structural=' + !!(mod && mod.hasStructural)
    };
  });
  ...
  T('Subnet CIDR change is structural', () => {
    ...
    return {
      pass: !!mod && mod.fields.some((f) => f.field.includes('CidrBlock')),
      detail: 'cidr field found=' + !!(mod && mod.fields.some((f) => f.field.includes('CidrBlock')))
    };
  });
  ...
  T('Instance subnet move detected', () => {
    const bl = structuredClone(current);
    const inst = bl.instances[0];
    const orig = inst.SubnetId;
    const other = bl.subnets.find((s) => s.SubnetId !== orig);
    if (other) {
      inst.SubnetId = other.SubnetId;
    }
    const d = computeDiff(bl, current);
    const mod = d.modified.find((r) => r.type === 'instances' && r.key === inst.InstanceId);
    return {
      pass: !!mod && mod.fields.some((f) => f.field.includes('SubnetId')),
      detail: 'SubnetId field=' + !!(mod && mod.fields.some((f) => f.field.includes('SubnetId')))
    };
  });
  ...
  T('Empty baseline: all added', () => {
    ...
    return {
      pass: d.total.added > 0 && d.total.removed === 0 && d.total.modified === 0,
      detail: 'added=' + d.total.added
    };
  });
  ...
  T('Identical snapshots: zero changes', () => {
    ...
    return {
      pass: d.total.added === 0 && d.total.removed === 0 && d.total.modified === 0,
      detail: 'unchanged=' + d.total.unchanged
    };
  });
  ...
  T('Multi-account new resources are added', () => {
    ...
    mc.vpcs.push({
      VpcId: 'vpc-acct2-prod',
      CidrBlock: '172.16.0.0/16',
      Tags: [{ Key: 'Name', Value: 'Acct2' }]
    });
    mc.instances.push({
      InstanceId: 'i-acct2-001',
      SubnetId: 'subnet-acct2-1',
      InstanceType: 'm5.large',
      State: { Name: 'running' },
      Tags: [{ Key: 'Name', Value: 'acct2-web' }]
    });
    const d = computeDiff(bl, mc);
    return {
      pass:
        d.added.some((r) => r.key === 'vpc-acct2-prod') &&
        d.added.some((r) => r.key === 'i-acct2-001'),
      detail:
        'vpc=' +
        d.added.some((r) => r.key === 'vpc-acct2-prod') +
        ', inst=' +
  ...
  T('Design mode does not corrupt diff', () => {
    const saved = window._designChanges || [];
    const savedM = window._designMode || false;
    window._designChanges = [
      { type: 'add_vpc', params: { name: 'DesignVPC', cidr: '10.250.0.0/16' } }
    ];
    window._designMode = true;
    const d = computeDiff(current, current);
    window._designChanges = saved;
    window._designMode = savedM;
    return {
      pass: d.total.added === 0 && d.total.removed === 0 && d.total.modified === 0,
      detail: 'changes=' + d.total.modified
    };
  });
...
window._edgeCaseTests.flow = function () {
  ...
  function mkCtx(opts) {
    ...
    const igw = 'igw-ft-1',
      nat = 'nat-ft-1';
    const rt1 = {
      RouteTableId: 'rtb-ft-pub',
      VpcId: v1,
      Routes: [
        { DestinationCidrBlock: '10.0.0.0/16', GatewayId: 'local' },
        { DestinationCidrBlock: '0.0.0.0/0', GatewayId: igw }
      ],
      Associations: [{ SubnetId: s1 }]
    };
    const rt2 = {
      RouteTableId: 'rtb-ft-priv',
      VpcId: v1,
      Routes: [
        { DestinationCidrBlock: '10.0.0.0/16', GatewayId: 'local' },
        { DestinationCidrBlock: '0.0.0.0/0', NatGatewayId: nat }
      ],
      Associations: [{ SubnetId: s2 }]
    };
    const rt3 = {
      RouteTableId: 'rtb-ft-v2',
      VpcId: v2,
      Routes: [{ DestinationCidrBlock: '10.1.0.0/16', GatewayId: 'local' }],
      Associations: [{ SubnetId: s3 }]
    };
    if (opts.tgw) {
      rt2.Routes.push({ DestinationCidrBlock: '10.1.0.0/16', TransitGatewayId: 'tgw-ft-1' });
      rt3.Routes.push({ DestinationCidrBlock: '10.0.0.0/16', TransitGatewayId: 'tgw-ft-1' });
    }
    if (opts.pcx) {
      rt2.Routes.push({ DestinationCidrBlock: '10.1.0.0/16', VpcPeeringConnectionId: 'pcx-ft-1' });
      rt3.Routes.push({ DestinationCidrBlock: '10.0.0.0/16', VpcPeeringConnectionId: 'pcx-ft-1' });
    }
    if (opts.vpce) {
      rt2.Routes.push({ DestinationCidrBlock: '0.0.0.0/0', VpcEndpointId: 'vpce-ft-gw' });
    }
    ...
    const nacl2E = opts.naclBlock
      ...
      : naclAllow.slice();
    const nacl3E = opts.naclRetBlock
      ? [
          {
            RuleNumber: 100,
            Protocol: '6',
      ...
      : naclAllow.slice();
    const nacls = [
      { NetworkAclId: 'acl-ft-1', VpcId: v1, Associations: [{ SubnetId: s1 }], Entries: naclAllow },
      { NetworkAclId: 'acl-ft-2', VpcId: v1, Associations: [{ SubnetId: s2 }], Entries: nacl2E },
      { NetworkAclId: 'acl-ft-3', VpcId: v2, Associations: [{ SubnetId: s3 }], Entries: nacl3E }
    ];
    const i1 = {
      InstanceId: 'i-ft-web',
      SubnetId: s1,
      ...
      SecurityGroups: [{ GroupId: 'sg-ft-web', GroupName: 'ft-web' }],
      State: { Name: 'running' },
      Tags: [{ Key: 'Name', Value: 'ft-web' }]
    };
    const i2 = {
      InstanceId: 'i-ft-app',
      SubnetId: s2,
      ...
      SecurityGroups: [{ GroupId: 'sg-ft-db', GroupName: 'ft-db' }],
      State: { Name: 'running' },
      Tags: [{ Key: 'Name', Value: 'ft-app' }]
    };
    const i3 = {
      InstanceId: 'i-ft-v2',
      SubnetId: s3,
      ...
      SecurityGroups: [{ GroupId: 'sg-ft-v2', GroupName: 'ft-v2' }],
      State: { Name: 'running' },
      Tags: [{ Key: 'Name', Value: 'ft-v2' }]
    };
    ...
    const rds = {
      DBInstanceIdentifier: 'ft-rds',
      DBInstanceClass: 'db.t3.micro',
      Engine: 'mysql',
      Endpoint: { Address: 'ft-rds.rds.amazonaws.com', Port: 3306 },
      DBSubnetGroup: { VpcId: v1, Subnets: [{ SubnetIdentifier: s2 }] },
      VpcSecurityGroups: [{ VpcSecurityGroupId: 'sg-ft-db' }]
    };
    const alb = {
      LoadBalancerArn: 'arn:aws:elb:ft:alb/ft-alb',
      LoadBalancerName: 'ft-alb',
      Type: 'application',
      Scheme: 'internet-facing',
      VpcId: v1,
      AvailabilityZones: [{ SubnetId: s1, ZoneName: 'us-east-1a' }],
      SecurityGroups: ['sg-ft-web']
    };
    const lam = {
      FunctionName: 'ft-lambda',
      Runtime: 'nodejs20.x',
      VpcConfig: { VpcId: v1, SubnetIds: [s2], SecurityGroupIds: ['sg-ft-db'] },
      State: 'Active'
    };
    const peerings = opts.pcx
      ? [
          {
            VpcPeeringConnectionId: 'pcx-ft-1',
            Status: { Code: 'active' },
            RequesterVpcInfo: { VpcId: v1, CidrBlock: '10.0.0.0/16' },
            AccepterVpcInfo: { VpcId: v2, CidrBlock: '10.1.0.0/16' }
          }
        ]
      : [];
    const tgwAtt = opts.tgw
      ? [
          {
            TransitGatewayAttachmentId: 'tgw-att-ft-1',
            TransitGatewayId: 'tgw-ft-1',
            ResourceId: v1
          },
          {
            TransitGatewayAttachmentId: 'tgw-att-ft-2',
            TransitGatewayId: 'tgw-ft-1',
            ResourceId: v2
          }
        ]
    ...
    return {
      vpcs: [
        { VpcId: v1, CidrBlock: '10.0.0.0/16' },
        { VpcId: v2, CidrBlock: '10.1.0.0/16' }
      ],
      subnets,
      sgs,
      rts,
      nacls,
      igws: [{ InternetGatewayId: igw, Attachments: [{ VpcId: v1 }] }],
      nats: [{ NatGatewayId: nat, VpcId: v1, SubnetId: s1 }],
      vpces: [],
      instances,
      albs: [alb],
      rdsInstances: [rds],
      ...
      subRT,
      subNacl,
      sgByVpc: { [v1]: sgs.filter((s) => s.VpcId === v1), [v2]: sgs.filter((s) => s.VpcId === v2) },
      tgByAlb: {},
      eniBySub: {},
      pubSubs: new Set([s1])
    };
  ...
  T('Cross-VPC TGW hop present', () => {
    ...
    const r = traceFlow(
      { type: 'instance', id: 'i-ft-app' },
      { type: 'instance', id: 'i-ft-v2' },
      { protocol: 'tcp', port: 443 },
      ctx
    );
    return {
      pass: r.path.some((h) => h.type === 'tgw') && !r.blocked,
      detail: 'tgw=' + r.path.some((h) => h.type === 'tgw') + ', hops=' + r.path.length
    };
  });
  ...
  T('VPC peering hop traced', () => {
    ...
    return {
      pass: r.path.some((h) => h.type === 'peering') && !r.blocked,
      detail: 'peering=' + r.path.some((h) => h.type === 'peering')
    };
  });
  ...
  T('VPCE gateway route in RT', () => {
    ...
    return {
      pass: !!rt && rt.Routes.some((r) => r.VpcEndpointId),
      detail: 'vpce route=' + !!(rt && rt.Routes.some((r) => r.VpcEndpointId))
    };
  });
  ...
  T('NACL blocks before SG', () => {
    ...
    return {
      pass:
        !!r.blocked && r.path.some((h) => h.type && h.type.includes('nacl') && h.action === 'deny'),
      detail: 'blocked=' + !!r.blocked + ', reason=' + (r.blocked ? r.blocked.reason : '')
    };
  });
  ...
  T('Stateless NACL outbound deny configured', () => {
    ...
    return {
      pass:
        !!nacl3 &&
        nacl3.Entries.some((e) => e.Egress && e.RuleAction === 'deny' && e.RuleNumber < 32767),
      detail:
        'outbound deny rule=' +
        !!(
          nacl3 &&
  ...
  T('ALB flow resolves to target', () => {
    const ctx = mkCtx({});
    const r = traceFlow(
      { type: 'subnet', id: 'subnet-ft-pub' },
      { type: 'alb', id: 'ft-alb' },
      { protocol: 'tcp', port: 443 },
      ctx
    );
    return {
      pass: r.path.some((h) => h.type === 'target'),
      detail: 'target=' + r.path.some((h) => h.type === 'target') + ', blocked=' + !!r.blocked
    };
  });
  ...
  T('Lambda resolves to VPC subnet', () => {
    const ctx = mkCtx({});
    const r = traceFlow(
      { type: 'lambda', id: 'ft-lambda' },
      { type: 'instance', id: 'i-ft-app' },
      { protocol: 'tcp', port: 3306 },
      ctx
    );
    return {
      pass: r.path[0] && r.path[0].subnetId === 'subnet-ft-priv',
      detail: 'srcSub=' + r.path[0]?.subnetId
    };
  });
  ...
  T('Internet to ALB flow traced', () => {
    ...
    return {
      pass: r.path.length >= 2,
      detail: 'hops=' + r.path.length + ', blocked=' + !!r.blocked
    };
  });
  ...
  T('No cross-VPC route is blocked', () => {
    ...
    return {
      pass: !!r.blocked && r.path.some((h) => h.type === 'cross-vpc' && h.action === 'block'),
      detail: 'blocked=' + !!r.blocked
    };
  });
...
window._edgeCaseTests.dep = function () {
  ...
  function mkCtx(opts) {
    ...
    const rtSh = {
      RouteTableId: 'rtb-dt-sh',
      VpcId: v1,
      Routes: [
        { DestinationCidrBlock: '10.0.0.0/16', GatewayId: 'local' },
        { DestinationCidrBlock: '0.0.0.0/0', NatGatewayId: natId }
      ],
      Associations: [{ SubnetId: s1 }, { SubnetId: s2 }, { SubnetId: s3 }]
    };
    const rtIso = {
      RouteTableId: 'rtb-dt-iso',
      VpcId: v2,
      Routes: [{ DestinationCidrBlock: '10.1.0.0/16', GatewayId: 'local' }],
      Associations: [{ SubnetId: s4 }]
    };
    if (opts.tgw) {
      rtSh.Routes.push({ DestinationCidrBlock: '10.1.0.0/16', TransitGatewayId: 'tgw-dt-1' });
    }
    const naclDef = {
      NetworkAclId: 'acl-dt-def',
      VpcId: v1,
      Associations: [{ SubnetId: s1 }, { SubnetId: s2 }, { SubnetId: s3 }],
      Entries: []
    };
    const naclIso = {
      NetworkAclId: 'acl-dt-iso',
      VpcId: v2,
      Associations: [{ SubnetId: s4 }],
      Entries: []
    };
    const i1 = {
      ...
      SubnetId: s1,
      VpcId: v1,
      SecurityGroups: [{ GroupId: sgA }],
      BlockDeviceMappings: [{ Ebs: { VolumeId: 'vol-dt-1' } }],
      Tags: [{ Key: 'Name', Value: 'dt-1' }]
    };
    const i2 = {
      InstanceId: 'i-dt-2',
      SubnetId: s2,
      VpcId: v1,
      SecurityGroups: [{ GroupId: sgB }],
      BlockDeviceMappings: [],
      Tags: [{ Key: 'Name', Value: 'dt-2' }]
    };
    const i3 = {
      InstanceId: 'i-dt-3',
      SubnetId: s3,
    ...
    const rds = {
      DBInstanceIdentifier: 'dt-rds',
      VpcSecurityGroups: [{ VpcSecurityGroupId: sgB }]
    };
    const alb = {
      LoadBalancerArn: 'arn:aws:elb:dt:alb-1',
      LoadBalancerName: 'dt-alb',
      SecurityGroups: [sgA],
      VpcId: v1
    };
    const lam = {
      FunctionName: 'dt-lambda',
      VpcConfig: { VpcId: v1, SubnetIds: [s1, s2], SecurityGroupIds: [sgA] }
    };
    const peerings = opts.pcx
      ? [
          {
            VpcPeeringConnectionId: 'pcx-dt-1',
            RequesterVpcInfo: { VpcId: v1 },
            AccepterVpcInfo: { VpcId: v2 }
          }
        ]
      : [];
    const tgwAtt = opts.tgw
      ? [
          {
            TransitGatewayAttachmentId: 'tgw-att-dt-1',
            TransitGatewayId: 'tgw-dt-1',
            ResourceId: v1
          },
          {
            TransitGatewayAttachmentId: 'tgw-att-dt-2',
            TransitGatewayId: 'tgw-dt-1',
            ResourceId: v2
          }
        ]
    ...
    return {
      vpcs: [
        { VpcId: v1, CidrBlock: '10.0.0.0/16', Tags: [{ Key: 'Name', Value: 'DtVPC1' }] },
        { VpcId: v2, CidrBlock: '10.1.0.0/16', Tags: [{ Key: 'Name', Value: 'DtVPC2' }] }
      ],
      subnets: [
        { SubnetId: s1, VpcId: v1, CidrBlock: '10.0.0.0/24' },
        { SubnetId: s2, VpcId: v1, CidrBlock: '10.0.1.0/24' },
        { SubnetId: s3, VpcId: v1, CidrBlock: '10.0.2.0/24' },
        { SubnetId: s4, VpcId: v2, CidrBlock: '10.1.0.0/24' }
      ],
      sgs,
      rts: [rtSh, rtIso],
      nacls: [naclDef, naclIso],
      igws: [{ InternetGatewayId: 'igw-dt-1', Attachments: [{ VpcId: v1 }] }],
      nats: [{ NatGatewayId: natId, VpcId: v1, SubnetId: s1 }],
      vpces: [],
      instances,
      albs: [alb],
      rdsInstances: [rds],
  ...
  T('Peering connects both VPCs', () => {
    ...
    return {
      pass: edges.some((e) => e.id === 'vpc-dt-1') && edges.some((e) => e.id === 'vpc-dt-2'),
      detail:
        'v1=' +
        edges.some((e) => e.id === 'vpc-dt-1') +
        ', v2=' +
        edges.some((e) => e.id === 'vpc-dt-2')
    };
  });
  ...
  T('Isolated subnet: no instance deps', () => {
    ...
    return {
      pass: !br.all.some((e) => e.id.startsWith('i-')),
      detail:
        'blast=' +
        br.all.length +
        ', instances=' +
        br.all.filter((e) => e.id.startsWith('i-')).length
    };
  });
  ...
  T('Fan-out RT associations correct', () => {
    ...
    return {
      pass: s1rt.length === 1 && s2rt.length === 1 && s3rt.length === 1,
      detail: 's1=' + s1rt.length + ', s2=' + s2rt.length + ', s3=' + s3rt.length
    };
  });
  ...
  T('NACL associated to subnet', () => {
    const ctx = mkCtx();
    const g = buildDependencyGraph(ctx);
    const assoc = (g['subnet-dt-1'] || []).filter(
      (e) => e.id === 'acl-dt-def' && e.rel === 'associated'
    );
  ...
  T('Lambda in multiple subnets', () => {
    ...
    return {
      pass: s1l.length === 1 && s2l.length === 1,
      detail: 's1->lam=' + s1l.length + ', s2->lam=' + s2l.length
    };
  });
  ...
  T('Design mode: graph still works', () => {
    ...
    const saved = window._designChanges || [];
    window._designChanges = [
      { type: 'add_vpc', params: { name: 'X', cidr: '10.250.0.0/16', VpcId: 'vpc-virt' } }
    ];
    const g = buildDependencyGraph(ctx);
    const br = getBlastRadius('vpc-dt-1', g, 5);
    window._designChanges = saved;
  ...
  T('Blast radius depth limiting', () => {
    const ctx = mkCtx();
    const g = buildDependencyGraph(ctx);
    const br1 = getBlastRadius('vpc-dt-1', g, 1);
    const br5 = getBlastRadius('vpc-dt-1', g, 5);
    const maxD1 = br1.all.length > 0 ? Math.max(...br1.all.map((e) => e.depth)) : 0;
    return {
      pass: br1.all.length <= br5.all.length && maxD1 <= 1,
      detail: 'd1=' + br1.all.length + ' (max=' + maxD1 + '), d5=' + br5.all.length
    };
  });
...
window._edgeCaseTests.firewall = function () {
  ...
  function mkCtx() {
    ...
    const naclAllow = {
      NetworkAclId: 'acl-fw-1',
      VpcId: v1,
      Associations: [{ SubnetId: s1 }, { SubnetId: s2 }],
      Entries: [
        {
          RuleNumber: 100,
          Protocol: '6',
    ...
    return {
      vpcs: [{ VpcId: v1, CidrBlock: '10.0.0.0/16' }],
      subnets,
      sgs: [sg1],
      rts: [rt1],
      nacls: [naclAllow],
  ...
  T('Modify SG inbound rule', () => {
    ...
    try {
      ...
      _fwResetAll();
      return {
        pass: hasRevoke && hasAuth,
        detail: 'revoke=' + hasRevoke + ', authorize=' + hasAuth
      };
    } finally {
      restoreState(saved);
    }
  ...
  T('Add route to route table', () => {
    ...
    try {
      ...
      const hasRoute = ctx.rts[0].Routes.some(
        (r) => r.DestinationCidrBlock === '0.0.0.0/0' && r.GatewayId === 'igw-fw-test'
      );
      const cli = _fwGenerateCli(_fwEdits).join('\n');
  ...
  T('Shadowed NACL rule warning', () => {
    ...
    const hasShadow =
      warnings.length > 0 && warnings.some((w) => w.toLowerCase().includes('shadowed'));
    return {
      pass: hasShadow,
      detail: 'warnings=' + warnings.length + ', msg=' + (warnings[0] || 'none')
    };
  });
  ...
  T('Duplicate NACL rule number rejected', () => {
    ...
    const errs = _fwValidateNaclRule(
      {
        RuleNumber: 100,
        Protocol: '6',
        CidrBlock: '10.0.0.0/24',
    ...
    return {
      ...
      detail:
        ...
        errs.length +
        ', msg=' +
        (errs.find((e) => e.toLowerCase().includes('duplicate')) || 'none')
    };
  });
  ...
  T('Undo restores original rule', () => {
    ...
    try {
      ...
      _fwResetAll();
      return {
        pass: afterDel === origLen - 1 && afterUndo === origLen,
        detail: 'orig=' + origLen + ', afterDel=' + afterDel + ', afterUndo=' + afterUndo
      };
    } finally {
      restoreState(saved);
    }
  ...
  T('Reset All restores snapshot', () => {
    ...
    try {
      ...
      return {
        pass: afterDel < origLen && afterReset === origLen,
        detail: 'orig=' + origLen + ', afterDel=' + afterDel + ', afterReset=' + afterReset
      };
    } finally {
      restoreState(saved);
    }
  ...
  T('SG eval reflects edit: deny then allow', () => {
    ...
    try {
      ...
      return {
        pass: denied && allowed,
        detail: 'before=' + before.action + ', after=' + after.action
      };
    } finally {
      restoreState(saved);
    }
...
window._runEdgeCaseTests = function (feature) {
  ...
  if (!tests[feature]) {
    console.error(
      '[EdgeTests] Unknown feature: ' + feature + '. Available: ' + Object.keys(tests).join(', ')
    );
    return null;
  }
  ...
  results.forEach((r) => {
    console.log(
      '%c' + (r.pass ? 'PASS' : 'FAIL') + ' %c' + r.name + ' %c' + r.detail,
