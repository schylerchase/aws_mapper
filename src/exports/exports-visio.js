...
export function exportVsdx(showToast) {
  ...
  enis.forEach((e) => {
    if (e.SubnetId && e.Attachment && e.Attachment.InstanceId) {
      instSubFromEni2[e.Attachment.InstanceId] = e.SubnetId;
    }
  });
  const volBySub = {};
  volumes.forEach((v) => {
    ...
    if (att && att.InstanceId) {
      if (knownInstIds2.has(att.InstanceId)) {
        return;
      }
      const sid = instSubFromEni2[att.InstanceId];
      if (sid) {
  ...
  rts.forEach((rt) => {
    (rt.Routes || []).forEach((r) => {
      ...
      if (r.GatewayId && r.GatewayId !== 'local') {
        entries.push({ id: r.GatewayId, type: clsGw(r.GatewayId) });
      }
      if (r.NatGatewayId) {
        entries.push({ id: r.NatGatewayId, type: 'NAT' });
      }
      if (r.TransitGatewayId) {
        entries.push({ id: r.TransitGatewayId, type: 'TGW' });
      }
      if (r.VpcPeeringConnectionId) {
        entries.push({ id: r.VpcPeeringConnectionId, type: 'PCX' });
      }
      entries.forEach((e) => {
        if (e.type === 'TGW' || e.type === 'PCX') {
          if (!sharedGwMap.has(e.id)) {
            sharedGwMap.set(e.id, { ...e, vpcIds: new Set() });
          }
          sharedGwMap.get(e.id).vpcIds.add(rt.VpcId);
        } else {
          if (!gwByVpc[rt.VpcId]) {
            gwByVpc[rt.VpcId] = new Map();
          }
          if (!gwByVpc[rt.VpcId].has(e.id)) {
            gwByVpc[rt.VpcId].set(e.id, e);
          }
        }
      });
    });
  ...
  function buildSubText(s) {
    ...
    const parts = [];
    if (si.length) {
      parts.push(si.length + ' EC2');
    }
    if (se.length) {
      parts.push(se.length + ' ENI');
    }
    if (sa.length) {
      parts.push(sa.length + ' ALB');
    }
    if (parts.length) {
    ...
    if (rt) {
      ...
      if (nonLocal.length) {
        ...
        nonLocal.forEach((r) => {
          ...
          const tgt =
            r.GatewayId || r.NatGatewayId || r.TransitGatewayId || r.VpcPeeringConnectionId;
  ...
  vpcLayouts.forEach((vl) => {
    ...
    if (vSgs.length) {
      ...
      vpcProps.push({
        ...
        val: vSgs
          .slice(0, 10)
          .map((sg) => sg.GroupName + ' (' + (sg.IpPermissions || []).length + ' in)')
          .join('; ')
      });
    }
    ...
    ss.forEach((s) => {
      ...
      if (nc) {
        sp.push({ label: 'NACL', val: nc.NetworkAclId });
      }
  ...
  if (sharedGwMap.size > 0) {
    ...
    sharedArr.forEach((gw, i) => {
      ...
      const gid = addRect(gwX, TGW_Y, 200, 60, st.fill, st.border, 2.5, gw.type + ': ' + truncNm, {
        fontSize: 12,
        fontColor: st.color,
        bold: true,
        hAlign: 'center'
