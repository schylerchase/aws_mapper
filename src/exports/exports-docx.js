...
function _docxTableRow(cells, isHeader) {
  ...
  cells.forEach(function (c) {
    h += '<w:tc><w:tcPr>';
    if (isHeader) {
      h += '<w:shd w:val="clear" w:fill="1B2A4A"/>';
    }
    h += '</w:tcPr><w:p><w:r>';
    if (isHeader) {
      h += '<w:rPr><w:b/><w:color w:val="FFFFFF"/><w:sz w:val="18"/></w:rPr>';
    } else {
      h += '<w:rPr><w:sz w:val="18"/></w:rPr>';
    }
...
function _docxBuildBody(enabled, isExec, title, author, date) {
  ...
  var sub = [];
  if (author) {
    sub.push('Prepared by ' + author);
  }
  sub.push(date);
  sub.push(isExec ? 'Executive Summary' : 'Technical Assessment');
  h += _docxP(sub.join('  |  '), 'Subtitle');
  h +=
    '<w:p><w:pPr><w:pBdr><w:bottom w:val="single" w:sz="12" w:color="1B2A4A"/></w:pBdr></w:pPr></w:p>';
  // TOC
  ...
  enabled.forEach(function (id) {
    if (id === 'exec-summary') {
      h += _docxExecSummary(isExec);
    } else if (id === 'compliance') {
      h += _docxCompliance(isExec);
    } else if (id === 'budr') {
      h += _docxBUDR(isExec);
    } else if (id === 'iam-review') {
      h += _docxIAMReview(isExec);
    } else if (id === 'inventory') {
      h += _docxInventory(isExec);
    } else if (id === 'action-plan') {
      h += _docxActionPlan(isExec);
    } else if (id === 'app-summary') {
      h += _docxAppSummary(isExec);
    } else if (id === 'architecture') {
      h += _docxP('(Architecture diagram — see HTML report for interactive map)');
    } else if (id === 'iac-recs') {
      h += _docxIaCRecs(isExec);
    }
  });
  // Footer
  h += _docxP('');
...
function _docxExecSummary(isExec) {
  ...
  var c = _rlCtx;
  if (!c) {
    return h + _docxP('No infrastructure data loaded.');
  }
  var counts = [
    ['VPCs', (c.vpcs || []).length],
  ...
  if (isExec) {
    h += _docxP(
      'This report covers the AWS infrastructure assessment performed on ' +
        (_rptState.date || 'today') +
        '. The environment spans ' +
        (c._regions ? c._regions.size : 1) +
        ' region(s) with ' +
        (c.vpcs || []).length +
        ' VPC(s) hosting ' +
        ((c.instances || []).length + (c.rdsInstances || []).length + (c.lambdaFns || []).length) +
    ...
    if (isExec) {
      ...
      h += _docxP(
        total +
          ...
          critHigh +
          ' are Critical or High severity and require immediate attention.'
      );
    }
    h += _docxTable(
      ['Severity', 'Count'],
...
function _docxCompliance(isExec) {
  ...
  if (!f || !f.length) {
    return h + _docxP('No compliance findings.');
  }
  if (isExec) {
    ...
    if (critFindings.length) {
      ...
      if (critFindings.length > maxShow) {
        h += _docxP(
          '... and ' + (critFindings.length - maxShow) + ' more. See XLSX for full details.'
        );
      }
    }
  } else {
    ...
    if (sorted.length > maxDocx) {
      h += _docxP(
        'Showing ' +
          maxDocx +
          ' of ' +
          sorted.length +
          ' findings. Export XLSX for the complete dataset.'
      );
    }
  }
  return h;
...
function _docxBUDR(isExec) {
  ...
  if (!filteredBudr.length) {
    return h + _docxP('No BUDR assessments.');
  }
  ...
  if (isExec) {
    ...
    h += _docxP(
      filteredBudr.length +
        ' resources assessed for disaster recovery readiness. ' +
        atRisk +
        ' resource(s) are at risk with insufficient backup coverage.'
    );
    h += _docxTable(
      ['DR Tier', 'Count'],
      [
...
function _docxIAMReview(isExec) {
  ...
  if (!_iamReviewData.length) {
    return h + _docxP('No IAM data available.');
  }
  ...
  if (isExec) {
    h += _docxP(
      _iamReviewData.length +
        ' IAM entities reviewed. ' +
        admins.length +
        ' have administrative access. ' +
        crossAcct.length +
        ' have cross-account trust relationships.'
    );
    if (admins.length) {
      h += _docxP('');
...
function _docxInventory(isExec) {
  ...
  if (!_rlCtx) {
    return h + _docxP('No data loaded.');
  }
  ...
  if (isExec) {
    ...
    h += _docxP(
      'The environment contains ' +
        filteredInv.length +
        ' tracked resources across the following categories:'
    );
    var typeCounts = {};
...
function _docxActionPlan(isExec) {
  ...
  if (!all.length) {
    return h + _docxP('No action items.');
  }
  ...
  if (isExec) {
    ...
    _PRIORITY_KEYS.forEach(function (t) {
      ...
      if (count > 5) {
        h += _docxP('... and ' + (count - 5) + ' more items');
      }
    });
  } else {
    var maxPerTier = 200;
    _PRIORITY_KEYS.forEach(function (t) {
      ...
      h += _docxTable(
        ['Severity', 'Control', 'Resource', 'Finding', 'Remediation'],
        rows.slice(0, maxPerTier)
      );
      if (total > maxPerTier) {
        h += _docxP('Showing ' + maxPerTier + ' of ' + total + '. See XLSX for complete list.');
      }
    });
  }
  return h;
...
function _docxIaCRecs(isExec) {
  ...
  if (isExec) {
    return (
      h +
      _docxP(
        'Infrastructure as Code recommendations are available in the detailed technical report.'
      )
    );
  }
  h += _docxP(
    'Terraform and CloudFormation snippets for remediation are available in the HTML report.'
  );
  return h;
}
