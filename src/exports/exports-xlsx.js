...
export function loadSheetJS() {
  ...
  return _tryLoad('libs/xlsx.bundle.min.js').catch(function () {
    return Promise.reject(
      new Error(
        'SheetJS (libs/xlsx.bundle.min.js) not found. XLSX export requires this library in the app directory.'
      )
    );
  });
}
...
function _xlsxSevStyle(sev) {
  var fg = {
    CRITICAL: _XLSX_COLORS.critFg,
    HIGH: _XLSX_COLORS.highFg,
    MEDIUM: _XLSX_COLORS.medFg,
    LOW: _XLSX_COLORS.lowFg
  };
  var bg = {
    CRITICAL: _XLSX_COLORS.critBg,
    HIGH: _XLSX_COLORS.highBg,
    MEDIUM: _XLSX_COLORS.medBg,
    LOW: _XLSX_COLORS.lowBg
  };
  return {
...
function _xlsxCellStyle(isStripe) {
  ...
  if (isStripe) {
    s.fill = { fgColor: { rgb: _XLSX_COLORS.stripeBg } };
  }
  return s;
}
...
function _xlsxAutoWidth(ws, data, minWidths) {
  ...
  if (minWidths) {
    minWidths.forEach(function (mw, i) {
      if (mw && i < colWidths.length) {
        colWidths[i] = Math.max(colWidths[i], mw);
      }
    });
  }
  ...
  if (typeof opts.sevCol === 'number') {
    ...
    for (let r = 1; r < data.length; r++) {
      ...
      if (ws[addr]) {
        ...
        if (val) {
          if (!_sevCache[val]) {
            _sevCache[val] = _xlsxSevStyle(val);
          }
  ...
  if (typeof opts.tierCol === 'number') {
    ...
    for (let r = 1; r < data.length; r++) {
      ...
      if (ws[addr]) {
        ...
        if (val) {
          if (!_tierCache[val]) {
            _tierCache[val] = _xlsxTierStyle(val);
          }
  ...
  if (typeof opts.effortCol === 'number') {
    ...
    for (let r = 1; r < data.length; r++) {
      ...
      if (ws[addr]) {
        ...
        if (val) {
          if (!_effortCache[val]) {
            _effortCache[val] = _xlsxEffortStyle(val);
          }
  ...
  for (let r = 1; r < data.length; r++) {
    ...
    for (let c = 0; c < headers.length; c++) {
      ...
      if (ws[addr] && !ws[addr].s) {
        ws[addr].s = sty;
      } else if (ws[addr] && ws[addr].s && !ws[addr].s.border) {
        ws[addr].s.border = _border;
      }
    }
  }
...
function _xlsxSummarySection(ws, row, label, cols) {
  ...
  for (let c = 1; c < cols; c++) {
    ...
    ws[a2] = {
      v: '',
      t: 's',
      s: { fill: { fgColor: { rgb: _XLSX_COLORS.headerBg } }, border: xlsxBorder() }
    };
  }
  if (cols > 1) {
    ws['!merges'] = (ws['!merges'] || []).concat([
      { s: { r: row, c: 0 }, e: { r: row, c: cols - 1 } }
    ]);
  }
}
...
function _rptBuildXlsxSummary(wb, preFilteredFindings) {
  ...
  for (let cc = tCol + 1; cc < cols; cc++) {
    ws[XLSX.utils.encode_cell({ r: r, c: cc })] = {
      v: '',
      t: 's',
  ...
  var subParts = [];
  if (_rptState.author) {
    subParts.push('Prepared by ' + _rptState.author);
  }
  ...
  if (c) {
    ...
    for (let i = 0; i < counts.length; i += 2) {
      ...
      ws[vAddr] = {
        ...
        t: 'n',
        s: {
          font: { bold: true, sz: 11, color: { rgb: _XLSX_COLORS.valueFg }, name: 'Calibri' },
          border: xlsxBorder(),
          alignment: { horizontal: 'center', vertical: 'center' }
        }
      };
  ...
  var _sumFindings =
    preFilteredFindings || _rptFilterByAccount(_complianceFindings || [], _rptGetAccountFilter());
    ...
    fws.forEach(function (fw) {
      ...
      ws[XLSX.utils.encode_cell({ r: r, c: 1 })] = {
        ...
        s: {
          ...
          border: xlsxBorder(),
          fill: isStripe ? { fgColor: { rgb: _XLSX_COLORS.stripeBg } } : undefined
        }
      };
      ...
      ws[XLSX.utils.encode_cell({ r: r, c: 2 })] = {
        v: critCt,
        t: 'n',
        s: critCt > 0 ? _xlsxSevStyle('CRITICAL') : _xlsxCellStyle(isStripe)
      };
      ws[XLSX.utils.encode_cell({ r: r, c: 3 })] = {
        v: highCt,
        t: 'n',
        s: highCt > 0 ? _xlsxSevStyle('HIGH') : _xlsxCellStyle(isStripe)
      };
      r++;
    });
    r++;
  ...
  if (_sumBudr.length) {
    ...
    _sumBudr.forEach(function (a) {
      if (a.profile && tc[a.profile.tier] !== undefined) {
        tc[a.profile.tier]++;
      }
    });
    [
      ['Protected', tc.protected || 0, 'protected'],
      ['Partial', tc.partial || 0, 'partial'],
      ['At Risk', tc.at_risk || 0, 'at_risk']
    ].forEach(function (t) {
...
function _rptBuildXlsxCompliance(wb, preFiltered) {
  ...
  var sorted = f.slice().sort(function (a, b) {
    var ta = _classifyTier(a),
      tb = _classifyTier(b);
    if (ta !== tb) {
      return (_PRIORITY_ORDER[ta] || 9) - (_PRIORITY_ORDER[tb] || 9);
    }
...
function _rptBuildXlsxBUDR(wb) {
  ...
  var sorted = filteredBudr.slice().sort(function (a, b) {
    return (
      (tierOrder[a.profile ? a.profile.tier : ''] || 9) -
      (tierOrder[b.profile ? b.profile.tier : ''] || 9)
    );
  });
  var rows = sorted.map(function (a) {
    ...
    var sigs = a.signals
      ...
      : '';
    return [
      _rptAccountLabel(a._accountId || a.account) || '',
      a.region || '',
      a.vpcId || '',
      a.type,
      a.id,
  ...
  rows.forEach(function (row, ri) {
    ...
    if (url) {
      budrLinks.push({ r: ri, c: 4, url: url });
    }
  });
...
function _rptBuildXlsxInventory(wb) {
  ...
  (c.subnets || []).forEach(function (s) {
    if (s.SubnetId) {
      _invSubVpc[s.SubnetId] = s.VpcId || '';
    }
  });
  var _invInstVpc = {};
  (c.instances || []).forEach(function (i) {
    if (i.InstanceId) {
      _invInstVpc[i.InstanceId] = i.VpcId || _invSubVpc[i.SubnetId] || '';
    }
  });
  function _a(r) {
  ...
  _af(c.rdsInstances).forEach(function (d) {
    ...
    rows.push([
      ...
      d.DBInstanceIdentifier,
      d.Engine + ' / ' + d.DBInstanceClass,
      d.MultiAZ ? 'Multi-AZ' : 'Single-AZ',
      d.StorageType || ''
    ]);
  });
  _af(c.albs).forEach(function (a) {
    rows.push([
      ...
      a.Type || 'application',
      a.Scheme || '',
      (a.AvailabilityZones || []).length + ' AZs'
    ]);
  });
  ...
  _af(c.volumes).forEach(function (v) {
    ...
    rows.push([
      ...
      v.VolumeId,
      _rptTagName(v),
      v.VolumeType + ' / ' + v.Size + 'GB',
      v.State || '',
      att ? att.Device || '' : 'unattached'
    ]);
  });
  ...
  _af(c.ecacheClusters).forEach(function (ec) {
    rows.push([
      ...
      ec.CacheClusterId,
      ec.CacheClusterId,
      ec.Engine + ' / ' + ec.CacheNodeType,
      ec.CacheClusterStatus || '',
      ec.NumCacheNodes + ' nodes'
    ]);
  });
  _af(c.redshiftClusters).forEach(function (rs) {
    rows.push([
      ...
      rs.VpcId || '',
      'Redshift',
      rs.ClusterIdentifier,
      rs.ClusterIdentifier,
      rs.NodeType,
      rs.ClusterStatus || '',
      rs.NumberOfNodes + ' nodes'
    ]);
  });
  ...
  (c.zones || []).forEach(function (z) {
    var recs =
      c.recsByZone && c.recsByZone[z.Id]
        ? c.recsByZone[z.Id].length
        : z.ResourceRecordSetCount || 0;
  ...
  (c.vpns || []).forEach(function (v) {
    rows.push([
      ...
      'VPN',
      v.VpnConnectionId,
      _rptTagName(v) || v.VpnConnectionId,
      v.Type || '',
      v.State || '',
      ''
    ]);
  ...
  (c.tgwAttachments || []).forEach(function (t) {
    rows.push([
      ...
      t.VpcId || '',
      'TGW Attachment',
      t.TransitGatewayAttachmentId || t.TransitGatewayId || '',
      _rptTagName(t) || '',
      t.ResourceType || '',
      t.State || '',
      ''
  ...
  rows.forEach(function (row, ri) {
    ...
    if (idUrl) {
      links.push({ r: ri, c: 4, url: idUrl });
    }
    if (vpc && vpc !== '-') {
      var vUrl = _awsConsoleUrl('VPC', vpc, region);
      if (vUrl) {
        links.push({ r: ri, c: 2, url: vUrl });
      }
    }
  });
  _xlsxAddSheet(wb, 'Inventory', headers, rows, {
    minWidths: [16, 14, 16, 16, 24, 22, 22, 18, 14],
    links: links
  });
}
...
function _rptBuildXlsxActionPlan(wb, preFiltered) {
  var all =
    preFiltered ||
    _rptFilterByAccount(
      (_complianceFindings || []).concat(_budrFindings || []),
...
async function _xlsxInjectLogo(zip) {
  ...
  try {
    ...
    if (s1.indexOf('<drawing') === -1) {
      s1 = s1.replace(
        '</worksheet>',
        '<drawing r:id="rId99" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"/></worksheet>'
      );
...
async function _xlsxPostProcess(wbBuf, sheetNames) {
  ...
  var result = await zip.generateAsync({
    type: 'blob',
...
function _rptBuildXlsxAppSummary(wb) {
  if (!_classificationData.length && _rlCtx) {
    runClassificationEngine(_rlCtx);
  }
  if (typeof _autoDiscoverApps === 'function') {
    _autoDiscoverApps();
  }
  if (!_appRegistry.length) {
    return;
  }
  ...
  _appRegistry.forEach(function (app) {
    ...
    matched.forEach(function (r) {
      if ((tierPri[r.tier] || 99) < (tierPri[bestTier] || 99)) {
        bestTier = r.tier;
      }
    });
  ...
  _xlsxAddSheet(wb, 'App Summary', sumHeaders, sumRows, {
    tierCol: 2,
    minWidths: [25, 15, 12, 10, 10, 10]
  });
  ...
  detailRows.forEach(function (row, ri) {
    ...
    if (url) {
      detLinks.push({ r: ri, c: 5, url: url });
    }
  });
  _xlsxAddSheet(wb, 'App Resources', detHeaders, detailRows, {
    tierCol: 3,
    minWidths: [25, 30, 15, 10, 20, 25],
    links: detLinks
  });
}
...
function _rptBuildXlsxIAMReview(wb) {
  ...
  _xlsxAddSheet(wb, 'IAM Review', headers, rows, {
    minWidths: [24, 10, 10, 8, 10, 10, 10, 28, 14, 14]
  });
...
function _rptBuildXlsxClassification(wb) {
  ...
  _xlsxAddSheet(wb, 'Classification', headers, rows, {
    tierCol: 3,
    minWidths: [28, 22, 16, 12, 10, 10, 20, 14]
  });
}
...
function _rptBuildXlsxFirewall(wb) {
  ...
  if (sgs.length) {
    ...
    _xlsxAddSheet(wb, 'Security Groups', sgHeaders, sgRows, {
      minWidths: [16, 14, 22, 22, 22, 14, 14, 16]
    });
  ...
  if (nacls.length) {
    ...
    _xlsxAddSheet(wb, 'NACLs', naclHeaders, naclRows, {
      minWidths: [16, 14, 22, 22, 22, 10, 10, 30]
    });
  }
  // Route Tables
  var rts = _af(c.rts);
  if (rts.length) {
    ...
    _xlsxAddSheet(wb, 'Route Tables', rtHeaders, rtRows, {
      minWidths: [16, 14, 22, 22, 22, 10, 30]
    });
  }
}
...
export async function generateXlsx() {
  ...
  try {
    ...
    var wb = XLSX.utils.book_new();
    _rptBuildXlsxSummary(wb);
    if (enabled.indexOf('classification') >= 0) {
      _rptBuildXlsxClassification(wb);
    }
    if (enabled.indexOf('compliance') >= 0) {
      _rptBuildXlsxCompliance(wb);
    }
    if (enabled.indexOf('budr') >= 0) {
      _rptBuildXlsxBUDR(wb);
    }
    if (enabled.indexOf('iam-review') >= 0) {
      _rptBuildXlsxIAMReview(wb);
    }
    if (enabled.indexOf('firewall') >= 0) {
      _rptBuildXlsxFirewall(wb);
    }
    if (enabled.indexOf('inventory') >= 0) {
      _rptBuildXlsxInventory(wb);
    }
    if (enabled.indexOf('action-plan') >= 0) {
      _rptBuildXlsxActionPlan(wb);
    }
    if (enabled.indexOf('app-summary') >= 0) {
      _rptBuildXlsxAppSummary(wb);
    }
