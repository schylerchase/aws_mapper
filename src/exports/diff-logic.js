...
function normalizeSG(sg) {
  ...
  function sortPerms(perms) {
    ...
    return perms
      .map((p) => {
        const np = { ...p };
        if (np.IpRanges) {
          np.IpRanges = [...np.IpRanges].sort((a, b) =>
            (a.CidrIp || '').localeCompare(b.CidrIp || '')
          );
        }
        if (np.Ipv6Ranges) {
          np.Ipv6Ranges = [...np.Ipv6Ranges].sort((a, b) =>
            (a.CidrIpv6 || '').localeCompare(b.CidrIpv6 || '')
          );
        }
        if (np.UserIdGroupPairs) {
          np.UserIdGroupPairs = [...np.UserIdGroupPairs].sort((a, b) =>
            (a.GroupId || '').localeCompare(b.GroupId || '')
          );
        }
        if (np.PrefixListIds) {
          np.PrefixListIds = [...np.PrefixListIds].sort((a, b) =>
            (a.PrefixListId || '').localeCompare(b.PrefixListId || '')
          );
        }
        return np;
      })
  ...
  if (clone.IpPermissions) {
    clone.IpPermissions = sortPerms(clone.IpPermissions);
  }
  if (clone.IpPermissionsEgress) {
    clone.IpPermissionsEgress = sortPerms(clone.IpPermissionsEgress);
  }
  return normalizeResource(clone);
}

function classifyChange(field) {
  if (DIFF_STRUCTURAL.has(field)) {
    return 'structural';
  }
  if (field === 'Tags' || field === 'TagSet' || field === 'Name' || field === 'Description') {
    return 'metadata';
  }
  return 'structural';
}
...
function _diffStr(obj) {
  ...
  if (_diffStrCache.has(obj)) {
    return _diffStrCache.get(obj);
  }
...
function _fieldDiff(normA, normB, path) {
  ...
  if (
    ...
    changes.push({
      ...
      old: normA,
      new: normB,
      kind: classifyChange(path.split('.').pop())
    });
    return changes;
  }
  if (Array.isArray(normA) && Array.isArray(normB)) {
    if (_diffStr(normA) !== _diffStr(normB)) {
      changes.push({
        field: path,
        old: normA,
        new: normB,
  ...
  allKeys.forEach((k) => {
    ...
    if (!(k in normA)) {
      changes.push({ field: sub, old: undefined, new: normB[k], kind: classifyChange(k) });
      return;
    }
    if (!(k in normB)) {
      changes.push({ field: sub, old: normA[k], new: undefined, kind: classifyChange(k) });
      return;
    }
...
function _diffResName(res, fallback) {
  ...
  return (
    res.DBInstanceIdentifier ||
    res.FunctionName ||
    res.serviceName ||
    res.CacheClusterId ||
    res.ClusterIdentifier ||
    fallback
  );
}
...
function computeDiff(baseline, current) {
  const results = {
    ...
    modified: [],
    unchanged: [],
    total: { added: 0, removed: 0, modified: 0, unchanged: 0 }
  };
