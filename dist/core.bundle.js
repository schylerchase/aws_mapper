...
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  ...
  function generatePsScript() {
    var script = [
      ...
      "    AWS CLI profile name (optional). -Profile is a supported alias. To create one:",
      "      aws configure --profile my-profile",
      "    Or for AWS SSO:",
      "      aws configure sso --profile my-profile",
      "      aws sso login --profile my-profile",
      "    Then run: ./export-aws-data.ps1 -Profile my-profile -AllRegions",
      ".PARAMETER Region",
      "    AWS region (optional). Required for single-region export unless the profile or environment already has a default region.",
      ".PARAMETER OutputDir",
      ...
      "#>",
      "[CmdletBinding()]",
      'param([Alias("p","Profile")][string]$AwsProfile,[Alias("r")][string]$Region,[Alias("o")][string]$OutputDir,[switch]$AllRegions,[string[]]$Profiles,[int]$MaxParallel=6)',
      '$ErrorActionPreference="Stop"',
      `if($AwsProfile -and $AwsProfile -notmatch '^[a-zA-Z0-9_-]+$'){Write-Error "Invalid profile";exit 1}`,
      ...
      `function Resolve-AwsRegion([string]$ProfileName,[string]$RequestedRegion,[switch]$AllowFallback){if($RequestedRegion){return $RequestedRegion};if($env:AWS_REGION -and $env:AWS_REGION -match '^[a-zA-Z0-9-]+$'){return $env:AWS_REGION};if($env:AWS_DEFAULT_REGION -and $env:AWS_DEFAULT_REGION -match '^[a-zA-Z0-9-]+$'){return $env:AWS_DEFAULT_REGION};$a=@("configure","get","region");if($ProfileName){$a+=@("--profile",$ProfileName)};try{$c=(& aws @a 2>$null|Out-String).Trim();if($LASTEXITCODE -eq 0 -and $c -match '^[a-zA-Z0-9-]+$'){return $c}}catch{};if($AllowFallback){return "us-east-1"};return $null}`,
      'function Get-RegionDiscoveryFlags([string]$ProfileName){$dr=Resolve-AwsRegion -ProfileName $ProfileName -RequestedRegion $Region -AllowFallback;$f=@();if($ProfileName){$f+=@("--profile",$ProfileName)};$f+=@("--region",$dr);return $f}',
      `function Write-RegionRequiredMessage([string]$ProfileName){$label=if($ProfileName){$ProfileName}else{"default"};Write-Host "";Write-Host "  No AWS region is configured for profile '$label'." -ForegroundColor Red;Write-Host "  Use -AllRegions, -Region us-east-1, or set a default with aws configure set region us-east-1." -ForegroundColor Yellow;Write-Host ""}`,
      "$profileList=@()",
      'if($Profiles){$profileList=@($Profiles|ForEach-Object{$_ -split ","}|ForEach-Object{$_.Trim()}|Where-Object{$_})}',
      "if($AwsProfile -and $profileList.Count -eq 0){$profileList=@($AwsProfile)}",
      '$awsFlags=@(); if($AwsProfile){$awsFlags+=@("--profile",$AwsProfile)}',
      'function Get-BaseFlags([string]$reg){$f=@();if($AwsProfile){$f+=@("--profile",$AwsProfile)};if($reg){$f+=@("--region",$reg)};return $f}',
      "$exports=@(",
      ...
      "  $probe=& aws @flags ec2 describe-vpcs --max-results 5 --output json 2>&1",
      '  if($LASTEXITCODE -ne 0){$raw=($probe|Out-String).Trim() -replace "\\s+"," ";if(-not $raw){$raw="Unknown AWS CLI error"};$d=if($raw -match "(UnauthorizedOperation|AccessDenied|AccessDeniedException|AuthorizationError|not authorized|is not authorized)"){"access denied - missing ec2:DescribeVpcs"}else{if($raw.Length -gt 220){$raw.Substring(0,220)}else{$raw}};Write-Host "  Preflight EC2 read access ERROR ($d)" -ForegroundColor Red;Write-Host "  Profile is logged in, but the selected AWS role needs ReadOnlyAccess or equivalent Describe/List/Get permissions." -ForegroundColor Yellow;@(@{label="Preflight EC2 read access";status="ERROR";detail=$d;rawDetail=$raw})|ConvertTo-Json -Depth 3|Out-File(Join-Path $OutPath "_export-log.json")-Encoding utf8;return}',
      ...
      '      else{$m=($r|Out-String).Trim() -replace "\\s+"," ";if(-not $m){$m="Unknown AWS CLI error"};$d=if($m -match "(UnauthorizedOperation|AccessDenied|AccessDeniedException|AuthorizationError|not authorized|is not authorized)"){"access denied - missing required read permission"}else{if($m.Length -gt 220){$m.Substring(0,220)}else{$m}};@{L=$_.Label;S="ERROR";D=$d;R=$m}}}',
      '    catch{$m=if($_.Exception.Message){$_.Exception.Message}else{"Unknown error"};@{L=$_.Label;S="ERROR";D=$m;R=$m}}}',
      '  foreach($r in $results){$c=if($r.S -eq "OK"){"Green"}elseif($r.S -eq "ERROR"){"Red"}else{"Yellow"};Write-Host("  {0,-30} {1} ({2})" -f $r.L,$r.S,$r.D)-ForegroundColor $c}',
      '  @($results|ForEach-Object{$e=@{label=$_.L;status=if($_.S -eq "SKIP"){"EMPTY"}else{$_.S};detail=$_.D};if($_.R){$e.rawDetail=$_.R};$e})|ConvertTo-Json -Depth 3|Out-File(Join-Path $OutPath "_export-log.json")-Encoding utf8',
      ...
      "function Test-HasResources([string[]]$f){try{$r=& aws @f ec2 describe-vpcs --query 'Vpcs[?IsDefault==`false`].VpcId' --output json 2>&1;if($LASTEXITCODE -ne 0){return $null};$v=$r|ConvertFrom-Json;return($v.Count -gt 0)}catch{return $null}}",
      "if($Profiles){",
      ...
      '    $AwsProfile=$prof;$awsFlags=@("--profile",$prof)',
      '    Write-Host "`n  === Profile: $prof ===" -ForegroundColor Magenta',
      "    $profDir=Join-Path $OutputDir $prof",
      "    if($AllRegions){",
      "      $regionFlags=Get-RegionDiscoveryFlags $prof",
      "      $rr=& aws @regionFlags ec2 describe-regions --query 'Regions[].RegionName' --output json 2>&1",
      ...
      '      $i=0;$skip=0;foreach($reg in $regions){$i++;$rf=Get-BaseFlags $reg;$has=Test-HasResources $rf;if($null -eq $has){Write-Host "  Region $reg access check failed; stopping profile." -ForegroundColor Red;Export-Region -RegionName $reg -OutPath(Join-Path $profDir $reg)-Parallel $MaxParallel;break};if(-not $has){$skip++;continue};Write-Host "  Region $i/$($regions.Count): $reg" -ForegroundColor Cyan',
      ...
      "    }else{$exportRegion=Resolve-AwsRegion -ProfileName $prof -RequestedRegion $Region;if(-not $exportRegion){Write-RegionRequiredMessage $prof;continue};Export-Region -RegionName $exportRegion -OutPath $profDir -Parallel $MaxParallel}}",
      "}elseif($AllRegions){",
      '  Write-Host "  Mode: All Regions (parallel x$MaxParallel)" -ForegroundColor Cyan',
      "  $regionFlags=Get-RegionDiscoveryFlags $AwsProfile",
      "  $rr=& aws @regionFlags ec2 describe-regions --query 'Regions[].RegionName' --output json 2>&1",
      '  if($LASTEXITCODE -ne 0){Write-Error "Failed: $rr";exit 1}',
      "  $regions=$rr|ConvertFrom-Json|Sort-Object",
      '  if(-not $OutputDir){$ts=Get-Date -Format "yyyyMMdd-HHmmss";$lb=if($AwsProfile){$AwsProfile}else{"default"};$OutputDir="./aws-export-${lb}-allregions-${ts}"}',
      "  New-Item -ItemType Directory -Path $OutputDir -Force|Out-Null",
      '  Write-Host "  $($regions.Count) regions -> $OutputDir`n"',
      '  $i=0;$skip=0;foreach($reg in $regions){$i++;$rf=Get-BaseFlags $reg;$has=Test-HasResources $rf;if($null -eq $has){Write-Host "  Region $reg access check failed; stopping export." -ForegroundColor Red;Export-Region -RegionName $reg -OutPath(Join-Path $OutputDir $reg)-Parallel $MaxParallel;break};if(-not $has){$skip++;Write-Host "  Region $i/$($regions.Count): $reg \u2014 empty, skipping" -ForegroundColor DarkGray;continue};Write-Host "  Region $i/$($regions.Count): $reg" -ForegroundColor Cyan',
      "    Export-Region -RegionName $reg -OutPath(Join-Path $OutputDir $reg)-Parallel $MaxParallel}",
      '  if($skip){Write-Host "  Skipped $skip empty regions" -ForegroundColor DarkGray}',
      "}else{",
      "  $exportRegion=Resolve-AwsRegion -ProfileName $AwsProfile -RequestedRegion $Region;if(-not $exportRegion){Write-RegionRequiredMessage $AwsProfile;exit 1}",
      '  if(-not $OutputDir){$ts=Get-Date -Format "yyyyMMdd-HHmmss";$lb=if($AwsProfile){$AwsProfile}else{"default"};$OutputDir="./aws-export-${lb}-${ts}"}',
      `  Write-Host "  Profile: $($AwsProfile ? $AwsProfile : 'default')  Region: $exportRegion  Output: $OutputDir\`n"`,
      "  Export-Region -RegionName $exportRegion -OutPath $OutputDir -Parallel $MaxParallel}",
      '$json=Get-ChildItem -Path $OutputDir -Filter "*.json" -Recurse -ErrorAction SilentlyContinue',
      '$af=@($json|Where-Object{$_.Name -ne "_export-log.json"});$logs=@($json|Where-Object{$_.Name -eq "_export-log.json"})',
      '$ok=0;$err=0;foreach($lg in $logs){try{$entries=@(Get-Content -LiteralPath $lg.FullName -Raw|ConvertFrom-Json);foreach($e in $entries){if($e.status -eq "OK"){$ok++}elseif($e.status -eq "ERROR"){$err++}}}catch{}}',
      ...
      'if($err -gt 0 -and $ok -eq 0){Write-Host "`n  Export failed: no importable AWS data files were written." -ForegroundColor Red;Write-Host "  $err AWS inventory call(s) were denied or failed. The selected role needs ReadOnlyAccess or equivalent Describe/List/Get permissions." -ForegroundColor Yellow;Write-Host "  Output: $OutputDir`n" -ForegroundColor Yellow;exit 2}',
      'if($err -gt 0){Write-Host "`n  Done with warnings! $fc data files ($ts2) -> $OutputDir; $err AWS API calls failed.`n" -ForegroundColor Yellow}else{Write-Host "`n  Done! $fc data files ($ts2) -> $OutputDir`n" -ForegroundColor Green}'
    ].join("\r\n");
    return script;
  }
